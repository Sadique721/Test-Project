...
DIAMETER_COMMAND_CODE_STA = 275
DIAMETER_COMMAND_CODE_STR = 275
...
def build_sta(hbh_id, ete_id):
    ...
    header = build_diameter_header(
        command_code=DIAMETER_COMMAND_CODE_STA,
...
def run_diameter_client(host='127.0.0.1', port=3868):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        hbh += 1
        ete += 1
        sta = build_sta(hbh, ete)
        print("[>] Sending STA Termination Request...")
        sock.sendall(sta)

        # Main loop: Receive STR or DWR
        while True:
            data = sock.recv(4096)
            if not data:
            ...
            elif header["command_code"] == DIAMETER_COMMAND_CODE_STR and not (header["flags"] & 0x80):
                print("[<] Received STA (Session-Termination-Answer)")
                print(data.hex())
