...
def build_ccr(hbh_id, ete_id):
    avps = b""
    avps += build_avp(263, 0x40, "RO-1300001")  # Session-Id
    avps += build_avp(264, 0x40, ORIGIN_HOST)
    avps += build_avp(296, 0x40, ORIGIN_REALM)
    ...
    avps += build_avp(416, 0x40, struct.pack("!I", 2))  # CC-Request-Number
    ...
    used_unit = b""
    rsu = build_avp(420, 0x40, struct.pack("!I", 30))
    used_unit += build_avp(437, 0x40, rsu)
    avps += build_avp(456, 0x40, used_unit)

    # Service-Identifier (voice)
    avps += build_avp(439, 0x40, struct.pack("!I", 1))
    
    # --- IMS-Information (876) ---
    ims_info = b''

    # Role-Of-Node (AVP 829, Vendor 10415)
    ims_info += build_avp(829, 0xC0, struct.pack("!I", 0), vendor_id=10415)

    # Calling-Party-Address (AVP 831, Vendor 10415)
    ims_info += build_avp(831, 0xC0, b"sip:caller@test.com", vendor_id=10415)

    # Called-Party-Address (AVP 832, Vendor 10415)
    ims_info += build_avp(832, 0xC0, b"sip:callee@test.com", vendor_id=10415)

    # SIP-Method (AVP 824, Vendor 10415)
    ims_info += build_avp(824, 0xC0, b"INVITE", vendor_id=10415)

    # Wrap IMS-Information
    ims_information_avp = build_avp(876, 0xC0, ims_info, vendor_id=10415)

    # --- Service-Information (873) ---
    service_info = ims_information_avp
...
def run_diameter_client(host='127.0.0.1', port=3868):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        print("[>] Sending CCR Update Request...")
        sock.sendall(ccr)
