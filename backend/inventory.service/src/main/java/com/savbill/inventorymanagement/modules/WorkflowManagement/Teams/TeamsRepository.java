...
public interface TeamsRepository extends JpaRepository<Teams, Long>, QuerydslPredicateExecutor<Teams> {
    ...
    @Query(value = "SELECT team_id, team_name FROM tblmteams WHERE is_deleted = false AND team_status = :status", nativeQuery = true)
    List<Object[]> findTeamIdAndNameByStatus(@Param("status") String status);

    @Query(value = "SELECT team_id, team_name FROM tblmteams WHERE is_deleted = false AND team_status = :status AND MVNOID = :mvnoId", nativeQuery = true)
    List<Object[]> findTeamIdAndNameByStatusAndMvnoId(@Param("status") String status, @Param("mvnoId") Integer mvnoId);

    @Query(value = "SELECT team_id, team_name FROM tblmteams WHERE is_deleted = false AND team_status = :status AND partnerid = :partnerId", nativeQuery = true)
    List<Object[]> findTeamIdAndNameByStatusAndPartnerId(@Param("status") String status, @Param("partnerId") Integer partnerId);

    @Query(value = "SELECT team_id, team_name FROM tblmteams WHERE is_deleted = false AND team_status = :status AND partnerid = :partnerId AND MVNOID = :mvnoId", nativeQuery = true)
    List<Object[]> findTeamIdAndNameByStatusAndPartnerIdAndMvnoId(@Param("status") String status, @Param("partnerId") Integer partnerId, @Param("mvnoId") Integer mvnoId);

    @Query(value = "SELECT team_id, team_name FROM tblmteams WHERE is_deleted = false AND team_status = :status AND partnerid = :partnerId AND MVNOID = :mvnoId AND lcoid = :lcoId", nativeQuery = true)
    List<Object[]> findTeamIdAndNameByStatusAndPartnerIdAndMvnoIdAndLcoId(@Param("status") String status, @Param("partnerId") Integer partnerId, @Param("mvnoId") Integer mvnoId, @Param("lcoId") Integer lcoId);

    @Query("SELECT new Teams(s.id, s.name) FROM Teams s WHERE s.isDeleted = false AND s.id =:id")
    Teams findAllLightTeamsById(@Param("id") Long id);
