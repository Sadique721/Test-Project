package com.savbill.inventorymanagement.modules.InventoryManagement.ippool.utils;

import com.savbill.inventorymanagement.core.exceptions.CustomValidationException;
import org.springframework.http.HttpStatus;

import java.util.ArrayList;
import java.util.List;

public class SubnetUtils {
    int baseIPnumeric;
    int netmaskNumeric;
    String IPinCIDRFormat;

    public SubnetUtils(String IPinCIDRFormat) throws NumberFormatException {
        this.IPinCIDRFormat = IPinCIDRFormat;
        String[] st = IPinCIDRFormat.split("\\/");
        if (st.length != 2)
            throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(), "Invalid IP Range format '" + IPinCIDRFormat + "', should be: xx.xx.xx.xx/xx", null);
        String symbolicIP = st[0];
        String symbolicCIDR = st[1];

        Integer numericCIDR = Integer.valueOf(symbolicCIDR);
        if (numericCIDR > 32)
            throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(), "IP Range can not be greater than 32", null);

        /* IP */
        st = symbolicIP.split("\\.");

        if (st.length != 4)
            throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(), "Invalid IP address: " + symbolicIP, null);

        int i = 24;
        baseIPnumeric = 0;

        for (int n = 0; n < st.length; n++) {
            int value = Integer.parseInt(st[n]);
            if (value != (value & 0xff)) {
                throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(), "Invalid IP address: " + symbolicIP, null);
            }
            baseIPnumeric += value << i;
            i -= 8;
        }

        /* netmask from CIDR */
        if (numericCIDR < 20)
            throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(), "Netmask IP Range can not be less than 20", null);
        netmaskNumeric = 0xffffffff;
        netmaskNumeric = netmaskNumeric << (32 - numericCIDR);
    }

    public String getIP() {
        return convertNumericIpToSymbolic(baseIPnumeric);
    }

    private String convertNumericIpToSymbolic(Integer ip) {
        StringBuffer sb = new StringBuffer(15);
        for (int shift = 24; shift > 0; shift -= 8) {
            // process 3 bytes, from high order byte down.
            sb.append(Integer.toString((ip >>> shift) & 0xff));
            sb.append('.');
        }
        sb.append(Integer.toString(ip & 0xff));
        return sb.toString();
    }

    public String getNetmask() {
        StringBuffer sb = new StringBuffer(15);
        for (int shift = 24; shift > 0; shift -= 8) {
            // process 3 bytes, from high order byte down.
            sb.append(Integer.toString((netmaskNumeric >>> shift) & 0xff));
            sb.append('.');
        }
        sb.append(Integer.toString(netmaskNumeric & 0xff));
        return sb.toString();
    }

    public List<String> getAvailableIPs(Integer numberofIPs) {
        ArrayList<String> result = new ArrayList<String>();
        int numberOfBits;
        for (numberOfBits = 0; numberOfBits < 32; numberOfBits++) {
            if ((netmaskNumeric << numberOfBits) == 0)
                break;
        }
        Integer numberOfIPs = 0;
        for (int n = 0; n < (32 - numberOfBits); n++) {
            numberOfIPs = numberOfIPs << 1;
            numberOfIPs = numberOfIPs | 0x01;
        }
        Integer baseIP = baseIPnumeric & netmaskNumeric;
        for (int i = 1; i < (numberOfIPs) && i <= numberofIPs; i++) {
            Integer ourIP = baseIP + i;
            String ip = convertNumericIpToSymbolic(ourIP);
            result.add(ip);
        }
        return result;
    }

    public String getHostAddressRange() {
        int numberOfBits;
        for (numberOfBits = 0; numberOfBits < 32; numberOfBits++) {
            if ((netmaskNumeric << numberOfBits) == 0)
                break;
        }
        Integer numberOfIPs = 0;
        for (int n = 0; n < (32 - numberOfBits); n++) {
            numberOfIPs = numberOfIPs << 1;
            numberOfIPs = numberOfIPs | 0x01;
        }
        Integer baseIP = baseIPnumeric & netmaskNumeric;
        String firstIp = convertNumericIpToSymbolic(baseIP + 1);
        String lastIp = convertNumericIpToSymbolic(baseIP + numberOfIPs);
        return firstIp + " - " + lastIp;
    }

    public String getNetworkIp(){
        Integer baseIP = baseIPnumeric & netmaskNumeric;
        String[] st = IPinCIDRFormat.split("\\/");
        return convertNumericIpToSymbolic(baseIP) + "/" + st[1];
    }

    public String getFirstIp(){
        Integer baseIP = baseIPnumeric & netmaskNumeric;
        return convertNumericIpToSymbolic(baseIP + 1);
    }

    public String getLastIp(){
        Integer baseIP = baseIPnumeric & netmaskNumeric;
        return convertNumericIpToSymbolic(baseIP + getNumberOfHosts());
    }

    public Integer getNumberOfHosts() {
        int numberOfBits;
        for (numberOfBits = 0; numberOfBits < 32; numberOfBits++) {
            if ((netmaskNumeric << numberOfBits) == 0)
                break;
        }
        Double x = Math.pow(2, (32 - numberOfBits));
        if (x == -1)
            x = 1D;
        return x.intValue() - 2;
    }

    public String getBroadcastAddress() {
        if (netmaskNumeric == 0xffffffff)
            return "0.0.0.0";
        int numberOfBits;
        for (numberOfBits = 0; numberOfBits < 32; numberOfBits++) {
            if ((netmaskNumeric << numberOfBits) == 0)
                break;
        }
        Integer numberOfIPs = 0;
        for (int n = 0; n < (32 - numberOfBits); n++) {
            numberOfIPs = numberOfIPs << 1;
            numberOfIPs = numberOfIPs | 0x01;
        }
        Integer baseIP = baseIPnumeric & netmaskNumeric;
        Integer ourIP = baseIP + numberOfIPs;
        String ip = convertNumericIpToSymbolic(ourIP);
        return ip;
    }

    public boolean contains(String IPaddress) {
        Integer checkingIP = 0;
        String[] st = IPaddress.split("\\.");
        if (st.length != 4)
            throw new NumberFormatException("Invalid IP address: " + IPaddress);
        int i = 24;
        for (int n = 0; n < st.length; n++) {
            int value = Integer.parseInt(st[n]);
            if (value != (value & 0xff)) {
                throw new NumberFormatException("Invalid IP address: "
                        + IPaddress);
            }
            checkingIP += value << i;
            i -= 8;
        }
        if ((baseIPnumeric & netmaskNumeric) == (checkingIP & netmaskNumeric))
            return true;
        else
            return false;
    }

    public boolean contains(SubnetUtils child) {
        Integer subnetID = child.baseIPnumeric;
        Integer subnetMask = child.netmaskNumeric;
        if ((subnetID & this.netmaskNumeric) == (this.baseIPnumeric & this.netmaskNumeric)) {
            if ((this.netmaskNumeric < subnetMask) == true && this.baseIPnumeric <= subnetID) {
                return true;
            }
        }
        return false;
    }
}