...
public interface ItemAssemblyRepo extends JpaRepository<ItemAssembly,Long> , QuerydslPredicateExecutor<ItemAssembly> {
    ...
    @Query(value = "SELECT ia.itemassembly_name FROM tblmitemassembly ia WHERE ia.id = :id AND is_deleted=false", nativeQuery = true)
    String findAssemblyNameById(@Param("id") Long id);
}
