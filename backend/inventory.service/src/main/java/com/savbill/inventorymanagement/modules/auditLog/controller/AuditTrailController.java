...
import com.savbill.inventorymanagement.core.controller.APIResponseController;
import com.savbill.inventorymanagement.core.dto.GenericDataDTO;
import com.savbill.inventorymanagement.core.dto.PaginationRequestDTO;
import com.savbill.inventorymanagement.core.utillity.PageableResponse;
import com.savbill.inventorymanagement.modules.auditLog.model.AuditSearchPojo;
import com.savbill.inventorymanagement.modules.auditLog.service.AuditTrailService;
...
@RestController
@RequestMapping(UrlConstants.BASE_API_URL +UrlConstants.BASE_INVENTORY_MANAGEMENT_API_URL+ UrlConstants.AUDIT_TRAIL)
public class AuditTrailController {
    ...
    @PreAuthorize("validatePermission(\"" + ACLMenuConstants.AUDIT_LOG + "\")")
    @GetMapping("/all")
    ...
    public GenericDataDTO getAuditTrailsByModule(@RequestBody AuditSearchPojo auditSearchPojo, HttpServletRequest request) {
        ...
        if (auditSearchPojo.getStartDate() != null && auditSearchPojo.getEndDate() != null) {
            ...
            log.info(LogConstant.REQUEST_FROM + request.getHeader("requestFrom") + LogConstant.REQUEST_FOR + "Fetch audit by Start-End Date " + auditSearchPojo.getEntityName()  + LogConstant.REQUEST_BY + getLoggedInUser().getUsername() + LogConstant.LOG_STATUS + LogConstant.LOG_NO_RECORD_FOUND  + LogConstant.LOG_STATUS_CODE  + HttpStatus.OK.value());
        } else {
        ...
        if (page.getData().isEmpty()) {
            ...
            log.info(LogConstant.REQUEST_FROM + request.getHeader("requestFrom") + LogConstant.REQUEST_FOR + "Unable to  Fetch audit data by entity name " + auditSearchPojo.getEntityName() + LogConstant.REQUEST_BY + getLoggedInUser().getUsername()  + LogConstant.LOG_STATUS + LogConstant.LOG_NO_RECORD_FOUND + LogConstant.LOG_STATUS_CODE  + HttpStatus.OK.value());
        }else{
