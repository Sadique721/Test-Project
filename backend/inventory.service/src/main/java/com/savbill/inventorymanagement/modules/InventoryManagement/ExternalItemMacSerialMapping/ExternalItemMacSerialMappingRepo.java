...
public interface ExternalItemMacSerialMappingRepo extends JpaRepository<ExternalItemMacSerialMapping, Long>, QuerydslPredicateExecutor<ExternalItemMacSerialMapping> {
    ...
    @Query(value = "select * from tbltexternalitemmacmapping t where t.item_id :id and t.is_deleted =false", nativeQuery = true)
    List<ExternalItemMacSerialMapping> findAllByItemId(@Param("id") Long id);

    List<ExternalItemMacSerialMapping> findAllByExternalItemId(Long externalItemId);
