...
public class BulkConsumptionServiceImp extends ExBaseAbstractService<BulkConsumptionDto, BulkConsumption, Long> {
    ...
    public BulkConsumptionDto saveEntity(BulkConsumptionDto bulkConsumptionDto) throws Exception {
        try {
            bulkConsumptionDto.setMvnoId(getMvnoIdFromCurrentStaff());
            ...
            bulkConsumptionDto.setApprovalStatus(CommonConstants.PENDING);
    ...
    public BulkConsumptionDto saveInwardApproval(Long bulckConsumptionId, String bulckConsumptionApprovalStatus, String bulckConsumptionApprovalRemark) {
        try {
            ...
            BooleanExpression booleanExpression = qBulkConsumption.isNotNull()
                    ...
                    .and(qBulkConsumption.approvalStatus.contains(CommonConstants.PENDING))
            ...
            List<BulkConsumption> bulkConsumptionList = Lists.newArrayList(bulkConsumptionRepository.findAll(booleanExpression))
                    .stream().filter(bulkConsumption -> bulkConsumption.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || bulkConsumption.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()).collect(Collectors.toList());
            if (bulkConsumptionList.size() > 0) {
                BulkConsumptionDto bulkConsumptionDto = null;
                bulkConsumptionDto = getEntityForUpdateAndDelete(bulkConsumptionList.get(0).getId());
                if(bulkConsumptionDto.getItemType().equalsIgnoreCase("Serialized Item")) {
                    if (bulckConsumptionApprovalStatus.equalsIgnoreCase(CommonConstants.APPROVE)) {
                        ...
                        bulkConsumptionDto.setApprovalStatus(CommonConstants.APPROVE);
                        bulkConsumptionDto.setApprovalRemark(bulckConsumptionApprovalRemark);
                        return super.updateEntity(bulkConsumptionDto);
                    }
                    if (bulckConsumptionApprovalStatus.equalsIgnoreCase(CommonConstants.REJECTED)) {
                        bulkConsumptionDto.setApprovalStatus(CommonConstants.REJECTED);
                    ...
                    if (bulckConsumptionApprovalStatus.equalsIgnoreCase(CommonConstants.APPROVE)) {
                        ...
                        bulkConsumptionDto.setApprovalRemark(bulckConsumptionApprovalRemark);
                        return super.updateEntity(bulkConsumptionDto);
                    } else if (bulckConsumptionApprovalStatus.equalsIgnoreCase(CommonConstants.REJECTED)) {
                        bulkConsumptionDto.setApprovalStatus(CommonConstants.REJECTED);
                        bulkConsumptionDto.setApprovalRemark(bulckConsumptionApprovalRemark);
    ...
    public boolean duplicateVerifyAtEdit(String name, Long id) throws Exception {
        try {
            ...
            if (name != null) {
                name = name.trim();
                Integer count;
                if (getMvnoIdFromCurrentStaff() == 1) count = bulkConsumptionRepository.duplicateVerifyAtSave(name);
        ...
        try {
            ...
            if (getMvnoIdFromCurrentStaff() != 1) {
                booleanExpression = booleanExpression
                        .and(qBulkConsumption.mvnoId.in(getMvnoIdFromCurrentStaff(), 1));
            }
...
//        return null;
    ...
    public BulkConsumption getBulkConsumptionById(long id){
        return bulkConsumptionRepository.findById(id).get();
    }
}
