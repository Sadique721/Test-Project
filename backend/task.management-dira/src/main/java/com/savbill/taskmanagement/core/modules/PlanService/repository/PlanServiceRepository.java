package com.savbill.taskmanagement.core.modules.PlanService.repository;
...
//    Integer duplicateVerifyAtSaveICName(@Param("icname") String icname, @Param("mvnoIds")List mvnoIds);
    ...
    @Query(value = "select t.servicename from TBLMSERVICES t" , nativeQuery = true)
    List<String> getAllServiceName();

    @Query("SELECT service.id from PlanService service where service.name in :names")
    List<Integer> findServiceIdByPlanName(@Param("names") List<String> names);
}
