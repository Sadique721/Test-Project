...
public interface StaffUserRepository extends JpaRepository<StaffUser, Integer>, QuerydslPredicateExecutor<StaffUser> {
    ...
    List<StaffUser> findAllByIdInAndStatusEqualsIgnoreCase(List<Integer> staffIdlist, String staffStatus);

    @Query(value = "select staffid from tblstaffuser t where  t.parent_staff_id=:parentStaffId",nativeQuery = true)
    Set<Integer> findAllByParentStaffIdAndIsDeleteFalse(Integer parentStaffId);

    List<StaffUser> findAllByEmailAndIsDeleteFalse(String email);
...
//
    ...
    StaffUser findById(NumberPath<Integer> id);
...
//
    ...
    @Query("SELECT t.id FROM StaffUser s JOIN s.team t WHERE s.id = :id")
    Set<Integer> findTeamIdsByStaffId(@Param("id") Integer id);
