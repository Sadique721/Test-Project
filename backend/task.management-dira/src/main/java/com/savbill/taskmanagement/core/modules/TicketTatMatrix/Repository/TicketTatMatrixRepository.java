package com.savbill.taskmanagement.core.modules.TicketTatMatrix.Repository;
...
public interface TicketTatMatrixRepository  extends JpaRepository<TicketTatMatrix,Long> , QuerydslPredicateExecutor<TicketTatMatrix> {
    ...
    @Query(value = "select count(*)as tab from tbltcasecategorytatmapping t1 where t1.ticket_tat_mapping_id =:id " ,nativeQuery = true)
    ...
    @Query(value = "select * from tblttickettatmatrix t where t.is_deleted = false and t.MVNOID in :mvnoIds",nativeQuery = true)
    Page<TicketTatMatrix> findAll(Pageable pageable, @Param("mvnoIds")List mvnoIds);

    @Query(value = "select * from tblttickettatmatrix t where t.is_deleted = false and (t.MVNOID = 1 or (MVNOID = :mvnoId and BUID in :buIds))",nativeQuery = true)
    Page<TicketTatMatrix> findAll(Pageable pageable, @Param("mvnoId") Integer mvnoId, @Param("buIds") List buIds);
    Page<TicketTatMatrix> findAllByAndIsDeletedFalse(Pageable pageable);

    Page<TicketTatMatrix>findAllByAndIsDeletedFalseAndMvnoIdIn(Pageable pageable, List<Integer> mvnoIds);
    Page<TicketTatMatrix>findAllByAndIsDeletedFalseAndMvnoIdInAndBuIdIn(Pageable pageable, List<Integer> mvnoIds, List<Long> buIds);




    TicketTatMatrix findByIdAndIsDeletedFalse(long id);

    TicketTatMatrix findByIsDefaultTrueAndMvnoId(int mvno);
