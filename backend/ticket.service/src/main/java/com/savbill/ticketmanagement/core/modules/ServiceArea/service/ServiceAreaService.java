package com.savbill.ticketmanagement.core.modules.ServiceArea.service;
...
import com.savbill.ticketmanagement.core.modules.ServiceArea.model.ServiceAreaDTO;
import com.savbill.ticketmanagement.core.modules.ServiceArea.repository.ServiceAreaPincodeRelRepository;
...
import com.savbill.ticketmanagement.rabbitmq.messages.DataShareMessage.SaveServiceAreaSharedDataMessge;
import com.savbill.ticketmanagement.rabbitmq.messages.DataShareMessage.UpdateServiceAreaSharedDataMessage;
    ...
    public void updateServiceArea(UpdateServiceAreaSharedDataMessage messge) {
        try {
            ...
            if (messge.getName() != null) {
                serviceArea = serviceAreaRepository.findById(messge.getId()).orElse(null);
                if (serviceArea != null) {
                    ...
                    serviceArea.setLastModifiedById(messge.getUpdatedById());
                    serviceAreaRepository.save(serviceArea);
                } else {
...
//
    ...
    public void saveServiceAreaData(ConsumerRecord<String, Object> records) {
        try {
            ObjectMapper objectMapper = new ObjectMapper();
            String jsonData = (String) records.value();
            SaveServiceAreaSharedDataMessge message = objectMapper.readValue(jsonData, SaveServiceAreaSharedDataMessge.class);
            if (message != null) {
                ServiceArea serviceArea = new ServiceArea();
                serviceArea.setId(message.getId());
                serviceArea.setName(message.getName());
    ...
    public void updateServiceAreaData(ConsumerRecord<String, Object> records) {
        ObjectMapper objectMapper = new ObjectMapper();
        String jsonData = (String) records.value();
        try {
            UpdateServiceAreaSharedDataMessage message = objectMapper.readValue(jsonData, UpdateServiceAreaSharedDataMessage.class);
            if (message != null) {
                ServiceArea serviceArea = serviceAreaRepository.findById(message.getId()).get();
                if (serviceArea != null) {
                    serviceArea.setName(message.getName());
                    serviceArea.setStatus(message.getStatus());
                    serviceArea.setLatitude(message.getLatitude());
