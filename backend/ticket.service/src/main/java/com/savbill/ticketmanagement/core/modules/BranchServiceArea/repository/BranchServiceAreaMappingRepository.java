package com.savbill.ticketmanagement.core.modules.BranchServiceArea.repository;
...
public interface BranchServiceAreaMappingRepository extends JpaRepository<BranchServiceAreaMapping, Long>, QuerydslPredicateExecutor<BranchServiceAreaMapping> {
    ...
    @Query(value = "select * from savbillconvergebss.tblmbranchservicearearel t where servicearea_id =:serviceAreaid", nativeQuery = true)
    List<BranchServiceAreaMapping> findAllByServiceareaId(@Param("serviceAreaid") Long serviceAreaid);

    @Query(value = "select * from savbillconvergebss.tblmbranchservicearearel t where servicearea_id in (:serviceAreaids)", nativeQuery = true)
