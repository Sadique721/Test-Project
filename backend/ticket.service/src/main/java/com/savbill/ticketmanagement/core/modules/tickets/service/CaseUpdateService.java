...
import com.savbill.ticketmanagement.core.dto.TimeUnitWithTotal;
...
import com.savbill.ticketmanagement.core.modules.ResolutionReasons.service.ResolutionReasonsService;
...
import com.savbill.ticketmanagement.core.modules.TicketFollowUp.Model.TicketFollowUpDTO;
import com.savbill.ticketmanagement.core.modules.TicketFollowUp.Service.TicketFollowUpService;
...
import com.savbill.ticketmanagement.core.modules.tickets.mapper.CaseUpdateMapper;
...
//    private MessageSender messageSender;
    ...
    @Autowired
    private CaseDocDetailsRepository caseDocDetailsRepository;
    @Autowired
    ResoultionFileMappingRepocitory resoultionFileMappingRepocitory;

    private final Logger log = Logger.getLogger(CaseUpdateService.class);
    ...
    @Transactional
    public CaseDTO updateEntity (CaseUpdateDTO entity, List<MultipartFile> file, Boolean isPickedUp, TicketFileUploadDTO ticketFileUploadDTO) {
        ...
        try {
            ...
            if (null != dbObj) {
                ...
                //Department
                if (null != dbObj.getDepartment() && null != entity.getDepartment() && !dbObj.getDepartment().equalsIgnoreCase(entity.getDepartment())) {
                    Map<String, String> valueMap = new HashMap<>();
                    valueMap.put(CaseConstants.OLD_VALUE, dbObj.getCaseStatus());
                    valueMap.put(CaseConstants.NEW_VALUE, entity.getStatus());
                    mainMap.put(CaseConstants.DEPARTMENT, valueMap);
                    dbObj.setDepartment(entity.getDepartment());
                }
                    ...
                    if(null != ticketFileUploadDTO) {
                        if (null != dbObj.getCurrentAssigneeId()) {
                            ...
                            if (oldAssignee != null && newAssignee != null) {
                                workflowAuditService.saveAudit(null, CommonConstants.WORKFLOW_EVENT_NAME.CASE,
                                        Math.toIntExact(dbObj.getCaseId()), dbObj.getCaseNumber(),
                                        newAssignee.getId(), newAssignee.getUsername(),
                                        CommonConstants.WORKFLOW_AUDIT_ACTION.REASSIGNED,
                                        LocalDateTime.now(),
                                        "Ticket reassigned to " + newAssignee.getFirstname() + " " + newAssignee.getLastname());
                            }
                        } else {
...
//                            String subFolderName = aCase.getCaseNumber().trim().replace("-", "_") + "/";
                            ...
                            if (file1 != null) {
                                ...
                                String lastSavedFilenameByTicketId = caseDocDetailsRepository.findLastSavedFilenameByTicketId(entity.getTicketId());
                                caseDoc = caseDocDetailsService.saveEntity(caseDoc);
                                fileMap.put(CaseConstants.OLD_VALUE,lastSavedFilenameByTicketId);
                                fileMap.put(CaseConstants.NEW_VALUE,caseDoc.getFilename());
                                mainMap.put(CaseConstants.CASE_DOCUMENT_UPLOAD,fileMap);
                            }
                        }
                    }
                ...
                if(ticketFileUploadDTO != null){
                    if(ticketFileUploadDTO.getSections() != null){
                        List<CustomerTicketFileMapping> fileMappings = new ArrayList<>();
                        for (SectionUploadDTO sectionUploadRequest : ticketFileUploadDTO.getSections()) {
                            if (sectionUploadRequest.getFiles() != null) {
                                ...
                                for (MultipartFile file2 : sectionUploadRequest.getFiles()) {
                                    if (!file2.isEmpty()) {
                                        ...
                                        CustomerTicketFileMapping fileMapping = new CustomerTicketFileMapping();
                                        fileMapping.setCustomerTicketMapping(dbObj.getCaseId());
                                ...
                                Optional<CustomerTicketFileMapping> existingMappingOpt = customerTicketFileMappingRepo.findByCustomerTicketMappingAndSection(dbObj.getCaseId(), sectionUploadRequest.getName());
                                CustomerTicketFileMapping fileMapping;
                                if (existingMappingOpt.isPresent()) {
                                    ...
                                    fileMapping = new CustomerTicketFileMapping();
                                    fileMapping.setCustomerTicketMapping(dbObj.getCaseId());
                                    fileMapping.setSection(sectionUploadRequest.getName());
                                    if (sectionUploadRequest.getOpticalRange() != null && !sectionUploadRequest.getOpticalRange().isEmpty()) {
                                        fileMapping.setOpticalRange(sectionUploadRequest.getOpticalRange());
                        ...
                        customerTicketFileMappingRepo.saveAll(fileMappings);
                    }
                }
...
//                            statusDetails.setResolutionId(entity.getResolutionId().longValue());
                            ...
                            sendCustomerTicketStatusChangeMessage(customers.getUsername(), entity.getStatus(), customers.getCountryCode(), customers.getMobile(), customers.getEmail(), customers.getMvnoId(), RMQConstants.SEND_CUSTOMER_STATUS_CHANGE_TEMPLATE, RMQConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, Integer.valueOf(entity.getTicketId().intValue()),customers.getBuId(), dbObj.getCustomerAdditionalEmail());//add parameters
                            if(entity.getStatus().equalsIgnoreCase("Closed")){
                                if(dbObj.getBuId()!=null){
                                    sendCustTicketCloseMessage(dbObj.getUserName(), dbObj.getMobile(), dbObj.getEmail(), dbObj.getCaseStatus(), dbObj.getMvnoId(), dbObj.getCaseNumber(),dbObj.getBuId(), dbObj.getCaseTitle(), dbObj.getCustomerAdditionalEmail());
                                }else{
                                    sendCustTicketCloseMessage(dbObj.getUserName(), dbObj.getMobile(), dbObj.getEmail(), dbObj.getCaseStatus(), dbObj.getMvnoId(), dbObj.getCaseNumber(),null, dbObj.getCaseTitle(), dbObj.getCustomerAdditionalEmail());
                                }
                        ...
                        if (map.getKey().equalsIgnoreCase(CaseConstants.CASE_DOCUMENT_UPLOAD)) {
                            CaseUpdateDetailsDTO updates = new CaseUpdateDetailsDTO();
                            updates.setEntitytype("File");
                            updates.setOperation("File Upload");
                            updates.setNewvalue(map.getValue().get(CaseConstants.NEW_VALUE));
                            updates.setOldvalue(map.getValue().get(CaseConstants.OLD_VALUE));
                            updates.setCaseUpdate(entity);
                            detailsList.add(updates);
...
//            }
            ...
            CloseTicketCheckMessage message = new CloseTicketCheckMessage(dbObj.getCustomersId(),dbObj.getCaseId().intValue(),dbObj.getCaseNumber(),dbObj.getCaseStatus());
//            messageSender.send(message, RMQConstants.QUEUE_SEND_UPDATED_TICKET_DATA_TO_APIGW);
            kafkaMessageSender.send(new KafkaMessageData(message,CloseTicketCheckMessage.class.getSimpleName(),"UPDATE_TICKET_DATA"));
...
//                    messageSender.send(ticketAssignMessege, RMQConstants.QUEUE_TICKET_ASSIGN_TEAM_SUCCESS);
                    kafkaMessageSender.send(new KafkaMessageData(ticketAssignMessege,TicketAssignMessege.class.getSimpleName(),KafkaConstant.TICKET_ASSIGN_TEAM_SUCCESS));
                }
            } else {
...
//                    messageSender.send(sendProblemDomainChangeMsg, RMQConstants.QUEUE_SEND_PROBLEM_DOMAIN_CHANGE_MSG);
                    kafkaMessageSender.send(new KafkaMessageData(sendProblemDomainChangeMsg,SendProblemDomainChangeMsg.class.getSimpleName(), KafkaConstant.SEND_PROBLEM_DOMAIN_CHANGE_MSG));
                }
            } else {
//                 log.error("Message of otp generated is not sent because '" + OTP_GENERATED + "' template is not present.");
    ...
    public void sendCreateTicketMessege(String customerName, String mobileNumber, String emailId, Integer mvnoId, String caseNumber) {
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                        TicketCreationMessage ticketCreationMessage = new TicketCreationMessage(mobileNumber,emailId, RMQConstants.TICKET_CREATION_SUCCESS, optionalTemplate.get(),customerName,caseNumber,mvnoId);
                    Gson gson = new Gson();
                    gson.toJson(ticketCreationMessage);
                    kafkaMessageSender.send(new KafkaMessageData(ticketCreationMessage,TicketCreationMessage.class.getSimpleName()));
...
public CaseUpdateDTO restartTATbeforeFollowupEndAndStatusChage(CaseUpdateDTO caseUpdateDTO){
    ...
    public List<FileMappingListDTO> getFilesByTicketId(Long ticketId){
        List<FileMappingListDTO> fileMappingList = new ArrayList<>();
        List<CustomerTicketFileMapping> customerInventoryFileMappingList = customerTicketFileMappingRepo.findByCustomerTicketMappingId(ticketId);
    ...
    public  List<FileMappingListDTO> convertToFileMappingList(List<CustomerTicketFileMapping> mappings, Long ticketId) {
        return mappings.stream()
                .collect(Collectors.groupingBy(CustomerTicketFileMapping::getSection))
                .entrySet().stream()
                .map(entry -> {
    ...
    public File getAssignTicketFile(CustomerTicketFileMapping customerInventoryMapping, String uniqueName,String section, Integer ticketId) {
        String PATH = clientServiceSrv.getClientSrvByName(ClientServiceConstant.TICKET_PATH).get(0).getValue();
    ...
    public void deleteFileFromDatabase(String uniquename){
        CustomerTicketFileMapping customerInventoryFileMapping = customerTicketFileMappingRepo.findByCustomerTicketByUniqueName(uniquename);
        if(customerInventoryFileMapping != null){
            customerTicketFileMappingRepo.delete(customerInventoryFileMapping);
        }
    }

    public Resource getAssignTicketDoc(CustomerTicketFileMapping customerInventoryMapping, String uniqueName, String section, Integer ticketId) {
        ...
        try {
            ...
            ex.printStackTrace();
            resource = null;
            log.error("Error while get Assign Inventory Doc : " + ex.getMessage() + " for inventoryId : " + customerInventoryMapping.getId());
        }
        return resource;
    }
            ...
            try {
                ...
                for (MultipartFile file : resoltionFiles) {
                   ...
                   String subfolderName=PATH+ File.separator+resolutionReasonsDTO.getName()+ File.separator;
                    ...
                    if (file1 != null) {
                        mapping.setUniquename(fileUtility.saveFileToServerForTicket(file1, subfolderName.trim()));
                    }
