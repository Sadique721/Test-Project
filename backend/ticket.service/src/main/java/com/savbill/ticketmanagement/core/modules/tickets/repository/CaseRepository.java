...
import com.savbill.ticketmanagement.core.modules.tickets.model.CaseSummaryDTO;
...
public interface CaseRepository extends JpaRepository<Case, Long>, QuerydslPredicateExecutor<Case> {
    ...
    List<Case> findAllByCaseIdIn(List<Integer> caseIds);
    ...
    @Query(value = "\t\tselect * from   savbillconvergebss.tblcases where case_status !='Resolved' and case_status!='Closed' and case_status!='rejected' and final_resolved_by_id IS null and final_closed_by_id  is null",nativeQuery = true)
...
//    List<Case> findAllByReasonSubCategoryId(@Param("reason_sub_category_id") Integer reason_sub_category_id);
    ...
    @Query("SELECT new com.savbill.ticketmanagement.core.modules.tickets.model.CaseSummaryDTO(c.caseId, c.caseStatus, c.createdate, c.nextFollowupTime) FROM Case c WHERE c.caseStatus NOT IN :caseStatuses")
    List<CaseSummaryDTO> findAllByCaseStatusNotIn(@Param("caseStatuses") String caseStatuses);

    @Query(value = "SELECT COUNT(*) \n" +
            "FROM savbillticketmanagement.tblcases t \n" +
            "WHERE t.case_status NOT IN ('Resolved', 'Closed') \n" +
            "  AND t.is_delete = FALSE \n" +
            "  AND t.current_assignee_id = :currentAssigneeId", nativeQuery = true)
    Integer getUnresolvedTicketCount(@Param("currentAssigneeId") Integer currentAssigneeId);
    /*@Query(value = "SELECT COUNT(*) FROM savbillticketmanagement.tblcases " +
            "WHERE case_status = 'In Progress'",
            nativeQuery = true)
    int countInProgressTickets();*/

    @Query(value = "SELECT COUNT(*) FROM tblcases t WHERE t.case_status = 'In Progress' and t.current_assignee_id = :staffId",nativeQuery = true)
    int countInProgressTickets(int staffId);
