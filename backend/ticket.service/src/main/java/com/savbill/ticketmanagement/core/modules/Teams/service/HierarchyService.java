package com.savbill.ticketmanagement.core.modules.Teams.service;
...
import com.savbill.ticketmanagement.core.modules.Teams.model.TeamHierarchyDTO;
import com.savbill.ticketmanagement.core.modules.Teams.repository.HierarchyRepository;
import com.savbill.ticketmanagement.core.modules.Teams.repository.QueryFieldRepo;
...
import com.savbill.ticketmanagement.core.modules.utils.WorkFlowQueryUtils;
...
import com.savbill.ticketmanagement.rabbitmq.messages.DataShareMessage.SaveHierarchyShareDataMessage;
import com.savbill.ticketmanagement.rabbitmq.messages.DataShareMessage.UpdateHierarchyShareDataMessage;
import com.savbill.ticketmanagement.rabbitmq.messages.WorkflowTicketMessage;
    ...
    public void saveHierachy(SaveHierarchyShareDataMessage message){
        try {
           ...
           List<QueryFieldMapping> queryFieldMappingList=new ArrayList<>();
            for(TeamHierarchyMapping hierarchyMapping: message.getTeamHierarchyMappingList()){
                queryFieldMappingList.addAll(hierarchyMapping.getQueryFieldList());
            }
            queryFieldRepo.saveAll(queryFieldMappingList);
            teamHierarchyMappingRepo.saveAll(message.getTeamHierarchyMappingList());
            ...
            if (!oldTeamHierarchyMapping.isEmpty()) {
                for (TeamHierarchyMapping teamHierarchyMapping : oldTeamHierarchyMapping) {
                    if (teamHierarchyMapping.getQueryFieldList() != null) {
                        oldQueryFieldMapping.addAll(teamHierarchyMapping.getQueryFieldList());
                    }
                }
                queryFieldRepo.deleteInBatch(oldQueryFieldMapping);
...
@Transactional
    ...
    List<ServiceArea> getServiceAreaFromEntity(Object entity) {
        ...
        if (Objects.nonNull(entity) && entity.getClass().equals(CaseDTO.class)) {
            Optional<ServiceArea> optionalServiceArea = serviceAreaRepository.findById(
                    Long.valueOf(customersService.get(((CaseDTO) entity).getCustomersId()).getServiceAreaId())
            );
            if (optionalServiceArea.isPresent()) {
                serviceAreaList.add(optionalServiceArea.get());
            } else {
                log.info("ServiceArea not found for customer");
                throw new CustomValidationException(APIConstants.NOT_FOUND,"ServiceArea not found",null);
            }
        }
...
@Transactional
    public List<StaffUserPojo> getStaffFromCurrentTeammapping(Integer id, Object entity) {
        ...
        if (entity instanceof CaseDTO) {
            ...
            Customers customers = customerRepository.findById(aCase.getCustomersId()).orElse(null);
            if (customers != null) {
...
//                            caseUpdateDTO.setStatus(CaseConstants.STATUS_ASSIGNED);
                        ...
                        //    caseUpdateService.sendAssignTicketMessege(aCase.getCustomers().getUsername(), aCase.getCustomers().getMobile(), aCase.getCustomers().getEmail(), aCase.getCustomers().getMvnoId(), aCase.getCaseNumber(), teams.getName(), nextFollowupDate, aCase.getCustomers().getUsername(), nextFollwupTime);
                            ...
                            try {
                                caseDTO = caseUpdateService.updateEntity(caseUpdateDTO, null, false, null);
                            } catch (Exception e) {
