...
import com.savbill.ticketmanagement.core.dto.*;
import com.savbill.ticketmanagement.core.dto.*;
...
import com.savbill.ticketmanagement.core.modules.Customers.dto.CustomerServicePlanDTO;
import com.savbill.ticketmanagement.core.modules.Customers.repository.CustomerRepository;
//import com.savbill.ticketmanagement.core.modules.ServiceArea.SubscriberMapper;
//import com.savbill.ticketmanagement.core.modules.common.LoggedInUser;
import com.savbill.ticketmanagement.core.modules.TicketRemark.service.TicketRemarkService;
...
import com.savbill.ticketmanagement.core.modules.tickets.model.*;
import com.savbill.ticketmanagement.core.modules.tickets.repository.CustomerTicketFileMappingRepo;
import com.savbill.ticketmanagement.core.modules.tickets.service.CaseDocDetailsService;
import com.savbill.ticketmanagement.core.modules.tickets.service.CaseService;
import com.savbill.ticketmanagement.core.modules.tickets.service.CaseUpdateService;
import com.savbill.ticketmanagement.core.modules.tickets.service.LiveCustomerNetworkDetailsService;
...
import com.savbill.ticketmanagement.core.service.FileSystemService;
...
import static com.savbill.ticketmanagement.core.modules.common.AuditableListener.MODULE;
...
//    }
    ...
    @PreAuthorize("validatePermission(\"" + MenuConstants.Ticket.TICKET_CREATE + "\")")
    ...
    public GenericDataDTO save(@Valid @RequestParam String entityDTO, @RequestParam(required = false, value = "file") List<MultipartFile> file, HttpServletRequest req) {
        ...
        try {
            ...
            if (caseDTO.getTicketClassification() == null || caseDTO.getTicketClassification().isEmpty()) {
                // For CWSC and Mobile App, set default to Reactive
                caseDTO.setTicketClassification("Reactive");
            }
...
//                ApplicationLogger.logger.error("Unable to create new Cases with name " + caseDTO.getCaseTitle() + ":  request: { From : {}}; Response : {{};Exception:{}}", req.getHeader("requestFrom"), genericDataDTO.getResponseCode(), genericDataDTO.getResponseCode());
            ...
            if (null != caseDTO.getCustomersId()) {
                customers = customersService.get(caseDTO.getCustomersId());
...
//
//                }
                caseDTO.setMvnoId(customers.getMvnoId());
            }
...
//                    AclConstants.OPERATION_CASE_ADD, req.getRemoteAddr(), null, caseDTO.getCaseId(), caseDTO.getCustomerName());
    ...
    @PreAuthorize("validatePermission(\"" + MenuConstants.TicketConversation.CHANGE_STATUS + "\")")
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_EDIT + "\")")
    ...
    public GenericDataDTO updateDetails(@RequestParam String caseUpdate, @RequestParam(value = "file", required = false) List<MultipartFile> file, @ModelAttribute TicketFileUploadDTO ticketFileUploadDTO, @RequestParam(value = "resoultionFileMappingDTO", required = false) String resoultionFileMappingDTO, @RequestParam(value = "resolutionFiles", required = false) List<MultipartFile> resolutionFiles, HttpServletRequest req) {
...
//            }
            ...
            caseDTO = caseUpdateService.updateEntity(convDTO, file, false, ticketFileUploadDTO);
...
//                    AclConstants.OPERATION_CASE_EDIT, req.getRemoteAddr(), null, caseDTO.getCaseId(), caseDTO.getCustomerName());
    ...
    public GenericDataDTO downloadDocument(@PathVariable Integer ticketId) {
        ...
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        try {
            List<FileMappingListDTO> customerInventoryFileMappingList = caseUpdateService.getFilesByTicketId(Long.valueOf(ticketId));
            if(customerInventoryFileMappingList.isEmpty()){
                ...
                genericDataDTO.setResponseMessage("No Documents found for download.");
            }
            else{
                genericDataDTO.setResponseCode(HttpStatus.OK.value());
                genericDataDTO.setData(null);
    ...
    public ResponseEntity<Resource> downloadDocument(@PathVariable Integer ticketId,@PathVariable String uniqueName,@PathVariable String section) {
        ...
        try {
            CustomerTicketFileMapping customerInventoryMapping = customerTicketFileMappingRepo.findByCustomerTicketByUniqueName(uniqueName);
            if (null == customerInventoryMapping) {
                return ResponseEntity.notFound().build();
            }
            resource =  caseUpdateService.getAssignTicketDoc(customerInventoryMapping,uniqueName,section,ticketId);
    ...
    public GenericDataDTO deleteDocument(@PathVariable Integer ticketId, @PathVariable String fileName, @PathVariable String uniqueName, @PathVariable String section) {
        ...
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        try {
            CustomerTicketFileMapping customerTicketMapping = customerTicketFileMappingRepo.findByCustomerTicketByUniqueName(uniqueName);
            ...
            File file = caseUpdateService.getAssignTicketFile(customerTicketMapping, uniqueName, section, ticketId);
            if (!file.exists()) {
...
//        }
//    }
    @PreAuthorize("validatePermission(\"" + MenuConstants.Ticket.TICKET + "\")")
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_VIEW + "\")")
    ...
    @PreAuthorize("validatePermission(\"" + MenuConstants.Ticket.TICKET + "\")")
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_VIEW + "\")")
    @PostMapping(value = UrlConstants.GET_CASES_BY_CUSTOMER + "/{custId}")
    public GenericDataDTO getCasesByCustomer(@RequestBody PaginationRequestDTO requestDTO,@PathVariable Integer custId, HttpServletRequest req) {
        ...
        try {
            ...
            return caseService.getAllCaseByCustomer(requestDTO, custId);
...
//                log.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR +"fetch All  case: " + LogConstants.REQUEST_BY + getLoggedInUser().getFirstName()+ LogConstants.LOG_STATUS +LogConstants.LOG_NOT_FOUND + LogConstants.LOG_INFO + LogConstants.LOG_NO_RECORD_FOUND+LogConstants.LOG_STATUS_CODE + RESP_CODE);
    ...
    @PreAuthorize("validatePermission(\"" + MenuConstants.Ticket.TICKET_DELETE + "\")")
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_DELETE + "\")")
    ...
    @PreAuthorize("validatePermission(\"" + MenuConstants.Ticket.TICKET_EDIT + "\")")
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_EDIT + "\")")
    @Override
    ...
    public GenericDataDTO getAll(@RequestBody PaginationRequestDTO requestDTO, HttpServletRequest req) {
        ...
        try {
            ...
            if (null != genericDataDTO) {
                ApplicationLogger.logger.info("Fetching All cases:  request: { From : {}}; Response : {{}{}}", getModuleNameForLog(), genericDataDTO.getResponseCode(), genericDataDTO.getResponseCode());
                return genericDataDTO;
            } else {
                ...
                ApplicationLogger.logger.error("Unable to fetch all cases  :  request: { From : {}}; Response : {{};Exception:{}}", getModuleNameForLog(), genericDataDTO.getResponseCode(), genericDataDTO.getResponseCode());
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_VIEW + "\")")
    ...
    public GenericDataDTO assignTicketFromTeam(@PathVariable Long caseId, @RequestParam(required = false) Integer teamId, @RequestParam String remark, HttpServletRequest req) {
        ...
        try {
            ...
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
            genericDataDTO.setTotalRecords(0);
            log.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "Assign All Ticket " + LogConstants.REQUEST_BY + getLoggedInUser().getFirstName() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
    ...
    public GenericDataDTO approveTicket(@RequestParam(name = "caseId") Long caseId, @RequestParam(name = "isApproveRequest") boolean isApproveRequest, @RequestParam(name = "remarks", required = false) String remarks, HttpServletRequest req) {
        ...
        try {
            ...
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
            log.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "approve Ticket" + LogConstants.LOG_BY_NAME + caseDTO.getCaseTitle() + LogConstants.REQUEST_BY + getLoggedInUser().getFirstName() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
            return genericDataDTO;
        } catch (Exception ex) {
            genericDataDTO.setResponseMessage(ex.getMessage());
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
...
//            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_PRODUCT_MANAGEMENT, AclConstants.OPERATION_PRODUCT_MANAGEMENT_ADD, req.getRemoteAddr(), null, outwardDto.getId(), outwardDto.getOutwardNumber());
            ...
            genericDataDTO.setResponseMessage(ex.getMessage());
            RESP_CODE = HttpStatus.NOT_ACCEPTABLE.value();
            log.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "assign PickedTicket" + LogConstants.LOG_BY_NAME + caseDTO.getCaseTitle() + LogConstants.REQUEST_BY + getLoggedInUser().getFirstName() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
...
//            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_PRODUCT_MANAGEMENT, AclConstants.OPERATION_PRODUCT_MANAGEMENT_ADD, req.getRemoteAddr(), null, outwardDto.getId(), outwardDto.getOutwardNumber());
            ...
            genericDataDTO.setResponseCode(HttpStatus.NOT_ACCEPTABLE.value());
            genericDataDTO.setResponseMessage(ex.getMessage());
            log.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "assign Every Staff FromList" + LogConstants.LOG_BY_NAME + caseDTO.getCaseTitle() + LogConstants.REQUEST_BY + getLoggedInUser().getFirstName() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    @PreAuthorize("validatePermission(\"" + MenuConstants.TicketConversation.CHANGE_STATUS + "\",\"" + MenuConstants.TicketConversation.CHANGE_PRIORITY + "\",\"" + MenuConstants.TicketConversation.BULK_REASIGN + "\")")
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_EDIT + "\")")
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_EDIT + "\")")
    ...
    public GenericDataDTO reassignTicket(@RequestParam(name = "caseId") Long caseId, HttpServletRequest req) {
        GenericDataDTO genericDataDTO = new GenericDataDTO();
...
//            + AclConstants.OPERATION_CUSTOMER_GET_DOCUMENT + "\")")
    ...
    public ResponseEntity<Resource> downloadDocument(@PathVariable Long docId, @PathVariable Long ticketId, HttpServletRequest req) {
        ...
        try {
            ...
            Customers customers = customerRepository.findById(caseDTO.getCustomersId()).orElse(null);
            if (null == caseDTO) {
                RESP_CODE = APIConstants.NOT_FOUND;
    ...
    @PreAuthorize("validatePermission(\"" + MenuConstants.Ticket.TICKET_ETR + "\")")
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_EDIT + "\")")
        ...
        try {
            ...
            if (null != genericDataDTO) {
                ...
                if (genericDataDTO.getDataList().isEmpty()) {
                    genericDataDTO = new GenericDataDTO();
                    genericDataDTO.setResponseCode(APIConstants.NO_CONTENT);
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_EDIT + "\")")
    ...
    public GenericDataDTO linkTicket(@RequestBody List<Integer> childTickets, @RequestParam(name = "linkTicketId") Integer linkTicketId, HttpServletRequest req) {
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        TraceContext traceContext = tracer.currentSpan().context();
        Integer RESP_CODE = APIConstants.FAIL;
        MDC.put("type", "Update");
...
//            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_PRODUCT_MANAGEMENT, AclConstants.OPERATION_PRODUCT_MANAGEMENT_ADD, req.getRemoteAddr(), null, outwardDto.getId(), outwardDto.getOutwardNumber());
            ...
            genericDataDTO.setResponseCode(HttpStatus.NOT_ACCEPTABLE.value());
            genericDataDTO.setResponseMessage(ex.getMessage());
            log.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "Fetch child Tickets  with id : " + caseId + LogConstants.REQUEST_BY + getLoggedInUser().getFirstName() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CASE_ALL + "\",\"" + AclConstants.OPERATION_CASE_ADD + "\")")
    ...
    @GetMapping("/count")
    public ResponseEntity<Map<String, Integer>> getTicketCount(@RequestParam(name = "staffId",required = true) Integer staffId) {
        int count = caseService.getMyInProgressTicketCount(staffId);
