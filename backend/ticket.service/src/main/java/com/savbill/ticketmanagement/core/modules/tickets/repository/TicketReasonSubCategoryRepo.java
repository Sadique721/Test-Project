...
public interface TicketReasonSubCategoryRepo extends JpaRepository<TicketReasonSubCategory, Long>, QuerydslPredicateExecutor<TicketReasonSubCategory> {
    ...
    @Query(value = "select * from tblmticketreasonsubcategory c join tbltticketsubcategoryreasoncategorymapping cm on cm.ticket_reason_sub_category_id = c.id where cm.ticket_reason_category_id IN :categoryIds and c.is_deleted = false and c.is_default_sub_problem_domain = true and c.BUID =:buId" , nativeQuery = true)
    List<TicketReasonSubCategory> findAllDefualtReasonSubCategoryUsingCategoryId(@Param("categoryIds") List<Integer> categoryIds , @Param("buId") Long buId);

    @Query(value = "select * from tblmticketreasonsubcategory c join tbltticketsubcategoryreasoncategorymapping cm on cm.ticket_reason_sub_category_id = c.id where cm.ticket_reason_category_id IN :categoryIds and c.is_deleted = false and c.is_default_sub_problem_domain = true" , nativeQuery = true)
    List<TicketReasonSubCategory> findAllDefualtReasonSubCategoryUsingCategoryId(@Param("categoryIds") List<Integer> categoryIds);

    List<TicketReasonSubCategory> findByIdIn(List<Long> ids);
