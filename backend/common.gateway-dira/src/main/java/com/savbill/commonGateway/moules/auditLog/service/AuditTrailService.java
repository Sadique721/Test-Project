package com.savbill.commonGateway.moules.auditLog.service;
...
import com.savbill.commonGateway.moules.auditLog.Constants.AuditLogConstants;

import com.savbill.commonGateway.moules.auditLog.model.AuditTrailResponseModel;
import com.savbill.commonGateway.moules.auditLog.model.CustomObject;
...
//import org.javers.repository.jql.Query;
        ...
        // Add byObject to the response map
         List<AuditTrailResponseModel> byObjects = changes.groupByCommit().stream()
                    ...
                    return byCommit.groupByObject().stream().map(byObject -> {
                        ...
                        if(snapshotResponse.get(0).equalsIgnoreCase("") || snapshotResponse.get(0).equalsIgnoreCase(" ")){auditTrail.setActionType("Delete");}
                        ...
                        auditTrail.setModuleName(CommonConstants.MODULE.MODULE_COMMON);
    ...
    public HashMap<String, Object> getOldAuditTrails(Changes changes, String moduleName, Integer pageIndex, Integer pageSize) {
        ...
        changes.groupByCommit().stream().filter(changesByCommit -> changesByCommit.getCommit().getProperties().get(AuditLogConstants.MVNOID).equals(String.valueOf(mvnoId))).forEach(byCommit -> {
            ...
            byCommit.groupByObject().forEach(byObject -> {
                ...
                if (moduleName.equals(CommonConstants.MODULE.MODULE_COMMON)) {
                    // Get property value based on global ID, module name, and commit ID
                    String propertyValue = getPropertyValue(byObject.getGlobalId().value(), entityType, byCommit.getCommit().getId());
                    // Iterate through property changes of the object
                    byObject.getPropertyChanges().forEach(propertyChange -> {
            ...
            byCommit.groupByObject().forEach(byObject -> {
                ...
                byObject.getPropertyChanges().forEach(propertyChange -> {
                    ...
                    if (!propertyChangeObj.get(AuditLogConstants.CHANGETYPE).getAsString().equals(AuditLogConstants.LISTCHANGE)) {
                        ...
                        auditTrail.setNewValue(propertyChange.getRight() != null ? propertyChange.getRight().toString() : "");
                        // Add the audit trail to the list
                        if(getLoggedInMvnoId()==auditTrail.getMvnoId()){
                            auditTrails.add(auditTrail);
                        }
...
//            }
    ...
    private String getActionType(String changeType, JsonObject propertyChangeObj, PropertyChange propertyChange) {
        switch (changeType) {
            ...
            case AuditLogConstants.TERMINALVALUE_CHANGE:
                return AuditLogConstants.DELETE;
            case AuditLogConstants.LIST_CHANGE:
            case AuditLogConstants.SET_CHANGE:
                return AuditLogConstants.UPDATE;
            default:
                return null;
    ...
    public List<String> getSnapshot(Changes changes, ChangesByObject byObject) {
        ...
        byObject.getPropertyChanges().forEach(propertyChange -> {
            ...
            if (!changeType.equals(AuditLogConstants.LISTCHANGE) || entityName.equals("BusinessVerticals")) {
                AuditTrailResponseModel auditTrail = new AuditTrailResponseModel();
                auditTrail.setModuleName(CommonConstants.MODULE.MODULE_COMMON);
                ...
                if(propertyChange.getPropertyName().contains("Time")){
                    if(oldObject!=null && isValidDate(oldObject.toString())){
                        DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
                        LocalDateTime formattedDateTime = LocalDateTime.parse(String.valueOf(oldObject));
                        oldObject = formattedDateTime.format(outputFormatter);
                    }
                    if(newObject!=null && isValidDate(newObject.toString())){
                        DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
                        LocalDateTime formattedDateTime = LocalDateTime.parse(String.valueOf(newObject));
                        newObject = formattedDateTime.format(outputFormatter);
                    }
                }
...
//        snapshotResponse.put("snapshot", snapshots);
    ...
    public static boolean isValidDate(String inDate) {
        try {
            LocalDateTime.parse(String.valueOf(inDate));
            return true;
...
//                        // Convert property change to JSON object
                            JsonObject propertyChangeObj = javers.getJsonConverter().toJsonElement(propertyChange).getAsJsonObject();
                            // Check if the module name matches the specified moduleName
                            if (moduleName.equalsIgnoreCase(CommonConstants.MODULE.MODULE_COMMON)) {
                                // Get property value based on global ID, module name, and commit ID
                                String propertyValue = getPropertyValue(byObject.getGlobalId().value(), entityType, byCommit.getCommit().getId());
                                String changeType = propertyChangeObj.get(AuditLogConstants.CHANGETYPE).getAsString();
                            ...
                            if (moduleName.equalsIgnoreCase(CommonConstants.MODULE.MODULE_COMMON)) {
                                ...
                                for (String pair : pairs) {
                                    ...
                                    if (keyValue.length == 2) {
                                        ...
                                        int commitMvnoId = byCommit.getCommit().getProperties().get(AuditLogConstants.MVNOID) != null ? Integer.parseInt(byCommit.getCommit().getProperties().get(AuditLogConstants.MVNOID)) : 0;
                                        if (commitMvnoId == currentMvnoId){
                                        if ((key.contains("name") || key.contains("pincode")) && value1.contains(entityName)) {
                                            AuditTrailResponseModel auditTrail = new AuditTrailResponseModel();
...
//    public String getEntityNameFromInstence(String className, Long id) {
//
//        switch (className) {
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.COUNTRY): {
//                try {
//                    Country country = auditUtil.findEntityById(className, id);
...
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.STATE): {
//                try {
//                    State state = auditUtil.findEntityById(className, id);
...
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.CITY): {
//                try {
//                    City city = auditUtil.findEntityById(className, id);
//                    return city.getName();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.PINCODE): {
//                try {
//                    Pincode pincode = auditUtil.findEntityById(className, id);
...
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.AREA): {
//                try {
//                    Area area = auditUtil.findEntityById(className, id);
//                    return area.getName();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.SERVICE_AREA): {
//                try {
//                    ServiceArea serviceArea = auditUtil.findEntityById(className, id);
...
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.INVESTMENT_CODE): {
//                try {
//                    InvestmentCode investmentCode = auditUtil.findEntityById(className, id);
//                    return investmentCode.getIcname();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.BUSINESS_UNIT): {
//                try {
//                    BusinessUnit businessUnit = auditUtil.findEntityById(className, id);
//                    return businessUnit.getBuname();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.SUB_BUSINESS_UNIT): {
//                try {
//                    SubBusinessUnit subBusinessUnit = auditUtil.findEntityById(className, id);
//                    return subBusinessUnit.getSubbuname();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.BANK_MANAGEMENT): {
//                try {
//                    BankManagement bankManagement = auditUtil.findEntityById(className, id);
//                    return bankManagement.getBankname();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.BRANCH_MANAGEMENT): {
//                try {
//                    Branch branch = auditUtil.findEntityById(className, id);
...
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.REGION_MANAGEMENT): {
//                try {
//                    Region region = auditUtil.findEntityById(className, id);
//                    return region.getRname();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.BUSINESS_VERTICLE): {
//                try {
//                    BusinessVerticals businessVerticals = auditUtil.findEntityById(className, id);
//                    return businessVerticals.getVname();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.SUB_BUSINESS_VERTICLE): {
//                try {
//                    SubBusinessVertical subBusinessVertical = auditUtil.findEntityById(className, id);
//                    return subBusinessVertical.getSbvname();
//                } catch (ClassNotFoundException e) {
//                    throw new RuntimeException(e);
//                }
//
//            }
//            case (CommonConstants.CLASS_NAME_FOR_SEARCH.DEPARTMENT): {
//                try {
//                    Department department = auditUtil.findEntityById(className, id);
...
//        }
//    }
    public List<Object> getChangesFromDto(PaginationRequestDTO dto) throws ClassNotFoundException {
        ...
        if (dto.getFilters() != null && !dto.getFilters().isEmpty()) {
            for (GenericSearchModel searchModel : dto.getFilters()) {
                ...
                String filterValue = searchModel.getFilterValue().trim();
                switch (filterColumn) {
                    case AuditLogConstants.AUDIT_USER:
                        changes = javers.findChanges(QueryBuilder
                                .anyDomainObject()
                                .byAuthorLikeIgnoreCase(filterValue)
                                .limit(dto.getPageSize())
                                .skip(dto.getPage() * dto.getPageSize())
                                .build());
                        totalRecords = javers.findSnapshots(QueryBuilder
                                        .anyDomainObject()
                                        .byAuthorLikeIgnoreCase(filterValue)
                                ...
                                .size();
                        break;
                    case AuditLogConstants.AUDIT_ENTITY_NAME:
                        String entityName = geEntityNameBYPath(filterValue);
                        changes = javers.findChanges(QueryBuilder
                                .byClass(Class.forName(entityName))
                                .limit(dto.getPageSize())
                                .skip(dto.getPage() * dto.getPageSize())
                                .build());
                        totalRecords = javers.findSnapshots(QueryBuilder
                                ...
                                .size();
                        break;
                    case AuditLogConstants.AUDIT_ACTION_TYPE:
                        SnapshotType snapshotType = SnapshotType.valueOf(filterValue.toUpperCase());
                        changes = javers.findChanges(QueryBuilder
                                .anyDomainObject()
                        ...
                        totalRecords = javers.findSnapshots(QueryBuilder
                                ...
                                .size();
                        break;
                    case AuditLogConstants.IP_ADDRESS:
                        changes = javers.findChanges(QueryBuilder
                                .anyDomainObject()
                                .withCommitPropertyLike(AuditLogConstants.IP_ADDRESS, filterValue)
                                .limit(dto.getPageSize())
                                .skip(dto.getPage() * dto.getPageSize())
                                .build());
                        totalRecords = javers.findSnapshots(QueryBuilder
                    ...
                    case "startDate":
                    case "endDate":
                        if (filterColumn.equals("startDate")) {
                        ...
                        break;
                    default:
                        throw new IllegalArgumentException("Unsupported fileter column: " + filterColumn);
                }
            }
        }
        list.add(changes);
    ...
    private String geEntityNameBYPath(String filterValue) {
        String path = null;
        switch (filterValue){
            case "Role":
                return path = "com.savbill.commonGateway.moules.SettingsManagement.RoleManagement." + filterValue;
            case "RoleACLEntry":
                return path = "com.savbill.commonGateway.moules.acl.domain." + filterValue;
            case "StaffUser":
                return path = "com.savbill.commonGateway.moules.SettingsManagement.StaffUserManagement." + filterValue;
            case "StaffUserLocationMapping":
                return path = "com.savbill.commonGateway.moules.SettingsManagement.StaffUserLocationMapping." + filterValue;
            default:
                throw new RuntimeException("Please Enter Valid Entity Name.");
        }
    }
}
