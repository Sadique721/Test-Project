...
public class SaveStaffUserSharedDataMessage {
    ...
    public  SaveStaffUserSharedDataMessage(StaffUser staffUser){
        ...
        this.tacacsAccessLevelGroup = staffUser.getTacacsAccessLevelGroup();
        ...
        if(staffUser.getDepartment() != null){
            this.departmentId = staffUser.getDepartment();
        }

    }

    public  SaveStaffUserSharedDataMessage(StaffUser staffUser, String name){
        ...
        this.businessUnitNameList = staffUser.getBusinessUnitNameList();
        this.serviceAreaNameList = staffUser.getServiceAreaNameList();
        if(staffUser.getServiceAreaNameList()!=null){
            for(ServiceArea serviceArea : staffUser.getServiceAreaNameList()){
                ServiceArea servArea = new ServiceArea();
                servArea.setId(serviceArea.getId());
                servArea.setAreaId(serviceArea.getAreaId());
                servArea.setMvnoId(serviceArea.getMvnoId());
                servArea.setIsDeleted(serviceArea.getIsDeleted());
                servArea.setName(serviceArea.getName());
                servArea.setCityid(serviceArea.getCityid());
                servArea.setStatus(serviceArea.getStatus());
                servArea.setLatitude(serviceArea.getLatitude());
                servArea.setLongitude(serviceArea.getLongitude());
                this.serviceAreasList.add(servArea);
            }
            this.serviceAreaNameList = serviceAreasList;
        }
        this.countryCode = staffUser.getCountryCode();
        this.phone = staffUser.getPhone();
        this.email =staffUser.getEmail();
