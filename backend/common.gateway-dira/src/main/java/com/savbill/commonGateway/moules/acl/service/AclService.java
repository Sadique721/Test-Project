...
import com.savbill.commonGateway.moules.acl.domain.AclMenuQueryEntry;
import com.savbill.commonGateway.moules.acl.domain.CustomACLEntry;
import com.savbill.commonGateway.moules.acl.mapper.AclClassMapper;
import com.savbill.commonGateway.moules.acl.mapper.AclMenuMapper;
import com.savbill.commonGateway.moules.acl.model.*;
import com.savbill.commonGateway.moules.acl.model.*;
import com.savbill.commonGateway.moules.acl.repository.AclClassRepository;
import com.savbill.commonGateway.moules.acl.repository.AclMenuRepository;
import com.savbill.commonGateway.moules.acl.repository.CustomACLEntryRepository;
...
public class AclService {
     ...
     * @return
     */
    public List<AclMenuStructureDTO>  createAclMenuStructure(String productName) {
        List<AclMenu> aclMenus = aclMenuRepository.findAllByIsDeleteFalseAndProduct(productName.toUpperCase());
        List<AclMenuDtoNew> aclMenuDtoNew = aclMenuMapper.domainToDTO(aclMenus, new CycleAvoidingMappingContext());
        List<AclMenuDtoNew> parent = aclMenuDtoNew.stream().filter(aclMenu -> aclMenu.getParentid()!=null && aclMenu.getParentid().equals(0L)).collect(Collectors.toList());
        return getChildMenuData(parent, aclMenuDtoNew);
    }

    public List<AclMenuStructureDTO>  createAclMenuStructure(String productName, List<Long> menuIds) {
        List<AclMenu> aclMenus = aclMenuRepository.findAllByIsDeleteFalseAndProductAndIdIn(productName.toUpperCase(), menuIds);
        List<AclMenuDtoNew> aclMenuDtoNew = aclMenuMapper.domainToDTO(aclMenus, new CycleAvoidingMappingContext());
        List<AclMenuDtoNew> parent = aclMenuDtoNew.stream().filter(aclMenu -> aclMenu.getParentid().equals(0L)).collect(Collectors.toList());
        if(productName.equals(Constants.IWF)) {
          parent = parent.stream().sorted(Comparator.comparing(AclMenuDtoNew::getPosition)).filter(aclMen -> aclMen.getProduct().equals(Constants.IWF)).collect(Collectors.toList());
        }
        return getChildMenuData(parent, aclMenuDtoNew);
    }

    /**
     * Method to fetch all iwf menu data and return menu structure
     * @return
     */

//    public List<AclMenuStructureDTO> createIwfAclMenuStructure(String productName) {
//        List<AclMenu> aclMenus = aclMenuRepository.findAllByIsDeleteFalseAndProduct(productName);
//        List<AclMenuDtoNew> aclMenuDtoNew = aclMenuMapper.domainToDTO(aclMenus, new CycleAvoidingMappingContext());
//        List<AclMenuDtoNew> parent = aclMenuDtoNew.stream().filter(aclMenu -> aclMenu.getParentid().equals(0L)).collect(Collectors.toList());
//        return getChildMenuData(parent, aclMenuDtoNew);
//    }
    ...
    public List<AclMenuStructureDTO> getChildMenuData(List<AclMenuDtoNew> parentDto, List<AclMenuDtoNew> aclMenuDtoNew) {
        if(!CollectionUtils.isEmpty(parentDto)) {
            ...
            for (AclMenuDtoNew aclDto: parentDto) {
                ...
                List<AclMenuDtoNew> childdto = aclMenuDtoNew.stream().filter(aclMenu -> aclMenu.getParentid()!=null && aclMenu.getParentid().equals(aclDto.getId())).collect(Collectors.toList());
...
//                        parentDTO.setSubmenu(childList);
    ...
    public List<RoleACLEntryDTO> fetchRoleAclEntryByRoleId(Long aLong) {
        List<RoleACLEntryDTO> roleACLEntries = roleAclRepository.findAClByRole(aLong);
//        List<RoleACLEntryDTO> roleACLEntryDTOS = roleACLEntries.stream().map(roleACLEntry ->
//                new RoleACLEntryDTO(roleACLEntry.getId(), roleACLEntry.getCode(), roleACLEntry.getMenuid(),roleACLEntry.getRole().getId().intValue())).collect(Collectors.toList());
        return roleACLEntries;
    }
}
