package com.savbill.commonGateway.moules.MasterManagement.BusinessUnit.repository;
...
//    Integer deleteVerify(@Param("id")Integer id);
    @Query(value = "select count(*) as tab from tbltstaffbusinessunitrel t1  where t1.businessunitid =:id ", nativeQuery = true)
    Integer deleteVerify(@Param("id") Integer id);

    @Query(value = "select count(*) as tab from tblmsubbusinessunit t1  where t1.businessunitid =:id and is_deleted=false", nativeQuery = true)
    Integer deleteVerifyForSubBusinessunit(@Param("id") Integer id);

    List<BusinessUnit> findAllByIdIn(List<Long> buids);

    List<BusinessUnit> findAllByIsDeletedIsFalseAndStatusAndIdIn(String status, List<Long> id);
    List<BusinessUnit> findAllByIsDeletedIsFalseAndStatusAndIdInAndMvnoIdIn(String status, List<Long> id, List<Integer> mvnoId);

    List<BusinessUnit> findAllByIsDeletedIsFalseAndStatus(String status);
    List<BusinessUnit> findAllByIsDeletedIsFalseAndStatusAndMvnoIdIn(String status, List<Integer> mvnoId);


    @Query(value = "select b.buname from BusinessUnit b where b.id in (:buid)")
    List<String>findAllByBusinessUnitnameById(List<Long>buid);
