package com.savbill.commonGateway.security.filter;
...
public class MyFilter extends GenericFilterBean {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		TraceContext traceContext = tracer.currentSpan().context();
			...
			ctx.addZuulRequestHeader(TRACE_ID,traceContext.traceIdString());
			...
			request.setAttribute(TRACE_ID,traceContext.traceIdString());
		}
		chain.doFilter(request, response);
	}

	private String getTokenFromHeaders(HttpServletRequest request) {
		String authHeader = request.getHeader(CommonConstants.AUTHORIZATION_HEADER_STRING);
		String apiKeyHeader = request.getHeader(CommonConstants.APIKEY_HEADER_STRING);
