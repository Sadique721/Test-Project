package com.savbill.commonGateway.moules.MasterManagement.State.repository;
...
public interface StateRepository extends JpaRepository<State, Integer>, QuerydslPredicateExecutor<State> {
    ...
    @Query("SELECT s FROM State s WHERE s.status = :status AND s.isDeleted = false " +
            "AND (s.mvnoId = :mvnoId OR s.mvnoId IS NULL OR s.mvnoId = 1 OR :mvnoId = 1) " +
            "ORDER BY s.id DESC")
    List<State> findAllByStatusAndMvnoId(@Param("status") String status, @Param("mvnoId") Integer mvnoId);
    ...
    Long countByNameAndIsDeletedIsFalseAndCountry_Id(String name, Integer country_id);
    Long countByNameAndIsDeletedIsFalseAndMvnoIdInAndCountry_Id(String name, List<Integer> mvnoId, Integer country_id);
    Long countByNameAndIdAndIsDeletedIsFalseAndCountry_Id(String name, Integer id, Integer country_id);
    Long countByNameAndIdAndIsDeletedIsFalseAndMvnoIdInAndCountry_Id(String name, Integer id, List<Integer> mvnoId, Integer country_id);


    @Query("SELECT new com.savbill.commonGateway.moules.MasterManagement.State.model.StatePojo(s.id, s.name, s.status, s.isDeleted, s.mvnoId, s.id, s.name) " +
            "FROM State s " +
            "WHERE s.status = :status AND s.isDeleted = false " +
            "AND (s.mvnoId = :mvnoId OR s.mvnoId IS NULL OR s.mvnoId = 1 OR :mvnoId = 1) " +
            "ORDER BY s.id DESC")
    List<StatePojo> findAllActiveStatesByStatusAndMvnoId(@Param("status") String status, @Param("mvnoId") Integer mvnoId);

    @Query(value = "SELECT name from State where id=:id")
