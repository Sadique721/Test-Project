package com.savbill.revenuemanagement.productmanagement.TimeBasePolicy.repository;
...
public interface TimeBasePolicyRepository extends JpaRepository<TimeBasePolicy, Long>  {
    ...
    @Query(value = "select * from tblmtimebasepolicy t where t.isdeleted = false"
            , countQuery = "select count(*) from tblmtimebasepolicy t where t.isdeleted = false"
            , nativeQuery = true)
