...
//    @GeneratedValue(strategy = GenerationType.IDENTITY)
    ...
    @Column(name = "discountexpdate", nullable = false, length = 40)
    ...
    public CustomerChargeHistory(CustomerChargeHistoryRevenue customerChargeHistory,Customers customers,boolean isnextBilldate,LocalDate maxDate,String type){
        ...
        this.discountExpDate=maxDate;
        if (customers.getCusttype().equalsIgnoreCase(CommonConstants.CUST_TYPE_POSTPAID)) {
            if (customerChargeHistory.getChargeType().equalsIgnoreCase(CommonConstants.CHARGE_TYPE_ADVANCE) && type!=null && type.equalsIgnoreCase("renew")){
                this.setNextBillDate(customers.getNextBillDate());
                this.setLastBillDate(customers.getNextBillDate());
            } else if (customerChargeHistory.getChargeType().equalsIgnoreCase(CommonConstants.CHARGE_TYPE_ADVANCE)) {
                if (!isnextBilldate) {
                    this.setLastBillDate(LocalDate.now());
                    if(customerChargeHistory.getBillingCycle() > 1)
                        this.setNextBillDate(LocalDate.now().plusMonths(customerChargeHistory.getBillingCycle()).withDayOfMonth(customers.getBillday()));
                    else
                        this.setNextBillDate(customers.getNextBillDate());
                }else {
            ...
            if (customerChargeHistory.getChargeType().equalsIgnoreCase(CommonConstants.CHARGE_TYPE_RECURRING)) {
                if (!isnextBilldate) {
                    ...
                    this.setNextBillDate(customers.getNextBillDate().plusMonths(customerChargeHistory.getBillingCycle()).withDayOfMonth(customers.getBillday()));
                    this.setLastBillDate(customers.getNextBillDate());
                }
            }
            if (customerChargeHistory.getChargeType().equalsIgnoreCase(CommonConstants.CHARGE_TYPE_NONRECURRING)) {
                this.setLastBillDate(LocalDate.now());
                this.setNextBillDate(LocalDate.now());
            }
        }
    }
