package com.savbill.revenuemanagement.core.auditLog.service;



import com.savbill.revenuemanagement.core.auditLog.Constants.AuditLogConstants;
import com.savbill.revenuemanagement.core.auditLog.domain.AuditLogEntry;
import com.savbill.revenuemanagement.core.auditLog.domain.QAuditLogEntry;
import com.savbill.revenuemanagement.core.auditLog.mapper.AuditLogMapper;
import com.savbill.revenuemanagement.core.auditLog.model.AuditLogEntryDTO;
import com.savbill.revenuemanagement.core.auditLog.model.AuditLogSearchRequestDTO;
import com.savbill.revenuemanagement.core.auditLog.queryScript.AuditSearchQueryScript;
import com.savbill.revenuemanagement.core.auditLog.repository.AuditLogRepository;
...
import com.savbill.revenuemanagement.core.constants.PGConstants;
...
import com.savbill.revenuemanagement.core.dto.common.GenericIdModel;
...
import com.savbill.revenuemanagement.core.util.PropertyReaderUtil;
...
public class AuditLogService extends ExBaseAbstractService<AuditLogEntryDTO, AuditLogEntry, Long> {
    ...
    public void addAuditEntry(String module,
                              ...
                              Integer invoiceID,
                              Integer debitDocGraceDays) throws Exception {
        ...
        auditLogEntry.setUserName(loggedInUser.getUsername());
        ...
        if (remark == null)
            auditLogEntry.setRemark(createRemark(module, operation, loggedInUser.getFullName(), refName,invoiceID));
        else
            auditLogEntry.setRemark(remark);
        auditLogEntry.setEntityRefId(refId);
        auditLogEntry.setPartnerId(loggedInUser.getPartnerId());
        auditLogEntry.setInvoiceID(invoiceID);
        auditLogEntry.setInvoiceNumber(invoiceNumber);
        auditLogEntry.setDebitDocGraceDays(debitDocGraceDays);
        AuditLogEntryDTO dto=saveEntity(auditLogEntry);

        kafkaMessageSender.send(new KafkaMessageData(dto,AuditLogEntryDTO.class.getSimpleName()));
    }
    public void addTranferAuditEntry(String module,
                              String operation,
                              String ipAddress,
                              String remark,
                              Long refId,
                              Long tocustId, Double amount
    ) throws Exception {
        LoggedInUser loggedInUser = (LoggedInUser) SecurityContextHolder.getContext()
                .getAuthentication().getPrincipal();
        AuditLogEntryDTO auditLogEntry = new AuditLogEntryDTO();
        ...
        AuditLogEntryDTO baseDto = saveEntity(auditLogEntry);
        baseDto.setAmount(amount);



// sender
        AuditLogEntryDTO sender = new AuditLogEntryDTO();
        BeanUtils.copyProperties(baseDto, sender);
        sender.setIsSender(true);
        sender.setToCustomerId(tocustId);

        kafkaMessageSender.send(new KafkaMessageData(sender, "AuditLogTransferDTO"));

// receiver
        AuditLogEntryDTO receiver = new AuditLogEntryDTO();
        BeanUtils.copyProperties(baseDto, receiver);
        receiver.setIsSender(false);
        receiver.setEntityRefId(tocustId);
        receiver.setToCustomerId(refId);

        kafkaMessageSender.send(new KafkaMessageData(receiver, "AuditLogTransferDTO"));
    ...
    private String createRemark(String module, String operation, String user, String objName, Integer invoiceID) {
...
//            Element el = opCache.get(operation);
    ...
    public GenericDataDTO getAllEntitiesbyEntityrefId(Long custId, PaginationRequestDTO paginationRequestDTO) {
        QAuditLogEntry qAuditLogEntry = QAuditLogEntry.auditLogEntry;
        BooleanExpression booleanExpression = qAuditLogEntry.isNotNull().and(qAuditLogEntry.entityRefId.eq(custId));
    ...
    public GenericDataDTO getSearchAudit(Long entityId, PaginationRequestDTO paginationRequestDTO) {
        QAuditLogEntry qAuditLogEntry = QAuditLogEntry.auditLogEntry;

        // Start with a base predicate
        BooleanExpression predicate = qAuditLogEntry.isNotNull().and(qAuditLogEntry.entityRefId.eq(entityId));


        if (paginationRequestDTO.getFilters() != null) {
            for (GenericSearchModel filter : paginationRequestDTO.getFilters()) {
                if (filter.getFilterColumn() != null && filter.getFilterValue() != null) {
                    String column = filter.getFilterColumn().trim().toLowerCase();
                    String value = filter.getFilterValue().trim();

                    switch (column) {
                        case "username":
                            predicate = predicate.and(qAuditLogEntry.userName.containsIgnoreCase(value));
                            break;
                        case "invoicenumber":
                            predicate = predicate.and(qAuditLogEntry.invoiceNumber.containsIgnoreCase(value));
                            break;
                        default:
        ...
        Page<AuditLogEntry> audit = auditLogRepository.findAll(predicate, pageRequest);

        GenericDataDTO genericDataDTO = new GenericDataDTO();
        return makeGenericResponse(genericDataDTO, audit);
    }
}
