...
//    }
    /**
     * Creates a Kafka producer for sending messages with String keys and KafkaMessageData values.
     * The producer is configured with various settings such as retries, timeouts, batch size, and compression type.
     *
    ...
    public KafkaProducer<String, KafkaMessageData> kafkaProducer() {
        ...
        config.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
        config.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class);
        config.put(ProducerConfig.BATCH_SIZE_CONFIG, 100000); // Default Kafka batch size
        config.put(ProducerConfig.RETRIES_CONFIG, 1); // Increased retries
        config.put(ProducerConfig.RETRY_BACKOFF_MS_CONFIG, 1000);
        config.put(ProducerConfig.ACKS_CONFIG, "all");
        config.put(ProducerConfig.LINGER_MS_CONFIG, 10);
        config.put(ProducerConfig.COMPRESSION_TYPE_CONFIG, "snappy");
        config.put(ProducerConfig.DELIVERY_TIMEOUT_MS_CONFIG, 3000); // 2 minutes
        config.put(ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG, 2000);
        config.put(ProducerConfig.TRANSACTION_TIMEOUT_CONFIG, 4000);
