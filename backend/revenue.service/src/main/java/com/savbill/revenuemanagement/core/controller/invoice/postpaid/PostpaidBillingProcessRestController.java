...
@Api(value = "SavbillBillingEngine postpaid Management", description = "REST APIs related to PostpaidBilling Entity!!!!", tags = "PostpaidBilling")
@RestController
@RequestMapping(UrlConstants.BASE_API_URL+"/postPaid")
@CrossOrigin
public class PostpaidBillingProcessRestController {
    ...
    public ResponseObject generatebill(@PathVariable("nextbilldate") String nextbilldate) {
        ...
        System.out.println("*********.29 server logs for invoice generation**************" + nextbilldate);
        System.out.println("*********.Sysytem Date************** " + LocalDateTime.now());

        synchronized (PostpaidBillingProcessRestController.class){
        response = postpaidInvoiceService.createPostpaidInvoice(nextbilldate,false);}
...
//    }
    ...
    @PreAuthorize("validatePermission(\"" + MenuConstants.postpaid_gene_bill_run +  "\" ,\"" + MenuConstants.postpaid_gene_trial_bill_run  + "\")")
    @RequestMapping(value = "/generateEarlybill/{earlybilldate}", method = RequestMethod.GET)
    public ResponseObject generateEarlybill(@PathVariable("earlybilldate") String earlybilldate) {
        ResponseObject response = new ResponseObject();
        logger.debug("[BillingProcessHelper] billgenerate() called with " + earlybilldate +" time: "+new Date().getTime());

        synchronized (PostpaidBillingProcessRestController.class){
            response = postpaidInvoiceService.createPostpaidInvoice(earlybilldate,true);}
        logger.debug("[BillingProcessHelper] billgenerate() end " + earlybilldate+" time: "+new Date().getTime());
        return response;
    }

    @RequestMapping(value = "/generateEarlybillBySchedular", method = RequestMethod.GET)
    public ResponseObject generateEarlybillBySchedular() {
        ResponseObject response = new ResponseObject();
        logger.debug("[BillingProcessHelper] billgenerate() called with " + LocalDate.now() +" time: "+new Date().getTime());

        logger.debug("[BillingProcessHelper] billgenerate() end " + LocalDate.now()+" time: "+new Date().getTime());
//        synchronized (PostpaidBillingProcessRestController.class){
//            postpaidInvoiceService.postpaidInvoiceScheduler();}
        postpaidInvoiceService.postpaidInvoiceScheduler();
        return response;
    }
}
