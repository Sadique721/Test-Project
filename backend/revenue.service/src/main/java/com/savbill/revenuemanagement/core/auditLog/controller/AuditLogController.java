package com.savbill.revenuemanagement.core.auditLog.controller;
        ...
        import com.savbill.revenuemanagement.core.constants.AclConstants;
...
public class AuditLogController extends ExBaseAbstractController<AuditLogEntryDTO> {
    ...
    public GenericDataDTO getAuditLogByParam(@RequestBody AuditLogSearchRequestDTO reqDTO) {
        ...
        this.MAX_PAGE_SIZE = Integer.parseInt(clientServiceSrv.getClientSrvByName
                (ClientServiceConstant.MAX_PAGE_SIZE).getValue());
        try {
            if (null == reqDTO) {
...
//            if (null != auditFor) {
//                if (auditFor.equalsIgnoreCase(AuditLogConstants.AUDIT_FOR_INVOICE))
//                    return GenericDataDTO.getGenericDataDTO(billRunService.getBillListForAuditFor());
...
//            }
    ...
    @PostMapping("/getAuditList/{cust_id}")
...
//            + MenuConstants.PostpaidCustomers.POSTPAID_CUSTOMER_SHIFT_LOCATION+ "\")")
    public GenericDataDTO getAuditForList(@PathVariable Long cust_id,@RequestBody PaginationRequestDTO paginationRequestDTO) {
        String SUBMODULE = getModuleNameForLog() + " [getAuditForList()] ";
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        ...
        try {
            if (null == cust_id) {
                ...
                genericDataDTO.setResponseCode(HttpStatus.NOT_ACCEPTABLE.value());
                logger.info("Unable to fetch Auditlog for "+cust_id+" :  request: { Response : {{}}", APIConstants.NULL_VALUE);
                return genericDataDTO;
            }
            genericDataDTO = auditLogService.getAllEntitiesbyEntityrefId(cust_id, paginationRequestDTO);
            ...
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
            logger.error("Unable to fetch Auditlog for"+cust_id+":  request: { Response : {{}};Error :{} ;Exception:{};",HttpStatus.NOT_ACCEPTABLE,APIConstants.ERROR_MESSAGE,ex.getStackTrace());
        }
        return genericDataDTO;
    }
    ...
    public GenericDataDTO getSearchAudit(@PathVariable Long entityIds, @RequestBody PaginationRequestDTO paginationRequestDTO) {
        ...
        try {
            ...
            genericDataDTO = auditLogService.getSearchAudit(entityIds, paginationRequestDTO);
