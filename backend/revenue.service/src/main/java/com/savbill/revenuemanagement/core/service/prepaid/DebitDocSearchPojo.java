...
public class DebitDocSearchPojo {
    ...
    private boolean isBoosterplan;

    private  Integer debitDocGraceDays;

    private List<Integer> custServiceMappingId;
    ...
    public DebitDocSearchPojo(DebitDocument debitDocument, boolean isBoosterplan) {
        this.custid = debitDocument.getCustomer().getId();
        this.customerName = debitDocument.getCustomer().getCustname();
        this.accountNumber = debitDocument.getCustomer().getAcctno();
        this.firstName = debitDocument.getCustomer().getFirstname();
        this.lastName = debitDocument.getCustomer().getLastname();
        this.paymentStatus = debitDocument.getPaymentStatus();
        if(debitDocument.getAdjustedAmount() != null) {
            this.adjustedAmount = (debitDocument.getAdjustedAmount() * 100) / 100; /**Invoice amount only 2 digit after point allowed**/
        }
        else{
        ...
        this.ispPayloadStatusCode=debitDocument.getIspPayloadStatusCode();
        this.qrCode = debitDocument.getQrCode();
        this.isBoosterplan = isBoosterplan;
        if(debitDocument.getDebitDocGraceDays() != null){
            this.debitDocGraceDays = debitDocument.getDebitDocGraceDays();
        }
    }
