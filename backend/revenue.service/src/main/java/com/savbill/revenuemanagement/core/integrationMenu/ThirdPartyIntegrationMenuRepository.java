...
public interface ThirdPartyIntegrationMenuRepository extends JpaRepository<ThirdPartyIntegrationMenu,Long>, JpaSpecificationExecutor<ThirdPartyIntegrationMenu> {
    ...
    @Query(nativeQuery = true, value = "SELECT t.client_name FROM tblmthirdpartymenu t WHERE t.event_name = :eventName AND t.mvno_id = :mvnoId AND t.status='Active' AND t.is_delete=0")
    List<String> findAllIntigrationByEventAndByMvnoId(
            @Param("eventName") String eventName,
