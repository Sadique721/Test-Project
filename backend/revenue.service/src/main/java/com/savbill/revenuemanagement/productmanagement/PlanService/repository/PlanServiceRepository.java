...
public interface PlanServiceRepository extends JpaRepository<PlanService, Integer>  {
    ...
    @Query("Select t from PlanService t where t.mvnoId in :mvnoIds")
    Page<PlanService> findAll(Pageable pageable, @Param("mvnoIds") List mvnoIds);

    @Query("Select t from PlanService t where t.mvnoId = 1 or (t.mvnoId = :mvnoId and t.buId in :buIds)")
...
//    Integer duplicateVerifyAtSaveICName(@Param("icname") String icname, @Param("mvnoIds")List mvnoIds);
  ...
  //  PlanService findAllById(@Param("planServices") List<PlanService> planServices);
}
