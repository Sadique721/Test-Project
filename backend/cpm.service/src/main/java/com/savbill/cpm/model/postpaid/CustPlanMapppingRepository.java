...
import com.savbill.cpm.modules.subscriber.model.CustomerDetailsDTO;
import feign.Param;
...
public interface CustPlanMapppingRepository extends JpaRepository<CustPlanMappping, Integer>, QuerydslPredicateExecutor<CustPlanMappping> {
    ...
    @Query(value = "SELECT p.POSTPAIDPLANID , p.NAME , c.expirydate " +
            "FROM TBLCUSTPACKAGEREL c " +
            "JOIN TBLMPOSTPAIDPLAN p ON c.planid = p.POSTPAIDPLANID " +
            "WHERE c.custid = :custId " +
            "AND c.startdate <= CURRENT_TIMESTAMP " +
            "AND c.expirydate >= CURRENT_TIMESTAMP " +
            "AND c.purchase_type IN :purchaseTypes AND p.offerprice > 0", nativeQuery = true)
    List<Object[]> findActivePlanDetailsWithExpirePlan(@Param("custId") Integer custId,
                                         @Param("purchaseTypes") List<String> purchaseTypes);
    ...
    @Query(value = "SELECT p.POSTPAIDPLANID , p.NAME , c.expirydate , p.offerprice " +
            ...
            "WHERE c.custid = :custId " +
            "AND c.startdate >= CURRENT_TIMESTAMP " +
            "AND c.purchase_type IN :purchaseTypes AND c.cust_plan_status IN ('Active', 'Hold') order by c.custpackageid desc", nativeQuery = true)
    List<Object[]> findFuturePlanDetails2(@Param("custId") Integer custId,
                                         @Param("purchaseTypes") List<String> purchaseTypes);
    ...
    @Query(value = "SELECT p.POSTPAIDPLANID , p.NAME , c.expirydate , p.offerprice " +
            ...
            "AND c.expirydate >= CURRENT_TIMESTAMP " +
            "AND c.purchase_type IN :purchaseTypes AND c.cust_plan_status IN ('Active', 'Hold') order by c.custpackageid desc", nativeQuery = true)
    List<Object[]> findActivePlanDetailsLatest2(@Param("custId") Integer custId,
                                               @Param("purchaseTypes") List<String> purchaseTypes);
    ...
    @Query(value = "SELECT p.POSTPAIDPLANID , p.NAME , c.expirydate , p.offerprice " +
            ...
            "WHERE c.custid = :custId " +
            "AND c.expirydate <= CURRENT_TIMESTAMP " +
            "AND c.purchase_type IN :purchaseTypes order by c.custpackageid desc", nativeQuery = true)
    List<Object[]> findExpiredPlanDetails2(@Param("custId") Integer custId,
                                          @Param("purchaseTypes") List<String> purchaseTypes);
...
//    List<String> findPlanNameByCustId(@Param("custId") Integer custId);
    ...
    @Query("SELECT new com.savbill.cpm.modules.subscriber.model.CustomerDetailsDTO(" +
