package com.savbill.cpm.modules.payments.controller;
...
import com.savbill.cpm.modules.FlutterWaveHelper.FlutterWaveService;
...
import com.savbill.cpm.modules.payments.service.PaymentService;
...
import com.savbill.cpm.pojo.api.SearchPaymentPojo;
import com.savbill.cpm.rabbitMq.MessageSender;
import com.savbill.cpm.rabbitMq.message.BudPayPaymentMessage;
...
public class PaytmPaymentController {
    ...
    public String getResponseRedirect(HttpServletRequest request, Model model) {
        ...
        if (payment.isPresent()) {
            ...
            recordPayment.setPaymode(CommonUtils.PAYMENT_MODE_ONLINE);
                ...
                if (isValideChecksum && parameters.containsKey("RESPCODE")) {
                    if (parameters.get("RESPCODE").equals("01")) {
                        ...
                        try {
                            ...
                            debitDocService.createInvoice(customers1, Constants.RENEW, "PAYTM", null,additionalInformationDTO, null,false,false,null,null);
    ...
    public void getResponseFromLink(HttpServletRequest request) {
        MDC.put("type", "Fetch");
        ApplicationLogger.logger.info("LinkResponse API Hit");
        Map<String, String[]> mapData = request.getParameterMap();
        TreeMap<String, String> parameters = new TreeMap<String, String>();
        ...
        try {
            ...
            if (isValideChecksum && parameters.containsKey("RESPCODE")) {
                if (parameters.get("RESPCODE").equals("01")) {
                    ...
                    CreditDocument doc = creditDocService.covertPaymentReqToCreditDoc(recordPayment);
                    ...
                    if (parameters.containsKey("status")) {
                       ...
                       //     logger.info("Payment For user "+recordPayment.getCustomer().getUsername()+" is Successfull :  request: { From : {}}; Response : {{}}", request.getHeader("requestFrom"), APIConstants.SUCCESS);
                            ...
                            if (customers.isPresent()) {
                                ...
                                customersService.sendCustPaymentSuccessMessage("Payment Success", customers.get().getUsername(), customerPayment.getPayment(), "Online", customers.get().getMvnoId(), customers.get().getCountryCode(), customers.get().getMobile(), customers.get().getEmail(), customers.get().getId(), customerPayment.getOrderId().toString(),LocalDate.now().toString(),customers.get().getBuId(),null);
                            }
                        } else {
                            result = "Payment Failed";
                            customerPayment.setStatus(result);
...
//                        model.addAttribute("homeRedirectUrl", clientServiceSrv.getValueByName(ClientServiceConstant.HOME_REDIRECT_URL_CAPTIVE_PORTAL) + "?customerId=" + customerPayment.getCustId() + "&transactionId=" + customerPayment.getOrderId() + "&amount=" + customerPayment.getPayment() + "&currency=" + this.clientServiceSrv.getValueByName(ClientServiceConstant.CURRENCY_FOR_PAYMENT));
                        ...
                        model.addAttribute("homeRedirectUrl", REDIRECT_CAPTIVE_URL + "?username=" + customers.get().getUsername() + "&password=" + customers.get().getPassword() + "&url=http://savbillnettech.com/");
                        RecordPaymentRequestDTO recordPaymentDTO = new RecordPaymentRequestDTO();
                        recordPaymentDTO.setPaymentAmount(customerPayment.getPayment());
...
//                        }
                        ...
                        try {
                            ...
                            debitDocService.createInvoice(customers1 , Constants.RENEW,"FLUTTERWAVE", null,additionalInformationDTO, null,false,false,null,null);
                        } catch (Exception e) {
                            logger.error("Payment Failed For Order Id "+orderId+" :  request: { From : {}}; Response : {{}};Error :{} ;", request.getHeader("requestFrom"),APIConstants.FAIL,request);
                        ...
                        if (customers.isPresent()) {
                            customersService.sendCustPaymentSuccessMessage("Payment Success", customers.get().getUsername(), customerPayment.getPayment(), "Online", customers.get().getMvnoId(), customers.get().getCountryCode(), customers.get().getMobile(), customers.get().getEmail(),customers.get().getId(),customerPayment.getPaymentDate().toString(), recordPaymentDTO.getReferenceNo(),customers.get().getBuId(),null);
            ...
            if (status.equalsIgnoreCase("success")) {
                ...
                if (customerPayment != null) {
                    ...
                    if (customers != null) {
                        ...
                        debitDocService.createInvoice(customers, CommonConstants.INVOICE_TYPE.CHANGE_PLAN, "", changePlanPojoFromCustomer.getRecordPayment(),null, null,changePlanNextBillDate,false,null,null);
                        model.addAttribute("result", "Payment Success!!");
                        parameters.put("referenece",reference);
                        parameters.put("status", status);
                        parameters.put("amount", String.valueOf(customerPayment.getPayment()));
