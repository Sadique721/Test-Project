package com.savbill.cpm.modules.cafRejectReason.Controller;
...
import com.savbill.cpm.modules.cafRejectReason.DTO.RejectReasonDto;
import com.savbill.cpm.modules.cafRejectReason.Service.RejectReasonService;
...
//	    @PreAuthorize("@roleAccesses.hasPermission('concurrent','createUpdateAccess',#request.getHeader('requestFrom'))")
    public ResponseEntity<?> addRejectReason(@RequestBody RejectReasonDto rejectReason,
        ...
        try {
            ...
            this.rejectReasonService.validateRequest(rejectReason, rejectReasonService.getMvnoIdFromCurrentStaff(), CommonConstants.OPERATION_ADD);
...
//	    @PreAuthorize("@roleAccesses.hasPermission('concurrent','createUpdateAccess',#request.getHeader('requestFrom'))")
    public ResponseEntity<?> updateRejectReason(@PathVariable Long id,
        ...
        try {
            String authTokenHeader = request.getHeader("Authorization");
            rejectReasonService.getEntityForUpdateAndDelete(id);
            rejectReason.setId(id);
            this.rejectReasonService.validateRequest(rejectReason, rejectReasonService.getMvnoIdFromCurrentStaff(), CommonConstants.OPERATION_UPDATE);
...
//	    @PreAuthorize("@roleAccesses.hasPermission('concurrent','deleteAccess',#request.getHeader('requestFrom'))")
    public ResponseEntity<?> deleteCustomer(@RequestParam(name = "rejectReasonId", required = true) Long rejectReasonId, HttpServletRequest request) {
        HashMap<String, Object> response = new HashMap<>();
        Integer responseCode = APIConstants.FAIL;
        TraceContext traceContext = tracer.currentSpan().context();
        MDC.put("type", "Delete");
        ...
        RejectReasonDto rejectReason = this.rejectReasonService.findById(rejectReasonId);
        try {
            rejectReasonService.getEntityForUpdateAndDelete(rejectReasonId);
            if (rejectReason == null) {
                response.put(APIConstants.MESSAGE,
                        "No record found for rejectReason with the given rejectReason id :" + rejectReasonId);
            } else {
