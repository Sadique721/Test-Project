...
import com.savbill.cpm.MicroSeviceDataShare.SharedMessages.ShiftlocationMessage;
...
import com.savbill.cpm.model.postpaid.*;
import com.savbill.cpm.modules.Area.domain.QArea;
...
import com.savbill.cpm.modules.BuildingMgmt.Domain.QBuildingManagement;
import com.savbill.cpm.modules.BuildingMgmt.Repository.BuildingMgmtRepository;
import com.savbill.cpm.modules.Pincode.domain.QPincode;
...
import com.savbill.cpm.modules.SubArea.Domain.QSubArea;
import com.savbill.cpm.modules.SubArea.Repository.SubAreaRepository;
...
import com.savbill.cpm.rabbitMq.message.CustAddressShiftingMsg;
import com.savbill.cpm.repository.common.StaffUserRepository;
import com.savbill.cpm.repository.postpaid.CustomerAddressRepository;
...
import com.savbill.cpm.repository.postpaid.ShiftLocationRepository;
...
public class CustomerAddressService extends AbstractService<CustomerAddress, CustomerAddressPojo, Integer> {
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.CustomerAddress', '1')")
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.controller.common.CustomerAddress', '4')")
    public void deleteCustomerAddress(Integer id) throws Exception {
        entityRepository.deleteById(id);
    }

//    @PreAuthorize("hasPermission('com.savbill.cpm.controller.common.CustomerAddress', '2')")
    public CustomerAddress getCustomerAddressForAdd(Integer custid) throws Exception {
        CustomerAddress add = new CustomerAddress();
        Customers cust = custService.get(custid);
...
//    @PreAuthorize("hasPermission('com.savbill.cpm.controller.common.CustomerAddress', '2')")
    public CustomerAddress saveCustomerAddress(CustomerAddress customerAddress) throws Exception {
        customerAddress.setCity(cityService.get(customerAddress.getCityId()));
        customerAddress.setState(stateService.get(customerAddress.getStateId()));
        customerAddress.setCountry(countryService.get(customerAddress.getCountryId()));
...
//        }
    ...
    public void validateRequest(CustomerAddressPojo pojo, Integer operation) {
        ...
        if (pojo.getStateId() != null) {
            if (stateService.get(pojo.getStateId()) == null) {
        ...
        if (pojo.getCityId() != null) {
            if (cityService.get(pojo.getCityId()) == null) {
        ...
        if (pojo.getCustomerId() != null) {
            if (custService.get(pojo.getCustomerId()) == null) {
        ...
        if (operation != CommonConstants.OPERATION_DELETE && findByAddressTypeAndCustomer(pojo.getAddressType(), custService.get(pojo.getCustomerId())) != null) {
    ...
    public ShiftLocationDTO shiftCustomerLocation(ShiftLocationDTO shiftLocationDTO, Integer custId) throws Exception {
        if (shiftLocationDTO != null && custId != null) {
            ...
            if (custId != null)
            {
                Customers customers = custService.get(custId);
                if (customers != null)
                {
                    if(shiftLocationDTO.getIsInvoiceCleared())
                    {
                        ...
                        sendCustOpenAddressShiftingMessage(customers.getUsername(), customers.getMobile(), customers.getEmail(), customers.getStatus(), customers.getMvnoId() , customers.getBuId());
                                ...
                                if (pojo.getFlag() != null && !"".equals(pojo.getFlag()) && pojo.getFlag().equalsIgnoreCase("approved")) {
                                    if(!flag) {
                                        ...
                                        if(shiftLocation.get().getShiftPartnerId()!=null && shiftLocation.get().getShiftPartnerId()!=1){
                                            Partner partner=partnerService.get(shiftLocation.get().getShiftPartnerId());
                                        ...
                                        if(shiftLocation.get().getBranchId()!=null){
                                            ...
                                            if(customers.getPartner()!=null) {
                                                Partner partner=partnerService.get(1);
                                                customers.setPartner(partner);
                                            }
                                        }
...
//                        messageSender.send(shiftlocationMessage, SharedDataConstants.QUEUE_PARTNER_SHIFT_LOCATION_SHARE_REVENUE);
            ...
            sendCustCloseAddressShiftingMessage(customers.getUsername(), customers.getMobile(), customers.getEmail(), customers.getStatus(), customers.getMvnoId(),customers.getBuId());
    ...
    public GenericDataDTO getShiftLocationApprovals(List<GenericSearchModel> filters, Integer page, Integer pageSize, String sortBy, Integer sortOrder) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1)
            booleanExpression = booleanExpression.and(qCustomerAddress.customer.mvnoId.in(1, getMvnoIdFromCurrentStaff()));
        if (getBUIdsFromCurrentStaff() != null && getBUIdsFromCurrentStaff().size() > 0) {
            booleanExpression = booleanExpression.and(qCustomerAddress.customer.mvnoId.eq(1).or(qCustomerAddress.customer.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qCustomerAddress.customer.buId.in(getBUIdsFromCurrentStaff()))));
        }
   ...
   public void sendCustOpenAddressShiftingMessage(String username, String mobileNumber, String emailId, String status, Integer mvnoId,Long buId) {
       try {
           ...
           if (optionalTemplate.isPresent()) {
               if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                   CustAddressShiftingMsg custAddressShiftingMsg = new CustAddressShiftingMsg(username, mobileNumber, emailId, status, mvnoId, RabbitMqConstants.CUSTOMER_OPEN_ADDRESS_SHIFTING_EVENT, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY,buId);
                   Gson gson = new Gson();
...
//                   messageSender.send(custAddressShiftingMsg, RabbitMqConstants.QUEUE_CUSTOMER_OPEN_ADDRESS_SHIFTING_NOTIFICATION);
    ...
    public void sendCustCloseAddressShiftingMessage(String username, String mobileNumber, String emailId, String status, Integer mvnoId,Long buId) {
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    CustAddressShiftingMsg custAddressShiftingMsg = new CustAddressShiftingMsg(username, mobileNumber, emailId, status, mvnoId, RabbitMqConstants.CUSTOMER_CLOSE_ADDRESS_SHIFTING_EVENT, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY,buId);
                    Gson gson = new Gson();
                    gson.toJson(custAddressShiftingMsg);
