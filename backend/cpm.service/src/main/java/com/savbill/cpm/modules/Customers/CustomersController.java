...
import com.savbill.cpm.modules.cafRejectReason.DTO.CafRejectDto;
...
import com.savbill.cpm.modules.subscriber.model.CustomerListPojo;
import com.savbill.cpm.modules.subscriber.model.DeactivatePlanReqDTO;
import com.savbill.cpm.modules.subscriber.model.DeactivatePlanReqDTOList;
...
import com.savbill.cpm.modules.subscriber.service.InvoiceCreationThread;
...
import com.savbill.cpm.service.common.CaptivePortalCustomerService;
import com.savbill.cpm.service.common.CustMilestoneDetailsService;
...
public class CustomersController extends ApiBaseController {
    ...
    public Page<CustomerShowDTO> getCustomersPagination(List<Integer> ids , PaginationRequestDTO paginationRequestDTO , String customerType){
        ...
        if (getBUIdsFromCurrentStaff().size() == 0 && getMvnoIdFromCurrentStaff() != 1){
            exp = exp.and(qCustomer.mvnoId.eq(1).or(qCustomer.mvnoId.eq(getMvnoIdFromCurrentStaff())));
        }
        else{
    ...
    public CustomerShowDTO convertChildCustomertodto(ChildCustomer childCustomer) {
        CustomerShowDTO customerShowDTO = new CustomerShowDTO();
        if (childCustomer != null) {
            // Map basic info
            customerShowDTO.setId(Math.toIntExact(childCustomer.getId()));
            customerShowDTO.setFirstname(childCustomer.getFirstName());
            customerShowDTO.setLastname(childCustomer.getLastName());
            customerShowDTO.setUsername(childCustomer.getUserName());
            customerShowDTO.setStatus(childCustomer.getStatus());
            customerShowDTO.setMobile(childCustomer.getMobileNumber());
            customerShowDTO.setEmail(childCustomer.getEmail());
            customerShowDTO.setName(childCustomer.getFirstName() + " " + childCustomer.getLastName());
            customerShowDTO.setConnectivity(true); // Assuming connectivity as true (or map as needed)
            customerShowDTO.setOutstanding(childCustomer.getWallet());
            customerShowDTO.setCusttype(childCustomer.getIsParent() != null && childCustomer.getIsParent() ? "Parent" : "Child");
            // Map additional fields as available
            customerShowDTO.setServiceareaname(""); // Fill if you can fetch service area
            customerShowDTO.setServiceArea("");     // Fill if you can fetch service area id/name
            customerShowDTO.setCalendarType("");    // Fill as needed
            customerShowDTO.setConnectionMode("");  // Fill as needed

            // Network details DTO (create empty or map if available)
            CustNetworkDetailsDTO custNetworkDetailsDTO = new CustNetworkDetailsDTO();
            custNetworkDetailsDTO.setServiceareaname(""); // Fill if available
            customerShowDTO.setNetworkDetails(custNetworkDetailsDTO);

            customerShowDTO.setIsinvoicestop(false); // Default or map if needed
        }
        return customerShowDTO;
    }
...
//            + AclConstants.OPERATION_POSTPAID_PLAN_VIEW + "\")")
    ...
    public ResponseEntity<?> getPlanByTypeServiceModeStatusAndMultipleServiceArea(@RequestParam(name = "type") String type,
        ...
        try {
            ...
            List<PostpaidPlanPojo> postpaidPlanList = postpaidPlanService.findPlanByTypeServiceModeStatusAndServiceArea(type, serviceId, serviceAreaId, mode, status, planGroup, planCategory, validity, unitsOfValidity, custId);
...
public GenericDataDTO getCustomerByServiceId(Long serviceId) throws Exception {
    ...
    public ResponseEntity<?> GetAllPlansByServiceArea(@RequestBody ServiceAreaFetchDTO serviceAreaFetchDTO,
                                      HttpServletRequest request,@RequestHeader(value = "rf", defaultValue = "bss") String requestFrom) {
        HashMap<String, Object> response = new HashMap<>();
        Integer responseCode = APIConstants.FAIL;
        TraceContext traceContext = tracer.currentSpan().context();
        Integer RESP_CODE = APIConstants.FAIL;
    ...
    public ResponseEntity<?> getPartnersByServiceAreaId(@Valid @PathVariable List<Integer> serviceAreaId, HttpServletRequest req) throws Exception {
        Integer RESP_CODE = APIConstants.FAIL;
        HashMap<String, Object> response = new HashMap<>();
        TraceContext traceContext = tracer.currentSpan().context();
        MDC.put("type", "Fetch");
        ...
        try {
            ...
            response.put("partnerList", partnerService.convertResponseModelIntoPojo(partnerService.getPartnersByServiceAreaId(serviceAreaId)));
            RESP_CODE = APIConstants.SUCCESS;
    ...
    public List<PostpaidPlan> getPlansByFilters(@RequestHeader(value = "rf", defaultValue = "bss") String requestFrom,@RequestBody GetPlansByFilter requestDto, @RequestParam (name = "parentPlanId", required = false)Integer parentPlanId, HttpServletRequest req){
        try {
            List<PostpaidPlan> postpaidPlanList = postpaidPlanService.getPlansByFilters(requestFrom, requestDto, parentPlanId);
...
//            snmpCounters.incrementGetCustomerByCidMacFailure();
    ...
    public ResponseEntity<?> saveCustomerWithLimitedData(@RequestBody CustomersPojo customersPojo, @RequestHeader(value = "rf", defaultValue = "bss") String requestFrom,HttpServletRequest req)  throws Exception {
        ...
        try {
            ...
            Integer responseCode = APIConstants.FAIL;
            LOGGER.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom")+ LogConstants.REQUEST_FOR +"Save Customer from CWSC"+ LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ce.getMessage()+LogConstants.LOG_STATUS_CODE+RESP_CODE);
            response.put(APIConstants.ERROR_MESSAGE, ce.getMessage());
            return apiResponse(responseCode, response);
    ...
    public ResponseEntity<?> getCustomerPlanListByAccountNo(@RequestParam(name = "accountNo") String accountNo,HttpServletRequest req) throws Exception {
        ...
        MDC.put("userName",getLoggedInUser().getUsername());
        MDC.put("spanId",traceContext.spanIdString());
        logger.info("Incoming request: accountNo={} user={} mvnoId={}",
                accountNo, getLoggedInUser().getUsername(), getLoggedInUser().getMvnoId());
        try {
            Map<String, Object> map= postpaidPlanService.getCustomerPlanListByAccountNo(accountNo, getLoggedInUser().getMvnoId());
            logger.debug("postpaidPlanService.getCustomerPlanListByAccountNo returned: {}", map);
            ...
            logger.debug("Fetched {} postpaidPlans: {}",
                    postpaidPlans.size(),
                    postpaidPlans.stream().map(LightPostpaidPlanDTO::getName).collect(Collectors.toList()));

            List<Map<String, Object>> filteredList = postpaidPlans.stream()
                    .filter(plan ->
                            plan.getMvnoId() != null
                                    && plan.getMvnoId().equals(getMvnoIdFromCurrentStaff())
                                    && (
                                    plan.getAccessibility() == null
                                            || "CustomerPortal".equals(plan.getAccessibility())
                                            || "Both".equals(plan.getAccessibility())
                            )
                                    && plan.getOfferprice() != null
                                    && plan.getOfferprice() > 0
                    )
                    .map(plan -> {
            ...
            logger.info("Filtered {} plans for mvnoId={}", filteredList.size(), getMvnoIdFromCurrentStaff());
    ...
    public ResponseEntity<?> upgradePlanByAccountNoAndPlanName(@RequestParam(name = "accountNo") String accountNo, @RequestParam(name = "packageName") String packageName, HttpServletRequest req) throws Exception {
        ...
        try {
            ...
            if(customers.getStatus().equalsIgnoreCase(CommonConstants.CUSTOMER_STATUS.ACTIVE)) {
                ...
                Customers customersPojo = customersService.get(custId);
                ...
                if (currentMvnoId == 1 || dataMvnoId.equals(currentMvnoId)) {
                    ...
                    if (customersforInvoice.size() > 1) {
                        ...
                        debitDocService.createInvoice(customersPojo, CommonConstants.INVOICE_TYPE.CHANGE_PLAN, "", deactivatePlanReqDTOList.getRecordPayment(), null, null, changePlanNextBillDate, false, null,null);
                    }
