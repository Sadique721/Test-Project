...
import com.savbill.cpm.model.postpaid.DebitDocumentSummary;
...
//    DebitDocument findFirstByOrderByDocnumberDesc(BooleanExpression eq);
    ...
    @Query("select d from DebitDocument d where d.customer.id= :customerId and d.totalamount-d.adjustedAmount >0 and d.endate < CURRENT_DATE ")
    List<DebitDocument> getInvoiceWhereServiceExpiredAndNotClear(@Param(value = "customerId") Integer customerId);

    @Query("select d from DebitDocument d where d.customer.id= :customerId and d.endate >= CURRENT_DATE ")
...
//    @Query("SELECT new com.savbill.cpm.rabbitMq.message.BillGenMessageData(dd.docnumber, concat( c.firstname,c.lastname), c.username, dd.billrunid, c.acctno, chg.name, tx.name, dd.startdate, dd.endate, b.branch_code, " +
...
//    Double getAmountByMvnoIdAndStatusIsNotVoid(Integer mvnoId,LocalDateTime duedate);
    ...
    @Query(value = "SELECT new com.savbill.cpm.pojo.DebitDocDetailDTO(dd.chargetype, dd.totalamount, dd.debitdocdetailid, d.docnumber, cust.username, d.billdate,c.id,c.tax.id) " +
            "FROM com.savbill.cpm.model.postpaid.DebitDocDetails dd " +
            "JOIN DebitDocument d on d.id = dd.debitdocumentid " +
            "JOIN Charge c on c.id = dd.chargeid " +
            "JOIN com.savbill.cpm.model.common.Customers cust ON d.customer.id = cust.id " +
    ...
    @Query(value = "SELECT  new com.savbill.cpm.model.postpaid.DebitDocumentSummary(d, SUM(d.totalamount) ) " +
            "FROM DebitDocument d " +
            "JOIN Customers cust ON d.customer.id = cust.id " +
