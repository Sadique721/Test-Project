...
import com.savbill.cpm.mapper.Creditdoc.CreditDocChargeRelMapper;
...
import com.savbill.cpm.modules.BankManagement.domain.QBankManagement;
import com.savbill.cpm.modules.BankManagement.repository.BankManagementRepository;
...
import com.savbill.cpm.modules.CustomerDBR.domain.QCustomerChargeDBR;
import com.savbill.cpm.modules.CustomerDBR.domain.QCustomerDBR;
import com.savbill.cpm.modules.CustomerDBR.repository.CustomerChargeDBRRepository;
...
import com.savbill.cpm.modules.Mvno.domain.QMvno;
...
import com.savbill.cpm.modules.subscriber.model.PaymentHistoryDTO;
...
import com.savbill.cpm.modules.xmlConversion.PaymentDetailsXml;
import com.savbill.cpm.pojo.CreditDoc.CreditDocUpdateDTO;
import com.savbill.cpm.pojo.PaymentListPojo;
import com.savbill.cpm.pojo.RecordPayment;
import com.savbill.cpm.pojo.SearchPayment;
...
import org.springframework.data.domain.PageImpl;  // If you return a Page manually
...
//    }
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.CreditDocument', '1')")
    public List<CreditDocument> getAllEntities(Integer pageNumber, int pageSize) {
//    	PageRequest pageRequest = PageRequest.of(pageNumber - 1, pageSize);
        return entityRepository.findAll().stream().filter(creditDic -> (creditDic.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || creditDic.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1) && (getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(creditDic.getBuID()))).collect(Collectors.toList());
    }
    ...
    public CreditDocument covertPaymentReqToCreditDoc(RecordPayment payment) throws Exception {
        ...
        if (payment != null && payment.getCustomerid() != null) {
            ...
            doc.setCustomer(customerService.getcustForCwsc(Integer.valueOf(payment.getCustomerid())));
...
//            doc.setCreditdocumentno(getPaymentInvoiceNo());
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.CreditDocument', '1')")
    public Page<CreditDocument> findCreditDocuments(SearchPayment search, PaginationRequestDTO requestDTO) {
        ...
        if (search.getCustomerid() != null) {
            exp = exp.and(qCreditDocument.customer.eq(customerService.get(Integer.valueOf(search.getCustomerid()))));
        }
...
//            List<Integer> list = creditDebitDocMappings.stream().map(CreditDocument::getId).collect(Collectors.toList());
        ...
        if (getLoggedInMvnoId() != 1) exp = exp.and(qCreditDocument.mvnoId.eq(getLoggedInMvnoId()));
    ...
    public CreditDocumentPojo convertCreditDocumentModelToCreditDocumentPojo(CreditDocument creditDocument) throws Exception {
        ...
        if (creditDocument != null) {
            ...
            if (CommonUtils.PAYMENT_MODE_CHEQUE.equalsIgnoreCase(creditDocument.getPaymode())) {
            ...
            } else if (CommonUtils.PAYMENT_MODE_ONLINE.equalsIgnoreCase(creditDocument.getPaymode())) {
...
//                    DebitDocument debitDocument = debitDocRepository.findById(creditDocument.getInvoiceId()).get();
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.CreditDocument', '1')")
    public Page<CreditDocument> serachPayment(SearchPaymentPojo searchPaymentPojo, PaginationRequestDTO requestDTO) {
        ...
        try {
            if (searchPaymentPojo != null) {
                ...
                creditDocumentsList = this.findCreditDocuments(payment, requestDTO);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    @Override
    public Page<CreditDocument> search(List<GenericSearchModel> filterList, Integer page, Integer pageSize, String sortBy, Integer sortOrder) {
    ...
    public Page<CreditDocument> getCustbyIdOrPayFromOrPayToOrPayStatus(String s1, PageRequest pageRequest) {
        return entityRepository.findAllByCustidContainingIgnoreCaseOrCust_NameAndIsDeleteIsFalse(s1, s1, s1, s1, s1, s1, pageRequest, getMvnoIdFromCurrentStaff());
    }

    public SearchPayment convertSearchPaymentPojoToSearchPayment(SearchPaymentPojo searchPaymentPojo) {
        ...
        if (searchPaymentPojo != null) {
            ...
            if (searchPaymentPojo.getCustomerid() != null) {
                payment.setCustomer(customerService.get(searchPaymentPojo.getCustomerid()));
            }
...
//            }
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.CreditDocument', '2')")
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.CreditDocument', '2')")
    public CreditDocument saveRecordPayment(RecordPayment recordPayment) throws Exception {
        recordPayment.setMvnoId(getMvnoIdFromCurrentStaff());
        CreditDocument doc = covertPaymentReqToCreditDoc(recordPayment);
            ...
            if (pojo.getCreditDocId() != null) {
                ...
                if (doc != null) {
                    ...
                    if (doc.getNextTeamHierarchyMappingId() == null && !isRevoked && customers.getLcoId() == null) {
                        if (Objects.equals(doc.getStatus(), CommonUtils.PAYMENT_STATUS_PENDING)) {
                            ...
                            if (clientServiceSrv.getValueByNameAndmvnoId(ClientServiceConstant.WORKFLOW_AUTOMATIC_ASSIGN,pojo.getMvnoId()).equals("TRUE")) {
                                ...
                                if (map.containsKey("staffId") && map.containsKey("nextTatMappingId")) {
                                    StaffUser assignedStaff = staffUserService.get(Integer.parseInt(map.get("staffId")));
...
//                                TAT functionality
                        ...
                        if (doc.getPaymode().equalsIgnoreCase(CommonConstants.PAYMENT_MODE.CREDIT_NOTE)) {
                            adjustCreditNote(Optional.of(doc), creditDebitDocMappings);
                        } else {
                            if (obj.getPaytype().equalsIgnoreCase(CommonConstants.CREDIT_DOC_STATUS.WITHDRAWAL)) {
                                adjustWithDrawal(doc);
                            } else {
                                adjustPayment(Optional.of(doc), creditDebitDocMappings);
                            }
                        }
                ...
                if (pojo!=null && pojo.getType()!=null && pojo.getType().equalsIgnoreCase("Payment")){
                    customerService.sendCustPaymentSuccessMessage(RabbitMqConstants.CUSTOMER_PAYMENT_SUCCESS, customers.getUsername(), pojo.getAmount(), pojo.getPaymode(), customers.getMvnoId(), customers.getCountryCode(), customers.getMobile(), customers.getEmail(), customers.getId(), pojo.getReciptNo(), String.valueOf(pojo.getPaymentdate()), customers.getBuId(), null);
                }
    ...
    public void validateRequest(RecordPaymentPojo pojo, Integer operation) {
        ...
        if (pojo != null && customerService.get(pojo.getCustomerid()) == null) {
...
//    }
    ...
    public GenericDataDTO approvePayment(SearchPaymentPojo entity) {
        logger.info("1379: approve Payment method start");
        ...
        logger.info("1387 credit doc : "+creditDocument.orElse(null));
        ...
        try {
            if (entity.getIdlist() != null) {
                ...
                if (idList.length > 0) {
                    for (String id : idList) {
                        doc = null;
                        doc = entityService.getEntityForUpdateAndDeleteForCWSC(Integer.valueOf(id));
                        logger.info("1408 credit doc : "+doc.toString());
                        StaffUser assignedUser = null;

                        if (doc.getStatus().equals(CommonUtils.PAYMENT_STATUS_PENDING)) {
                            if (clientServiceRepository.findValueByNameandMvnoId(ClientServiceConstant.WORKFLOW_AUTOMATIC_ASSIGN,getLoggedInMvnoId()).equals("TRUE")) {
                                logger.info("1413 WORKFLOW_AUTOMATIC_ASSIGN : TRUE");
                                ...
                                } else if (creditDocument.get().getStatus().equalsIgnoreCase(CommonUtils.PAYMENT_STATUS_PENDING)) {
...
//                                    addLedgeAfterApproval(creditDocument.get());
                                ...
                                logger.info("1475 WORKFLOW_AUTOMATIC_ASSIGN : other than TRUE");
                                ...
                                if (NextApprovalmap.containsKey("assignableStaff")) {
                                    ...
                                    sendCustPaymentVerificationMessage(customers.getUsername(), customers.getMobile(), customers.getEmail(), customers.getStatus(), customers.getMvnoId(), creditDocument.get().getReciptNo(), creditDocument.get().getAmount(), String.valueOf(creditDocument.get().getPaymentdate()),customers.getBuId());
...
//                            doc.setRemarks(entity.getRemarks());
                            doc = entityService.save(doc);
                            logger.info("1542 doc.getStatus().equalsIgnoreCase(CommonConstants.PAYMENT_STATUS_APPROVED)"+doc.getStatus().equalsIgnoreCase(CommonConstants.PAYMENT_STATUS_APPROVED));
                            if(doc.getStatus().equalsIgnoreCase(CommonConstants.PAYMENT_STATUS_APPROVED)) {
                                ...
                                if (!creditDocument.get().getPaytype().equalsIgnoreCase(CommonConstants.CREDIT_DOC_STATUS.WITHDRAWAL)) {
                                    logger.info("===========================+++++++++Calling kafka : creditDocMessage from cpm to revenue++++++++++++==============================");
                                    logger.info("===========================+++++++++Message creditDocMessage "+creditDocMessage.toString()+"++++++++++++==============================");
                                    kafkaMessageSender.send(new KafkaMessageData(creditDocMessage, CreditDocMessage.class.getSimpleName()));
                                    logger.info("===========================+++++++++Kafka calldone : creditDocMessage from cpm to revenue++++++++++++==============================");
                                }
//                                messageSender.send(creditDocMessage, RabbitMqConstants.QUEUE_CREDIT_DOCUMENT_APPROVED_REVENUE);
                                logger.info("1550 Calling custPackStatusUpdate "+doc.toString());
                                custPackStatusUpdate(doc);
                                logger.info("1552 Return from custPackStatusUpdate ");
                                List<WorkflowAudit> workflowAudits = workflowAuditRepository.findAllByEntityId(doc.getId());
        ...
        if ("success".equalsIgnoreCase(flashMsg) && doc != null) {
            String paymode = clientServiceRepository.findValueByNameandMvnoId(ClientServiceConstant.Allow_Transfer_To_Wallet_Mode,getLoggedInMvnoId());
            String[] array = paymode.split(",");

            if (Arrays.stream(array).anyMatch(creditDocument.get().getPaymode()::equals) && (creditDocument.get().getStatus().equalsIgnoreCase(CommonConstants.CREDIT_DOC_STATUS.FULLY_ADJUSTED) || creditDocument.get().getStatus().equalsIgnoreCase(CommonConstants.CREDIT_DOC_STATUS.ADVANCE_PAYMENT) || creditDocument.get().getStatus().equalsIgnoreCase(CommonConstants.CREDIT_DOC_STATUS.PARTIAL_ADJUSTED) || creditDocument.get().getStatus().equalsIgnoreCase("Approved"))) {
            ...
            logger.info("1577: approve Payment method end");
    ...
    public void custPackStatusUpdate(CreditDocument creditDocument) {
        logger.info("1597 Inside custPackStatusUpdate ");
        try{
            if(creditDocument.getInvoiceId()!=null) {
                Optional<DebitDocument> debitDocument = debitDocRepository.findById(creditDocument.getInvoiceId());
                logger.info("1595 debitdocment" + debitDocument.orElse(null));
                if (debitDocument.isPresent()) {
                    ...
                    logger.info("1598 totalCreditNoteGenerated" + totalCreditNoteGenerated);
                    ...
                    if ((totalCreditNoteGenerated >= debitDocument.get().getTotalamount()) && creditDocument.getPaytype().equalsIgnoreCase(CommonConstants.TRANS_CREDIT_NOTE)) {
                        custPlanMapppings.forEach(custPlanMappping -> {
                            logger.info("1602 custPlanMappping loop" + custPlanMappping.toString());
                            ...
                            if (custPlanMappping.getStartDate().isAfter(LocalDateTime.now())) {
                                logger.info("1609: "+custPlanMappping.getStartDate().isAfter(LocalDateTime.now()));
                            ...
                            if (custPlanMappping.getStartDate().isAfter(custPlanMappping.getEndDate())) {
                                logger.info("1618 : "+custPlanMappping.getStartDate().isAfter(custPlanMappping.getEndDate()));
                                ...
                                custPlanMappping.setExpiryDate(custPlanMappping.getStartDate().plusSeconds(1));
                                 ...
                                 logger.info("1623 : custPlanMappping.ExpiryDate "+custPlanMappping.getExpiryDate());
        ...
        try {
            if (entity.getIdlist() != null) {
                ...
                if (idList != null && idList.length > 0) {
                    for (String id : idList) {
                        doc = null;
                        doc = entityService.getEntityForUpdateAndDelete(Integer.valueOf(id));
                        if (doc.getStatus().equals(CommonUtils.PAYMENT_STATUS_PENDING)) {
                            if (clientServiceRepository.findValueByNameandMvnoId(ClientServiceConstant.WORKFLOW_AUTOMATIC_ASSIGN,getLoggedInMvnoId()).equals("TRUE")) {
                                if (!loggedInUser.getUsername().equalsIgnoreCase("admin")) {
                                    ...
                                    if (map.containsKey("staffId") && map.containsKey("nextTatMappingId")) {
                                        ...
                                        StaffUser staffUsers = staffUserService.get(staffId);
                                        creditDocument.get().setNextTeamHierarchyMappingId(Integer.valueOf(map.get("nextTatMappingId")));
                                        creditDocument.get().setApproverid(Integer.valueOf(map.get("nextTatMappingId")));
...
//                                        workflowAuditService.saveAudit(map.containsKey("eventId") ? Integer.parseInt(map.get("eventId")) : null, CommonConstants.WORKFLOW_EVENT_NAME.PAYMENT, doc.getId(), doc.getReferenceno(), staffUsers.getId(), staffUsers.getUsername(), CommonConstants.WORKFLOW_AUDIT_ACTION.ASSIGNED, LocalDateTime.now(), "Assigned to :- " + staffUsers.getUsername());
                                        ...
                                        creditDocument.get().setNextTeamHierarchyMappingId(null);
                                        creditDocument.get().setStatus(CommonUtils.PAYMENT_STATUS_REJECTED);
...
//                            messageSender.send(creditDocMessage, RabbitMqConstants.QUEUE_CREDIT_DOCUMENT_APPROVED_REVENUE);
    ...
    public String assemblePaymentXML(CreditDocument doc) {
        ...
        return paymentXML;*/
        return assemblePaymentXML(doc, CommonUtils.ADDR_TYPE_PRESENT, null, null);
    }
    ...
    public String assemblePaymentXML(CreditDocument doc, String addressType, CustomerAddress address, Integer debitDocId) {
        try {
            if (debitDocId != null) {
                DebitDocument docDebit = debitDocService.get(debitDocId);
    ...
    public List<CreditDocument> getCreditDocByCustomer(Customers customers, Boolean Is_reversed) {
        return this.entityRepository.findAllByCustomer(customers).stream().filter(data -> data.getIs_reversed() == false && (data.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || data.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1)).collect(Collectors.toList());
    }

    public List<CreditDocument> getTdspendingCreditDocByCustomer(Customers customers) {
        return this.entityRepository.findAllByCustomer(customers).stream().filter(data -> data.getTds_received() == false && (data.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || data.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1)).collect(Collectors.toList());
    }

    public List<CreditDocument> getAllPaymentByCustomer(Customers customers) {
        return this.entityRepository.findAllByCustomer(customers).stream().collect(Collectors.toList()).stream().filter(creditDic -> creditDic.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || creditDic.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1).collect(Collectors.toList());
    }

    public List<PaymentHistoryDTO> getByCustId(Integer custId) {
        List<CreditDocument> creditDocuments = creditDocRepository.getAllByCustomer_IdAndPaytypeNotIgnoreCaseAndTypeNotIgnoreCaseOrderByIdDesc(custId, "CREDITNOTE", "creditnote").stream().filter(creditDic -> creditDic.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || creditDic.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1).collect(Collectors.toList());
    ...
    public List<CreditDocument> getAllByCustomer_IdOrderByIdDesc(Integer id) {
        return entityRepository.getAllByCustomer_IdOrderByIdDesc(id).stream().filter(creditDic -> creditDic.getMvnoId() == getMvnoIdFromCurrentStaff() || creditDic.getMvnoId() == null).collect(Collectors.toList());
    }
    ...
    @Override
    public CreditDocument get(Integer id) {
        CreditDocument creditDocument = super.get(id);
        if (creditDocument != null && (getMvnoIdFromCurrentStaff() == 1 || (creditDocument.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || creditDocument.getMvnoId() == 1)))
            return creditDocument;
        return null;
    }

    public CreditDocument getcwsc(Integer id) {
        CreditDocument creditDocument = super.get(id);
        return creditDocument;
    }


    public CreditDocument getEntityForUpdateAndDelete(Integer id) {
        CreditDocument creditDocument = get(id);
        if (creditDocument == null || !(getMvnoIdFromCurrentStaff() == 1 || getMvnoIdFromCurrentStaff().intValue() == creditDocument.getMvnoId().intValue()))
            throw new CustomValidationException(APIConstants.FAIL, Constants.MVNO_DELETE_UPDATE_ERROR_MSG, null);
        return creditDocument;
    }

    public CreditDocument getEntityForUpdateAndDeleteForCWSC(Integer id) {
        CreditDocument creditDocument = getcwsc(id);
        return creditDocument;
    }
    ...
    private StaffLedgerDetails saveStaffLedgerDetails(Optional<CreditDocument> creditDocument, StaffUser loggedInUser) {
        try {
            ...
            staffLedgerDetails.setStaff(loggedInUser);
    ...
    public GenericDataDTO getPaymentApprovalsList(List<GenericSearchModel> filters, Integer page, Integer pageSize, String sortBy, Integer sortOrder) {
        ...
        long apiStart = System.currentTimeMillis(); // Start timer
        ...
        if (getMvnoIdFromCurrentStaff() != 1)
            booleanExpression = booleanExpression.and(qCreditDocument.mvnoId.in(1, getMvnoIdFromCurrentStaff()));
        if (getBUIdsFromCurrentStaff() != null && getBUIdsFromCurrentStaff().size() > 0) {
            booleanExpression = booleanExpression.and(qCreditDocument.mvnoId.eq(1).or(qCreditDocument.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qCreditDocument.customer.buId.in(getBUIdsFromCurrentStaff()))));
        }

        long queryStart = System.currentTimeMillis();
       // Page<CreditDocument> paginationList = entityRepository.findAll(booleanExpression, pageRequest);

        Page<CreditDocumentProjectionDTO> paginationList =
                creditDocRepository.findAllProjected(booleanExpression, pageRequest);

        long queryEnd = System.currentTimeMillis();
        logger.info("{} DB call took {} ms", SUBMODULE, (queryEnd - queryStart));
...
//        }).collect(Collectors.toList()));
        ...
        logger.info("{} API execution took {} ms", SUBMODULE, (apiEnd - apiStart));
        return genericDataDTO;
    }
...
//        messageSender.send(creditDocMessage, RabbitMqConstants.QUEUE_CREDIT_DOCUMENT_APPROVED_SUCCESS);
    ...
    public void adjustCreditNote(Optional<CreditDocument> creditDocument, List<CreditDebitDocMapping> creditDebitDocMappings) {
        for (CreditDebitDocMapping creditDebitDocMapping : creditDebitDocMappings) {
            DebitDocument debitDocument = debitDocRepository.findById(creditDebitDocMapping.getDebtDocId()).orElse(null);
            ...
            if (creditDocument.isPresent() && creditDocument.get().getCustomer().getId().intValue() == 1) {
                if (creditDocument.get().getInvoiceId() != null)
                {
                    if (debitDocument.getIsCNEnable()) {
                        ...
                        reversalPaymentForOrg(creditDocument.get().getAmount(), debitDocument,true);
                        return;
                    }
                }
                return;
...
//            messageSender.send(creditDocMessage, RabbitMqConstants.QUEUE_CREDIT_DOCUMENT_APPROVED_SUCCESS);
            //messageSender.send(creditDocMessage, RabbitMqConstants.QUEUE_CREDIT_DOCUMENT_KPI);
            debitDocService.adjustBillToSubisuInvoiceWithCreditNote(creditDocument.get().getAmount(),debitDocument);
        }
    }
...
//                }
    ...
    public void adjustPayment(Optional<CreditDocument> creditDocument, List<CreditDebitDocMapping> creditDebitDocMappings) {
...
//        }
    ...
    public void sendCustPaymentVerificationMessage(String username, String mobileNumber, String emailId, String status, Integer mvnoId, String reciptNo, Double paymentAmount, String paymentDate,Long buId) {
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    CustPaymentVerificationMsg custPaymentVerificationMsg = new CustPaymentVerificationMsg(username, mobileNumber, emailId, status, mvnoId, reciptNo, paymentAmount, paymentDate, RabbitMqConstants.CUSTOMER_PAYMENT_VERIFICATION_EVENT, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY,buId);
                    Gson gson = new Gson();
                    ...
                    for (CustomerServiceMapping customerServiceMapping : customerServiceMappings) {
                        ...
                        if (!CollectionUtils.isEmpty(planGroups)) {
                            ...
                            if (!CollectionUtils.isEmpty(custPlanMappPgList)) {
                                ...
                                custPlanMappPgList = custPlanMappPgList.stream().filter(CommonUtils.distinctByKey(CustPlanMappping::getId)).collect(Collectors.toList());
                            }
                        }
...
//            creditDocument.setCreditdocumentno(getInvoiceNo());
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.CreditDocument', '1')")
    public Page<CreditDocumentSearchPojo> getCreditDocuments(SearchPaymentPojo search, PaginationRequestDTO requestDTO) {
        QCreditDocument qCreditDocument = QCreditDocument.creditDocument;
        QDebitDocument qDebitDocument = QDebitDocument.debitDocument;
        ...
        if (search.getType() != null && !"null".equals(search.getType()) && !"".equals(search.getType())) {
            ...
            if (search.getType().equalsIgnoreCase("payment")) {
                exp = exp.or(qCreditDocument.paytype.startsWithIgnoreCase(CommonConstants.CREDIT_DOC_STATUS.WITHDRAWAL));
                exp = exp.or(qCreditDocument.paytype.startsWithIgnoreCase(CommonConstants.CREDIT_DOC_STATUS.TRANSFERRED));
            }
        }
        ...
        if (!StringUtils.isEmpty(search.getPaystatus()) && !"null".equals(search.getReferenceno()) && !"-1".equalsIgnoreCase(search.getPaystatus())) {
            if(search.getPaystatus().equalsIgnoreCase("Approved")){
                exp = exp.and(qCreditDocument.status.equalsIgnoreCase(search.getPaystatus()).or(qCreditDocument.status.equalsIgnoreCase("Fully Adjusted")));
            }else{
                exp = exp.and(qCreditDocument.status.equalsIgnoreCase(search.getPaystatus()));
            }
        }
...
//            }
                ...
                QueryResults<CreditDocumentSearchPojo> queryResults = queryFactory
                    .select(Projections.constructor(
                            ...
                            Expressions.stringTemplate(
                                    "CASE WHEN lower({0}) = 'advance' THEN {1} ELSE {0} END",
                                    qCreditDocument.paytype,
                                    qCreditDocument.type
                            ),
                            qCreditDocument.paymentreferenceno,
                            qCreditDocument.adjustedAmount,
                            qCreditDocument.createdate,
                            qCreditDocument.customer.acctno
                    ))
                    ...
                    .leftJoin(qDebitDocument).on(qCreditDocument.invoiceId.eq(qDebitDocument.id))// Join with debitdocument table
                         .leftJoin(qMvno).on(qCreditDocument.mvnoId.eq(qMvno.id.intValue()))
                    .where(exp)
        ...
        JPAQueryFactory queryFactory = new JPAQueryFactory(entityManager);
            ...
            List<CreditDocumentSearchPojo> creditDocumentPojos = queryResults.getResults();
      //  creditDocumentPojos.forEach(creditDocumentSearchPojo -> {if(creditDocumentSearchPojo.getFilename()!=null){creditDocumentSearchPojo.setFilename(PATH+creditDocumentSearchPojo.getFilename());}});
            for (CreditDocumentSearchPojo dto : creditDocumentPojos) {
                if (dto.getApproverid() != null) {
                    staffUserRepository.findById(dto.getApproverid()).ifPresent(staff -> {
                        dto.setCreatedByName(staff.getUsername());
                    });
                }
            }
        long totalRecords = queryResults.getTotal();
    ...
    public void savecreditDoc(CreditDocMessageList messages) {
        try{
            for (CreditDocMessage message : messages.getCreditDocMessageList()) {
                ...
                System.out.println("******* before savecreditDoc"+ message);
                creditDocument = creditDocRepository.save(creditDocument);
                System.out.println("******* after savecreditDoc"+ message);
