package com.savbill.cpm.modules.Voucher.service;
...
import com.savbill.cpm.model.common.FieldType;
...
import com.savbill.cpm.modules.Voucher.domain.QVoucher;
import com.savbill.cpm.modules.Voucher.domain.Voucher;
import com.savbill.cpm.modules.Voucher.module.*;
import com.savbill.cpm.modules.Voucher.module.*;
...
import com.savbill.cpm.modules.VoucherBatch.domain.QBSSVoucherBatch;
import com.savbill.cpm.modules.VoucherBatch.repository.BSSVoucherBatchRepository;
import com.savbill.cpm.modules.VoucherBatch.service.VoucherBatchService;
import com.savbill.cpm.modules.VoucherConfiguration.domain.QVoucherConfiguration;
...
import com.savbill.cpm.rabbitMq.message.VoucherCodeMessage;
...
import com.savbill.cpm.utils.EncryptVoucher;
import com.savbill.cpm.utils.RandomStringGenerator;
        ...
        try {
            ...
            if(optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    ...
                    VoucherCodeMessage voucherCodeMessage = new VoucherCodeMessage(countryCode, mobileNo, voucherCode, mvnoId, RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, RabbitMqConstants.CUSTOMER_VOUCHER_TEMPLATE, optionalTemplate.get() , buId);
...
//                    } else
    ...
    @Override
    public List<Map<String, String>> dataToExport(String batchName, String status) {
        QVoucher qVoucher = QVoucher.voucher;
        BooleanExpression expForVoucher = qVoucher.isNotNull();
...
//        List<Tuple> queryResult = query.select(qVoucher.code, qPlan.plantype,qVoucher.status,  qPlan.name, qPlan.offerprice, qPlan.startDate, qPlan.endDate).from(qVoucher, qVoucherBatch, qVoucherConfiguration, qPlan).where(qVoucherBatch.voucherBatchId.eq(qVoucher.voucherBatch.voucherBatchId).and(qVoucherConfiguration.id.eq(qVoucherBatch.voucherConfiguration.id)).and(qVoucherConfiguration.plan.id.eq(qPlan.id)).and(expForVoucher)).fetch();
        List<Tuple> queryResult = query
                ...
                .from(qVoucher)
                .join(qVoucherBatch).on(qVoucherBatch.voucherBatchId.eq(qVoucher.voucherBatch.voucherBatchId))
                .join(qVoucherConfiguration).on(qVoucherConfiguration.id.eq(qVoucherBatch.voucherConfiguration.id))
