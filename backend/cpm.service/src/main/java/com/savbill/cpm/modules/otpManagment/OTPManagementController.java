package com.savbill.cpm.modules.otpManagment;
...
import com.savbill.cpm.service.common.OTPManagmentService;
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_OTP_ALL + "\",\"" + AclConstants.OPERATION_OTP_ADD + "\")")
    ...
    @PostMapping
     public ResponseEntity<?> add(@RequestBody OTPManagementDto otpProfile, HttpServletRequest request) {
        MDC.put("type", "create");
         HashMap<String, Object> response = new HashMap<>();
        Integer RESP_CODE = APIConstants.FAIL;
         try {
            OTPManagement otpManagement=otpManagementService.saveOtpProfile(otpProfile,request);
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_OTP_ALL + "\",\"" + AclConstants.OPERATION_OTP_EDIT + "\")")
    ...
    @PutMapping("/{profileId}")
     public ResponseEntity<?> update(@Valid @RequestBody UpdateOTPManagementDto optManagement, HttpServletRequest request) {
        HashMap<String, Object> response = new HashMap<>();
        MDC.put("type", "Update");
        Integer RESP_CODE = APIConstants.FAIL;
        ...
        try {
            ...
            OTPManagement otpManagement = otpManagementService.updateOtpProfile(optManagement,request);
            response.put(OTP_PROFILE, otpManagement);
            RESP_CODE = APIConstants.SUCCESS;
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_OTP_ALL + "\",\"" + AclConstants.OPERATION_OTP_DELETE + "\")")
    ...
    public ResponseEntity<?> delete(@PathVariable(name = "profileId", required = true) Long profileId, HttpServletRequest request) {
        HashMap<String, Object> response = new HashMap<>();
        MDC.put("type", "Delete");
        Integer RESP_CODE = APIConstants.FAIL;

        try {
            otpManagementService.deleteOtpProfileById(profileId);
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_OTP_ALL + "\",\"" + AclConstants.OPERATION_OTP_VIEW + "\")")
    ...
    @GetMapping("/{profileIds}")
     public ResponseEntity<?> get(@PathVariable(name = "profileIds", required = true) Long profileId ) {
        ...
        try {
            OTPManagement otpManagement=otpManagementService.getOtpProfileById(profileId);
             response.put(OTP_PROFILE, otpManagement);
             RESP_CODE = APIConstants.SUCCESS;
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_OTP_ALL + "\",\"" + AclConstants.OPERATION_OTP_VIEW + "\")")
    ...
    public ResponseEntity<?> getByName(@PathVariable(name = "name", required = true) String name) {
        ...
        try {
            List<OTPManagement> otpManagementList=otpManagementService.getOtpProfileByProfileName(name);
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_OTP_ALL + "\",\"" + AclConstants.OPERATION_OTP_VIEW + "\")")
    ...
    public ResponseEntity<?> getAll(HttpServletRequest request) {
        ...
        try {
            List<OTPManagement> otpManagementList=otpManagementService.findAll();
