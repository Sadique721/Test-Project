...
import com.savbill.cpm.constants.CaseConstants;
...
import com.savbill.cpm.modules.DebitDocumentInventoryRel.DebitDocumentInventoryRelRepository;
...
import com.savbill.cpm.modules.Matrix.domain.QTatMatrixWorkFlowDetails;
...
import com.savbill.cpm.modules.mvnoDocDetails.domain.MvnoDocDetails;
...
import com.savbill.cpm.modules.subscriber.model.DeactivatePlanReqModel;
...
import com.savbill.cpm.pojo.api.CreditDebitDataPojo;
import com.savbill.cpm.pojo.api.CreditDebitMappingPojo;
...
import com.savbill.cpm.repository.common.ShorterRepository;
...
//                        tatUtils.sendNotificationToStaff(details);
    ...
    public void runSchedulerForDunning() throws Exception {
        log.info("XXXXXXXXXXXX----------CRON TIME_FOR_DUNNING_SCHEDULER START---------XXXXXXXXXXXX");
        SchedulerAudit schedulerAudit = new SchedulerAudit();
        schedulerAudit.setStartTime(LocalDateTime.now());
        ...
        if (!schedulerLockService.isSchedulerLocked(CronjobConst.TIME_FOR_DUNNING)) {
            ...
            try {
                ...
                UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken(user, null, new ArrayList<>());
                SecurityContextHolder.getContext().setAuthentication(auth);
                ApplicationLogger.logger.debug("***** Savbill Api gateway scheduler  for dunning Starting *****");
                System.out.println("********* Savbill Api gateway schedular for dunning started*****");
...
//        if(!CollectionUtils.isEmpty(pojos)){
                ...
                schedulerAudit.setEndTime(LocalDateTime.now());
                schedulerAudit.setDescription("Time For Dunning Scheduler Run Success");
                schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_SUCCESS);
                schedulerAudit.setTotalCount(null);
//        }
            } catch (Exception ex) {
                ...
                log.info("XXXXXXXXXXXX---------- Time For Dunning Scheduler Locked released ---------XXXXXXXXXXXX");
            }
        } else {
            schedulerAudit.setEndTime(LocalDateTime.now());
            schedulerAudit.setDescription("Time For Dunning Scheduler Lock held by another instance");
            schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_LOCKED);
            schedulerAuditService.saveEntity(schedulerAudit);
            log.warn("XXXXXXXXXXXX----------Time For Dunning Scheduler Locked held by another instance---------XXXXXXXXXXXX");
        }
...
//        }
    ...
    public void runSchedulerForPartnerCount() {
        ...
        if (!schedulerLockService.isSchedulerLocked(CronjobConst.TIME_FOR_PARTNER_COUNT)) {
            ...
            try {
                ApplicationLogger.logger.debug("***** Savbill Api gateway scheduler  for partner count Starting *****");
                updatePartnerCount();
...
//        ApplicationLogger.logger.debug("***** Savbill Api gateway scheduler  for Activating service in ezBill  Starting *****");
                            ...
                            switch (action.getAction()) {
                                case CommonConstants.DUNNING_ACTION_TYPE_EMAIL:
                                    sendEmailNotificationForDunning(
                                            customersService.getCustomersForDunning(type, dunningSector, category, action.getDays(), customerPayType, branchIds, partnerIds), ccEmail ,action.getDays());
                                    break;
                                case CommonConstants.DUNNING_ACTION_TYPE_SMS:
                                    sendSMSNotificationForDunning(
                                            customersService.getCustomersForDunning(type, dunningSector, category, action.getDays(), customerPayType, branchIds, partnerIds), ccMobile , action.getDays());
                                    break;
...
//    }
    ...
    void sendEmailNotificationForDunning(List<Customers> customers, String ccEmail, Integer dateDiff) {
        for (Customers tuple : customers) {
            ...
            if (tuple.getWalletbalance() > 0) {
                try {
                    Customers customers1 = customersRepository.findById(tuple.getId()).orElse(null);
                    if (customers1 == null) continue;
                        ...
                        if (tuple.getStatus().equalsIgnoreCase("Active") && optionalTemplate.isPresent()) {
                            ...
                            if (template.isSmsEventConfigured() || template.isEmailEventConfigured()) {
                                CustomerDunningMessage dunningMessage = new CustomerDunningMessage(
                                        ...
                                        RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, tuple,
                                        Math.abs(customers1.getWalletbalance()), currencySymbol, LocalDate.now(), null);
    ...
    void sendSMSNotificationForDunning(List<Customers> customers, String ccMobile , Integer dateDiff) {
       // List<Customers> distinctCustomers = customers.stream().distinct().collect(Collectors.toList());
        for (Customers customer : customers) {
            ...
            if (customer.getWalletbalance() > 0) {
                try {
                    Customers customers1 = customersRepository.findById(customer.getId()).get();
                        ...
                        if (customer.getStatus().equalsIgnoreCase("Active")) {
                            if (optionalTemplate.isPresent()) {
                                if (optionalTemplate.get().isSmsEventConfigured()
                                    ...
                                    CustomerDunningMessage dunningMessage = new CustomerDunningMessage(
                                            RabbitMqConstants.CUSTOMER_DUNNING_TEMPLATE_HEADER, optionalTemplate.get(),
                                            RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, customer,
                                            Math.abs(customers1.getWalletbalance()), currencySymbol, LocalDate.now(),null);
                                ...
                                if (optionalTemplate.isPresent()) {
                                    if (optionalTemplate.get().isSmsEventConfigured()
                                        ...
                                        StaffExpiredMassage dunningMessage = new StaffExpiredMassage(
                                                RabbitMqConstants.EXPIRED_DOCUMENT_TEMPLATE_HEADER, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, staffUser, customerDocDetails1.getCustomer().getUsername(),customerDocDetails1.getCustomer().getMvnoId(),buId);
                                        dunningMessage.setEmailConfigured(false);
                                        dunningMessage.setSmsConfigured(true);
                                        Gson gson = new Gson();
                                        gson.toJson(dunningMessage);
                                ...
                                if (optionalTemplate2.isPresent()) {
                                    if (optionalTemplate2.get().isSmsEventConfigured()
                                        ...
                                        CustomerExpiredDocumentMessage dunningMessage = new CustomerExpiredDocumentMessage(
                                                RabbitMqConstants.CUSTOMER_DUNNING_DOCUMENT_TEMPLATE_HEADER, optionalTemplate2.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, customer, staffUser.getUsername());
                                        dunningMessage.setEmailConfigured(false);
                                        dunningMessage.setSmsConfigured(true);
                                        Gson gson = new Gson();
                                        gson.toJson(dunningMessage);
...
//                                            messageSender.send(dunningMessage, RabbitMqConstants.QUEUE_BSS_DOCUMENT_DUNNING_STAFF);
                                    ...
                                    if (optionalTemplate2.isPresent()) {
                                        if (optionalTemplate2.get().isSmsEventConfigured()
                                                || optionalTemplate2.get().isEmailEventConfigured()) {
                                            CustomerExpiredDocumentMessage dunningMessage = new CustomerExpiredDocumentMessage(
                                                    RabbitMqConstants.CUSTOMER_DUNNING_DOCUMENT_DEACTIVATION_TEMPLATE, optionalTemplate2.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, customer, staffUser.getUsername());
                                            dunningMessage.setEmailConfigured(true);
                                            dunningMessage.setSmsConfigured(false);
                                            Gson gson = new Gson();
                                            gson.toJson(dunningMessage);
...
//                                            messageSender.send(dunningMessage, RabbitMqConstants.QUEUE_PARTNER_DUNNING_DOCUMENT_DEACTIVATION_STAFF);
                                    ...
                                    if (optionalTemplate2.isPresent()) {
                                        if (optionalTemplate2.get().isSmsEventConfigured()
                                                || optionalTemplate2.get().isEmailEventConfigured()) {
                                            CustomerExpiredDocumentMessage dunningMessage = new CustomerExpiredDocumentMessage(
                                                    RabbitMqConstants.CUSTOMER_DUNNING_DOCUMENT_DEACTIVATION_TEMPLATE_HEADER, optionalTemplate2.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, customer, staffUser.getUsername());
                                            dunningMessage.setEmailConfigured(true);
                                            dunningMessage.setSmsConfigured(false);
                                            Gson gson = new Gson();
                                            gson.toJson(dunningMessage);
                        ...
                        if (partnerIds.contains(partner.getId().longValue())) {
                            if (partner.getStatus().equalsIgnoreCase("Active")) {
                                ...
                                if (optionalTemplate.isPresent()) {
                                    if (optionalTemplate.get().isSmsEventConfigured()
                                            || optionalTemplate.get().isEmailEventConfigured()) {
                                        StaffExpiredMassage dunningMessage = new StaffExpiredMassage(RabbitMqConstants.EXPIRED_DOCUMENT_TEMPLATE_HEADER, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, staffUser, partnerdocDetails1.getPartner().getName(),partnerdocDetails1.getPartner().getMvnoId(),partnerdocDetails1.getPartner().getBuId().intValue());
                                        dunningMessage.setEmailConfigured(false);
                                        dunningMessage.setSmsConfigured(true);
                                        Gson gson = new Gson();
                                        gson.toJson(dunningMessage);
                                ...
                                if (optionalTemplate2.isPresent()) {
                                    if (optionalTemplate.get().isSmsEventConfigured()
                                        ...
                                        PartnerExpiredDocumentMessage dunningMessage = new PartnerExpiredDocumentMessage(
                                                RabbitMqConstants.PARTNER_DUNNING_DOCUMENT_TEMPLATE_HEADER, optionalTemplate2.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, partner, staffUser.getUsername());
                                        dunningMessage.setEmailConfigured(false);
                                        dunningMessage.setSmsConfigured(true);
                                        Gson gson = new Gson();
                                        gson.toJson(dunningMessage);
                            ...
                            if (partner.getStatus().equalsIgnoreCase("Active")) {
                                ...
                                if (optionalTemplate.isPresent()) {
                                    if (optionalTemplate.get().isSmsEventConfigured()
                                        ...
                                        StaffExpiredMassage dunningMessage = new StaffExpiredMassage(
                                                RabbitMqConstants.EXPIRED_DOCUMENT_TEMPLATE_HEADER, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, staffUser, partnerdocDetails1.getPartner().getName(),partnerdocDetails1.getPartner().getMvnoId(),buId);
                                        dunningMessage.setEmailConfigured(true);
                                        dunningMessage.setSmsConfigured(false);
                                        Gson gson = new Gson();
                                        gson.toJson(dunningMessage);
                        ...
                        if (partnerIds.contains(partner.getId().longValue())) {
                            ...
                            if (optionalTemplate.isPresent()) {
                                if (optionalTemplate.get().isSmsEventConfigured()
                                    ...
                                    StaffExpiredMassage dunningMessage = new StaffExpiredMassage(
                                            RabbitMqConstants.PARTNER_DUNNING_DOCUMENT_DEACTIVATION_STAFF_TEMPLATE_HEADER, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, staffUser, partnerdocDetails1.getPartner().getName(),partnerdocDetails1.getPartner().getMvnoId(),buId);
                                    dunningMessage.setEmailConfigured(true);
                                    dunningMessage.setSmsConfigured(true);
                                    Gson gson = new Gson();
                                    gson.toJson(dunningMessage);
                                    ...
                                    if (optionalTemplate.isPresent()) {
                                        if (optionalTemplate.get().isSmsEventConfigured()
                                            ...
                                            CustomerDeactivationMessage dunningMessage = new CustomerDeactivationMessage(
                                                    ...
                                                    RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, customer,
                                                    infoPojo.getClosingBalance(), currencySymbol, "Insufficient Balance/Plan Expired", planname, planDate.get(0).toString());
                                            dunningMessage.setEmailConfigured(true);
                                            dunningMessage.setSmsConfigured(true);
                                            Gson gson = new Gson();
                                            gson.toJson(dunningMessage);
...
//                }
    ...
    void sendEmailNotificationForAdvance(List<Customers> customers, Integer dateDiff, String ccEmail, String token, Boolean isGeneratePaymentLink) {
        ...
        log.info("EMAIL FOR ADVANCE DUNNING STARTED FOR {} CUSTOMERS", customers.size());
        ...
        try {
            ...
            customerBatches.parallelStream().forEach(batch -> {
                // CRITICAL FIX: Extract IDs ONLY from the current batch, not the full list
                List<Integer> batchIds = batch.stream().map(Customers::getId).collect(Collectors.toList());

                // 1. Fetch auxiliary data for this batch
                Map<Integer, Boolean> flagMap = getFlagsForDunningDuplicationEmailNew(batchIds);
                Map<Integer, List<LocalDate>> expiryDatesMap = getExpiryDates(batchIds, dateDiff);
                Map<Integer, List<String>> postpaidPlanMap = getPostpaidPlans(batchIds);
                Map<Integer, Double> paymentMap = getTotalPayMent(batchIds);
                Map<Integer, List<LocalDate>> startDateMap = getStartDates(batchIds);
                Map<Integer, List<LocalDate>> endDateMap = getEndDates(batchIds);
                Map<Integer, List<LocalDateTime>> dueDateMap = getDuedate(batchIds, dateDiff);
                Map<Integer, Double> taxAmountMap = getTaxAmounts(batchIds);
                Map<Integer, Double> subtotalMap = getSubtotal(batchIds);
                Map<Integer, Double> totalDueMap = getTotalDue(batchIds);
                Map<Integer, List<String>> debitdocId = getDebidocId(batchIds, dateDiff, token);

                // 2. Fetch Tax Percentages (Isolated try-catch)
                Map<Integer, Double> taxPercentageMap = new HashMap<>();
                try {
                    ResponseEntity<Map<Integer, List<Double>>> taxResponse = revenueClient.getTaxPercentagesByCustomers(token, batchIds);
                    if (taxResponse != null && taxResponse.getStatusCode().is2xxSuccessful() && taxResponse.getBody() != null) {
                        taxResponse.getBody().forEach((custId, taxList) -> {
                            double totalTax = taxList.stream().filter(Objects::nonNull).mapToDouble(Double::doubleValue).sum();
                            taxPercentageMap.put(custId, totalTax);
                        });
                    }
                } catch (Exception e) {
                    log.error("Failed to fetch tax percentages for batch {}: {}", batchIds, e.getMessage());
                }

                // 3. Fetch Wallet Balances (Isolated try-catch + Key normalization)
                Map<Integer, Double> walletBalanceMap = new HashMap<>();
                try {
                    List<CustomerLedgerDtlsPojo> pojoList = batchIds.stream().map(id -> {
                        CustomerLedgerDtlsPojo p = new CustomerLedgerDtlsPojo();
                        p.setCustId(id);
                        return p;
                    }).collect(Collectors.toList());
                    ...
                    if (response != null && response.getStatusCode().is2xxSuccessful() && response.getBody() instanceof Map) {
                        ...
                        Map<?, ?> details = (Map<?, ?>) body.get("customerWalletDetails");
                        if (details != null) {
                            details.forEach((k, v) -> {
                                Integer cid = Integer.valueOf(k.toString());
                                Double bal = Double.valueOf(v.toString());
                                walletBalanceMap.put(cid, bal);
                            });
                        }
                    }
                } catch (Exception e) {
                    log.error("Failed to fetch wallet balances for batch {}: {}", batchIds, e.getMessage());
                }

                // 4. Generate Links & Notifications
                Map<Integer, String> paymentUrlMap = (Boolean.TRUE.equals(isGeneratePaymentLink))
                        ? generatePaymentLinks(batchIds, token)
                        : new HashMap<>();
                ...
                if (!optionalTemplate.isPresent()) {
                    log.error("Dunning template not found!");
                    return;
                }

                TemplateNotification template = optionalTemplate.get();

                // 5. Iterative Notification Dispatch
                batch.forEach(customer -> {
                    if (Boolean.TRUE.equals(flagMap.getOrDefault(customer.getId(), false))) {
                        try {
                            customersRepository.updateDunningStatus(customer.getId(), LocalDateTime.now());

                            List<String> plans = postpaidPlanMap.getOrDefault(customer.getId(), Collections.emptyList());
                            if (!plans.isEmpty() && "Active".equalsIgnoreCase(customer.getStatus())) {

                                customer.setPlanName(plans.get(0));
                                ClientService cs = clientService.getCurrencyByNameAndMvnoId(ClientServiceConstant.CURRENCY_SYMBOL, customer.getMvnoId());

                                String debitDocNum = Optional.ofNullable(debitdocId.get(customer.getId()))
                                        .filter(l -> !l.isEmpty()).map(l -> l.get(l.size() - 1)).orElse(null);

                                sendAdvanceDunningEmailNotificationInBatch(
                                        customer, template,
                                        expiryDatesMap.getOrDefault(customer.getId(), Collections.emptyList()),
                                        startDateMap.getOrDefault(customer.getId(), Collections.emptyList()),
                                        endDateMap.getOrDefault(customer.getId(), Collections.emptyList()),
                                        (cs != null ? cs.getValue() : ""),
                                        paymentUrlMap.get(customer.getId()),
                                        paymentMap.getOrDefault(customer.getId(), 0.0),
                                        taxAmountMap.getOrDefault(customer.getId(), 0.0),
                                        taxPercentageMap.getOrDefault(customer.getId(), 0.0),
                                        dueDateMap.getOrDefault(customer.getId(), Collections.emptyList()),
                                        subtotalMap.getOrDefault(customer.getId(), 0.0),
                                        totalDueMap.getOrDefault(customer.getId(), 0.0),
                                        walletBalanceMap.getOrDefault(customer.getId(), 0.0),
                                        debitDocNum
                                );
                            }
                        } catch (Exception e) {
                            log.error("Error dispatching notification for customer {}: {}", customer.getId(), e.getMessage());
                        }
                    }
                });
            });
        } catch (Exception e) {
            log.error("Fatal error in sendEmailNotificationForAdvance: ", e);
        }
    }
...
//                        Map<Integer, String> invoiceNumber = getInvoiceNumber(customerIds, dateDiff);
                        ...
                        Map<Integer, Double> taxPercentageMap = new HashMap<>();
                        try {
                            ResponseEntity<Map<Integer, List<Double>>> taxResponse = revenueClient.getTaxPercentagesByCustomers(token, customerIds);
                            if (taxResponse != null && taxResponse.getStatusCode().is2xxSuccessful() && taxResponse.getBody() != null) {
                                taxResponse.getBody().forEach((customerId, taxList) -> {
                        ...
                        try {
                            ...
                            if (!pojoList.isEmpty()) {
                                ...
                                if (response != null && response.getBody() instanceof Map) {
                                    ...
                                    Map<Integer, Double> responseMap = (Map<Integer, Double>) ((Map) response.getBody()).get("customerWalletDetails");
    ...
    void sendSMSNotificationForDunningAdvance(List<Customers> customers, Integer dateDiff, String ccMobile, String token, Boolean isGeneratepaymentLink) {
        ...
        log.info("SMS FOR ADVANCE DUNNING STARTED FOR {} CUSTOMERS", customers.size());
        try {
            int batchSize = 500;
            List<List<Customers>> customerBatches = Lists.partition(customers, batchSize);

            customerBatches.parallelStream().forEach(batch -> {
                // FIX 1: Extract IDs ONLY from the current 500-customer batch
                List<Integer> batchIds = batch.stream().map(Customers::getId).collect(Collectors.toList());

                // FIX 2: Fetch data using ONLY the batchIds
                Map<Integer, Boolean> flagMap = getFlagsForDunningDuplicationNew(batchIds);
                Map<Integer, List<LocalDate>> expiryDatesMap = getExpiryDates(batchIds, dateDiff);
                Map<Integer, List<String>> postpaidPlanMap = getPostpaidPlans(batchIds);
                Map<Integer, Double> paymentMap = getTotalPayMent(batchIds);
                Map<Integer, List<LocalDate>> startDateMap = getStartDates(batchIds);
                ...
                try {
                    ...
                    log.error("Tax fetch failed for batch: {}", batchIds);
                }

                // Fetch Wallet Balances (Corrected Scoping)
                Map<Integer, Double> walletBalanceMap = new HashMap<>();
                try {
                    List<CustomerLedgerDtlsPojo> pojoList = batchIds.stream().map(id -> {
                        CustomerLedgerDtlsPojo p = new CustomerLedgerDtlsPojo();
                    ...
                    if (response != null && response.getStatusCode().is2xxSuccessful() && response.getBody() instanceof Map) {
                        ...
                        if (details != null) {
                            details.forEach((k, v) -> {
                                // FIX 3: Convert key to String then Integer to handle JSON type variance
                                Integer cid = Integer.valueOf(k.toString());
                                Double bal = Double.valueOf(v.toString());
                                walletBalanceMap.put(cid, bal);
                            });
                    ...
                    log.error("Wallet fetch failed for batch: {}", batchIds);
                }
                ...
                if (!optionalTemplate.isPresent()) return;
                TemplateNotification template = optionalTemplate.get();

                // FIX 4: Process batch items. Removed 'throw RuntimeException' to prevent 1 failure killing 499 other SMS
                batch.forEach(customer -> {
                    if (Boolean.TRUE.equals(flagMap.getOrDefault(customer.getId(), false))) {
                        try {
                            customersRepository.updateDunningStatus(customer.getId(), LocalDateTime.now());
                            ...
                            if (!plans.isEmpty() && "Active".equalsIgnoreCase(customer.getStatus())) {
                                ...
                                customer.setPlanName(plans.get(0));
                                ClientService cs = clientService.getCurrencyByNameAndMvnoId(ClientServiceConstant.CURRENCY_SYMBOL, customer.getMvnoId());
                                String currencySymbol = (cs != null) ? cs.getValue() : "";

                                String invNum = Optional.ofNullable(debitdocId.get(customer.getId()))
                                        .filter(l -> !l.isEmpty()).map(l -> l.get(l.size() - 1)).orElse(null);

                                // Send SMS
                                sendAdvanceDunningNotificationInBatch(customer, template,
                                        expiryDatesMap.getOrDefault(customer.getId(), Collections.emptyList()),
                                        startDateMap.getOrDefault(customer.getId(), Collections.emptyList()),
                                        endDateMap.getOrDefault(customer.getId(), Collections.emptyList()),
                                        currencySymbol, paymentUrlMap.get(customer.getId()),
                                        paymentMap.getOrDefault(customer.getId(), 0.0),
                                        taxAmountMap.getOrDefault(customer.getId(), 0.0),
                                        taxPercentageMap.getOrDefault(customer.getId(), 0.0),
                                        dueDateMap.getOrDefault(customer.getId(), Collections.emptyList()),
                                ...
                                // Send CC SMS
                                if (ccMobile != null && ccMobile.length() > 1) {
                                    String originalMobile = customer.getMobile();
                                    customer.setMobile(ccMobile);
                                    sendAdvanceDunningNotificationInBatch(customer, template,
                                            expiryDatesMap.get(customer.getId()), startDateMap.get(customer.getId()),
                                            endDateMap.get(customer.getId()), currencySymbol, paymentUrlMap.get(customer.getId()),
                                            paymentMap.get(customer.getId()), taxAmountMap.get(customer.getId()),
                                            taxPercentageMap.get(customer.getId()), dueDateMap.get(customer.getId()),
                                            subtotalMap.get(customer.getId()), totalDueMap.get(customer.getId()),
                                            walletBalanceMap.get(customer.getId()), invNum);
                                    customer.setMobile(originalMobile); // Restore original mobile
                                }
                            }
                        } catch (Exception e) {
                            log.error("Error processing SMS for customer {}: {}", customer.getId(), e.getMessage());
                        }
                    }
                });
            });
        } catch (Exception e) {
            log.error("Fatal error in SMS notification scheduler: ", e);
        }
    }
    ...
    public void PartnerDeactivateMessageSend(Partner partner, Integer staffId) {
        ...
        if (optionalTemplate.isPresent()) {
            if (optionalTemplate.get().isSmsEventConfigured()
                    || optionalTemplate.get().isEmailEventConfigured()) {
                PartnerExpiredDocumentMessage dunningMessage = new PartnerExpiredDocumentMessage(
                        RabbitMqConstants.PARTNER_DUNNING_DOCUMENT_DEACTIVATION_TEMPLATE_HEADER, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, partner, staffUser.getUsername());
                dunningMessage.setEmailConfigured(true);
                dunningMessage.setSmsConfigured(true);
                Gson gson = new Gson();
                gson.toJson(dunningMessage);
...
//        messageSender.send(mvnoDocumentDunningMessage , RabbitMqConstants.QUEUE_SEND_MVNO_PAYMENT_REMINDER_NOTIFICATION);
    ...
    @Async
    @Scheduled(cron = "${cronJobTimeForKRACustomerSync}")
    public void runSchedulerForKRACustomerSync() {
        log.info("XXXXXXXXXXXX----------CRON TIME_FOR_KRA_CUSTOMER_SYNC_SCHEDULER START---------XXXXXXXXXXXX");
        SchedulerAudit schedulerAudit = new SchedulerAudit();
        schedulerAudit.setStartTime(LocalDateTime.now());
        schedulerAudit.setSchedulerName(CronjobConst.SCHEDULER_AUDIT.TIME_FOR_KRA_CUSTOMER_SYNC_SCHEDULER);
        if (!schedulerLockService.isSchedulerLocked(CronjobConst.TIME_FOR_KRA_CUSTOMER_SYNC)) {
            schedulerLockService.acquireSchedulerLock(CronjobConst.TIME_FOR_KRA_CUSTOMER_SYNC);
            try {
                log.info("********* KRA CUSTOMER SYNC SCHEDULER STARTED ********");
                kraUtils.syncCustomersToKRA();
                schedulerAudit.setEndTime(LocalDateTime.now());
                schedulerAudit.setDescription("KRA Customer Sync Scheduler Run Success");
                schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_SUCCESS);
                schedulerAudit.setTotalCount(null);
                log.info("********* KRA CUSTOMER SYNC SCHEDULER ENDED *********");
            } catch (Exception ex) {
                schedulerAudit.setEndTime(LocalDateTime.now());
                schedulerAudit.setDescription(ExceptionUtils.getRootCauseMessage(ex));
                schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_FAILURE);
                log.error(ex.toString(), ex);
                log.error("**********KRA Customer Sync Scheduler Showing ERROR***********");
            } finally {
                schedulerAuditService.saveEntity(schedulerAudit);
                schedulerLockService.releaseSchedulerLock(CronjobConst.TIME_FOR_KRA_CUSTOMER_SYNC);
                log.info("XXXXXXXXXXXX---------- KRA Customer Sync Scheduler Lock released ---------XXXXXXXXXXXX");
            }
        } else {
            schedulerAudit.setEndTime(LocalDateTime.now());
            schedulerAudit.setDescription("KRA Customer Sync Scheduler Lock held by another instance");
            schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_LOCKED);
            schedulerAuditService.saveEntity(schedulerAudit);
            log.warn("XXXXXXXXXXXX----------KRA Customer Sync Scheduler Lock held by another instance---------XXXXXXXXXXXX");
        }
    }


    @Async
    @Scheduled(cron = "${cronJobTimeForKRAPlanSync}")
    public void runSchedulerForKRAPlanSync() {
        log.info("XXXXXXXXXXXX----------CRON TIME_FOR_KRA_PLAN_SYNC_SCHEDULER START---------XXXXXXXXXXXX");
        SchedulerAudit schedulerAudit = new SchedulerAudit();
        schedulerAudit.setStartTime(LocalDateTime.now());
        schedulerAudit.setSchedulerName(CronjobConst.SCHEDULER_AUDIT.TIME_FOR_KRA_PLAN_SYNC_SCHEDULER);
        if (!schedulerLockService.isSchedulerLocked(CronjobConst.TIME_FOR_KRA_PLAN_SYNC)) {
            schedulerLockService.acquireSchedulerLock(CronjobConst.TIME_FOR_KRA_PLAN_SYNC);
            try {
                log.info("********* KRA PLAN SYNC SCHEDULER STARTED ********");
                kraUtils.syncPlansToKRA();
                schedulerAudit.setEndTime(LocalDateTime.now());
                schedulerAudit.setDescription("KRA Plan Sync Scheduler Run Success");
                schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_SUCCESS);
                schedulerAudit.setTotalCount(null);
                log.info("********* KRA PLAN SYNC SCHEDULER ENDED *********");
            } catch (Exception ex) {
                schedulerAudit.setEndTime(LocalDateTime.now());
                schedulerAudit.setDescription(ExceptionUtils.getRootCauseMessage(ex));
                schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_FAILURE);
                log.error(ex.toString(), ex);
                log.error("**********KRA Plan Sync Scheduler Showing ERROR***********");
            } finally {
                schedulerAuditService.saveEntity(schedulerAudit);
                schedulerLockService.releaseSchedulerLock(CronjobConst.TIME_FOR_KRA_PLAN_SYNC);
                log.info("XXXXXXXXXXXX---------- KRA Plan Sync Scheduler Lock released ---------XXXXXXXXXXXX");
            }
        } else {
            schedulerAudit.setEndTime(LocalDateTime.now());
            schedulerAudit.setDescription("KRA Plan Sync Scheduler Lock held by another instance");
            schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_LOCKED);
            schedulerAuditService.saveEntity(schedulerAudit);
            log.warn("XXXXXXXXXXXX----------KRA Plan Sync Scheduler Lock held by another instance---------XXXXXXXXXXXX");
        }
    }
...
public void sendPlanExpiryNotification() {
    ...
    if (!schedulerLockService.isSchedulerLocked(CronjobConst.TIME_FOR_PLAN_EXPIRY_NOTIFICATION)) {
        ...
        try {
                ...
                for (CustPlanMappping custPlanMappping : custPlanMapppingList) {
                    if (custPlanMappping.getCustomer() != null) {
                        ...
                        if (custPlanMappping.getCustomer().getBuId() != null) {
                            ...
                            sendPlanExpiryMessage(custPlanMappping.getCustomer().getEmail(), custPlanMappping.getCustomer().getMobile(), custPlanMappping.getCustomer().getUsername(), PlanName, custPlanMappping.getEndDate().toString(), custPlanMappping.getCustomer().getAltemail(), custPlanMappping.getCustomer().getCountryCode(), custPlanMappping.getCustomer().getMvnoId(), null, custPlanMappping.getCustomer().getId());
                            System.out.println("--------------------------------------PLAN EXPIRY MESSAGE INITIATED--------------------------------------------------");
                        }
            ...
            if(schedulerAudit.getEndTime()==null){
                schedulerAudit.setEndTime(LocalDateTime.now());
            }
            if(schedulerAudit.getStatus()==null){
                schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_SUCCESS);
            }
            schedulerAudit.setDescription("Expiry Plan Notification Scheduler Run Success");
            schedulerAuditService.saveEntity(schedulerAudit);
    ...
    public void sendPlanExpiryMessage(String emailId, String mobileNumber, String customerUsername, String planName, String expiryDate,String ccMail, String countryCode, Integer mvnoid, Integer buid,Integer custId){
        //create a message and send it to notification.

        PlanExpiryNotificationMessage planExpiryNotificationMessage = new PlanExpiryNotificationMessage(emailId,mobileNumber,customerUsername,planName,expiryDate,ccMail,countryCode,mvnoid,buid,custId);
...
//        messageSender.send(planExpiryNotificationMessage, RabbitMqConstants.QUEUE_SEND_PLAN_EXPIRY_NOTIFICATION);
        System.out.println("*********************************PLAN EXPIRY MESSAGE SENT TO NOTIFICATION******************************"+planExpiryNotificationMessage);
    ...
    Map<Integer, String> generatePaymentLinks(List<Integer> customerIds, String token) {
        return customerIds.stream()
                .collect(Collectors.toMap(
                        id -> id,
                        id -> { String hash = shorterRepository.findLatestShorterByCustIdAndIshashusedIsFalse(id).stream()
                                .findFirst()
                                .map(Shorter::getHash)
    ...
    void sendAdvanceDunningNotificationInBatch(Customers customer, TemplateNotification template,
        ...
        CustomerDunningMessage dunningMessage = new CustomerDunningMessage(
                ...
                template,
                RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY,
                customer,
                ...
                if (customerServiceMappingList.size() > 0) {
                    System.out.println("................. Service Resume Started....................");
                     customerServiceMappingList.forEach(customerServiceMapping -> {
                        ...
                        custserviceMappingIds.add(customerServiceMapping.getId());
                        idToDateMap.put(customerServiceMapping.getId(), customerServiceMapping.getServiceHoldDate().toLocalDate());
                        idToDateDiffMap.put(customerServiceMapping.getId(), Long.valueOf(ChronoUnit.DAYS.between(customerServiceMapping.getServiceHoldDate(), customerServiceMapping.getServiceResumeDate())));
                    });
