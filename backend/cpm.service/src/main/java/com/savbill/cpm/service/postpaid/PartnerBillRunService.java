...
import com.savbill.cpm.model.postpaid.PartnerBillRun;
import com.savbill.cpm.pojo.SearchPartnerBillRun;
import com.savbill.cpm.pojo.TriggerBillRun;
import com.savbill.cpm.pojo.api.PartnerBillRunPojo;
import com.savbill.cpm.pojo.api.SearchPartnerBillRunPojo;
import com.savbill.cpm.repository.postpaid.PartnerBillRunRepository;
...
import com.savbill.cpm.model.postpaid.QPartnerBillRun;
...
// 	}
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.PartnerBillRun', '1')")
...
//    		billRunURL="http://167.71.236.158:40080/billing-engine-1.0/billingprocess/generatebill/{billrundate}";
            ...
            billRunURL = CommonConstants.PBILL_RUN_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
                    .replace("{port}", CommonUtils.getInvoiceServer().getWebport())
                    .replace("{billrundate}", billRunDate);

        } else {
            billRunURL = CommonConstants.PARTNER_BILL_RUN_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
                    .replace("{port}", CommonUtils.getInvoiceServer().getWebport())
                    .replace("{billrundate}", billRunDate)
...
//        	billRunURL=billRunURL.replace("{billrundate}", billRunDate).replace("{custid}", "2");
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.PartnerBillRun', '2')")
    public boolean generateInvoice(String billRunId) throws Exception {
        String generatePDFURL = CommonConstants.PGERERATE_INVOICE_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
                .replace("{port}", CommonUtils.getInvoiceServer().getWebport())
    ...
    public boolean emailInvoices(String billRunId) throws Exception {
        String emailPDFURL = CommonConstants.PEMAIL_INVOICE_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
                .replace("{port}", CommonUtils.getInvoiceServer().getWebport())
                .replace("{billrunid}", billRunId);
