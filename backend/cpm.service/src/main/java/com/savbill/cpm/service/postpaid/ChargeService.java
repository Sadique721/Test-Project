...
import com.savbill.cpm.modules.servicePlan.repository.ServiceChargemappingRepo;
...
import com.savbill.cpm.rabbitMq.message.ChargeMessage;
import com.savbill.cpm.rabbitMq.message.InventoryChargeMessage;
...
public class ChargeService extends AbstractService<Charge, ChargePojo, Integer> {
    ...
    public List<Charge> getAllByChargeType(String chargeType) {
        return entityRepository.findByChargetype(chargeType).stream().filter(charge -> charge.getMvnoId() == getMvnoIdFromCurrentStaff() || charge.getMvnoId() == null && charge.getIsDelete().equals(false)).collect(Collectors.toList());
    }
    ...
    public List<ChargePojo> getAllChargePojoByChargeType(String chargeType) {
        ...
        if ((Objects.isNull(businessUnit.getPlanBindingType())) || (CommonConstants.PREDEFINED).equalsIgnoreCase(businessUnit.getPlanBindingType())) {
            chargePojoList = entityRepository.findAllByChargetypeAndIsDeleteIsFalse(chargeType).stream().map(data ->
                    ...
                    .stream().filter(charge -> (charge.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || charge.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1) && (charge.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(charge.getBuId())) &&
    ...
    public List<ChargePojo> getAllChargePojoByChargeTypeAndChargeCategory(String type, String category) {
        return entityRepository.findAllByChargetypeAndChargecategory(type, category).stream().map(data ->
                        chargeMapper.domainToDTO(data, new CycleAvoidingMappingContext())).collect(Collectors.toList())
                .stream().filter(charge -> (charge.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || charge.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1) && (charge.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(charge.getBuId()))).collect(Collectors.toList());
    }
...
//                                    ((CommonConstants.ENTERPRISE).equalsIgnoreCase(chargePojo.getBusinessType()))).collect(Collectors.toList());
    ...
    public Charge saveCharge(Charge charge) {
    	if(getMvnoIdFromCurrentStaff() != null) {
            ...
            charge.setMvnoName(mvnoRepository.findMvnoNameById(getMvnoIdFromCurrentStaff().longValue()));
    	}
    ...
    public boolean duplicateVerifyAtSave(String name) {
        ...
        if (name != null) {
            ...
            if(getMvnoIdFromCurrentStaff() == 1) count = entityRepository.duplicateVerifyAtSave(name);
                ...
                if(getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0) count = entityRepository.duplicateVerifyAtSave(name, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
                else count = entityRepository.duplicateVerifyAtSave(name, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
            }
            if (count == 0) {
                flag = true;
            }
...
//            }
    ...
    @Override
    public Page<Charge> search(List<GenericSearchModel> filterList, Integer page, Integer pageSize, String sortBy, Integer sortOrder) {
        String SUBMODULE = MODULE + " [search()] ";
        PageRequest pageRequest = generatePageRequest(page, pageSize,"id1", sortOrder);
        try {
            QCharge qCharge = QCharge.charge;
            BooleanExpression combinedExp = qCharge.isDelete.eq(false);
            ...
            if (getMvnoIdFromCurrentStaff() != 1) {
                if (getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().isEmpty() || Objects.isNull(businessUnit.getPlanBindingType())) {
                    combinedExp = combinedExp.and(qCharge.mvnoId.in(Arrays.asList(getMvnoIdFromCurrentStaff(), 1)));
                } else if (businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.PREDEFINED)) {
                    combinedExp = combinedExp.and(qCharge.mvnoId.in(Arrays.asList(getMvnoIdFromCurrentStaff())))
                            .and(qCharge.buId.in(getBUIdsFromCurrentStaff()))
                            .and(qCharge.businessType.isNull().or(qCharge.businessType.equalsIgnoreCase(CommonConstants.RETAIL)));
                } else if (businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.ON_DEMAND)) {
                    combinedExp = combinedExp.and(qCharge.mvnoId.in(Arrays.asList(getMvnoIdFromCurrentStaff())))
                            .and(qCharge.buId.in(getBUIdsFromCurrentStaff()))
                            .and(qCharge.businessType.equalsIgnoreCase(CommonConstants.ENTERPRISE));
                }
            }

            for (GenericSearchModel searchModel : filterList) {
                if (searchModel.getFilterColumn() != null) {
                    String col = searchModel.getFilterColumn().trim();
                    String val = searchModel.getFilterValue();
                    if (val == null || val.isEmpty()) continue;

                    if (col.equalsIgnoreCase(SearchConstants.ANY)) {
                        BooleanExpression chargecategory = qCharge.chargecategory.toLowerCase().contains(val.toLowerCase());
                        BooleanExpression taxName = qCharge.tax.name.toLowerCase().contains(val.toLowerCase());
                        BooleanExpression chargeName = qCharge.name.toLowerCase().contains(val.toLowerCase());
                        BooleanExpression status = qCharge.status.toLowerCase().contains(val.toLowerCase());
                        BooleanExpression chargeAmount = qCharge.price.stringValue().contains(val);

                        List<CommonList> commonLists = commonListRepository.findAllByType("chargeType");
                        commonLists = commonLists.stream().filter(x -> x.getText().toLowerCase().contains(val.toLowerCase())).collect(Collectors.toList());
                        BooleanExpression chargeType = qCharge.chargetype.in(commonLists.stream().map(CommonList::getValue).collect(Collectors.toList()));

                        combinedExp = combinedExp.and(chargeName.or(chargecategory).or(chargeType).or(taxName).or(chargeAmount).or(status));
                    } else if (col.equalsIgnoreCase(SearchConstants.CHARGE_NAME)) {
                        combinedExp = combinedExp.and(qCharge.name.toLowerCase().contains(val.toLowerCase()));
                    } else if (col.equalsIgnoreCase(SearchConstants.CHARGE_CATEGORY)) {
                        combinedExp = combinedExp.and(qCharge.chargecategory.toLowerCase().contains(val.toLowerCase()));
                    } else if (col.equalsIgnoreCase(SearchConstants.CHARGE_TYPE)) {
                        combinedExp = combinedExp.and(qCharge.chargetype.toLowerCase().contains(val.toLowerCase()));
                    } else if (col.equalsIgnoreCase(SearchConstants.IS_KRA_SYNCED)) {
                        combinedExp = combinedExp.and(qCharge.status.equalsIgnoreCase(CommonConstants.ACTIVE_STATUS))
                                .and(qCharge.isKraSynced.eq(Boolean.parseBoolean(val)));
                    } else {
                        throw new RuntimeException("Please Provide Search Column!");
                    }
                }
            }
            return entityRepository.findAll(combinedExp, pageRequest);
        } catch (Exception ex) {
            ApplicationLogger.logger.error(SUBMODULE + ex.getMessage(), ex);
            throw ex;
        }
    ...
    public Page<Charge> getChargeByNameOrTypeOrCategory(String s1, PageRequest pageRequest) {
        BusinessUnit businessUnit = new BusinessUnit();
        ...
        BooleanExpression mvnoIds=qCharge.mvnoId.in(Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
        ...
        if(getMvnoIdFromCurrentStaff() == 1) {
            charges=entityRepository.findAll(isDeleted.and(chargeName.or(chargecategory).or(chargeType).or(taxName).or(chargeAmount).or(status)),pageRequest);
            //charges= entityRepository.findAllByNameOrChargetypeOrChargecategory(s1, s1, s1, pageRequest);
        }
        if(getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0 || Objects.isNull(businessUnit.getPlanBindingType())) {
            charges=entityRepository.findAll(isDeleted.and(mvnoIds).and(chargeName.or(chargecategory).or(chargeType).or(taxName).or(chargeAmount).or(status)),pageRequest);
            //charges= entityRepository.findAllByNameOrChargetypeOrChargecategoryAndMvnoIdIn(s1, s1, s1, pageRequest, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
        } else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.PREDEFINED))) {
            BooleanExpression mvnoId=qCharge.mvnoId.in(Arrays.asList(getMvnoIdFromCurrentStaff()));
            BooleanExpression buids=qCharge.buId.in(getBUIdsFromCurrentStaff());
            charges=entityRepository.findAll(isDeleted.and(mvnoId).and(buids).and(businessTypeRetail).and(chargeName.or(chargecategory).or(chargeType).or(taxName).or(chargeAmount).or(status)),pageRequest);
        }
        else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.ON_DEMAND))) {
            BooleanExpression mvnoId=qCharge.mvnoId.in(Arrays.asList(getMvnoIdFromCurrentStaff()));
            BooleanExpression buids=qCharge.buId.in(getBUIdsFromCurrentStaff());
            charges=entityRepository.findAll(isDeleted.and(mvnoId).and(buids).and(businessTypeEnterprise).and(chargeName.or(chargecategory).or(chargeType).or(taxName).or(chargeAmount).or(status)),pageRequest);
            //charges= entityRepository.findAllByNameOrChargetypeOrChargecategoryAndMvnoIdInAndEnterprise(s1, s1, s1, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
        }
        return charges;
    }
    public Page<Charge> getChargeByName(String s1, PageRequest pageRequest) {
        BusinessUnit businessUnit = new BusinessUnit();
        Page<Charge> charges = null;
        if (getBUIdsFromCurrentStaff().size() == 1) {
            businessUnit = businessUnitRepository.findById(getBUIdsFromCurrentStaff().get(0)).get();
        ...
        if(getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0 || Objects.isNull(businessUnit.getPlanBindingType())) {
            charges= entityRepository.findAllByNameAndMvnoIdIn(s1, pageRequest, Arrays.asList(getMvnoIdFromCurrentStaff()   , 1));
        } else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.PREDEFINED))) {
            charges= entityRepository.findAllByNameAndMvnoIdInAndRetail(s1, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
        }
        else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.ON_DEMAND))) {
            charges= entityRepository.findAllByNameAndMvnoIdInAndEnterprise(s1, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
        }
        return charges;
    }

    public Page<Charge> getChargeByType(String s1, PageRequest pageRequest) {
        BusinessUnit businessUnit = new BusinessUnit();
        Page<Charge> charges = null;
        if (getBUIdsFromCurrentStaff().size() == 1) {
            businessUnit = businessUnitRepository.findById(getBUIdsFromCurrentStaff().get(0)).get();
        ...
        if(getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0 || Objects.isNull(businessUnit.getPlanBindingType())) {
            charges= entityRepository.findAllByChargetypeAndMvnoIdIn(s1, pageRequest, Arrays.asList(getMvnoIdFromCurrentStaff()   , 1));
        } else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.PREDEFINED))) {
            charges= entityRepository.findAllByChargetypeAndMvnoIdInAndRetail(s1, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
        }
        else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.ON_DEMAND))) {
            charges= entityRepository.findAllByChargetypeAndMvnoIdInAndEnterprise(s1, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
        }
        return charges;
    }

    public Page<Charge> getChargeByCategory(String s1, PageRequest pageRequest) {
        BusinessUnit businessUnit = new BusinessUnit();
        Page<Charge> charges = null;
        if (getBUIdsFromCurrentStaff().size() == 1) {
            businessUnit = businessUnitRepository.findById(getBUIdsFromCurrentStaff().get(0)).get();
        ...
        if(getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0 || Objects.isNull(businessUnit.getPlanBindingType())) {
            charges= entityRepository.findAllByChargecategoryAndMvnoIdIn(s1, pageRequest, Arrays.asList(getMvnoIdFromCurrentStaff()   , 1));
        } else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.PREDEFINED))) {
            charges= entityRepository.findAllByChargecategoryAndMvnoIdInAndRetail(s1, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
        }
        else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.ON_DEMAND))) {
            charges= entityRepository.findAllByChargecategoryAndMvnoIdInAndEnterprise(s1, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
        }
        return charges;
    }
    ...
    public Page<Charge> getList(Integer pageNumber, Integer customPageSize, String sortBy, Integer sortOrder,
        ...
        if (null == filterList || 0 == filterList.size()) {
            if (getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0 || Objects.isNull(businessUnit.getPlanBindingType())) {
                if(getMvnoIdFromCurrentStaff() != 1){
                    charges = entityRepository.findAll(pageRequest, Arrays.asList(1, getMvnoIdFromCurrentStaff()));
                }else{
                    charges = entityRepository.findAllWithoutMvno(pageRequest);
                }

            } else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.PREDEFINED))) {
                charges = entityRepository.findAllByRetail(pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
            } else if ((businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.ON_DEMAND))) {
                charges = entityRepository.findAllByEnterprise(pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
            }
        } else {
            charges = search(filterList, pageNumber, customPageSize, sortBy, sortOrder);
        }
        return  charges;
    }
...
//        log.warn("Total execution time for Charge.get: " + (endTime - startTime) + "ms");
    ...
    public Charge getEntityForUpdateAndDelete(Integer id) {
        Charge charge = get(id);
        if(charge == null || (!(getMvnoIdFromCurrentStaff() == 1 || getMvnoIdFromCurrentStaff().intValue() == charge.getMvnoId().intValue())&& (charge.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(charge.getBuId()))))
...
//    }
    ...
    public List<ServiceChargeMapping> getchargeByService(String serviceId) {
        List<Integer> chargeIdList = getFilteredChargeIds();
    ...
    public List<Integer> getFilteredChargeIds() {
                ...
                if (!currencySymbol.isEmpty()) {
                    ...
                    if (optionalTemplate.isPresent()) {
                        if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                           ...
                           // PaymentLinkMessage paymentLinkMessage = new PaymentLinkMessage(message, customerName, currencySymbol, paymentAmount, url, mvnoId, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, countryCode, mobileNumber, emailId);
                            Gson gson = new Gson();
...
//                            messageSender.send(chargeMessage, RabbitMqConstants.QUEUE_CHARGE_MGMTN_SUCCESS);
    ...
    public List<ChargePojo> getAllCustomerDirectChargesByServiceId(Integer serviceId) {
        List<ChargePojo> chargePojoList = new ArrayList<>();
        BusinessUnit businessUnit = new BusinessUnit();
        if (getBUIdsFromCurrentStaff().size() == 1) {
            businessUnit = businessUnitRepository.findById(getBUIdsFromCurrentStaff().get(0)).get();
...
//        chargePojoList = charges.stream().map(charge -> chargeMapper.domainToDTO(charge, new CycleAvoidingMappingContext())).collect(Collectors.toList());
    ...
    public List<ChargePojo> getchargeByPriceCompare(double price, String priceCompare, boolean isDeleted) {
        ...
        if(getLoggedInMvnoId() != -1)
            expression = expression.and(qCharge.mvnoId.eq(getLoggedInMvnoId()));
    ...
    public List<ChargePojo> getChargeByMvno(Integer mvnoId) {
        List<Integer> mvnoIds = new ArrayList<>(Collections.singletonList(getLoggedInMvnoId()));
        mvnoIds.add(mvnoId);
