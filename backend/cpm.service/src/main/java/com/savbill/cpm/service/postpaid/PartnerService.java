...
import com.savbill.cpm.MicroSeviceDataShare.SharedMessages.SavePartnerSharedDataMessage;
import com.savbill.cpm.MicroSeviceDataShare.SharedMessages.UpdatePartnerSharedDataMessage;
...
import com.savbill.cpm.modules.PartnerLedger.domain.QPartnerLedger;
...
import com.savbill.cpm.modules.PartnerLedger.repository.PartnerLedgerRepository;
...
import com.savbill.cpm.pojo.api.PartnerAddBalancePojo;
import com.savbill.cpm.pojo.api.PartnerPojo;
import com.savbill.cpm.pojo.api.PartnerTransferBalancePojo;
import com.savbill.cpm.pojo.api.WithdrawCommissionDto;
import com.savbill.cpm.rabbitMq.MessageSender;
import com.savbill.cpm.rabbitMq.message.PartnerMessage;
...
public class PartnerService extends AbstractService<Partner, PartnerPojo, Integer> {
    ...
    public Page<Partner> getList(Integer pageNumber, Integer customPageSize, String sortBy, Integer sortOrder, List<GenericSearchModel> filterList) {
        ...
        if (null == filterList || 0 == filterList.size())
            ...
            else return entityRepository.findAll(pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
    ...
    public Page<Partner> searchEntity(String searchText, Integer pageNumber, int pageSize) {
        ...
        if (getBUIdsFromCurrentStaff().size() == 0)
            ...
            return entityRepository.findAllByNameAndIsDeleteIsFalse(searchText, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
    }

    public List<Partner> getAllActiveEntities() {
        List<Partner> partners = entityRepository.findByStatusAndIsDeleteIsFalse(CommonConstants.ACTIVE_STATUS).stream().filter(partner -> partner.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || partner.getMvnoId() == 1 || partner.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()).collect(Collectors.toList());
    ...
    public List<Partner> getAllEntities() {
        try {
            ...
            return IterableUtils.toList(entityRepository.findAll(aBoolean)).stream().filter(partner -> partner.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || partner.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() && (partner.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(partner.getBuId()) || partner.getId() == 1)).collect(Collectors.toList());
        } catch (Exception ex) {
...
//    }
        ...
        try {
            if (partner != null) {
                ...
                if (null != partner.getCountry()) {
                    Country country = countryService.get(partner.getCountry());
    ...
    public void validateRequest(PartnerPojo pojo, Integer operation) {
        ...
        if (operation.equals(CommonConstants.OPERATION_ADD) || operation.equals(CommonConstants.OPERATION_UPDATE)) {
            ...
            if (pojo.getParentpartnerid() != null && get(pojo.getParentpartnerid()) == null) {
    ...
    public List<Partner> getAllParentPartners(Integer id) {
        return entityRepository.getAllParentPartners(id).stream().filter(partner -> partner.getMvnoId() == getMvnoIdFromCurrentStaff() || partner.getMvnoId() == null).collect(Collectors.toList());
    }
...
//        }
    ...
    public List<AuditForResponseModel> getPartnerListForAuditFor() {
        ...
        try {
            List<Partner> partnerList = getAllActiveEntities().stream().filter(partner -> partner.getMvnoId() == getMvnoIdFromCurrentStaff() || partner.getMvnoId() == null).collect(Collectors.toList());
...
//    }
    ...
    public boolean checkPartnerUniqueEmail(String email, Integer partnerId) {
        ...
        List<Partner> partnerList = entityRepository.findAllByEmailAndIsDeleteIsFalseOrderByIdDesc(email).stream().filter(partner -> partner.getMvnoId() == getMvnoIdFromCurrentStaff() || partner.getMvnoId() == null).collect(Collectors.toList());
...
//                        }
    ...
    public List<Partner> getPartnersByServiceAreaId(List<Integer> serviceAreaId) {
        QPartner qPartner = QPartner.partner;
        BooleanExpression booleanExpression = qPartner.isNotNull().and(qPartner.isDelete.eq(false));
        if (getLoggedInUserId() != 1) {
            List<Integer> serviceIDs = new ArrayList<Integer>();
