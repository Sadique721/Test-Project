...
public interface PriceBookRepository extends JpaRepository<PriceBook, Long>, QuerydslPredicateExecutor<PriceBook> {
    ...
    @Query("SELECT new com.savbill.cpm.modules.PriceGroup.domain.PriceBook(pb.id, pb.bookname, pb.status, pb.isDeleted) " +
            "FROM PriceBook pb " +
    ...
    @Query("SELECT new com.savbill.cpm.modules.PriceGroup.domain.PriceBook(pb.id, pb.bookname, pb.status, pb.isDeleted) " +
            "FROM PriceBook pb " +
            "WHERE pb.status = 'ACTIVE' AND (pb.mvnoId = :mvnoId OR pb.mvnoId = 1)")
    List<PriceBook> findActiveByMvnoId(@Param("mvnoId") Integer mvnoId);

    @Query("SELECT new com.savbill.cpm.modules.PriceGroup.domain.PriceBook(pb.id, pb.bookname, pb.status, pb.isDeleted) " +
            "FROM PriceBook pb " +
            "WHERE pb.status = 'ACTIVE' AND (pb.mvnoId = :mvnoId OR pb.mvnoId = 1) AND pb.buId IN :buIds")
    List<PriceBook> findActiveByMvnoIdAndBuIds(@Param("mvnoId") Integer mvnoId,
