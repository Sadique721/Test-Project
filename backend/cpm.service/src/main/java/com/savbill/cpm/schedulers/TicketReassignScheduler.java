...
import com.savbill.cpm.modules.tickets.domain.TicketReasonCategoryTATMapping;
import com.savbill.cpm.modules.tickets.service.TicketReasonCategoryService;
...
//    private AuthenticationManager authenticationManager;
    ...
    public void runSchedulerForTicket() {
        ...
        if (!schedulerLockService.isSchedulerLocked(CronjobConst.TIME_FOR_REASSIGN_TICKET)) {
            ...
            try {
//        ApplicationLogger.logger.debug("***** Savbill Api gateway scheduler  for ticker action  Starting *****");
                System.out.println("***** Savbill Api gateway scheduler  for ticket action  Starting *****");
        ...
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    TicketAssignMessege ticketAssignMessege = new TicketAssignMessege(username, mobileNumber, emailId, mvnoId, RabbitMqConstants.TICKET_ASSIGN_SUCCESS, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, caseNumber, name, nextFollowupDate, staffusername);
                    Gson gson = new Gson();
