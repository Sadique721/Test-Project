...
public interface WorkflowAuditRepository extends JpaRepository<WorkflowAudit, Long>, QuerydslPredicateExecutor<WorkflowAudit> {
    ...
    value = "select * from tbltworkflowaudit t where (t.event_id =:eventid and t.custcaf_id =:id)")
    ...
    @Query(nativeQuery = true,
            value = "select * from tbltworkflowaudit t where t.plan_id =:id")
    Page<WorkflowAudit> findByplan_id(@Param("id") Integer id, Pageable pageable);

    //Getall workflowauditbycreditDocid with pagination
    @Query(nativeQuery = true,
            value = "select * from tbltworkflowaudit t where t.creditdoc_id =:id")
    Page<WorkflowAudit> findBycreditdoc_id(@Param("id") Integer id, Pageable pageable);

    @Query(nativeQuery = true,
            value = "select * from tbltworkflowaudit t where t.custpackage_id =:id")
    Page<WorkflowAudit> findBycustPackage_id(@Param("id") Integer id, Pageable pageable);

    @Query(nativeQuery = true,
            value = "select * from tbltworkflowaudit t where t.custcaf_id =:custcaf_id and t.staff_id =:staff_id and t.status ='Pending'")
    WorkflowAudit findByCustCAFANDStaffUser(@Param("custcaf_id") Integer custcaf_id, @Param("staff_id") Integer staff_id);

    @Query(nativeQuery = true,
            value = "select * from tbltworkflowaudit t where t.creditdoc_id =:creditdoc_id and t.staff_id =:staff_id and t.status ='Pending'")
    WorkflowAudit findByCreditDocIDANDStaffUser(@Param("creditdoc_id") Integer creditdoc_id, @Param("staff_id") Integer staff_id);

    @Query(nativeQuery = true,
            value = "select * from tbltworkflowaudit t where t.plan_id =:plan_id and t.staff_id =:staff_id and t.status ='Pending'")
    WorkflowAudit findByPlanIdANDStaffUser(@Param("plan_id") Integer plan_id, @Param("staff_id") Integer staff_id);

    @Query(nativeQuery = true,
            value = "select * from tbltworkflowaudit t where t.custpackage_id =:custpackage_id and t.staff_id =:staff_id and t.status ='Pending'")
    ...
    @Query(nativeQuery = true,
            value = "select t.staff_id  from tblcustomercafassignment t where t.custcaf_id =:id")
    Integer findByNextApprover (@Param("id") Integer id);

    @Query(nativeQuery = true,
            value = "select t.staff_id  from tblcustomercafassignment t where t.creddoc_id =:id")
    Integer findByPaymentNextApprover (@Param("id") Integer id);

    @Query(nativeQuery = true,
            value = "select t.staff_id  from tblcustomercafassignment t where t.custpackage_id =:id")
    Integer findByCustPackIdNextApprover (@Param("id") Integer id);

    @Query(nativeQuery = true,
            value = "select t.staff_id  from tblcustomercafassignment t where t.plan_id =:id")
    Integer findByPlanNextApprover (@Param("id") Integer id);

    @Query(nativeQuery = true,
    value = "select t2.username from tbltcreditdoc t \n" +
            "left join\n" +
            "tblcustomers t2 \n" +
    ...
    @Query(nativeQuery = true,
    value = "select max(t.workflow_audit_id), t.* from tbltworkflowaudit t where t.custpackage_id =:id and t.next_staff_status ='Approved'")
