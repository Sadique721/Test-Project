...
import com.savbill.cpm.rabbitMq.message.PaymentLinkMessage;
import com.savbill.cpm.rabbitMq.message.PaymentSuccess;
                ...
                if (!currencySymbol.isEmpty()) {
                    ...
                    if (optionalTemplate.isPresent()) {
                        if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                            ...
                            PaymentLinkMessage paymentLinkMessage = new PaymentLinkMessage(message, customerName, currencySymbol, paymentAmount, url, mvnoId, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, countryCode, mobileNumber, emailId,buId);
                            Gson gson = new Gson();
...
//                            messageSender.send(paymentLinkMessage, RabbitMqConstants.QUEUE_BSS_CUSTOMER_PAYMENT_LINK);
    ...
    private void sendCustPaymentSuccessMessage(String message, String customerName, Double paymentAmount, String paymentMode, String url, Integer mvnoId, String countryCode, String mobileNumber, String emailId, Integer userId, String reciptNo, String paymentDate) {
        try {
            {
                ...
                if (optionalTemplate.isPresent()) {
                    if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                        ...
                        PaymentSuccess paymentSuccessMessage = new PaymentSuccess(message, customerName, currencySymbol, paymentAmount, paymentMode, mvnoId, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, countryCode, mobileNumber, emailId, userId, reciptNo, paymentDate,buId,null,null);
                        Gson gson = new Gson();
