...
import com.savbill.cpm.modules.payments.util.CCAvenueHelper;
import com.savbill.cpm.modules.payments.util.PGHelper;
import com.savbill.cpm.modules.payments.util.PayuPGHelper;
...
import com.savbill.cpm.modules.placeOrder.repository.OrderRepository;
...
import com.savbill.cpm.modules.purchaseDetails.service.PaymentGatewayResponseService;
            ...
            if (orderDTO != null) {
                ...
                if (orderDTO.getCustId() != null && orderDTO.getCustId() != 0) {
                    if ((pgDTO != null && pgDTO.getUserenableflag())) {
                        customersPojo = customerMapper.domainToDTO(customersService.get(orderDTO.getCustId().intValue())
                        ...
                        if (customersPojo != null) {
                            if (orderDTO.getOrdertype().equalsIgnoreCase(PGConstants.ORDER_TYPE_PLAN)) {
                                PostpaidPlan plan = planService.get(orderDTO.getEntityid().intValue());
                    ...
                    if ((pgDTO != null && pgDTO.getPartnerenableflag())) {
                        partnerPojo = partnerService.convertPartnerModelToPartnerPojo(partnerService.get(orderDTO.getPartnerId().intValue()));
                        if (partnerPojo != null) {
...
//                                customersPojo.setEmail(partnerPojo.getEmail());
                ...
                if (purchaseDTO != null) {
                    if (purchaseDTO.getAmount() != null) {
                        ...
                        purchaseDTO.setPaymentstatus(CommonUtils.getPaymentStatus().get(PGConstants.PENDING_STATUS));
                        purchaseDTO.setPurchaseStatus(CommonUtils.getPaymentStatus().get(PGConstants.PENDING_STATUS));
    ...
    public PurchaseDetailsDTO processPayment(Map<String, Object> response, HttpServletRequest request) throws Exception {
        ...
        if (null != responseModel) {
            ...
            if (pgStatus.equalsIgnoreCase(PGConstants.SUCCESS_STATUS)) {
                sysStatus = CommonUtils.getPaymentStatus().get(PGConstants.SUCCESSFUL_STATUS);
                ...
                sysStatus = CommonUtils.getPaymentStatus().get(PGConstants.PENDING_STATUS);
            } else {
                sysStatus = CommonUtils.getPaymentStatus().get(PGConstants.FAILED_STATUS);
            }
            ...
            if (sysTxnId != null && sysTxnId.trim().length() > 0) {
                ...
                if (null != purchaseDetailsDTO.getPaymentstatus()
                    ...
                    if (purchaseDetailsDTO.getTransid() != null && purchaseDetailsDTO.getTransid().trim().length() > 0) {
                        ...
                        purchaseDetailsDTO.setPurchaseStatus(CommonUtils.getPaymentStatus().get(PGConstants.PENDING_STATUS));
