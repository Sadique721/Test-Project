...
import com.savbill.cpm.mapper.postpaid.BillRunMapper;
import com.savbill.cpm.model.common.Customers;
import com.savbill.cpm.model.postpaid.BillRun;
import com.savbill.cpm.pojo.SearchBillRun;
import com.savbill.cpm.pojo.TriggerBillRun;
import com.savbill.cpm.pojo.api.BillRunPojo;
import com.savbill.cpm.rabbitMq.MessageSender;
import com.savbill.cpm.rabbitMq.message.CustomerBillingMessage;
import com.savbill.cpm.repository.postpaid.BillRunRepository;
...
import com.savbill.cpm.model.postpaid.QBillRun;
...
// 	}
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.BillRun', '1')")
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.BillRun', '4')")
...
//    		billRunURL="http://167.71.236.158:40080/billing-engine-1.0/billingprocess/generatebill/{billrundate}";
            ...
            generatePaymentURL = CommonConstants.PAYMENT_RECEIPT_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
        ...
        try {
            ...
            if (response != null) {
                ...
                HashMap<String, Object> responseMap = (HashMap<String, Object>) CommonUtils.convertJsonToHashMap(responseBody);
    ...
    public boolean generateInvoice(String billRunId) throws Exception {
        String generatePDFURL = CommonConstants.GERERATE_INVOICE_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
                .replace("{port}", CommonUtils.getInvoiceServer().getWebport())
    ...
    public boolean revertInvoice(String invoiceId) throws Exception {
        String revertInvocieURL = CommonConstants.REVERT_INVOICE_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
                .replace("{port}", CommonUtils.getInvoiceServer().getWebport())
...
//    	emailPDFURL=emailPDFURL.replace("{billrunid}",billRunId);
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.BillRun', '2')")
