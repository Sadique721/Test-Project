...
import com.savbill.cpm.model.postpaid.ChargeMapper;
...
import com.savbill.cpm.modules.InventoryManagement.ExternalItemManagement.domain.QExternalItemManagement;
...
import com.savbill.cpm.modules.InventoryManagement.Product_Plan_Mapping.domain.QProductplanmapping;
...
import com.savbill.cpm.modules.InventoryManagement.productCategory.*;
...
import com.savbill.cpm.modules.InventoryManagement.warehouse.WareHouseManagmentServiceAreamappingRepo;
...
//            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_PRODUCT_MANAGEMENT, AclConstants.OPERATION_PRODUCT_MANAGEMENT_ADD, req.getRemoteAddr(), null, outwardDto.getId(), outwardDto.getOutwardNumber());
    ...
    public boolean duplicateVerifyAtSave(String name) throws Exception {
        ...
        if (name != null) {
            name = name.trim();
            Integer count;
            if(getMvnoIdFromCurrentStaff() == 1) count = productRepository.duplicateVerifyAtSave(name);
    ...
    public boolean duplicateProductIdVerifyAtSave(String productId) throws Exception {
        ...
        if (productId != null) {
            ...
            Integer count;
            if(getMvnoIdFromCurrentStaff() == 1) count = productRepository.duplicateProductIdVerifyAtSave(productId);
    ...
    public boolean duplicateVerifyAtEdit(String name, Long id) throws Exception {
        ...
        if (name != null) {
            ...
            if (count >= 1) {
                Integer countEdit;
                if(getMvnoIdFromCurrentStaff() == 1) countEdit = productRepository.duplicateVerifyAtEdit(name, Math.toIntExact(id));
    ...
    public boolean duplicateProductIdVerifyAtEdit(String productId, Long id) throws Exception {
        ...
        if (productId != null) {
            productId = productId.trim();
            Integer count;
            if(getMvnoIdFromCurrentStaff() == 1) count = productRepository.duplicateVerifyAtSave(productId);
            else count = productRepository.duplicateVerifyAtSave(productId, mvnoIds);
            if (count >= 1) {
                Integer countEdit;
                if(getMvnoIdFromCurrentStaff() == 1) countEdit = productRepository.duplicateProductIdVerifyAtEdit(productId, Math.toIntExact(id));
