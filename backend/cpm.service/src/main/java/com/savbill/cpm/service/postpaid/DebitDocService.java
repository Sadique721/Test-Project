...
import com.savbill.cpm.modules.ChangePlanDTOs.ChangePlanMessageList;
...
import com.savbill.cpm.modules.Customers.SendCustomerPaymentDTO;
...
import com.savbill.cpm.modules.subscriber.model.PurchasedHistoryMapper;
...
import com.savbill.cpm.pojo.DebitDocDetailDTO;
...
public class DebitDocService extends AbstractService<DebitDocument, DebitDocumentPojo, Integer> {
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.DebitDocument', '1')")
...
//    	PageRequest pageRequest = PageRequest.of(pageNumber - 1, pageSize);
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.DebitDocument', '2')")
    ...
    public Page<DebitDocSearchPojo> searchInvoice(SearchDebitDocsPojo searchDebitDocsPojo, PaginationRequestDTO paginationDTO, boolean isInvoiceVoid) {
        ...
        try {
            ...
            if(searchDebitDocsPojo.getCustomerid()==null || (searchDebitDocsPojo.getCustomerid()!=null && searchDebitDocsPojo.getCustomerid().intValue()!=1 && searchDebitDocsPojo.getCustomerid().intValue()!=2))
            {
                ...
                if(getMvnoIdFromCurrentStaff()!=null && getMvnoIdFromCurrentStaff()!=1 && getBUIdsFromCurrentStaff()!=null && getBUIdsFromCurrentStaff().isEmpty()) {
                    exp = exp.and(qDebitDocument.customer.mvnoId.eq(1).or(qDebitDocument.customer.mvnoId.eq(getMvnoIdFromCurrentStaff())));
                }

                if (getBUIdsFromCurrentStaff().size() != 0)
                    exp = exp.and(qDebitDocument.customer.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qDebitDocument.customer.buId.in(getBUIdsFromCurrentStaff())));
...
//    }
    ...
    public void voidInvoiceInBatch(List<Integer> invoiceIds) {
        if (invoiceIds != null) {
            for (Integer id : invoiceIds) {
                voidInvoice(id, "customer terminated");
            }
        }
    }

    @Transactional
    public GenericDataDTO voidInvoice(Integer invoiceId, String invoiceCancelRemark) {
        GenericDataDTO genericDataDTO = new GenericDataDTO();

        try {
            DebitDocument debitDocumentForCustomer = super.get(invoiceId);
            QDebitDocument qDebitDocument = QDebitDocument.debitDocument;
                ...
                for (Integer CustRefId : cprids) {
                    ...
                    if (subisinvoiceId.size() > 0) {
                        for (Integer id : subisinvoiceId) {
                            voidInvoice(id, invoiceCancelRemark);
                        }
                    }
                }
            }
...
//        });
    ...
    public void createInvoiceAfterStartBilling(Customers customer) throws InterruptedException {
        ...
        if (custPlanMapppings.stream().filter(custPlanMappping -> custPlanMappping.getPlanGroup() != null).findAny().isPresent()) {
            List<CustPlanMappping> list = custPlanMapppings.stream().filter(CommonUtils.distinctByKey(CustPlanMappping::getStartDate)).collect(Collectors.toList());
...
//            messageSender.send(appproveOrgInvoiceMessage,RabbitMqConstants.QUEUE_APPROVE_ORG_INVOICE_REVENUE);
    ...
    @Transactional
    public GenericDataDTO approveDebitDoc(Long invoiceId, boolean isApproveRequest, String remark) {
        GenericDataDTO genericDataDTO = new GenericDataDTO();
...
//                List<CustPlanMappping> custPlanMapppingList = custPlanMappingRepository.findAllByDebitdocidAndCustPlanStatus(debitDocument.getId(), CommonConstants.STOP_STATUS);
                ...
                adjustBillToSubisuInvoiceWithCreditNote(debitDocument.getAdjustedAmount(), debitDocument);
    ...
    public void adjustPaymentWithNewInvoice(List<Integer> creditDocumentList, Double invoiceAmount, DebitDocument newDebitDocument, Double totalAdjustedAmount) {
        ...
        for (Integer crid : creditDocumentList) {
            ...
            if (!document.getStatus().equalsIgnoreCase(CommonUtils.PAYMENT_STATUS_PENDING)) {
...
//            messageSender.send(message, RabbitMqConstants.QUEUE_CUST_PLAN_MAPPING_UPDATE);
    ...
    public void createInvoice(Customers customers, String type, String paysource, RecordPaymentPojo recordPaymentDTO, AdditionalInformationDTO additionalInformationDTO, List<CustChargeDetailsPojo> custChargeDetailsPojos, boolean changePlanNextBillDate, Boolean isAutoPaymentRequired, List<CreditDocumentPaymentPojo> paymentPojoList, Integer payingChildCustId){
        ...
        createDataSharedService.sendChangePlanForAllMicroService(newCustPlanMapppingList,oldCustPlanMapppingList,customerChargeHistoryList,type,renewalId,null,null,null,paysource, recordPaymentDTO,additionalInformationDTO, overrideChargeIds,changePlanNextBillDate,isAutoPaymentRequired,paymentPojoList,payingChildCustId);
    ...
    public void createInvoice(Set<Customers> customersList,String type,Integer parentId,List<Integer> childIds, RecordPaymentPojo recordPaymentDTO, List<CustChargeDetailsPojo> custChargeDetailsPojos,boolean changePlanNextBillDate,Boolean isAutoPaymentRequired,List<CreditDocumentPaymentPojo> paymentPojoList,Integer payingChildId){
        logger.info("========Inside createInvoice 1 ========= ");
        ...
        createDataSharedService.sendChangePlanForAllMicroService(newCustPlanMapppingList,oldCustPlanMapppingList,customerChargeHistoryList,type,renewalId,null,parentId,childIds,"", recordPaymentDTO,null,overrideChargeIds,false,isAutoPaymentRequired,paymentPojoList,payingChildId);
