package com.savbill.cpm.modules.SectorMaster.Controller;
...
import com.savbill.cpm.modules.SectorMaster.Service.SectorMasterService;
...
public class SectorMasterController extends ExBaseAbstractController<SectorMasterDTO> {
    ...
    @Override
    public GenericDataDTO save(@Valid @RequestBody SectorMasterDTO entityDTO, BindingResult result, Authentication authentication, HttpServletRequest req) throws Exception {
    ...
    @Override
    @PutMapping("/update")
    public GenericDataDTO update(@Valid @RequestBody SectorMasterDTO entityDTO, BindingResult result, Authentication authentication, HttpServletRequest req) throws Exception {
        TraceContext traceContext = tracer.currentSpan().context();
        MDC.put("type", "Update");
        MDC.put("userName", sectorMasterService.getLoggedInUser().getUsername());
        MDC.put(LogConstants.TRACE_ID, req.getHeader(LogConstants.TRACE_ID));
    ...
    //        String oldname=sectorMasterService.getById(entityDTO.getId()).getSname();
            ...
            if(currentMvnoId==1 || dataMvnoId.equals(currentMvnoId)){
                ...
                if (flag) {
                    String updatedvalues = CommonUtils.getUpdatedDiff(sectorMasterMapper.domainToDTO(sectorMasterold, new CycleAvoidingMappingContext()), entityDTO);
    ...
    public GenericDataDTO search(@RequestParam(required = false, defaultValue = "${request.defaultPage}") Integer page
        ...
        try {
            genericDataDTO = sectorMasterService.search(filter.getFilter(), page, pageSize, sortBy,sortOrder,req);
    ...
    public GenericDataDTO getAllWithoutPagination() {
        ...
        try {
            List<SectorMasterDTO> list = sectorMasterService.getAllEntities().stream().filter(x -> !x.getIsDeleted()).collect(Collectors.toList());
