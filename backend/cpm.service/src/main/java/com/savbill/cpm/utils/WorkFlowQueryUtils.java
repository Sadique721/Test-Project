...
import com.savbill.cpm.model.lead.*;
...
import com.savbill.cpm.modules.CaseCustomerDetails.repository.CaseCustomerDetailsRepository;
...
import com.savbill.cpm.modules.CustomerDBR.service.CustomerDBRService;
...
import com.savbill.cpm.modules.InventoryManagement.CustomerInventoryMapping.QCustomerInventoryMapping;
...
import com.savbill.cpm.modules.Teams.domain.QTeamHierarchyMapping;
...
import com.savbill.cpm.repository.LeadCustomerAddressRepository;
...
import com.savbill.cpm.schedulers.ApiGatewayScheduler;
...
WorkFlowQueryUtils {
    ...
    private static final String MODULE = " [WorkFlowQueryUtils] ";
                    ...
                    for (QueryFieldMapping queryFieldMapping : queryFieldMappingList) {
                        switch (queryFieldMapping.getQueryField()) {
                            ...
                            case CommonConstants.SHIFT_LOCATION_CONDITION.CITY: {
                                City city = cityService.get(customerAddressPojo.getCityId());
                                switch (queryFieldMapping.getQueryOperator()) {
                                    case "==": {
                                        condition = city.getName().equalsIgnoreCase(queryFieldMapping.getQueryValue());
                                        break;
                            ...
                            case CommonConstants.SHIFT_LOCATION_CONDITION.STATE: {
                                State state = stateService.get(customerAddressPojo.getStateId());
                                switch (queryFieldMapping.getQueryOperator()) {
                                    case "==": {
                                        condition = state.getName().equalsIgnoreCase(queryFieldMapping.getQueryValue());
                                        break;
...
//                    }
    ...
    public List<StaffUserPojo> assignCAFToStaffFromTeam(List<ServiceArea> serviceAreaList, Long buId, Teams team) {
        ...
        List<StaffUserPojo> returnList;
        Set<StaffUser> staffList = team.getStaffUser();
        staffList = staffList.stream().filter(staffUser -> staffUser.getStatus().equalsIgnoreCase(CommonConstants.ACTIVE_STATUS)).collect(Collectors.toSet());
        ...
        returnList = tempStaffList.stream().map(staffUser -> staffUserMapper.domainToDTO(staffUser, new CycleAvoidingMappingContext())).collect(Collectors.toList());
...
//                    }
    ...
    public void checkAction(String actionName, String eventName, Object entity) {
        switch (eventName) {
                ...
                if (entity instanceof CustomersPojo) {//CAF
                    ...
                    switch (actionName) {
                        ...
                        case CommonConstants.CAF_ACTION.ACTIVATION:
                            ...
                            break;
                        case CommonConstants.CAF_ACTION.Service_Activation:
                            activateCustomerInRadius(customers);
                            break;
                        case CommonConstants.CAF_ACTION.Document_Upload_Digital:
                            documentUploadDigital(customers);
                            break;
            ...
            case CommonConstants.WORKFLOW_EVENT_NAME.SHIFT_LOCATION: {
                if (entity instanceof CustomerAddressPojo) {//shift location
                    CustomerAddressPojo
                            customerAddressPojo = (CustomerAddressPojo) entity;
                    Optional<Customers> customers = customersRepository.findById(customerAddressPojo.getCustomerId());
                    CustomersPojo customersPojo = customerMapper.domainToDTO(customers.get(), new CycleAvoidingMappingContext());

                    switch (actionName) {
                        case CommonConstants.SHIFT_LOCATION_ACTION.INVENTORY_ASSIGNMENT:
                            checkInventoryAssignMent(customersPojo);
                            break;
                        case CommonConstants.SHIFT_LOCATION_ACTION.Service_Activation:
                            activateCustomerInRadius(customersPojo);
                            break;
                        case CommonConstants.SHIFT_LOCATION_ACTION.Document_Upload_Digital:
                            documentUploadDigital(customersPojo);
                            break;

                        case CommonConstants.SHIFT_LOCATION_ACTION.REMOVE_INVENTORY:
                            checkInventoryForTermination(customersPojo);
                            break;
                    }
                }
                break;
...
//                            }
                            if(customerApprove.getIsWalletSetteled()) {
                                CustomerTerminationMessage cutomerCustomerTerminationMessage = new CustomerTerminationMessage();
...
//                                messageSender.send(cutomerCustomerTerminationMessage, SharedDataConstants.QUEUE_CUSTOMER_TERMINATION_DATA_REVENUE);
    ...
    private void approvePayment(CreditDocument creditDocument) {
        ...
        if (creditDocument.getPaymode().equalsIgnoreCase(CommonConstants.PAYMENT_MODE.CREDIT_NOTE)) {
            creditDocService.adjustCreditNote(Optional.of(creditDocument), creditDebitDocMappings);
        } else {
            if (creditDocument.getPaytype().equalsIgnoreCase(CommonConstants.CREDIT_DOC_STATUS.WITHDRAWAL)) {
                ...
                creditDocService.adjustPayment(Optional.of(creditDocument), creditDebitDocMappings);
            }
        }
...
//        postpaidPlan.setNextTeamHierarchyMapping(null);
    ...
    @Transactional
    public void activateCustomerInRadius(CustomersPojo customers) {
        String nmsEnable = clientServiceRepository.findValueByNameAndMvnoId(NMSIntegrationConstants.NMS_INTEGRATION.NMS_INTEGRATION_ENABLE, getMvnoIdFromCurrentStaff());
//        if(nmsEnable.equalsIgnoreCase(NMSIntegrationConstants.NMS_INTEGRATION.TRUE_FLAG)){
//        Customers customer = customersService.get(customers.getId());
//        customer.setStatus(CommonConstants.CUSTOMER_STATUS_SERVICE_ACTIVATION);
//        customersService.update(customer);
//        }
        if (customers != null) {
            CaftoCustomerMessage caftoCustomerMessage = new CaftoCustomerMessage();
            caftoCustomerMessage.setCustomerId(customers.getId());
            caftoCustomerMessage.setLoggedInUser(getLoggedInUserId());
            caftoCustomerMessage.setStatus(CommonConstants.CUSTOMER_STATUS_ACTIVE);
            kafkaMessageSender.send(new KafkaMessageData(caftoCustomerMessage, CaftoCustomerMessage.class.getSimpleName(), KafkaConstant.SERVICE_ACTIVATION));
        }
    }

    @Transactional
    public void documentUploadDigital(CustomersPojo customers) {

        if (customers != null) {
            boolean isSmartGadgetDocUploaded = customerServiceMappingRepository.existsByCustIdAndIsSgImgUpload(customers.getId());
            if(isSmartGadgetDocUploaded){
            Customers customer = customersService.get(customers.getId());
            customer.setStatus(CommonConstants.CUSTOMER_STATUS_ACTIVE);
            customer.setNextTeamHierarchyMapping(null);
            customersService.update(customer);
            ...
            throw new CustomValidationException(HttpStatus.SC_EXPECTATION_FAILED, "Smart Gadget Document not uploaded", new Throwable());
        }
    }
    }
    ...
    private void checkInvoiceGenerated(CustomersPojo customers) {
        Customers checkCustomer = customersService.get(customers.getId());
