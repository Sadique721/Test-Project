...
import com.savbill.cpm.modules.InventoryManagement.productOwner.ProductOwnerDto;
import com.savbill.cpm.modules.InventoryManagement.productOwner.ProductOwnerMapper;
        ...
        if (wareHouseList.size() > 0) {
            ...
            if (getMvnoIdFromCurrentStaff() != 1) {
                if (outwardNumber != null) {
                    paginationList = outwardRepository.findAllByoutwardNumberContainingIgnoreCaseAndDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(outwardNumber, warehouseResult, warehouseDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
                } else {
                    paginationList = outwardRepository.findAllByDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(warehouseResult, warehouseDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
                }
            }
        }
...
//        }
        if (getMvnoIdFromCurrentStaff() != 1) {
            if (outwardNumber != null) {
                paginationList = outwardRepository.findAllByoutwardNumberContainingIgnoreCaseAndDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(outwardNumber, popResult, popDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            } else {
                paginationList = outwardRepository.findAllByDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(popResult, popDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            }
        }
        return paginationList;
    }
...
//        }
        if (getMvnoIdFromCurrentStaff() != 1) {
            if (outwardNumber != null) {
                ...
                QOutward qOutward=QOutward.outward;
                BooleanExpression booleanExpression=qOutward.isNotNull();
                booleanExpression=booleanExpression.and(qOutward.createdById.eq(getLoggedInUserId()).and(qOutward.isDeleted.eq(false)).and(qOutward.mvnoId.in(1,getMvnoIdFromCurrentStaff())));
                paginationList= (List<Outward>) outwardRepository.findAll(booleanExpression);
...
//        }
        if (getMvnoIdFromCurrentStaff() != 1) {
            if (outwardNumber != null) {
                paginationList = outwardRepository.findAllByoutwardNumberContainingIgnoreCaseAndDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(outwardNumber, resultStaffId, partnerDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            } else {
                paginationList = outwardRepository.findAllByDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(resultStaffId, partnerDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            }
        }
        return paginationList;
    }
...
//        }
        if (getMvnoIdFromCurrentStaff() != 1) {
            if (outwardNumber != null) {
                paginationList = outwardRepository.findAllByoutwardNumberContainingIgnoreCaseAndDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(outwardNumber, serviceAreaStaffResult, serviceAreaDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            } else {
                paginationList = outwardRepository.findAllByDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(serviceAreaStaffResult, serviceAreaDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            }
        }
        return paginationList;
    }
