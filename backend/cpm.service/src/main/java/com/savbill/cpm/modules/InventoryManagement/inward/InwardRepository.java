...
public interface InwardRepository extends JpaRepository<Inward, Long>, QuerydslPredicateExecutor<Inward> {
	...
	@Query(nativeQuery = true, value = "select t.inward_id from tbltinward t where t.outward_id =:id")
	Integer findInwardIdByOutwardId(@Param("id") Integer id);

	@Query(nativeQuery = true, value = "select * from tbltinward t where t.product_id =:id")
