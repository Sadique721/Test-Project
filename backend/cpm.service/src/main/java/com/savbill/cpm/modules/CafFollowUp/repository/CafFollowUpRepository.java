...
public interface CafFollowUpRepository extends JpaRepository<CafFollowUp, Long>,QuerydslPredicateExecutor<CafFollowUp>{
	...
	@Query(name = "select * from tbltcaffollowup where assignee_id=:assignee_id and status <> :status",nativeQuery = true)
	Page<CafFollowUp> findByStaffUserIdAndStatusNot(@Param("assignee_id") Integer assignee_id,  @Param("status") String status,Pageable pageable);
