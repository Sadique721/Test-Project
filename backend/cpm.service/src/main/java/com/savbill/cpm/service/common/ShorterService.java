...
import com.savbill.cpm.model.common.Shorter;
...
import com.savbill.cpm.utils.CodeGenerator;
...
public class ShorterService {
    ...
    public Shorter createShortUrl(String url, Integer custId, Double amount, String username, String planname, LocalDateTime planDueDate) {
        ...
        if (url != null) {
            ...
            shorter.setMvnoId(subscriberService.getMvnoIdFromCurrentStaff());
...
//        }
        ...
        shorter.setCustName((Objects.toString(customer.getFirstname(), "") + " " + Objects.toString(customer.getLastname(), "")).trim());
        ...
        if (isActive || isNewActivation) {
            Object[] latestPlan = subscriberService.getActivePlanListByCustomerIdWithExpireDate(shorter.getCustId()).stream().findFirst().orElse(null);
            if (latestPlan != null) {
                shorter.setPlanId(((BigInteger) latestPlan[0]).intValue());
                shorter.setPlanName(String.valueOf(latestPlan[1]));
                //shorter.setPlanDueDate(((Timestamp) latestPlan[2]).toLocalDateTime());
                if (latestPlan != null && latestPlan.length > 2 && latestPlan[2] != null) {
                    shorter.setPlanDueDate(
                            ((Timestamp) latestPlan[2]).toLocalDateTime()
                    );
                } else {
                    shorter.setPlanDueDate(LocalDateTime.now());
                }
            }
            ...
            if (shorter.getLinkType().equals(CommonConstants.RENEW_PAYMENT) && isActive) {
                ...
                shorter.setPlanDueDate(
                        ((Timestamp) renewPlans.get(0)[2]).toLocalDateTime()
                );
            } else if (shorter.getLinkType().equals(CommonConstants.CURRENT_PAYMENT)) {
                if (isActive){
                    ...
                    if (debitDocument != null) {
                        ...
                        shorter.setAmount(Math.abs(difference) < 0.1 ? 0.0 : difference);
                        shorter.setInvoiceId(debitDocument.getId());
                        shorter.setPlanDueDate(debitDocument.getEndate());
                    } else {
                        List<Object[]> renewPlans = subscriberService.getRenewPlanListByCustomerId(shorter.getCustId(), true);
                        BigDecimal value = (BigDecimal) renewPlans.get(0)[3];
                        shorter.setAmount(value != null ? value.doubleValue() : 0.0);
                        shorter.setPlanDueDate(
                                ((Timestamp) renewPlans.get(0)[2]).toLocalDateTime()
