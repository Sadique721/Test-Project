...
import com.savbill.cpm.service.common.CustQuotaService;
...
public class CustPlanMappingService extends AbstractService<CustPlanMappping, CustPlanMapppingPojo, Long> {
    ...
    @Autowired
    CustPlanMappingReader custPlanMappingReader;
...
//            Optional<Customers> customers = customersRepository.findById(pojo.getId());
            Optional<Customers> customers = Optional.ofNullable(customersService.get(pojo.getId()));
...
//            redisTemplates.opsForValue().set("custPlanMappping:" + custPlanMappping.getId(),custPlanMappping, Duration.ofMinutes(1)); // Cache for future
        ...
        log.warn(">>>>>>>>>>>>>>>>>>>>>>>>>>>  " + entity.getId() + " ::::: inner inner inner inner s1 >>>>>>>>>>>>>>>>>>>>>>>>>>>   " + (startTime_s2 - startTime_s1));
        return custPlanMappping;
    }
...
//        List<CustPlanMappping> custPlanMapppingList = custPlanMappingRepository.findAllByIdIn(custServIds);
...
//        List<CustomerServiceMapping> customerServiceMappings = customerServiceMappingRepository.findAllByIdIn(customerServiceMappingIds);
...
//        List<CustPlanMappping> custPlanMapppingList = custPlanMappingRepository.getAllByCustServiceMappingIds(custServIds);
        List<CustPlanMappping> custPlanMapppingList = custPlanMappingReader.fetchUpdatedPlans(custServIds);
...
//                    messageSender.send(message, RabbitMqConstants.QUEUE_DBR_SERVICE_HOLD_RESUME);
                    ...
                    customerServiceMappings.forEach(customerServiceMapping -> {
                        ...
                        if (customerServiceMapping.getUuid() != null) {
                            ...
                            customersService.createHSNService(customerServiceMapping.getCustId(), username, getLoggedInUser().getUsername(), getLoggedInMvnoId(), customerServiceMapping.getCustId(), customerServiceMapping.getId());
                        }
...
//                    custPlanMapppings = custPlanMapppings.stream().filter(custPlanMappping -> custPlanMappping.getCustPlanStatus().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.ACTIVE)).collect(Collectors.toList());
                    custPlanMapppings = custPlanMapppings.stream()
                            .filter(custPlanMappping -> custPlanMappping.getCustPlanStatus().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.ACTIVE))
                            .filter(custPlanMappping -> custPlanMappping.getExpiryDate() != null && custPlanMappping.getExpiryDate().isAfter(LocalDateTime.now()))
                ...
                case StatusConstants.CUSTOMER_SERVICE_STATUS.TERMINATE: {
                    ...
                    logger.debug("=== STOP DEBUG START === Customer: {} | Total plans: {} | Now: {}",
                            custPlanMapppings.get(0).getCustomer().getId(), custPlanMapppings.size(), now);

                    custPlanMapppings.forEach(p ->
                            logger.debug("BEFORE - Plan ID: {} | Status: '{}' | Expiry: {} | isPlanExpiryNotified: {} | isVoid: {}",
                                    p.getId(), p.getCustPlanStatus(), p.getExpiryDate(),
                                    p.getIsPlanExpiryNotified(), p.getIsVoid()));

                    custPlanMapppings = custPlanMapppings.stream()
                            .filter(planMappping -> planMappping.getExpiryDate() != null && !planMappping.getExpiryDate().isBefore(now))
                            .collect(Collectors.toList());

                    logger.debug("After filtering, count={} plans to STOP", custPlanMapppings.size());

                    custPlanMapppings.forEach(p ->
                            logger.debug("PLAN TO STOP - ID: {} | Status: '{}' | Expiry: {} | isPlanExpiryNotified: {}",
                                    p.getId(), p.getCustPlanStatus(), p.getExpiryDate(), p.getIsPlanExpiryNotified()));

                    custPlanMapppings.forEach(custPlanMappping -> {
                        logger.debug("UPDATING Plan ID: {}", custPlanMappping.getId());
                        custPlanMappping.setIsPlanExpiryNotified(true);
                        custPlanMappping.setEndDate(now);
                        custPlanMappping.setExpiryDate(now);
                        custPlanMappping.setIsVoid(Boolean.TRUE);
                        if (custPlanMappping.getStartDate().isAfter(custPlanMappping.getEndDate())) {
                            custPlanMappping.setStartDate(now);
                            custPlanMappping.setEndDate(now.plusSeconds(1));
                            custPlanMappping.setExpiryDate(now.plusSeconds(1));
                        }

                        logger.debug("UPDATED Plan ID: {} | New Expiry: {} | New isPlanExpiryNotified: {} | New isVoid: {}",
                                custPlanMappping.getId(), custPlanMappping.getExpiryDate(),
                                custPlanMappping.getIsPlanExpiryNotified(), custPlanMappping.getIsVoid());
                    });

                    logger.debug(" === STOP DEBUG END - {} plans STOPPED ===", custPlanMapppings.size());
                }
                break;
                case StatusConstants.CUSTOMER_SERVICE_STATUS.INGRACE: {
                    ...
                    Integer graceDays = Integer.valueOf(clientServiceRepository.findValueByNameandMvnoId("graceperiod", getLoggedInMvnoId()));
                    Long systemPromiseToPayCount = Long.valueOf(clientServiceRepository.findValueByNameandMvnoId(ClientServiceConstant.PROMISETOPAY_COUNT, getLoggedInMvnoId()));
...
//                            || custPlanMappping.getCustPlanStatus().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.STOP));
            ...
            if (!CollectionUtils.isEmpty(custPlanMapppings)) {
                custPlanMapppings.forEach(custPlanMappping -> {
                    updateEndAndStartDate(custPlanMappping, custPlanMappping.getEndDate(),status);
                });
...
//                    List<Integer> custServiceIds = list.stream().filter(custServiceMapping -> custServiceMapping.getInvoiceType() != null  && custServiceMapping.getInvoiceType().equalsIgnoreCase(StatusConstants.INVOICE_TYPE.GROUP)).map(CustomerServiceMapping::getId).collect(Collectors.toList());
     ...
     */
    public void updateEndAndStartDate(CustPlanMappping custPlanMappping, LocalDateTime updatedDateTime,String status) {
        custPlanMappping.setEndDate(updatedDateTime);
            if (!StatusConstants.CUSTOMER_SERVICE_STATUS.RESUME.equalsIgnoreCase(status) && !StatusConstants.CUSTOMER_SERVICE_STATUS.HOLD.equalsIgnoreCase(status)) {
    ...
    public void expirePlanByCprId(Integer id) {
        logger.info("++++++++Test fut. expiry start : CPR id:  "+id+"++++++++++++++++");
        CustPlanMappping custPlanMappping = custPlanMapppingRepository.findById(id).get();
        logger.info("++++++++Test fut. expiry custPlanMappping :"+custPlanMappping.toString()+"++++++++++++++++");
        custPlanMappping.setCustPlanStatus(CommonConstants.STOP_STATUS);

        if (custPlanMappping.getStartDate().isAfter(LocalDateTime.now())) {
            logger.info("++++++++Test fut. expiry 1ST IF COND :"+custPlanMappping.getStartDate().isAfter(LocalDateTime.now())+"++++++++++++++++");
            ...
            logger.info("++++++++945  Test fut. expiry custPlanMappping EXPIRY :"+custPlanMappping.getExpiryDate()+"++++++++++++++++");
        } else {
            logger.info("++++++++Test fut. expiry else  :"+custPlanMappping.getStartDate().isAfter(LocalDateTime.now())+"++++++++++++++++");
            ...
            logger.info("++++++++949  Test fut. expiry custPlanMappping EXPIRY :"+custPlanMappping.getExpiryDate()+"++++++++++++++++");
        }
        if (custPlanMappping.getStartDate().isAfter(custPlanMappping.getEndDate())) {
            logger.info("++++++++Test fut. expiry 2nd IF COND :"+custPlanMappping.getStartDate().isAfter(custPlanMappping.getEndDate())+"++++++++++++++++");
            ...
            logger.info("++++++++955  Test fut. expiry custPlanMappping EXPIRY :"+custPlanMappping.getExpiryDate()+"++++++++++++++++");
        }
        custPlanMappping.setIsVoid(true);
        custPlanMapppingRepository.save(custPlanMappping);
        logger.info("++++++++Test fut. expiry custPlanMappping saved successfully++++++++++++++++");
    ...
    /**
     * Method to expire old plan for CAF in case of Change Plan
     * @param custPlanMappping
     * @param oldDebitdocIds
     * @return
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public CustPlanMappping stopAndSaveOldCpr(CustPlanMappping custPlanMappping, List<Integer> oldDebitdocIds) {
        if (custPlanMappping != null) {
            //change CPR
            ...
            Integer custServiceMappingId = custPlanMappping.getCustServiceMappingId();
