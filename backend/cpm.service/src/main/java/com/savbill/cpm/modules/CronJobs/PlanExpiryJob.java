...
import com.savbill.cpm.constants.NotificationConfigConstant;
...
import com.savbill.cpm.modules.Notification.model.NotificationConfigDTO;
import com.savbill.cpm.modules.Notification.model.NotificationDTO;
import com.savbill.cpm.modules.Notification.repository.NotificationRepeatRepository;
import com.savbill.cpm.modules.Notification.service.NotificationService;
import com.savbill.cpm.modules.ippool.model.CustomIpExpiryModel;
import com.savbill.cpm.modules.ippool.model.CustomPlanExpiryModel;
...
public class PlanExpiryJob {
    ...
    @Scheduled(cron = "${planexpiryjob}")
    public void cronJobSch() throws Exception {
        log.info("XXXXXXXXXXXX----------Plan Expiry Scheduler Scheduler START---------XXXXXXXXXXXX");
        SchedulerAudit schedulerAudit = new SchedulerAudit();
        schedulerAudit.setStartTime(LocalDateTime.now());
        schedulerAudit.setSchedulerName(CronjobConst.SCHEDULER_AUDIT.PLAN_EXPIRY_JOB_SCHEDULER);
        if (!schedulerLockService.isSchedulerLocked(CronjobConst.PLAN_EXPIRY_JOB)) {
            schedulerLockService.acquireSchedulerLock(CronjobConst.PLAN_EXPIRY_JOB);
            try {
...
//                                }
//                            }
                                    if (configDTO.getConfig_attribute().equalsIgnoreCase(NotificationConfigConstant.CONFIG_ATTR_PLAN_EXPIRY)) {
                    ...
                    schedulerAudit.setDescription("Plan Expiry Scheduler Success");
                ...
                schedulerLockService.releaseSchedulerLock(CronjobConst.PLAN_EXPIRY_JOB);
                log.info("XXXXXXXXXXXX---------- Plan Expiry Scheduler Locked released ---------XXXXXXXXXXXX");
            }
        } else {
            schedulerAudit.setEndTime(LocalDateTime.now());
            schedulerAudit.setDescription("Plan Expiry Scheduler Lock held by another instance");
            schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_LOCKED);
            schedulerAuditService.saveEntity(schedulerAudit);
            log.warn("XXXXXXXXXXXX----------Plan Expiry Scheduler Locked held by another instance---------XXXXXXXXXXXX");
        }
    }
...
//    public List<Map<String, String>> quotaExpiryMapBuilder(List<CustQuotaDtlsPojo> custQuotaDtlsPojos, String usage) {
//        List<Map<String, String>> mapList = new ArrayList<>();
