package com.savbill.cpm.modules.MtnPayment.service;
...
import com.savbill.cpm.modules.MtnPayment.model.*;
import com.savbill.cpm.modules.MtnPayment.model.*;
...
import com.savbill.cpm.modules.payments.controller.PaytmPaymentController;
...
import com.savbill.cpm.modules.subscriber.model.RecordPaymentRequestDTO;
...
import com.savbill.cpm.service.common.SendRestAPIService;
...
//    }
    ...
    public CustomerPayment putStatusString(String status,CustomerPayment customerPayment,Long orderId,Integer planId) throws Exception {
        if(status!= null && customerPayment != null){
            ...
            Integer testMvno =  paytmPaymentController.getMvnoIdFromCurrentStaff();
...
//                            billchargeThread.start();
                            ...
                            if(postpaidPlan.getPlanGroup().equalsIgnoreCase(CommonConstants.PLAN_GROUP_RENEW) || postpaidPlan.getPlanGroup().equalsIgnoreCase(SubscriberConstants.PLAN_PURCHASE_REGISTRATION_AND_RENEWWAL)) {
                                debitDocService.createInvoice(customers1, Constants.RENEW, this.clientServiceSrv.getValueByName(ClientServiceConstant.PAYEMNT_GATEWAY) + "  " + recordPaymentDTO.getReferenceNo(), null, additionalInformationDTO, null, false,false,null,null);
                            }
                            else{
                                debitDocService.createInvoice(customers1, Constants.ADD_ON, this.clientServiceSrv.getValueByName(ClientServiceConstant.PAYEMNT_GATEWAY) + "  " + recordPaymentDTO.getReferenceNo(), null, additionalInformationDTO, null, false,false,null,null);
                            }
                        ...
                        if (customers.isPresent()) {
                            customersService.sendCustPaymentSuccessMessage("Payment Success", customers.get().getUsername(), customerPayment.getPayment(), "Online", customers.get().getMvnoId(), customers.get().getCountryCode(), customers.get().getMobile(), customers.get().getEmail(), customers.get().getId(), customerPayment.getPaymentDate().toString(), recordPaymentDTO.getReferenceNo(),customers.get().getBuId(),requestDTO.getPlanId());
...
//                }
    ...
    public CustomerPayment setCustomerPayment(CustomerPaymentDto customerPaymentDto){
        ...
        if(customerPaymentDto.getOrderId() == null) {
            customerPayment.setOrderId(CommonUtils.generateId(Long.parseLong(String.valueOf(CommonUtils.gen()))));
        }
        else{
    ...
    public void createCustomerOrAddPlan(MtnBuyPlanDTO mtnBuyPlanDTO , CustomerPayment customerPayment ,PostpaidPlan newpostpaidPlan) throws Exception {
        ...
        Customers customers = customersRepository.findByUsernameAndMvnoId(mobileNumber , getMvnoIdFromCurrentStaff()).orElse(null);
        customerPayment.setStatus("Success");
...
//                messageSender.send(customMessage, RabbitMqConstants.QUEUE_APIGW_CUSTOMER_NOTIFICATION);
                ...
                customersService.sendCustPaymentSuccessMessage("Payment Success", newcustomersPojo.getUsername(), newpostpaidPlan.getOfferprice(), "Online", newcustomersPojo.getMvnoId(), newcustomersPojo.getCountryCode(), newcustomersPojo.getMobile(), newcustomersPojo.getEmail(), newcustomersPojo.getId(), "3839323923", LocalDate.now().toString(),newcustomersPojo.getBuId(), newpostpaidPlan.getId());
            }
        }
        customerPaymentRepository.save(customerPayment);
    }
    ...
    public void renewOrAddonPlan(Customers customers , PostpaidPlan plan) throws Exception {
        ...
        CustomersBasicDetailsPojo basicDetailsPojo = customChangePlanDTO.getCustomersBasicDetailsPojo();
        try {
            customersService.sendCustPaymentSuccessMessage("Payment Success", customers.getUsername(), plan.getOfferprice(), "Online", customers.getMvnoId(), customers.getCountryCode(), customers.getMobile(), customers.getEmail(), customers.getId(), "test2992299", LocalDate.now().toString(),customers.getBuId(),plan.getId());
            ...
            if(plan.getPlanGroup().equalsIgnoreCase(CommonConstants.PLAN_GROUP_RENEW) || plan.getPlanGroup().equalsIgnoreCase(SubscriberConstants.PLAN_PURCHASE_REGISTRATION_AND_RENEWWAL)) {
                debitDocService.createInvoice(customers, Constants.RENEW, "OCS", null, null, null, false,false,null,null);
            }
            else{
                debitDocService.createInvoice(customers, Constants.ADD_ON, "OCS", null, null, null, false,false,null,null);
            }
        }catch (Exception e ){
    ...
    public CustomersPojo createMTNCustomers(String username,PostpaidPlan plan){
        ...
        ClientService basePlanId = clientServiceSrv.getByNameAndMvnoIdEquals("BASE_PLAN_ID" , getMvnoIdFromCurrentStaff());
        if(basePlanId == null){
        ...
        customers.setPassword(String.valueOf(CommonUtils.gen6Digit()));
...
//                    mapping = customersService.generateConnectionNumber(mapping);
        ...
        recordPaymentDTO.setType("Payment");
        recordPaymentDTO.setMvnoId(getMvnoIdFromCurrentStaff());
