package com.savbill.cpm.modules.PurchaseOrder.Service;
...
import com.savbill.cpm.modules.PurchaseOrder.Domain.QPurchaseOrder;
   ...
   @Autowired
    ...
    private GenericDataDTO getpoNumber(String filterValue, PageRequest pageRequest) {
        ...
        try {
            ...
            if(getMvnoIdFromCurrentStaff() == 1) {
                ...
                booleanExpression = booleanExpression.and(qPurchaseOrder.mvnoId.in(getMvnoIdFromCurrentStaff(), 1));
                purchaseOrders = repository.findAll(booleanExpression, pageRequest);
            }
    ...
    public boolean duplicateVerifyAtSave(String ponumber) {
        ...
        if (ponumber != null) {
            ...
            Integer count;
            if(getMvnoIdFromCurrentStaff() == 1) count = repository.duplicateVerifyAtSave(ponumber);
            else {
                if(getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff() == null)
                    count = repository.duplicateVerifyAtSave(ponumber, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
                else
                    count = repository.duplicateVerifyAtSave(ponumber, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
            }
            if (count == 0) {
                flag = true;
            }
