...
public class ParentChildMappingControllor {
    ...
    public ResponseEntity<?> getParentChildMapping(@PathVariable String username){
        return parentChildMappingRelService.getParentChildRel(username);
    }
...
//        GenericDataDTO response = ne/w GenericDataDTO();
    ...
    public ResponseEntity<?> getParentChildByNumber(@PathVariable String mobilenumber){
        ...
        try {
            logger.info("Fetching Parent-Child mapping for mobilenumber: {}", mobilenumber);
                List<ParentChildMappingRel> mappings = parentChildMappingRelService.getParentChildMappingList(mobilenumber);
                if (!mappings.isEmpty()) {
