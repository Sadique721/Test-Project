package com.savbill.cpm.modules.TimeBasePolicy.controller;
...
import com.savbill.cpm.rabbitMq.message.TimeBasePolicyDetailsListMessage;
import com.savbill.cpm.rabbitMq.message.TimeBasePolicyDetailsMessage;
import com.savbill.cpm.rabbitMq.message.TimeBasePolicyMessage;
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_TIME_BASE_POLICY_ALL + "\",\"" + AclConstants.OPERATION_TIME_BASE_POLICY_ADD + "\")")
    ...
    @Override
    public GenericDataDTO save(@Valid @RequestBody TimeBasePolicyDTO entityDTO, BindingResult result, Authentication authentication, HttpServletRequest req) throws Exception {
        ...
        try {
            ...
            boolean flag = timeBasePolicyService.duplicateVerifyAtSave(entityDTO.getName());
            TimeBasePolicyDTO timeBasePolicyDTO = null;
            if (flag) {
                if (getMvnoIdFromCurrentStaff() != null) {
                    entityDTO.setMvnoId(getMvnoIdFromCurrentStaff());
                    entityDTO.setMvnoName(mvnoRepository.findMvnoNameById(getMvnoIdFromCurrentStaff().longValue()));
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_TIME_BASE_POLICY_ALL + "\",\"" + AclConstants.OPERATION_TIME_BASE_POLICY_EDIT + "\")")
    ...
    @Override
    public GenericDataDTO update(@Valid @RequestBody TimeBasePolicyDTO entityDTO, BindingResult result, Authentication authentication, HttpServletRequest req) throws Exception {
        TraceContext traceContext = tracer.currentSpan().context();
        HashMap<String, Object> response = new HashMap<>();
        MDC.put("type", "Update");
        MDC.put("userName", getLoggedInUser().getUsername());
        ...
        try {
            ...
            TimeBasePolicyDTO td = timeBasePolicyService.getEntityById(entityDTO.getId());
            String updatedValues = CommonUtils.getUpdatedDiff(td, entityDTO);
            boolean flag = timeBasePolicyService.duplicateVerifyAtEdit(entityDTO.getName(), entityDTO.getId().intValue());
            if (flag) {
