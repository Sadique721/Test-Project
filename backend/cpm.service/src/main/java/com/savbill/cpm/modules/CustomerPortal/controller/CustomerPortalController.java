package com.savbill.cpm.modules.CustomerPortal.controller;
...
import com.savbill.cpm.modules.InventoryManagement.inventoryMapping.InventoryMappingService;
import com.savbill.cpm.modules.InventoryManagement.inventoryMapping.NMSIntegrationResDto;
...
import com.savbill.cpm.modules.placeOrder.service.PurchaseThread;
...
import com.savbill.cpm.rabbitMq.message.CustChangePasswordMsg;
...
//    private CaseReasonService caseReasonService;
    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginPojo pojo) throws Exception {
        ...
        try {
            Optional<ChildCustomer> childCustomerList = childCustomerRepo.findByUserNameAndMvnoIdInAndIsdeleted(pojo.getUsername(),Arrays.asList(childCustomerService.getMvnoIdFromCurrentStaff().longValue(),1L),false);
            if (childCustomerList.isPresent() && (childCustomerList.get().getIsParent().equals(true) && !childCustomerService.hasMultipleParents(pojo.getUsername(),childCustomerService.getMvnoIdFromCurrentStaff().longValue()))) {
    ...
    public GenericDataDTO updateCustomersPassword(@RequestBody PasswordPojo pojo, HttpServletRequest req) throws NoSuchFieldException {
        ...
        try {
            ...
            Integer currentMvnoId = customersService.getLoggedInMvnoId();
            if(currentMvnoId==1 || dataMvnoId.equals(currentMvnoId)){
                ...
                sendCustChangePasswordMessage(customers.getUsername(), customers.getMobile(), customers.getEmail(), customers.getStatus(), customers.getMvnoId(), customers.getBuId(), customers.getPassword());
                ...
                customerNotesDto.setNotes(AclConstants.OPERATION_CHANGE_PASSWORD+": " +remark);
                customerNotesDto.setCustId(Math.toIntExact(customers.getId().longValue()));
...
//                    AclConstants.OPERATION_CUSTOMER_RESET_SELFCARE_PASSWORD, req.getRemoteAddr(), null, customers.getId().longValue(), customers.getFullName());
    ...
    public GenericDataDTO forgotPassword(@RequestBody ForgotPassowrdDTO pojo) throws NoSuchFieldException {
        ...
        try {
            ...
            customersService.getEntityForUpdateAndDelete(customers.getId());
            if (customers == null) {
    ...
    public GenericDataDTO updateProfile(@RequestBody UpdateProfileDTO pojo) throws NoSuchFieldException {
        ...
        try {
            ...
            Customers customers = customersService.getEntityForUpdateAndDelete(pojo.getId());
    ...
    public GenericDataDTO changeCustomerPassword(@RequestBody ChangePasswordDTO pojo) throws NoSuchFieldException {
        ...
        try {
            ...
            if (customers.getPassword().equals(pojo.getOldPassword())) {
                ...
                createDataSharedService.updateCustomerEntityForAllMicroServce(customerMapper.dtoToDomain(customersPojo, new CycleAvoidingMappingContext()),custPlanMapppingList,customerServiceMappinList);
                /*method called for cust change password send notification*/
                sendCustChangePasswordMessage(customers1.getUsername(), customers1.getMobile(), customers1.getEmail(), customers1.getStatus(), customers1.getMvnoId(),customers1.getBuId(),customers1.getPassword());
...
//    }
    ...
    public GenericDataDTO placeOrder(@RequestBody OrderDTO requestDTO, HttpServletRequest request) {
        ...
        String SUBMODULE = "Payment gateway" + " [getSubscriberPLanDetails()] ";
        MDC.put("type", "Create");
        Customers customers = customersService.get(requestDTO.getCustId().intValue());
        try {
            PostpaidPlan postpaidPlan = planService.get(requestDTO.getEntityid().intValue());
...
//    }
    ...
    public ResponseEntity<Resource> downloadDocument(@PathVariable Long docId, @PathVariable Integer custId) {
        ...
        try {
            ...
            CustomerDocDetailsDTO docDetailsDTO = customerDocDetailsService.getEntityById(docId);
            if (null == docDetailsDTO) {
                return ResponseEntity.notFound().build();
            }
            FileSystemService service = SpringContext.getBean(FileSystemService.class);
            resource = service.getCustDoc(customers.getUsername().trim(), docDetailsDTO.getUniquename());
            //resource=service.getInvoice("12123");
            String contentType = "application/octet-stream";
            if (resource != null && resource.exists()) {
...
//                }
    ...
    public void sendCustChangePasswordMessage(String username, String mobileNumber, String emailId, String status, Integer mvnoId,Long buId,String password) {
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    CustChangePasswordMsg custChangePasswordMsg = new CustChangePasswordMsg(username, mobileNumber, emailId, status, mvnoId, RabbitMqConstants.CUSTOMER_CHANGE_PASSWORD_EVENT, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY,buId,password);
                    Gson gson = new Gson();
