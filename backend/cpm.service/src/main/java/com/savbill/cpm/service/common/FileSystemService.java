...
public class FileSystemService {
    ...
    public Resource getCustDoc(String userName, String file) {
       ...
       List<ClientServicePojo> clientServices= clientServiceSrv.getClientSrvByName(ClientServiceConstant.CUST_DOC_READ_PATH);
        String path=clientServices.stream().filter(i->i.getMvnoId().equals(clientServiceSrv.getLoggedInMvnoId())).map(j->j.getValue()).collect(Collectors.toList()).get(0);
