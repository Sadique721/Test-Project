package com.savbill.cpm.modules.subscriber.controller;
...
import com.savbill.cpm.core.dto.*;
...
import com.savbill.cpm.modules.subscriber.service.*;
import com.savbill.cpm.modules.subscriber.model.*;
import com.savbill.cpm.modules.subscriber.service.*;
...
import com.savbill.cpm.pojo.customer.service.TerminateServiceInBulkPojo;
...
public class SubscriberController {
    ...
    public GenericDataDTO changeQos(@RequestBody ChangeQosRequestDTO requestDTO, HttpServletRequest req) {
        ...
        try {
            ...
            genericDataDTO.setTotalPages(1);
            genericDataDTO.setCurrentPageNumber(1);
            String updatedValues = CommonUtils.getUpdatedDiff(subscriberService.get(requestDTO.getCustId()), requestDTO);
            RESP_CODE = APIConstants.SUCCESS;
    ...
    public GenericDataDTO changeVoiceDetails(@RequestBody CustomerVoiceDetailsDTO pojo, HttpServletRequest req) {
        ...
        try {
            ...
            String updatedValues = CommonUtils.getUpdatedDiff(customersService.convertCustomersModelToCustomersPojo(customers), pojo);
            genericDataDTO.setResponseMessage(HttpStatus.OK.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.OK.value());
    ...
    public GenericDataDTO updateLatitudeAndLongitude(@RequestBody CustomerLocationDTO pojo, HttpServletRequest req) {
        ...
        try {
            ...
            String updatedValues = CommonUtils.getUpdatedDiff(customers, pojo);
...
//            logger.info("Fetching voice details of customer " + customers.getUsername() + " :  request: { From : {}}; Response : {{} code:{}}", getModuleNameForLog(), genericDataDTO.getResponseCode(), genericDataDTO.getResponseMessage());
    ...
    public GenericDataDTO changeQuota(@RequestBody QuotaDtlsModel requestDTO, HttpServletRequest req) {
        ...
        try {
            ...
            String updatedValues = CommonUtils.getUpdatedDiff(customers, requestDTO);
            genericDataDTO.setResponseMessage(HttpStatus.OK.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.OK.value());
...
///			logger.info("Fetching All Hierarchy :  request: { From : {}}; Response : {{} code:{}}",req.getHeader("requestFrom"),genericDataDTO.getResponseCode(), genericDataDTO.getResponseMessage());
    ...
    public GenericDataDTO updateMacDetails(@RequestBody MacUpdateModel dto) {
        ...
        try {
            ...
            Customers customers = customersService.get(dto.getCustId());
            String updatedValues = CommonUtils.getUpdatedDiff(customers, dto);
            if (null == customers) {
                genericDataDTO.setResponseMessage("Customer not found!");
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                return genericDataDTO;
            ...
            genericDataDTO.setData(subscriberService.updateMacDetails(dto));
            genericDataDTO.setResponseMessage(HttpStatus.OK.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.OK.value());
            genericDataDTO.setTotalRecords(1);
            genericDataDTO.setPageRecords(1);
...
//            MenuConstants.PostpaidCustomerCAF.POSTPAID_CAF_CUSTOMER_PLANS + "\")")
    ...
    @GetMapping("/getCustomerPlans/{customerId}")
    public GenericDataDTO getCustomerPlans(
            @PathVariable Integer customerId,
            @RequestParam(name = "type", required = true) String type,
            @RequestParam(name = "serviceId", required = false) Integer serviceId,
            @RequestParam(name = "isNotChangePlan", required = false, defaultValue = "false") Boolean isNotChangePlan,
        ...
        try {
            ...
            if (customerId == null) {
                ...
                response.setResponseMessage("Please provide customer id!");
                return response;
            }
            ...
            auditLogService.addAuditEntry(
                    AclConstants.ACL_CLASS_CUSTOMER,
                    AclConstants.OPERATION_CUSTOMER_VIEW,
            ...
            List<CustomerPlansModel> plans = new ArrayList<>();

            switch (type.toUpperCase()) {
                case "ACTIVE":
                    plans = subscriberService.getActivePlanList(customerId, isNotChangePlan);
                    break;
                case "FUTURE":
                    plans = subscriberService.getFuturePlanList(customerId, true);
                    break;
                case "EXPIRED":
                    plans = subscriberService.getExpiredPlanList(customerId, true);
                    break;
                case "ALL":
                    plans.addAll(subscriberService.getActivePlanList(customerId, isNotChangePlan));
                    plans.addAll(subscriberService.getFuturePlanList(customerId, true));
                    plans.addAll(subscriberService.getExpiredPlanList(customerId, true));
                    break;
                default:
                    response.setResponseCode(HttpStatus.BAD_REQUEST.value());
                    response.setResponseMessage("Invalid plan type! Use ACTIVE, FUTURE, or EXPIRED");
                    return response;
            }


            if (serviceId != null) {
                plans = subscriberService.getActivePlanListForServiceId(plans, serviceId);
            }

            RESP_CODE = APIConstants.SUCCESS;
            return GenericDataDTO.getGenericDataDTO(plans);

        } catch (Exception ex) {
            ex.printStackTrace();
            response.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            response.setResponseMessage("Failed to fetch plans!");
            return response;
...
//            logger.error("Unable to apply charge for customer " + customersService.get(requestDTO.getCustId()).getUsername() + " :  request: { From : {}}; Response : {{} code:{}}exception:{}", req.getHeader("requestFrom"), genericDataDTO.getResponseCode(), genericDataDTO.getResponseMessage(), ex.getStackTrace());
    ...
    public GenericDataDTO calculateReverseCharge(@PathVariable Integer chargeId, HttpServletRequest req) {
        TraceContext traceContext = tracer.currentSpan().context();
        Integer RESP_CODE = APIConstants.FAIL;
        MDC.put("type", "Fetch");
        MDC.put("userName", getLoggedInUser().getUsername());
        ...
        try {
            ...
            CustChargeDetails custChargeDetails = this.custChargeService.get(chargeId);
    ...
    public GenericDataDTO updateContactDetails(@RequestBody ContactDetailsDTO contactDetailsDTO, HttpServletRequest req)
        ...
        try {
            ...
            String updatedValues = CommonUtils.getUpdatedDiff(customers, contactDetailsDTO);
    ...
    public GenericDataDTO updateBasicDetails(@RequestBody BasicDetailsDTO basicDetailsDTO, HttpServletRequest req)
        ...
        try {
            ...
            String updatedValues = CommonUtils.getUpdatedDiff(customers, basicDetailsDTO);
    ...
    public GenericDataDTO updateNetworkDetails(@RequestBody NetworkDetailsDTO networkDetailsDTO, HttpServletRequest req)
        ...
        try {
            ...
            String updatedValues = CommonUtils.getUpdatedDiff(customers, networkDetailsDTO);
    ...
    public GenericDataDTO updateAddressDetails(@RequestBody AddressUpdateDTO addressUpdateDTO, HttpServletRequest req)
        ...
        try {
            ...
            Customers customers = customersService.get(addressUpdateDTO.getCustId());
            if (null == customers) {
                genericDataDTO.setResponseMessage("Customer not found!");
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                RESP_CODE = HttpStatus.NOT_FOUND.value();
            ...
            String updatedValues = CommonUtils.getUpdatedDiff(customers, addressUpdateDTO);
    ...
    public GenericDataDTO updateStaffPassword(@RequestBody StaffPasswordPojo pojo, HttpServletRequest req) throws NoSuchFieldException {
        ...
        try {
            ...
            StaffUser staffUser = staffUserService.get(pojo.getStaffId());
            if (staffUser == null) {
    ...
    public GenericDataDTO resetStaffPassword(@RequestBody StaffPasswordPojo pojo, HttpServletRequest req) throws NoSuchFieldException {
    ...
    public GenericDataDTO changeStaffPassword(@RequestBody ChangePasswordDTO pojo, HttpServletRequest req) throws NoSuchFieldException {
        ...
        try {
            ...
            StaffUser staffUser = staffUserService.get(pojo.getId());
            if (staffUser == null) {
                genericDataDTO.setResponseMessage("User not found!");
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                RESP_CODE = HttpStatus.NOT_FOUND.value();
...
//                logger.info("Forgot password for user " + staffUser.getUsername() + " :  request: { From : {},}; Response : {{} code:{}}", getModuleNameForLog(), genericDataDTO.getResponseCode(), genericDataDTO.getResponseMessage());
    ...
    public GenericDataDTO updateProfile(@RequestBody UpdateProfileDTO pojo, HttpServletRequest req) throws NoSuchFieldException {
        ...
        try {
            ...
            StaffUser staffUser = staffUserService.get(pojo.getId());
            String updatedValues = CommonUtils.getUpdatedDiff(staffUser, pojo);
            if (staffUser == null) {
                genericDataDTO.setResponseMessage("User not found! Please Enter Valid User !");
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                RESP_CODE = HttpStatus.NOT_FOUND.value();
...
//            logger.error("Unable to fetch reservable paymeent for customer " + customersService.get(customerId).getUsername() + " :  request: { From : {}}; Response : {{} code:{};exception:{}}", getModuleNameForLog(), genericDataDTO.getResponseCode(), genericDataDTO.getResponseMessage(), ex.getStackTrace());
    ...
    public GenericDataDTO updatePayment(@RequestBody RecordPaymentRequestDTO requestDTO, @PathVariable Integer id, HttpServletRequest req) {
        ...
        try {
            ...
            Customers customers = customersService.get(requestDTO.getCustId());
            String updatedValues = CommonUtils.getUpdatedDiff(requestDTO, customers);
            if (null == customers) {
                genericDataDTO.setResponseMessage("Customer not found!");
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                RESP_CODE = HttpStatus.NOT_FOUND.value();
            ...
            CreditDocument document = this.creditDocService.get(id);
            if (null == document) {
    ...
    public GenericDataDTO getCaseCountByCustomer(@PathVariable Integer custId, HttpServletRequest req) throws Exception {
        ...
        try {
            ...
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
            logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch case by customer" + LogConstants.LOG_BY_NAME + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + e.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public GenericDataDTO checkCustomerUnique(@Valid @RequestBody SubscriberUniqueChequeReqDTO reqDTO,
        ...
        try {
            ...
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch fetch unique user for " + customersService.get(reqDTO.getSubscriberId()).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + e.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public ResponseEntity<Resource> downloadInvoice(@PathVariable Integer invoiceid, Model model, HttpServletRequest req) {
        TraceContext traceContext = tracer.currentSpan().context();
        MDC.put("type", "Fetch");
        MDC.put("userName", getLoggedInUser().getUsername());
        MDC.put(LogConstants.TRACE_ID, req.getHeader(LogConstants.TRACE_ID));
        ...
        try {
            DebitDocument doc = debitDocService.get(invoiceid);
...
//    @PreAuthorize("validatePermission(\"" + MenuConstants.PARTNER_DOCS_DOWNLOAD + "\")")
    ...
    public ResponseEntity<Resource> downloadDocument(@PathVariable Long docId, @PathVariable Integer custId, HttpServletRequest req) {
        ...
        try {
            ...
            if (null == customers) {
                RESP_CODE = APIConstants.NOT_FOUND;
                logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Download document for customer " + customersService.get(custId).getUsername() + " for doccument id" + docId + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_NOT_FOUND + LogConstants.LOG_INFO + LogConstants.LOG_NO_RECORD_FOUND + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return ResponseEntity.notFound().build();
            }
            ...
            FileSystemService service = SpringContext.getBean(FileSystemService.class);
            resource = service.getCustDoc(customers.getUsername().trim(), docDetailsDTO.get().getUniquename());
            // resource=service.getInvoice("12123");
            String contentType = "application/octet-stream";
            if (resource != null && resource.exists()) {
            ...
            RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Download document for customer " + customersService.get(custId).getUsername() + " for doccument id" + docId + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
    ...
    public GenericDataDTO getSubscriberPopup(@PathVariable Integer custId, HttpServletRequest req) throws Exception {
        ...
        try {
            if (custId == null) {
                ...
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                RESP_CODE = APIConstants.NOT_FOUND;
                logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch fetch Popup details for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_NOT_FOUND + LogConstants.LOG_INFO + LogConstants.LOG_NO_RECORD_FOUND + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            genericDataDTO.setResponseMessage(HttpStatus.OK.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.OK.value());
            ...
            genericDataDTO.setCurrentPageNumber(1);
            RESP_CODE = APIConstants.SUCCESS;
            logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Popup details for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_SUCCESS + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } catch (Exception e) {
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Popup details for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + e.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
            //ApplicationLogger.logger.error(SUBMODULE + e.getStackTrace(), e);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
    ...
    private GenericDataDTO updateSubscriberPopup(@RequestBody SubscriberPopupDTO subscriberPopupDTO, HttpServletRequest req) {
        ...
        try {
            ...
            Customers customers = customersService.get(subscriberPopupDTO.getCustId());
            if (customers == null) {
                genericDataDTO.setResponseMessage("Customer Not Found !");
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
            ...
            //ApplicationLogger.logger.error(SUBMODULE + e.getStackTrace(), e);
            RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "Update subscriber popup for customer" + LogConstants.LOG_BY_NAME + customersService.get(subscriberPopupDTO.getCustId()).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + e.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public GenericDataDTO getSubscriberCurrentPlan(@PathVariable Integer customerId, HttpServletRequest req) {
        ...
        try {
            if (null == customerId) {
                ...
                RESP_CODE = HttpStatus.NOT_ACCEPTABLE.value();
                genericDataDTO.setResponseCode(HttpStatus.NOT_ACCEPTABLE.value());
                logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch subscriber plan for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            Customers customers = customersService.get(customerId);
            if (null == customers) {
            ...
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch subscriber plan for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public GenericDataDTO changePlan(@RequestBody ChangePlanRequestDTO requestDTO,
        ...
        try {
                ...
                if (requestDTO.getPlanList() != null) {
                    ...
                    for (PostpaidPlan plan : requestDTO.getPlanList()) {
                        ...
                        try {
                            ...
                            debitDocService.createInvoice(customer, Constants.RENEW, "", null,null, null,false,false,null,null);
                        } catch (Exception e) {
                            //ApplicationLogger.logger.error(SUBMODULE + e.getStackTrace(), e);
                            RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
            ...
            if (ex instanceof RuntimeException) {
                ...
                genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
                RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
                logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "update to update plan for customer " + customersService.get(requestDTO.getCustId()).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
    ...
    public GenericDataDTO changePlan01(@RequestBody ChangePlanRequestDTOList requestDTOs, @RequestHeader(value = "rf", defaultValue = "bss") String requestFrom, HttpServletRequest req) throws Exception {
        String SUBMODULE = MODULE + " [changePlan()] ";
        TraceContext traceContext = tracer.currentSpan().context();
        MDC.put("type", "Fetch");
        MDC.put("userName", getLoggedInUser().getUsername());
        ...
        if (requestDTOs != null) {
            ...
            if (getLoggedInUserPartnerId() != CommonConstants.DEFAULT_PARTNER_ID) {
                if (requestFrom.equals("pw") && !getLoggedInUser().getLco() && customersService.isPartnerBalanceInsufficient(requestDTOs, getLoggedInUserPartnerId())) {
                ...
                if (requestFrom.equals("pw") && getLoggedInUser().getLco() && customersService.isPartnerBalanceInsufficientForLCO(requestDTOs, getLoggedInUserPartnerId())) {
                    genericDataDTO.setResponseMessage("Partner has Insufficient balance!");
                    RESP_CODE = HttpStatus.NOT_ACCEPTABLE.value();
            ...
            if (requestDTOs != null) {
                ...
                for (ChangePlanRequestDTO requestDTO : requestDTOs.getChangePlanRequestDTOList()) {
                    if (null == requestDTO.getCustId()) {
                        genericDataDTO.setResponseMessage("Please provide customer id!");
                        RESP_CODE = HttpStatus.NOT_ACCEPTABLE.value();
                        logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch change plan for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                        return genericDataDTO;
                    }
...
//                        if (requestDTO.getNewPlanList() != null) {
                        ...
                        for (NewPlanBindWithOldPlan newPlanBindWithOldPlan : requestDTO.getPlanBindWithOldPlans()) {
                            if (newPlanBindWithOldPlan.getNewPlanId() != null) {
                                ...
                                if (requestDTO.getCustServiceMappingId() != null) {
                                    List<CustomerServiceMapping> customerServiceMappings = customerServiceMappingRepository.findAllByIdIn(Collections.singletonList(requestDTO.getCustServiceMappingId()));
                    ...
                    if (requestDTO.getPurchaseType() != null) {
                        if (requestDTO.getPlanId() != null) {
                            PostpaidPlan postpaidPlan = postpaidPlanService.get(requestDTO.getPlanId());
                            if (postpaidPlan != null) {
                                auditChangePlan = getLoggedInUser().getFullName() + " renewed plan with type "
                                        + requestDTO.getPurchaseType() + " and plan is "
                                        + postpaidPlan.getDisplayName();
//                                logger.info("Fetching All Hierarchy :  request: { From : {}}; Response : {{} code:{}}", req.getHeader("requestFrom"), genericDataDTO.getResponseCode(), genericDataDTO.getResponseMessage());
                            }
                        }
                    ...
                    customerNotesDto.setNotes(AclConstants.OPERATION_CUSTOMER_CHANGE_PLAN+": "+auditChangePlan);
                    customerNotesDto.setCustId(customers.getId());
                    customerNotesDto.setCreatedBy(String.valueOf(getLoggedInUser().getUserId()));
                    customerNotesDto.setCreatedByName(getLoggedInUser().getFullName());
                    customersService.saveNotes(customerNotesDto,Long.valueOf(getLoggedInUser().getUserId()));
                ...
                try {
                    if (requestDTOs.getChangePlanRequestDTOList().get(0).getPurchaseType().equalsIgnoreCase("Addon")) {
                        if (customersforInvoice.size() > 1) {
                            ...
                            debitDocService.createInvoice(parentCustomers, Constants.ADD_ON, "", requestDTOs.getRecordPayment(),null, null,false,false,null,requestDTOs.getChildId());
                        }
                    } else {
                        if (customersforInvoice.size() > 1) {
                            Integer parentId = custIdOptional.get();
                            ...
                            debitDocService.createInvoice(parentCustomers, Constants.RENEW, "", requestDTOs.getRecordPayment(),null, null,false,false,null,requestDTOs.getChildId());
                        }
                    }
//                            if (requestDTOs.getChangePlanRequestDTOList().get(0).getPurchaseType().equalsIgnoreCase("Addon")){
...
//                            flag = false;
    ...
    public GenericDataDTO getSubscriberChargeForIpRollback(@PathVariable Integer customerId, HttpServletRequest req) {
        ...
        try {
            if (null == customerId) {
                ...
                RESP_CODE = HttpStatus.NOT_ACCEPTABLE.value();
                genericDataDTO.setResponseCode(HttpStatus.NOT_ACCEPTABLE.value());
                logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Customer ip rollback for customer" + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_NOT_CREATED + LogConstants.LOG_INFO + "Discount with same name already exist" + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            Customers customers = customersService.get(customerId);
            if (null == customers) {
            ...
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch ip rollback for  customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public GenericDataDTO allocateIp(@RequestBody AllocateIpDTO requestDTO, HttpServletRequest req) {
        ...
        try {
            ...
            if (ex instanceof RuntimeException) {
                ...
                genericDataDTO.setResponseMessage(ex.getMessage());
                genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
                logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Ip address for customer " + customersService.get(requestDTO.getCustId()).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
    ...
    public GenericDataDTO getSubscriberAllocatedIp(@PathVariable Integer customerId, HttpServletRequest req) {
        ...
        try {
            ...
            if (ex instanceof RuntimeException) {
                ...
                genericDataDTO.setResponseMessage(ex.getMessage());
                genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
                logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Allocated ip address for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
    ...
    public GenericDataDTO releaseIp(@RequestBody IPPoolDtlsDTO requestDTO, @PathVariable Integer customerId,
        ...
        try {
            ...
            if (ex instanceof RuntimeException) {
                ...
                genericDataDTO.setResponseMessage(ex.getMessage());
                genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
                logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "relese ip addresss: for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
    ...
    public GenericDataDTO replaceIp(@RequestBody ReplaceIPDTO requestDTO, @PathVariable Integer customerId,
        ...
        try {
            ...
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "update ip address for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public GenericDataDTO rollBackIP(@RequestBody ReverseChargeRequestDTO requestDTO, @PathVariable Integer customerId,
        ...
        try {
            ...
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "rollback ipaddress for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public GenericDataDTO changeIPExpiry(@RequestBody ChangeIPExpiryDTO requestDTO, @PathVariable Integer customerId,
        ...
        try {
            ...
            if (ex instanceof RuntimeException) {
                ...
                genericDataDTO.setResponseMessage(ex.getMessage());
                genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
                logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "update ip expiry for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
    ...
    public GenericDataDTO adjustPayment(@RequestBody AdjustPaymentDTO requestDTO, @PathVariable Integer customerId, HttpServletRequest req) {
        ...
        try {
            ...
            if (null == customers) {
                ...
                RESP_CODE = APIConstants.NOT_FOUND;
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch adjust Payment for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_NOT_FOUND + LogConstants.LOG_INFO + LogConstants.LOG_NO_RECORD_FOUND + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            ...
            if (ex instanceof RuntimeException) {
                ...
                genericDataDTO.setResponseMessage(ex.getMessage());
                genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
                logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch adjust Payment for customer " + customersService.get(customerId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
    ...
    public GenericDataDTO cancelPlan(@RequestBody CancelPlanRequestDTO requestDTO, HttpServletRequest req) {
        ...
        try {
            ...
            if (custPlanMappping == null) {
                ...
                RESP_CODE = HttpStatus.NOT_ACCEPTABLE.value();
                genericDataDTO.setResponseCode(HttpStatus.NOT_ACCEPTABLE.value());
                logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch ancel plan for customer " + customersService.get(requestDTO.getCustId()).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_NOT_FOUND + LogConstants.LOG_INFO + LogConstants.LOG_NO_RECORD_FOUND + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            CustPlanMapppingPojo customerPlansModel = customerMapper.mapCustPlanMapToCustPlanMapPojo(custPlanMappping,
                    new CycleAvoidingMappingContext());
            ...
            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_CUSTOMER, AclConstants.OPERATION_CUSTOMER_CANCEL_PLAN,
                    req.getRemoteAddr(), null, customers.getId().longValue(), customers.getFullName());
            logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch ancel plan for customer " + customersService.get(requestDTO.getCustId()).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_SUCCESS + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } catch (Exception ex) {
            if (ex instanceof RuntimeException) {
                //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
                genericDataDTO.setResponseMessage(ex.getMessage());
                genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
                logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch ancel plan for customer " + customersService.get(requestDTO.getCustId()).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
    ...
    public GenericDataDTO getSubscriberCharges(@PathVariable Integer custId, HttpServletRequest req) {
        ...
        try {
            ...
            if (null == customers) {
                ...
                RESP_CODE = APIConstants.NOT_FOUND;
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch subscriber charges for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_NOT_FOUND + LogConstants.LOG_INFO + LogConstants.LOG_NO_RECORD_FOUND + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            genericDataDTO.setResponseMessage(HttpStatus.OK.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.OK.value());
            ...
            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_CUSTOMER, AclConstants.OPERATION_CUSTOMER_APPLY_CHARGE,
                    req.getRemoteAddr(), null, customers.getId().longValue(), customers.getFullName());
            logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch subscriber charges for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_SUCCESS + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } catch (Exception ex) {
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "subscriber charges for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public GenericDataDTO getSubscriberUpdates(@PathVariable Integer custId, HttpServletRequest req) {
        ...
        try {
            ...
            if (null == customers) {
                ...
                RESP_CODE = APIConstants.NOT_FOUND;
                genericDataDTO.setResponseCode(HttpStatus.NOT_FOUND.value());
                logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Subscriber updates for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_NOT_FOUND + LogConstants.LOG_INFO + LogConstants.LOG_NO_RECORD_FOUND + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                return genericDataDTO;
            }
            genericDataDTO.setResponseMessage(HttpStatus.OK.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.OK.value());
            ...
            genericDataDTO.setTotalPages(1);
            genericDataDTO.setCurrentPageNumber(1);
            logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Subscriber updates for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_SUCCESS + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } catch (Exception ex) {
            //ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch Subscriber updates for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + ex.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
    ...
    public ResponseEntity<?> searchByInvoiceType(@RequestBody PaginationRequestDTO requestDTO, @PathVariable String invoiceType, HttpServletRequest req) {
        TraceContext traceContext = tracer.currentSpan().context();
        MDC.put("type", "Fetch");
        MDC.put("userName", getLoggedInUser().getUsername());
        MDC.put(LogConstants.TRACE_ID, req.getHeader(LogConstants.TRACE_ID));
        ...
        try {
            ...
            customerList = subscriberService.searchByInvoiceType(requestDTO.getFilters(),
                    ...
                    invoiceType, requestDTO.getStatus());
            Integer Response = 0;
...
//	}
    ...
    public ResponseEntity<?> changeCustomerDiscountServiceLevel(@RequestBody List<ChangeDiscountDTO> requestDTO, @PathVariable Integer custId) {
        ...
        try {
            ...
            Integer currentMvnoId = subscriberService.getLoggedInMvnoId();
    ...
    public ResponseEntity<?> fetchCustomerDiscountServiceLevel(@PathVariable Integer custId, @RequestParam(required = false, defaultValue = "true") boolean isExpiredRequired, HttpServletRequest req) {
        TraceContext traceContext = tracer.currentSpan().context();
        MDC.put("type", "Fetch");
        MDC.put("userName", getLoggedInUser().getUsername());
        MDC.put(LogConstants.TRACE_ID, req.getHeader(LogConstants.TRACE_ID));
        ...
        try {
            CustomersService custService = SpringContext.getBean(CustomersService.class);
            result = custService.fetchCustomerDiscountDetailServiceLevel(custId, isExpiredRequired);
            response.put("discountDetails", result);
            RESP_CODE = APIConstants.SUCCESS;
...
//			customersService.validateRequest(pojo, CommonConstants.OPERATION_ADD);
            ...
            createDataSharedService.sendChangePlanForAllMicroService(Collections.singletonList(custPlanMappingRepository.findById(pojo.getCustPackageId())), null, customerChargeHistoryList, "isCAFCustomer", null, customerServiceMappingList, null, null, "", null,null, null,false,false,null,null);
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CUSTOMER_ALL + "\",\"" + AclConstants.OPERATION_CUSTOMER_CHANGE_PLAN + "\")")
    ...
    //            debitDocService.createInvoice(customers, null, 200, new HashSet<Integer>(debitDocIds), null, paymentOwner, paymentOwnerId, false, CommonConstants.INVOICE_TYPE.CHANGE_PLAN);
                ...
                if (customersforInvoice.size() > 1) {
                    ...
                    debitDocService.createInvoice(customers, CommonConstants.INVOICE_TYPE.CHANGE_PLAN, "", requestDTOs.getRecordPayment(),null, null,changePlanNextBillDate,false,null,null);
                }
                ...
                if (requestDTOs.getDeactivatePlanReqDTOS().get(0).getChangePlanDate() == null || !(requestDTOs.getDeactivatePlanReqDTOS().get(0).getChangePlanDate().equalsIgnoreCase("Next Bill Date"))) {
                    for(DeactivatePlanReqDTO dto:list){
                        for(CustPlanMappping custPlanMappping:dto.getPlanMapppingList()){
                ...
                auditLogService.addAuditEntry(AclConstants.ACL_CLASS_CHANGE_PLAN, AclConstants.OPERATION_CHANGE_PLAN_ADD, req.getRemoteAddr(), remarks, updatedCustomer.getId().longValue(), updatedCustomer.getFullName()
                );
//                Save audit remark as a note in customer notes
                CustomerNotesDto customerNotesDto = new CustomerNotesDto();
                customerNotesDto.setNotes(AclConstants.OPERATION_CHANGE_PLAN_ADD+": "+requestDTOs.getDeactivatePlanReqDTOS().get(0).getRemark());
                customerNotesDto.setCustId(customers.getId());
                customerNotesDto.setCreatedBy(String.valueOf(getLoggedInUser().getUserId()));
                customerNotesDto.setCreatedByName(getLoggedInUser().getFullName());
                customersService.saveNotes(customerNotesDto,Long.valueOf(getLoggedInUser().getUserId()));
...
//            Customers customers = customersService.get(requestDTOs.getCustId());
            ...
            if (customers.getStatus().equalsIgnoreCase(SubscriberConstants.NEW_ACTIVATION)) {
                List<CustPlanMappping> planMapppingList = requestDTOs.getPlanMapppingList();
...
//            + AclConstants.OPERATION_CUSTOMER_CANCEL_PLAN + "\")")
    ...
    public ResponseEntity<?> removeService(@PathVariable Integer planId, @RequestParam Integer custId, @RequestParam Integer planMapId, @RequestParam(required = false) Integer reasonId, HttpServletRequest req) {
        ...
        try {
            ...
            PostpaidPlan plan = postpaidPlanService.getEntityForUpdateAndDelete(planId);
            if (plan != null) {
 ...
 //          List<CustomerPlansModel> planList = customersService.getPlanByCustServiceList(customerId, status, isAllRequired, isNotChangePlan);
//
//            if (planList == null || planList.isEmpty()) {
//                List<CustomerServiceMapping> serviceDetails = customerServiceMappingRepository.findByCustId(customerId);
//                List<CustomerPlansModel> fallbackList = new ArrayList<>();
//
//                for (CustomerServiceMapping mapping : serviceDetails) {
//                    CustomerPlansModel dto = new CustomerPlansModel();
//                    dto.setServiceId(Math.toIntExact(mapping.getServiceId()));
//
//                    // Fetch the PlanService for the given serviceId
//                    PlanService planService = planServiceRepository.findById(Math.toIntExact(mapping.getServiceId())).orElse(null);
//
//                    if (planService != null) {
//                        dto.setService(planService.getName()); // Or getDisplayName() if preferred
//                    } else {
//                        // No service found, set service field to null or "Unknown"
//                        dto.setService(null);
//                    }
//
//                    // Set all other fields from mapping regardless of planService presence
//                    dto.setCustId(mapping.getCustId());
//                    dto.setCustPlanStatus(mapping.getStatus());
//                    dto.setConnection_no(mapping.getConnectionNo());
//                    dto.setNickname(mapping.getNickName());
//                    dto.setInvoiceType(mapping.getInvoiceType());
//                    dto.setDiscountType(mapping.getDiscountType());  // Corrected field from mapping
//                    dto.setDiscount(mapping.getDiscount());
//                    dto.setDiscountExpiryDate(mapping.getDiscountExpiryDate());
//                    dto.setCustServMappingStatus(mapping.getStatus());
//                    dto.setServiceHoldDate(mapping.getServiceHoldDate());
//                    dto.setServiceHoldRemarks(mapping.getServiceHoldRemarks());
//                    dto.setServiceHoldBy(mapping.getServiceHoldBy());
//                    dto.setServiceResumeDate(mapping.getServiceResumeDate());
//                    dto.setServiceResumeRemarks(mapping.getServiceResumeRemarks());
//                    dto.setServiceEndDate(mapping.getExpiryDate() != null ? mapping.getExpiryDate().atStartOfDay() : null);
//                    dto.setRemarks(mapping.getRemarks());
//                    dto.setCreatedate(mapping.getCreatedate());
//                    dto.setCreatebyname(mapping.getCreatedByName());
//
//                    fallbackList.add(dto);
//                }
//                genericDataDTO.setDataList(fallbackList);
//            } else {
//                genericDataDTO.setDataList(planList);
//            }
            RESP_CODE = APIConstants.SUCCESS;
...
//            genericDataDTO.setDataList(customersService.getPlanByCustServiceList(customerId, status, isAllRequired, isNotChangePlan));
            genericDataDTO.setDataList(customersService.getPlanByCustServiceList1(customerId, status, isAllRequired, isNotChangePlan));
        } catch (Exception ex) {
            ApplicationLogger.logger.error(SUBMODULE + ex.getStackTrace(), ex);
            genericDataDTO.setResponseMessage(HttpStatus.EXPECTATION_FAILED.getReasonPhrase());
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
    ...
    public ResponseEntity<?> holdServiceInBulk(@Valid @RequestBody DeactivatePlanReqDTOList planReqDTOList, HttpServletRequest req) throws Exception {
        ...
        try {
            ...
            String auditMsg = String.format(
                    "Service Hold: %s performed service Hold ",
                    getLoggedInUser().getFullName()
            );

            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_SERVICE_HOLD, AclConstants.OPERATION_SERVICE_HOLD,
                    req.getRemoteAddr(), auditMsg, planReqDTOList.getCustId(), "");



            // Save audit remark as a note in customer notes
            CustomerNotesDto customerNotesDto = new CustomerNotesDto();
            customerNotesDto.setNotes(AclConstants.OPERATION_SERVICE_HOLD+": "+planReqDTOList.getDeactivatePlanReqModels().get(0).getRemarks());
            customerNotesDto.setCustId(Math.toIntExact(planReqDTOList.getCustId().longValue()));
            LoggedInUser loggedInUser = (LoggedInUser) SecurityContextHolder.getContext()
                    .getAuthentication().getPrincipal();
            customerNotesDto.setCreatedBy(String.valueOf(loggedInUser.getUserId()));
            customerNotesDto.setCreatedByName(loggedInUser.getFirstName());
            customersService.saveNotes(customerNotesDto,Long.valueOf(loggedInUser.getUserId()));
    ...
    public ResponseEntity<?> resumeServiceInBulk(@Valid @RequestBody DeactivatePlanReqDTOList planReqDTOList, HttpServletRequest req) throws Exception {
        ...
        try {
            ...
            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_SERVICE_RESUME, AclConstants.OPERATION_SERVICE_RESUME,
    ...
    public GenericDataDTO extendPlanValidityInBulk(@RequestBody ExtendPlanValidityInBulk extendPlanValidityInBulk, HttpServletRequest req) {
        ...
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        try {
            ExtendPlanValidity extendPlanValidity = extendPlanValidityInBulk.getExtendPlanValidity().get(0);
            ...
            if (extendPlanValidity.getDownStartDate() != null && extendPlanValidity.getDownEndDate() != null) {
                newValue.add(String.valueOf(extendPlanValidity.getDownStartDate()));
                newValue.add(String.valueOf(extendPlanValidity.getDownEndDate()));
            } else {
                newValue.add("Start/End date not provided");
            }
            if (extendPlanValidity.getNumberOfDays() != null) {
                newValue.add("NumberOfDays=" + extendPlanValidity.getNumberOfDays());
            }
            newValue.add(extendPlanValidity.getExtend_validity_remarks() != null ? extendPlanValidity.getExtend_validity_remarks() : "");
            genericDataDTO.setDataList(customerPlansModelList);
            genericDataDTO.setResponseCode(HttpStatus.OK.value());
            String fieldName = getLoggedInUser().getFullName();
            String remark = getLoggedInUser().getFullName()
                    + " extended plan validity by " + extendPlanValidity.getNumberOfDays()
                    + " days. Remarks: " + extendPlanValidity.getExtend_validity_remarks();
...
//
            CustomerNotesDto customerNotesDto = new CustomerNotesDto();
            customerNotesDto.setNotes(AclConstants.OPERATION_EXTEND_VALIDITY+": "+extendPlanValidity.getExtend_validity_remarks());
            customerNotesDto.setCustId(Math.toIntExact(customerPlansModelList.get(0).getCustomer().getId()));
            customerNotesDto.setCreatedBy(String.valueOf(getLoggedInUser().getUserId()));
            customerNotesDto.setCreatedByName(getLoggedInUser().getFirstName());
            customersService.saveNotes(customerNotesDto,Long.valueOf(getLoggedInUser().getUserId()));
...
private List<String> extractOldExtendValidityData(List<ExtendPlanValidity> extendList) {
    List<String> oldValue = new ArrayList<>();
    // Get custPlanId from payload
    Integer custPlanId = extendList.stream()
            .map(ExtendPlanValidity::getCustPlanMapppingId)
            .findFirst()
            .orElse(null);
    if (custPlanId == null) return oldValue;
    // Fetch old mappings
    List<CustPlanExtendValidityMapping> existingMappings =
            custPlanExtendValidityMappingRepository.findAllByCustServiceMappingId(custPlanId);
    if (!existingMappings.isEmpty()) {
        // Sort safely even if some records have null startDate
        existingMappings.sort(
                Comparator.comparing(
                        CustPlanExtendValidityMapping::getDownTimeStartDate,
                        Comparator.nullsLast(Comparator.naturalOrder())
                ).reversed()
        );
        CustPlanExtendValidityMapping latestData = existingMappings.get(0);
        // Add safely (null check before adding)
        oldValue.add(latestData.getDownTimeStartDate() != null ? latestData.getDownTimeStartDate().toString() : "");
        oldValue.add(latestData.getDownTimeExpiryDate() != null ? latestData.getDownTimeExpiryDate().toString() : "");
        oldValue.add(latestData.getExtendValidityremarks() != null ? latestData.getExtendValidityremarks() : "");
    }
    return oldValue;
}
    ...
    public ResponseEntity<?> stopServiceInBulk(@Valid @RequestBody DeactivatePlanReqDTOList planReqDTOList, HttpServletRequest req) throws Exception {
        ...
        try {
            ...
            String auditMsg = String.format(
                    "Service Stop: %s performed service stop  ",
                    getLoggedInUser().getUsername()

            );
            auditLogService.addAuditEntry(AclConstants.ACL_CLASS_SERVICE_STOP, AclConstants.OPERATION_SERVICE_STOP,
                    req.getRemoteAddr(), auditMsg, planReqDTOList.getCustId(), "");
            // Save audit remark as a note in customer notes
            CustomerNotesDto customerNotesDto = new CustomerNotesDto();
//            String reason=planReqDTOList.get
            String remark=planReqDTOList.getDeactivatePlanReqModels().get(0).getRemarks();
            customerNotesDto.setNotes(AclConstants.OPERATION_SERVICE_STOP+": "+remark);
            customerNotesDto.setCustId(Math.toIntExact(planReqDTOList.getCustId().longValue()));
            LoggedInUser loggedInUser = (LoggedInUser) SecurityContextHolder.getContext()
                    .getAuthentication().getPrincipal();
            customerNotesDto.setCreatedBy(String.valueOf(loggedInUser.getUserId()));
                ...
                try {
                    if (requestDTOs.getChangePlanRequestDTOList().get(0).getPurchaseType().equalsIgnoreCase("Addon")) {
                        if (customersforInvoice.size() > 1) {
                            ...
                            debitDocService.createInvoice(parentCustomers, Constants.ADD_ON, "", requestDTOs.getRecordPayment(),null, null,false,requestDTOs.getChangePlanRequestDTOList().get(0).getIsAutoPaymentRequired(),requestDTOs.getChangePlanRequestDTOList().get(0).getCreditDocumentPaymentPojoList(),null);
                        }
                    } else {
                        if (customersforInvoice.size() > 1) {
                            Integer parentId = custIdOptional.get();
                            ...
                            debitDocService.createInvoice(parentCustomers, Constants.RENEW, "", requestDTOs.getRecordPayment(),null, null,false,requestDTOs.getChangePlanRequestDTOList().get(0).getIsAutoPaymentRequired(),requestDTOs.getChangePlanRequestDTOList().get(0).getCreditDocumentPaymentPojoList(),null);
                        }
                    }
                } catch (Exception e) {
                    logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch change plan for user " + parentCustomers.getId() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + e.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
...
//    @PreAuthorize("validatePermission(\"" + MenuConstants.pre_cust_invoices_void + MenuConstants.post_cust_invoices_void + "\")")
    @GetMapping(value = "/voidPlan")
    public GenericDataDTO voidPlan(@RequestParam(name = "custpackageid") Integer custpackageid, @RequestParam(name = "planCancelRemarks", required = false) String planCancelRemarks, HttpServletRequest req) {
        Integer RESP_CODE = APIConstants.FAIL;
        TraceContext traceContext = tracer.currentSpan().context();
        MDC.put("type", "Delete");
        MDC.put("userName", getLoggedInUser().getUsername());
        ...
        try {
            genericDataDTO=subscriberService.voidPlan(custpackageid,planCancelRemarks,req);
        } catch (Exception e) {
            ...
            logger.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "Void Plan Id: " + custpackageid + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + e.getMessage() + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        } finally {
            MDC.remove("type");
            MDC.remove("userName");
            MDC.remove("traceId");
