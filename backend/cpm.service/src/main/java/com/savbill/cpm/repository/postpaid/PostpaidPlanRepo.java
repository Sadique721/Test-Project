...
public interface PostpaidPlanRepo extends JpaRepository<PostpaidPlan, Integer>, QuerydslPredicateExecutor<PostpaidPlan> {
     ...
     */
    @Query("SELECT DISTINCT new com.savbill.cpm.modules.Customers.LightPostpaidPlanDTO(pp.id, ps.name, pp.name, pp.offerprice, pp.mvnoId, qp.qosspeed, qp.qosspeed,mv.profileImage,mv.logo_file_name, pp.plantype, ps.id, pp.validity, pp.unitsOfValidity) from PostpaidPlan pp " +
    ...
    @Query("SELECT new com.savbill.cpm.modules.Customers.LightPostpaidPlanDTO(pp.id, ps.name, pp.name, pp.offerprice, pp.mvnoId, qp.qosspeed, qp.qosspeed, mv.profileImage, mv.logo_file_name, pp.plantype, ps.id, pp.validity, pp.unitsOfValidity, pp.param1, pp.param2) " +
    ...
    @Query("SELECT new com.savbill.cpm.pojo.api.postpaidPlanFetchPojo(" +
    ...
    @Query("SELECT new com.savbill.cpm.pojo.api.postpaidPlanFetchPojo(" +
            "e.id, e.serviceId, e.mvnoId, e.offerprice) " +
            "FROM PostpaidPlan e " +
            "WHERE e.id IN :planIds")
    List<postpaidPlanFetchPojo> findAllPlanDetailsByIdsOptimized(@Param("planIds") List<Integer> planIds);


    @Query("SELECT p FROM PostpaidPlan p LEFT JOIN FETCH p.serviceAreaNameList WHERE p.id = :id")
    PostpaidPlan findPlanWithServiceAreas(@Param("id") Integer id);

    @Query("SELECT DISTINCT p FROM PostpaidPlan p LEFT JOIN FETCH p.serviceAreaNameList WHERE p.isKraSynced = false AND p.isDelete = false AND lower(p.status) = 'active'")
    List<PostpaidPlan> findAllUnsyncedActivePlans();
