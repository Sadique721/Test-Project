...
import com.savbill.cpm.modules.Branch.model.BranchDTO;
import com.savbill.cpm.modules.Branch.service.BranchService;
import com.savbill.cpm.modules.Customers.CaptiveCustomerResponseDTO;
...
import com.savbill.cpm.rabbitMq.message.BudpayChangePlanMessage;
import com.savbill.cpm.rabbitMq.message.CustPackageRelMessage;
...
import com.savbill.cpm.repository.postpaid.CityRepository;
...
import com.savbill.cpm.modules.LocationMaster.repository.LocationMasterMappingRepository;
...
import com.savbill.cpm.modules.Voucher.module.ValidateCrudTransactionData;
import com.savbill.cpm.modules.Voucher.module.VoucherStatus;
import com.savbill.cpm.modules.Voucher.repository.VoucherRepository;
...
import static com.savbill.cpm.modules.Reseller.mapper.WifiUtils.validateEmailAddress;
...
//        customers.setPlangroupid(null);
        ...
        if(isServiceAreaPartnerBind(Math.toIntExact(parentCustomers.getServicearea().getId()))) {
...
//        customers.setCustomerLocations(customerLocationMappingDtos);
    ...
    public CustomerListPojo getCustomerByUsernameAndMac(String username , String mac , String countryCode) throws Exception {
        ...
        Integer globalmvnoId = subscriberService.getMvnoIdFromCurrentStaff();
                    ...
                    if(!customerLocationMappingList.isEmpty()){
                        ...
                        if(customerLocationMappingList.get(0).getCustId() != null) {
                            ...
                            if (isQuotaAvailable(customerLocationMappingList.get(0).getCustId().intValue())) {
                                ...
                                mvnoId = customersService.getMvnoIdFromCurrentStaff();
                    ...
                    if (!customerLocationMappingList.isEmpty()) {
                        ...
                        if (customerLocationMappingList.get(0).getCustId() != null) {
                            ...
                            if(parentCustomer != null){
                               ...
                               captiveCustomerResponseDTO.setIsPartnerBindWithServiceArea(isServiceAreaPartnerBind(Math.toIntExact(parentCustomer.getServicearea().getId())));
                            }
                        }
                    }
                }
...
//            customersRepository.save(customerMapper.dtoToDomain(customersPojo, new CycleAvoidingMappingContext()));
    ...
    public CustomersPojo addDefaultValue(CustomersPojo pojo) {
        ...
        if(pojo.getServiceareaid() != null) {
            ...
            if(serviceArea.isPresent()) {
                customerAddressPojo.setAddressType(CommonUtils.ADDR_TYPE_PRESENT);
                ...
                pojo.setCustcategory(CommonUtils.CREDIT_CLASS_GOLD);
                pojo.setCustcategory(CommonUtils.CREDIT_CLASS_GOLD);
    ...
    public Boolean isServiceAreaPartnerBind(Integer serviceArea){
        log.info("enter in partner for serviceare");
        Boolean flag = false;
        log.info("flag intial set to false");
        List<Integer> serviceareaids = new ArrayList<>();
        serviceareaids.add(serviceArea);
        List<Partner> partnerList = partnerService.getPartnersByServiceAreaId(serviceareaids);
...
//                messageSender.send(message, RabbitMqConstants.QUEUE_APIGW_CUSTOMER_PACKAGE_REL);
        ...
        if(finalUsername != null){
            ...
            if(paidUser != null){
                ...
                customersService.sendCustApprovalRegistrationMessage(localDateTime.toString(), planNamesBuilder.toString(), paidUser.getUsername(), paidUser.getPassword(), pojo.getCountryCode(), pojo.getMobile(), pojo.getEmail(), pojo.getMvnoId(), pojo.getStatus(), pojo.getAcctno());
            }
            else{
                log.info("Paid customer not find with mobile number, Send child customer username and password");
                customersService.sendCustApprovalRegistrationMessage(localDateTime.toString(), planNamesBuilder.toString(), pojo.getUsername(), pojo.getPassword(), pojo.getCountryCode(), pojo.getMobile(), pojo.getEmail(), pojo.getMvnoId(), pojo.getStatus(), pojo.getAcctno());
            }
        }
        else{
