package com.savbill.cpm.modules.childcustomer.implemetation;
...
import com.savbill.cpm.modules.childcustomer.UpdateChildCustometMessesge;
import com.savbill.cpm.modules.childcustomer.dto.CafChildCustomerApproveMessege;
...
import com.savbill.cpm.modules.childcustomer.entity.QChildCustomer;
import com.savbill.cpm.modules.childcustomer.parentchildmappingrel.ParentChildMappingRel;
import com.savbill.cpm.modules.childcustomer.parentchildmappingrel.ParentChildMappingRelService;
import com.savbill.cpm.modules.childcustomer.parentchildmappingrel.ParentChildMappingRepo;
import com.savbill.cpm.modules.childcustomer.repository.ChildCustomerRepo;
import com.savbill.cpm.modules.childcustomer.service.ChildCustomerService;
import com.savbill.cpm.pojo.PaginationDetails;
import com.savbill.cpm.pojo.api.LoginPojo;
import com.savbill.cpm.pojo.api.PasswordPojo;
import com.savbill.cpm.rabbitMq.RabbitMqConstants;
import com.savbill.cpm.rabbitMq.message.ChildCustomerRegistrationSuccessMsg;
...
//        MDC.put("spanId", traceContext.spanIdString());
        ...
        try {
            ...
            boolean isDuplicate = duplicateVerifyAtSave(pojo.getUserName());
//            if(!isDuplicate){
...
//            }
            if (isDuplicate) {
                ...
                childCustomer.setMvnoId(Long.valueOf(getLoggedInUser().getMvnoId()));
...
//                   List<String> planNames = custPlanMapppingRepository.findPlanNameByCustId(pojo.getParentCustId().intValue());
                if (customers != null && customers.length > 0) {
                    ...
                    ChildCustomerRegistrationSuccessMsg childCustomerRegistrationSuccessMsg = new ChildCustomerRegistrationSuccessMsg(LocalDateTime.now().toString(), null, childCustomer.getUserName(), childCustomer.getPassword(), innerArray[6].toString(), childCustomer.getMobileNumber(), childCustomer.getEmail(), childCustomer.getMvnoId(), "Registration Succes", RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, CommonConstants.ACTIVE_STATUS, childCustomer.getBuId(), childCustomer.getParentAccountNumber(), (String) innerArray[1]);
...
//                List<ParentChildMappingRel> parentChildMappingRel = parentChildMappingRepo.findByChildUsernameAndParentCustomerAndMvnoAndIsDeleteIsFalse(pojo.getUserName(), pojo.getParentCustId(), getLoggedInUser().getMvnoId().longValue());
                Long count = parentChildMappingRepo.countByChildUsernameAndParentCustomerAndMvnoAndIsDeleteIsFalse(pojo.getUserName(), pojo.getParentCustId(), getLoggedInUser().getMvnoId().longValue());
                if (count <= 0) {
...
//                Mvno mvno = optionalMvno.get();
    ...
    public List<ChildCustomer> getChildCustomer() {
        List<Long> mvnoIds = Arrays.asList(getMvnoIdFromCurrentStaff().longValue(), 1L);
    ...
    public ResponseEntity<?> updatechildCustByParentID(ChildCustPojo pojo) {
        ...
        try {
            Optional<ChildCustomer> childCustByPArentIdAndUsername = childCustomerRepo.findByParentCustIdAndUserNameAndMvnoId(pojo.getId(), pojo.getUserName(), getMvnoIdFromCurrentStaff().longValue());
            Optional<ChildCustomer> optionalChild = childCustomerRepo.findByIdAndMvnoId(pojo.getId(), getMvnoIdFromCurrentStaff().longValue());
    ...
    public boolean duplicateVerifyAtSave(String userName) {
        ...
        if (userName != null) {
            userName = userName.trim();
            Integer count;
            if (getMvnoIdFromCurrentStaff() == 1) count = childCustomerRepo.duplicateVerifyAtSave(userName);
            else
                count = childCustomerRepo.duplicateVerifyAtSave(userName, Arrays.asList(getMvnoIdFromCurrentStaff().longValue(), 1L));
            if (count == 0) {
                flag = true;
            }
        }
...
//            }
    ...
    public HashMap<String, Object> childLogin(LoginPojo pojo, Optional<ChildCustomer> childCustomerList) {
        ...
        try {
            if (childCustomerList == null || !childCustomerList.isPresent()) {
                ...
                response.put(CommonConstants.RESPONSE_MESSAGE, "Username or Password is wrong");
                return response;
            }
            ...
            if (pojo.getPassword().equals(childCustomer.getPassword())) {
                ...
                response.put("userId", childCustomer.getParentCustId());
    ...
    @Override
    public Page<ChildCustomer> getAllChildCustomer(Integer pageNumber, Integer pageSize) {
        PageRequest pageRequest = PageRequest.of(pageNumber - 1, pageSize);
        return childCustomerRepo.findAllByMvnoIdAndIsdeletedOrderByIdDesc(getMvnoIdFromCurrentStaff().longValue(), false, pageRequest);
    }
    ...
    @Override
    public ResponseEntity<?> updateChildPassword(ChangePasswordPojo pojo) {
        ...
        try {
            ...
            Long mvnoId = getMvnoIdFromCurrentStaff().longValue();
            Optional<ChildCustomer> optionalCustomer = childCustomerRepo.findByUserNameAndMvnoIdInAndIsdeleted(pojo.getUserName(), Arrays.asList(mvnoId, 1L),false);
            ...
            if (optionalCustomer.isPresent()) {
                ...
                if (pojo.getOldPassword().equals(customer.getPassword())) {
                    ...
                    Customers customers = customersRepository.findByUsernameAndMvnoId(pojo.getUserName(), getMvnoIdFromCurrentStaff()).orElse(null);
                    if (customers != null) {
