package com.savbill.cpm.modules.PurchaseOrder.Controller;
...
import com.savbill.cpm.modules.PurchaseOrder.Mapper.PurchaseOrderMapper;
import com.savbill.cpm.modules.PurchaseOrder.Repository.PurchaseOrderRepository;
import com.savbill.cpm.modules.PurchaseOrder.Service.PurchaseOrderService;
    ...
    public GenericDataDTO save(@Valid @RequestParam String spojo, @RequestParam(value = "file", required = false) MultipartFile file) throws Exception {
       ...
       dto = new ObjectMapper().registerModule(new JavaTimeModule()).
        ...
        if(getMvnoIdFromCurrentStaff() != null) {
            dto.setMvnoId(getMvnoIdFromCurrentStaff());
        }
        MDC.put("type", "Create");
...
//    }
    ...
    public GenericDataDTO search (@RequestParam(required = false, defaultValue = "${request.defaultPage}") List<GenericSearchModel>
        ...
        return purchaseOrderService.search(page, pageSize, sortOrder, sortBy, filter);
    }
    ...
    public ResponseEntity<Resource> downloadDocument(@PathVariable String ponumber, @PathVariable Integer custId) {
        ...
        try {
            ...
            logger.error("Unable to Download document for customer " + customersService.get(custId).getUsername() + " for doccument id" + ponumber + " :  request: { From : {}}; Response : {{} code:{};Exception:{}}", getModuleNameForLog(), HttpStatus.EXPECTATION_FAILED, APIConstants.FAIL, ex.getStackTrace());
