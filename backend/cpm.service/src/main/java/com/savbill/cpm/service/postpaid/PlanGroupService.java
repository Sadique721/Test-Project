...
import com.savbill.cpm.modules.InventoryManagement.Product_Plan_Group_Mapping.ProductPlanGroupMappingRepository;
import com.savbill.cpm.modules.InventoryManagement.Product_Plan_Group_Mapping.QProductPlanGroupMapping;
...
import com.savbill.cpm.rabbitMq.message.PlanGroupMsg;
...
//                            .collect(Collectors.toList());
    ...
    public Page<PlanGroup> getPlanGroupList(Integer pageNumber, Integer customPageSize, String sortBy, Integer sortOrder, List<GenericSearchModel> filterList, Integer mvnoId) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            booleanExpression = booleanExpression.and(qPlanGroup.mvnoId.in(Arrays.asList(getMvnoIdFromCurrentStaff(), 1)));
        }
    ...
    public BooleanExpression getPlanGroupList() {
        ...
        BooleanExpression booleanExpression = qPlanGroup.isNotNull().and(qPlanGroup.isDelete.eq(false));
        if (getMvnoIdFromCurrentStaff() != 1)
            booleanExpression = booleanExpression.and(qPlanGroup.mvnoId.in(getMvnoIdFromCurrentStaff(), 1));
    ...
    public PlanGroup findPlanGroupById(Integer planGroupId, Integer mvnoId) {
        try {
            ...
            if (getBUIdsFromCurrentStaff().size() != 0)
                boolExp = boolExp
                        .and(qPlanGroup.mvnoId.eq(1)
                                .or(qPlanGroup.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qPlanGroup.buId.in(getBUIdsFromCurrentStaff()))));
...
//            }
    ...
    public Page<PlanGroup> search(List<GenericSearchModel> filterList, Integer page, Integer pageSize, String sortBy, Integer sortOrder) {
        String SUBMODULE = MODULE + " [search()] ";
        QPlanGroup qPlanGroup = QPlanGroup.planGroup;
        BooleanExpression booleanExpression = qPlanGroup.isNotNull().and(qPlanGroup.isDelete.eq(false));
        if (getMvnoIdFromCurrentStaff()!=1){
            booleanExpression.and(qPlanGroup.mvnoId.eq(getMvnoIdFromCurrentStaff()));
    ...
    public Page<PlanGroup> getPlanGroupByNameAndPlanType(String s1, PageRequest pageRequest) {
        ...
        if (getBUIdsFromCurrentStaff().size() == 0)
            return entityRepository.findByPanGroupNameOrPlanTypeContainingIgnoreCaseAndIsDeleteIsFalseAndMvnoIdIn(s1, s1, pageRequest, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
        else
            return entityRepository.findByPanGroupNameOrPlanTypeContainingIgnoreCaseAndIsDeleteIsFalseAndMvnoIdIn(s1, s1, pageRequest, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
    }
    ...
    public PlanGroup savePlanGroup(PlanGroupDTO planGroupDTO, Integer mvnoId) throws Exception {
        try {
            ...
            planGroup.setMvnoName(mvnoRepository.findMvnoNameById(getMvnoIdFromCurrentStaff().longValue()));
...
//            planGroup.setServicearea(serviceArea);
            ...
            if (planGroupDTO.getPlanMappingList() != null) {
                ...
                for (PlanGroupMappingDTO planGroupMappingDTO : planGroupDTO.getPlanMappingList()) {
                    ...
                    planGroupMappingDTO.setMvnoName(mvnoRepository.findMvnoNameById(getMvnoIdFromCurrentStaff().longValue()));
...
//                            Charge charge= chargeRepository.findById(postpaidPlanCharge.getCharge().getId()).get();
    ...
    public PlanGroup updatePlanGroup(PlanGroupDTO planGroupDTO, Integer mvnoId) {
        try {
            DecimalFormat df = new DecimalFormat("0.00");
            planGroupDTO.setMvnoId(getMvnoIdFromCurrentStaff());
            PlanGroup planGroup = getEntityForUpdateAndDelete(planGroupDTO.getPlanGroupId());
...
//                }
    ...
    public void deletePlanGroupById(Integer planGroupId, Integer mvnoId) {
        ...
        try {
            ...
            PlanGroup planGroup = getEntityForUpdateAndDelete(planGroupId);
  ...
  /*  @Override
    public PlanGroup get(Integer id) {
        ...
        if (getMvnoIdFromCurrentStaff() == 1 || ((planGroup.getMvnoId().intValue() == getMvnoIdFromCurrentStaff().intValue() || planGroup.getMvnoId() == 1) && (planGroup.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(planGroup.getBuId()))))
            return planGroup;
        return null;
    }*/
    ...
    public PlanGroup getEntityForUpdateAndDelete(Integer id) {
        PlanGroup planGroup = get(id);
        if (planGroup == null || (!(getMvnoIdFromCurrentStaff() == 1 || getMvnoIdFromCurrentStaff().intValue() == planGroup.getMvnoId().intValue()) && (planGroup.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(planGroup.getBuId()))))
...
//    }
    ...
    public GenericDataDTO updatePlanGroupAssignment(CustomerCafAssignmentPojo pojo) throws NoSuchFieldException {
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        if (pojo.getPlanGroupId() != null && pojo.getStaffId() != null) {
            PlanGroup planGroup = getEntityForUpdateAndDelete(pojo.getPlanGroupId());
...
//            workflowAuditService.planAssignByStaffID(pojo, plan, customerCafAssignment, getLoggedInUserId());
    ...
    public GenericDataDTO getPlanGroupApprovalsList(List<GenericSearchModel> filters, Integer page, Integer pageSize, String sortBy, Integer sortOrder) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1)
            booleanExpression = booleanExpression.and(qPlanGroup.mvnoId.in(1, getMvnoIdFromCurrentStaff()));
        if (getBUIdsFromCurrentStaff() != null && getBUIdsFromCurrentStaff().size() > 0) {
            booleanExpression = booleanExpression.and(qPlanGroup.mvnoId.eq(1).or(qPlanGroup.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qPlanGroup.buId.in(getBUIdsFromCurrentStaff()))));
        }
...
//            PlanGroup planGroup = planGroupRepository.findById(planGroupId).get();
    ...
    public PlanGroup getPlanGroupEntity(PlanGroup planGroup) throws Exception {
        ...
        try {
            ...
            finalPlanGroupEntity.setMvnoId(getMvnoIdFromCurrentStaff());
