...
public interface CustomerPackageRepository extends JpaRepository<CustomerPackage, Long>, QuerydslPredicateExecutor<CustomerPackage> {
    ...
    @Query(nativeQuery = true, value = "select * from tblcustpackagerel t where t.custpackageid =:id")
    CustomerPackage findByCustPackId(@Param("id") Integer id);

    @Query(nativeQuery = true, value = "select t.planid from tblcustpackagerel t where t.custpackageid =:id")
...
//    @Query(value = "select count(t.planid) > 0 from tblcustpackagerel t where t.planid =:planid and t.is_delete=false and t.MVNOID in :mvnoIds")
//    boolean customerPlanBindingIsAlreadyExists(@Param("planid")Integer planid, @Param("mvnoIds") List mvnoIds);
//
//    @Query("select count(t.planid) > 0 from tblcustpackagerel t where t.planid =:planid and t.is_delete=false and (t.MVNOID = 1 or (t.MVNOID = :mvnoId and t.BUID in :buIds))")
