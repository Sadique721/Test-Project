...
import com.savbill.cpm.model.common.QCustomers;
import com.savbill.cpm.model.common.QStaffUser;
import com.savbill.cpm.model.lead.LeadMaster;
import com.savbill.cpm.model.lead.QLeadMaster;
import com.savbill.cpm.model.postpaid.*;
import com.savbill.cpm.model.radius.LiveUser;
import com.savbill.cpm.model.radius.QLiveUser;
...
import com.savbill.cpm.modules.InventoryManagement.outward.QOutward;
...
import com.savbill.cpm.modules.InventoryManagement.productOwner.ProductOwner;
import com.savbill.cpm.modules.InventoryManagement.productOwner.ProductOwnerRepository;
import com.savbill.cpm.modules.InventoryManagement.productOwner.QProductOwner;
import com.savbill.cpm.modules.InventoryManagement.warehouse.*;
...
import com.savbill.cpm.modules.PartnerLedger.domain.QPartnerLedgerDetails;
import com.savbill.cpm.modules.PartnerLedger.domain.QPartnerPayment;
...
import com.savbill.cpm.modules.tickets.domain.QCaseAssignment;
...
public class DashboardServiceImpl implements DashboardService {
    ...
    @Override
    public Map<String, Integer> getAllCounts(String authHeader, Integer staffId) {
        int leadCount = 0;
        int ticketCount =0;
        Map<String, Integer> result = new HashMap<>();
        Map<String, Object> ticketMap = null;
        try {
            Map<String, Object> leadMap = leadFeignClient.getLeadCount(authHeader,staffId);
            leadCount = (leadMap.get("leadCount") != null)
                    ? Integer.parseInt(leadMap.get("leadCount").toString())
                    : 0;
        } catch (Exception e) {
            logger.error("Issue while fetch MyLead count. Default ticketCount to 0", e);
        }
        try {
            ticketMap = ticketFeignClient.getTicketCount(authHeader,staffId);
            ticketCount = (ticketMap.get("ticketCount") != null)
                ? Integer.parseInt(ticketMap.get("ticketCount").toString())
                : 0;
        } catch (Exception e) {
            logger.error("Issue while fetch MyTicket count. Default ticketCount to 0", e);
        }
        int cafCount = customersRepository.countCafByStatus("NewActivation",staffId);
        result.put("leadCount", leadCount);
        result.put("ticketCount", ticketCount);
        result.put("cafCount", cafCount);
    ...
    /*@Override
    public Map<String, Integer> getAllCounts(String userName, String authHeader) {

        System.out.println("TOKEN_PASSED_TO_SERVICE = " + authHeader);

        // Lead count
        Map<String, Object> leadMap = leadFeignClient.getLeadCount(authHeader, userName);

        Integer leadCount = (leadMap.get("leadCount") != null)
                ? Integer.parseInt(leadMap.get("leadCount").toString())
                : 0;

        // Ticket count
        Map<String, Object> ticketMap = ticketFeignClient.getTicketCount(authHeader, userName);

        Integer ticketCount = (ticketMap.get("ticketCount") != null)
                ? Integer.parseInt(ticketMap.get("ticketCount").toString())
                : 0;

        // CAF count (DB)
        int cafCount = customersRepository.countCafByStatusAndUser("NewActivation", userName);
        ...
        System.out.println("Lead Count = " + leadCount);
        System.out.println("Ticket Count = " + ticketCount);
        System.out.println("CAF Count = " + cafCount);
...
*/
   ...
   /* @Override
    public Map<String, Integer> getAllCounts(String token, String userName) {

        GenericDataDTO leadDto = leadFeignClient.getLeadCount(token, userName);
        int leadCount = Integer.parseInt(leadDto.getData().toString());

        GenericDataDTO ticketDto = ticketFeignClient.getTicketCount(token, userName);
       int ticketCount = Integer.parseInt(ticketDto.getData().toString());
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                if (staffUserService.getBUIdsFromCurrentStaff().size() != 0) {
                    booleanExpression = booleanExpression.and(qCustomers.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCustomers.buId.in(staffUserService.getBUIdsFromCurrentStaff())).and(qCustomers.servicearea.id.in(staffUserService.getServiceAreaIdList())));

                } else {
                    booleanExpression = booleanExpression.and(qCustomers.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCustomers.servicearea.id.in(staffUserService.getServiceAreaIdList())));
                }
            }
    ...
    public Map<String, String> getNewlyActivatedCustomer(Long mvnoId) {
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                if (staffUserService.getBUIdsFromCurrentStaff().size() != 0) {
                    booleanExpression = booleanExpression.and(qCustomer.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCustomer.buId.in(staffUserService.getBUIdsFromCurrentStaff())).and(qCustomer.servicearea.id.in(staffUserService.getServiceAreaIdList())));
                } else {
                    booleanExpression = booleanExpression.and(qCustomer.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCustomer.servicearea.id.in(staffUserService.getServiceAreaIdList())));
                }
            }
    ...
    public Map<String, Double> getMonthWiseCollection(Long mvnoId, String year) {
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                if (staffUserService.getBUIdsFromCurrentStaff().size() != 0) {
                    booleanExpression = booleanExpression.and(qCreditDocument.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCreditDocument.buID.in(staffUserService.getBUIdsFromCurrentStaff())).and(qCreditDocument.customer.servicearea.id.in(staffUserService.getServiceAreaIdList())));

                } else {
                    booleanExpression = booleanExpression.and(qCreditDocument.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCreditDocument.customer.servicearea.id.in(staffUserService.getServiceAreaIdList())));
                }
            }
    ...
    public Map<String, Double> nextTenDaysReceivablePayment(Long mvnoId) throws ParseException {
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                if (staffUserService.getBUIdsFromCurrentStaff().size() != 0) {
                    booleanExpression = booleanExpression.and(qPostpaidPlan.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qPostpaidPlan.buId.in(staffUserService.getMvnoIdFromCurrentStaff())).and(qCustPlanMappping.customer.servicearea.id.in(staffUserService.getServiceAreaIdList())));

                } else {
                    booleanExpression = booleanExpression.and(qPostpaidPlan.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCustPlanMappping.customer.servicearea.id.in(staffUserService.getServiceAreaIdList())));
...
//    Tickets APIs
    ...
    public Map<String, Long> totalOpenTickets(Long mvnoId) {
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                if (staffUserService.getBUIdsFromCurrentStaff().size() != 0) {
                    booleanExpression = booleanExpression.and(qCase.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCase.buId.in(staffUserService.getBUIdsFromCurrentStaff())).and(qCase.customers.servicearea.id.in(staffUserService.getServiceAreaIdList())));
                } else {
                    booleanExpression = booleanExpression.and(qCase.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCase.customers.servicearea.id.in(staffUserService.getServiceAreaIdList())));
                }
            }
...
//                dataMap.put("Total", Objects.requireNonNull(result.get(qCreditDocument.amount.count())).doubleValue());
    ...
    public Map<String, Double> partnerWisePayment(Long mvnoId) throws ParseException {
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                ...
                booleanExpression = booleanExpression.and(qPartnerPayment.partner.id.in(queryForPartner.select(qPartnerServiceAreaMapping.partnerId).from(qPartnerServiceAreaMapping).where(qPartnerServiceAreaMapping.serviceId.in(serviceIDs))).and(qPartnerPayment.partner.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff())));
            }
    ...
    public Map<String, Double> monthWiseVolumeUsages(Long mvnoId, Integer year) {
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                booleanExpression = booleanExpression.and(qCustQuotaDetails.customer.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()).and(qCustQuotaDetails.customer.servicearea.id.in(staffUserService.getServiceAreaIdList())));
            }
            ...
            if (staffUserService.getLoggedInUserId() != 1) {
                ...
                if (staffUserService.getBUIdsFromCurrentStaff().size() != 0) {
                    booleanExpression = booleanExpression.and(qPartnerLedgerDetails.partner.id.in(queryForPartner.select(qPartnerServiceAreaMapping.partnerId)
                            ...
                            and(qPartnerLedgerDetails.partner.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff())));
                } else {
                    booleanExpression = booleanExpression.and(qPartnerLedgerDetails.partner.id.in(queryForPartner.select(qPartnerServiceAreaMapping.partnerId)
    ...
    public List<Map<String, String>> availableInventoryProductWise(Long mvnoId) {
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                ...
                booleanExpression = booleanExpression.and(qInward.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()))
    ...
    public List<Map<String, String>> staffAndProductWiseInventories(Long mvnoId) {
        ...
        try {
            if (staffUserService.getLoggedInUserId() != 1) {
                List<Integer> serviceIDs = staffUserService.getServiceAreaIdList().stream().map(Long::intValue).collect(Collectors.toList());
                booleanExpression = booleanExpression.and(qOutward.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff()))
...
//                    dataMap.put("wareHouseName", result.get(qInward.wareHouseId.name));
    ...
    public GenericDataDTO getLeadApprovals(PaginationRequestDTO paginationRequestDTO) {
        ...
        if (staffUserService.getMvnoIdFromCurrentStaff() != 1) {
            booleanExpression = booleanExpression.and(leadMaster.mvnoId.in(1, staffUserService.getMvnoIdFromCurrentStaff()));
        }
        if (staffUserService.getBUIdsFromCurrentStaff() != null && staffUserService.getBUIdsFromCurrentStaff().size() > 0) {
            booleanExpression = booleanExpression.and(leadMaster.mvnoId.eq(1L).or(leadMaster.mvnoId.eq(staffUserService.getMvnoIdFromCurrentStaff().longValue()).and(leadMaster.buId.in(staffUserService.getBUIdsFromCurrentStaff()))));
        }
