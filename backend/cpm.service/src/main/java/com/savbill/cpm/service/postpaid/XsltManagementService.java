...
import com.savbill.cpm.model.postpaid.QXsltManagement;
import com.savbill.cpm.model.postpaid.XsltManagement;
import com.savbill.cpm.pojo.XslManagementPojo;
import com.savbill.cpm.repository.postpaid.XsltManagementRepository;
...
public class XsltManagementService extends AbstractService<XsltManagement, XslManagementPojo, Integer> {
    ...
    public List<XsltManagement> getAllActiveEntities() {
        return xsltManagementRepository.findByStatus("Y")
        		.stream().filter(tax -> tax.getMvnoId() == getMvnoIdFromCurrentStaff() || tax.getMvnoId() == null).collect(Collectors.toList());
    }
    ...
    public List<XsltManagement> getListByType(String type) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            exp = exp.and((qXsltManagement.mvnoId.in(getMvnoIdFromCurrentStaff(), 1)));
        }
        if (getBUIdsFromCurrentStaff().size() != 0) {
            if(getMvnoIdFromCurrentStaff()!=1){
                exp = exp.and(qXsltManagement.mvnoId.eq(1).or(qXsltManagement.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qXsltManagement.buId.in(getBUIdsFromCurrentStaff()))));
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.XsltManagement', '1')")
...
//    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.XsltManagement', '2')")
...
//    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.XsltManagement', '4')")
