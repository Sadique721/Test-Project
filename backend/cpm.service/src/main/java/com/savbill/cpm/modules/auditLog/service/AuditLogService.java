package com.savbill.cpm.modules.auditLog.service;
...
import com.savbill.cpm.modules.auditLog.domain.QAuditLogEntry;
...
import com.savbill.cpm.core.dto.GenericIdModel;
...
import com.savbill.cpm.modules.auditLog.mapper.AuditLogMapper;
import com.savbill.cpm.modules.auditLog.model.AuditLogEntryDTO;
import com.savbill.cpm.modules.auditLog.model.AuditLogSearchRequestDTO;
import com.savbill.cpm.modules.auditLog.queryScript.AuditSearchQueryScript;
import com.savbill.cpm.modules.auditLog.repository.AuditLogRepository;
import com.savbill.cpm.modules.subscriber.queryScript.SubscriberSearchQueryScript;
...
public class AuditLogService extends ExBaseAbstractService<AuditLogEntryDTO, AuditLogEntry, Long> {
    ...
    // Record audit entry for record payment
    @Async
    public void addPaymentAuditEntry( AuditLogEntryDTO dto
    ) throws Exception {
        AuditLogEntryDTO auditLogEntry = new AuditLogEntryDTO();


        auditLogEntry.setEmployeeName(dto.getEmployeeName());
        auditLogEntry.setEmployeeId(dto.getEmployeeId());
        auditLogEntry.setUserName(dto.getUserName());
        auditLogEntry.setUserId(dto.getUserId());
        auditLogEntry.setModule(dto.getModule());
        auditLogEntry.setOperation(dto.getOperation());
        auditLogEntry.setIpAddress(dto.getIpAddress());
//        if (remark == null)
//            auditLogEntry.setRemark(createRemark(module, operation, loggedInUser.getFullName(), refName));
//        else
        auditLogEntry.setRemark(dto.getRemark());
        auditLogEntry.setEntityRefId(dto.getEntityRefId());
        auditLogEntry.setPartnerId(dto.getPartnerId());
        saveEntity(auditLogEntry);
    }
