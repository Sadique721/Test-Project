...
import com.savbill.cpm.MicroSeviceDataShare.SharedMessages.SaveTeamHierarchyMappingMessage;
import com.savbill.cpm.MicroSeviceDataShare.SharedMessages.UpdateTeamHierarchyMappingMessage;
...
import com.savbill.cpm.model.common.*;
import com.savbill.cpm.model.lead.LeadChangeAssigneePojo;
import com.savbill.cpm.model.lead.LeadMaster;
import com.savbill.cpm.model.lead.LeadQuotationChangeAssigneePojo;
...
import com.savbill.cpm.modules.PartnerLedger.mapper.PartnerPaymentMapper;
...
import com.savbill.cpm.modules.Teams.domain.*;
import com.savbill.cpm.modules.Teams.mapper.HierarchyMapper;
import com.savbill.cpm.modules.Teams.model.HierarchyDTO;
import com.savbill.cpm.modules.Teams.model.ShowHiearachyDTO;
import com.savbill.cpm.modules.Teams.model.TeamOrderDTO;
...
//import com.savbill.cpm.modules.tickets.service.CaseUpdateService;
...
import com.savbill.cpm.utils.WorkFlowQueryUtils;
...
//                        Teams teams = teamsRepository.findById(Long.valueOf(teamHierarchyMapping.getTeamId())).orElse(null);
            ...
            hierarchyList.stream().filter(hierarchy -> (hierarchy.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || hierarchy.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1) && (getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(hierarchy.getBuId()))).forEach(hierarchy -> {
                Map<String, String> teamsList = new HashMap<>();
                ShowHiearachyDTO showHiearachyDTO = new ShowHiearachyDTO();
                showHiearachyDTO.setId(hierarchy.getId());
                showHiearachyDTO.setHierarchyName(hierarchy.getHierarchyName());
        ...
        if (filterList.size() > 0) {
            for (GenericSearchModel genericSearchModel : filterList) {
                switch (genericSearchModel.getFilterColumn()) {
                    case "hierarchyname":
                        booleanExpression = booleanExpression.and(qHierarchy.hierarchyName.containsIgnoreCase(genericSearchModel.getFilterValue()).and(qHierarchy.mvnoId.in(getMvnoIdFromCurrentStaff())));
                        break;
                }
            }
        }
        if (getMvnoIdFromCurrentStaff() != 1)
            booleanExpression = booleanExpression.and(qHierarchy.mvnoId.in(1, getMvnoIdFromCurrentStaff()));
        if (getBUIdsFromCurrentStaff().size() != 0)
            booleanExpression = booleanExpression.and(qHierarchy.mvnoId.eq(1).or(qHierarchy.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qHierarchy.buId.in(getBUIdsFromCurrentStaff()))));
...
//    }
    ...
    @Override
    public boolean duplicateVerifyAtSave(String evenName) {
        boolean flag = false;
        if (evenName != null) {
            if (getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0) {
                flag = hierarchyRepository.existsByBuIdIsNullAndMvnoIdInAndEventNameAndIsDeleted(Arrays.asList(getMvnoIdFromCurrentStaff(), 1), evenName, false);
            } else {
                flag = hierarchyRepository.existsByMvnoIdInAndBuIdInAndEventNameAndIsDeleted(Arrays.asList(getMvnoIdFromCurrentStaff(), 1), getBUIdsFromCurrentStaff(), evenName, false);
            }
        } else {
            ...
            if (hierarchy.isPresent()) {
                ...
                if (currentTeamMapping != null && currentTeamMapping.getTeamAction() != null && isApproveRequest != null) {
                    if (isApproveRequest) {
                        ...
                        if(CommonConstants.WORKFLOW_EVENT_NAME.CAF.equalsIgnoreCase(eventName) && CommonConstants.CAF_ACTION.Service_Activation.equalsIgnoreCase((currentTeamMapping.getTeamAction()))){
                            map.put(CommonConstants.CAF_ACTION.Service_Activation,CommonConstants.YES_STATUS);
                        }
                ...
                try {
                    if (nextTeamMapping != null) {
                        ...
                        if (staffId != 0 && nextTeamMapping != null) {
                            ...
                            Teams currentTeam = teamsRepository.findById(Long.valueOf(nextTeamMapping.getTeamId())).orElse(null);
                            if (currentTeam != null) {
                                map.put("teamName", currentTeam.getName());
                            }
    ...
    public List<TeamHierarchyDTO> getApproveProgress(String eventName, Long entityId) {
        ...
        try {
            switch (eventName) {
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.SHIFT_LOCATION: {
                    CustomerAddress customerAddress = customerAddressService.get(entityId.intValue());
...
//                }
    ...
    List<ServiceArea> getServiceAreaFromEntity(Object entity) {
        ...
        if (Objects.nonNull(entity) && entity.getClass().equals(CustomersPojo.class)) {
            ...
            serviceAreaList.add(serviceAreaService.getRepository().findById(customersService.get(((CaseDTO) entity).getCustomersId()).getServicearea().getId()).get());
            ...
            Partner partner = partnerService.get(((PartnerPaymentDTO) entity).getPartnerId());
...
//                        }
                            ...
                            sendWorkflowAssignActionMessage(assignedToStaff.getCountryCode(), assignedToStaff.getPhone(), assignedToStaff.getEmail(), customers.getMvnoId(), assignedToStaff.getFullName(), action);
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.PLAN: {
                    ...
                    if (map.containsKey("staffId") && map.containsKey("nextTatMappingId")) {
                        ...
                        sendWorkflowAssignActionMessage(assignedToStaff.getCountryCode(), assignedToStaff.getPhone(), assignedToStaff.getEmail(), assignedToStaff.getMvnoId(), assignedToStaff.getFullName(), action);
                    }
                    break;
                }
                case CommonConstants.WORKFLOW_EVENT_NAME.PAYMENT: {
...
//                            sendWorkflowAssignActionMessage(assignedToStaff.getCountryCode(), assignedToStaff.getPhone(), assignedToStaff.getEmail(), assignedToStaff.getMvnoId(), assignedToStaff.getFullName(), action);
    ...
    public void leadManagementWorkflowRequest(LeadMgmtWfDTO leadMgmtWfDTO) {
        StaffUser loggedInUser = staffUserService.get(leadMgmtWfDTO.getCurrentLoggedInStaffId());
        ...
        if (leadMgmtWfDTO.getStatus().equals(CommonConstants.LEADINQ) || leadMgmtWfDTO.getStatus().equals(CommonConstants.LEADRINQ)) {
            if (clientServiceSrv.getValueByNameAndmvnoId(ClientServiceConstant.WORKFLOW_AUTOMATIC_ASSIGN, leadMgmtWfDTO.getMvnoId().intValue()).equals("TRUE")) {
                ...
                if (map.containsKey("staffId") && map.containsKey("nextTatMappingId")) {
                    TeamHierarchyMapping teamHierarchyMapping = teamHierarchyMappingRepo.findById(Integer.valueOf(map.get("nextTatMappingId"))).orElse(null);
                    Teams teams = teamsRepository.findById(Long.valueOf(teamHierarchyMapping.getTeamId())).orElse(null);
                    StaffUser assignedStaffUser = staffUserService.get(Integer.valueOf(map.get("staffId")));
                    ...
                    sendWorkflowAssignActionMessage(assignedStaffUser.getCountryCode(), assignedStaffUser.getPhone(), assignedStaffUser.getEmail(), assignedStaffUser.getMvnoId(), assignedStaffUser.getFullName(), action);
                    ...
                    sendWorkflowAssignActionMessage(loggedInUser.getCountryCode(), loggedInUser.getPhone(), loggedInUser.getEmail(), loggedInUser.getMvnoId(), loggedInUser.getFullName(), action);
                }
            } else {
                sendUpdateDto.setNextApproveStaffId(loggedInUser.getId());
                sendUpdateDto.setNextTeamMappingId(null);
...
//                                               tatUtils.saveOrUpdateDataForTatMatrix( tatMapDetails, staffUser.get(),subscriber.getId(),null);
                ...
                sendWorkflowAssignActionMessage(loggedInUser.getCountryCode(), loggedInUser.getPhone(), loggedInUser.getEmail(), leadMgmtWfDTO.getMvnoId().intValue(), loggedInUser.getFullName(), action);
            }
...
//            messageSender.send(sendApproverForLeadMsg, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL);
    ...
    @Override
    public HierarchyDTO getEntityById(Long aLong) throws Exception {
        QHierarchy qHierarchy = QHierarchy.hierarchy;
...
//                messageSender.send(sendApproverForLeadMsg, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL);
                ...
                sendWorkflowAssignActionMessage(assignedUser.getCountryCode(), assignedStaffUser.getPhone(), assignedStaffUser.getEmail(), leadMaster.getMvnoId().intValue(), assignedUser.getFullName(), action);
...
//                    messageSender.send(sendApproverForLeadMsg, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL);
    ...
    public LeadMgmtWfDTO assignFromStaffListForLead(Integer nextAssignStaff, String eventName, LeadMgmtWfDTO
        ...
        if (map.containsKey("staffId") && map.containsKey("nextTatMappingId")) {
            ...
            //workflowAuditService.saveAudit(Integer.parseInt(map.get("eventId")), CommonConstants.EVENT_NAME.LEAD, leadMaster.getId().intValue(), leadMaster.getFirstname(), loggedInUser.getId(), loggedInUser.getUsername(), leadMgmtWfDTO.isApproveRequest() ? CommonConstants.WORKFLOW_AUDIT_ACTION.APPROVED : CommonConstants.WORKFLOW_AUDIT_ACTION.REJECTED, LocalDateTime.now(), leadMgmtWfDTO.isApproveRequest() ? CommonConstants.WORKFLOW_AUDIT_ACTION.APPROVED + " with remarks : " + leadMgmtWfDTO.getRemark() + " by :- " + loggedInUser.getUsername() : CommonConstants.WORKFLOW_AUDIT_ACTION.REJECTED + "with remarks : " + leadMgmtWfDTO.getRemark() + " by :- " + loggedInUser.getUsername());
            //workflowAuditService.saveAudit(Integer.parseInt(map.get("eventId")), CommonConstants.EVENT_NAME.LEAD, leadMaster.getId().intValue(), leadMaster.getFirstname(), assignedUser.getId(), assignedUser.getUsername(), CommonConstants.WORKFLOW_AUDIT_ACTION.ASSIGNED, LocalDateTime.now(), " Assigned to :- " + assignedUser.getUsername());
            sendWorkflowAssignActionMessage(assignedUser.getCountryCode(), assignedUser.getPhone(), assignedUser.getEmail(), leadMaster.getMvnoId().intValue(), assignedUser.getFullName(), CommonConstants.WORKFLOW_MSG_ACTION.LEAD);
            sendUpdateDto.setNextTeamMappingId(Integer.valueOf(map.get("nextTatMappingId")));
...
//            messageSender.send(sendApproverForLeadMsg, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL);
    ...
    public void sendWorkflowAssignActionMessage(String countryCode, String mobileNumber, String emailId, Integer
            mvnoId, String staffPersonName, String action)
    {
        ...
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    ...
                    WorkflowTicketMessage workflowTicketMessage = new WorkflowTicketMessage(RabbitMqConstants.WORKFLOW_ASSIGN_ACTION_MESSAGE, RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, optionalTemplate.get(), staffPersonName, mobileNumber, emailId, mvnoId, action, buId);
                    Gson gson = new Gson();
    ...
    public GenericDataDTO assignEveryStaff(Integer entityId, String eventName, Boolean isApproveRequest) throws
        ...
        try {
            ...
            switch (eventName) {
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.PLAN: {
                    PostpaidPlan postpaidPlan = postpaidPlanService.get(entityId);
...
//                leadMaster.setRejectReasonId(Math.toIntExact(leadReasonMgmtWfDTO.getRejectedReasonMasterId()));
    ...
    public GenericDataDTO reassignWorkflowGetStaffList(Integer entityId, String eventName) {
        ...
        try {
            ...
            switch (eventName) {
                case CommonConstants.WORKFLOW_EVENT_NAME.CAF: {
                    CustomersPojo customersPojo = customerMapper.domainToDTO(customersService.get(entityId), new CycleAvoidingMappingContext());
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.PLAN: {
                    PostpaidPlanPojo postpaidPlanPojo = postpaidPlanMapper.domainToDTO(postpaidPlanService.get(entityId), new CycleAvoidingMappingContext());
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.PLAN_GROUP: {
                    PlanGroup planGroupDTO = planGroupService.get(entityId);
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.SHIFT_LOCATION: {
                    CustomerAddressPojo customerAddressPojo = customerAddressMapper.domainToDTO(customerAddressService.get(entityId), new CycleAvoidingMappingContext());
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.PAYMENT:
                case CommonConstants.WORKFLOW_EVENT_NAME.CREDIT_NOTE: {
                    CreditDocument creditDocument = creditDocService.get(entityId);
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.TERMINATION: {
                    CustomersPojo customersPojo1 = customerMapper.domainToDTO(customersService.get(entityId), new CycleAvoidingMappingContext());
            ...
            if (currentAssignee != 0) {
                StaffUser staffUser = staffUserService.get((int) currentAssignee);
...
//                            genericDataDTO.setData(CaseConstants.CHANGE_PROBLEM_DOMAIN);
    ...
    public GenericDataDTO reassignWorkflow(Integer entityId, String eventName, Integer assignToStaffId, String remark) {
        ...
        try {
            StaffUser assignToStaff = staffUserService.get(assignToStaffId);
            Customers customers = customersRepository.findById(entityId).orElse(null);
...
//                            tatMatrixWorkFlowDetails=tatMatrixWorkFlowDetailsRepo.findByStaffIdAndEntityIdAndEventName(customers.getCurrentAssigneeId(), customers.getId().intValue(),CommonConstants.WORKFLOW_EVENT_NAME.CAF);
                        ...
                        sendWorkflowAssignActionMessage(assignToStaff.getCountryCode(), assignToStaff.getPhone(), assignToStaff.getEmail(), customers.getMvnoId(), assignToStaff.getFullName(), action);
                    }
                    break;
                }
                case CommonConstants.WORKFLOW_EVENT_NAME.TERMINATION: {
...
//                            tatMatrixWorkFlowDetails=tatMatrixWorkFlowDetailsRepo.findByStaffIdAndEntityIdAndEventName(customers.getCurrentAssigneeId(), customers.getId().intValue(),CommonConstants.WORKFLOW_EVENT_NAME.TERMINATION);
                ...
                case CommonConstants.WORKFLOW_EVENT_NAME.PLAN: {
                    ...
                    if (postpaidPlan != null) {
                        ...
                        sendWorkflowAssignActionMessage(assignToStaff.getCountryCode(), assignToStaff.getPhone(), assignToStaff.getEmail(), assignToStaff.getMvnoId(), assignToStaff.getFullName(), action);
                    }
                    break;
                }
                case CommonConstants.WORKFLOW_EVENT_NAME.PLAN_GROUP: {
    ...
    public void leadQuotationWorkflowRequest(LeadQuotationWfDTO leadQuotationWfDTO) {
        StaffUser loggedInUser = staffUserService.get(leadQuotationWfDTO.getCurrentLoggedInStaffId());
...
//            messageSender.send(sendLeadQuotationMessage, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL_QUOTATION);
    ...
    public GenericDataDTO approveLeadQuotation(LeadQuotationWfDTO leadQuotationWfDTO, GenericDataDTO genericDataDTO) {
        LeadQuotationWfDTO sendUpdateDto = new LeadQuotationWfDTO();
        List<StaffUserPojo> staffUserPojos = new ArrayList<>();
...
//                messageSender.send(sendLeadQuotationMessage, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL_QUOTATION);
                ...
                sendWorkflowAssignActionMessage(assignedUser.getCountryCode(), assignedStaffUser.getPhone(), assignedStaffUser.getEmail(), assignedUser.getMvnoId(), assignedUser.getFullName(), action);
                //updating tatmapping id in apigw lead
...
//                messageSender.send(sendLeadQuotationMessage, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL_QUOTATION);
    ...
    public LeadQuotationWfDTO assignFromStaffListForLeadQuotation(Integer nextAssignStaff, String eventName, LeadQuotationWfDTO
        ...
        if (map.containsKey("staffId") && map.containsKey("nextTatMappingId")) {
            ...
            //workflowAuditService.saveAudit(Integer.parseInt(map.get("eventId")), CommonConstants.EVENT_NAME.LEAD_QUOTATION, leadQuotationDetails.getQuotationDetailId().intValue(), leadQuotationDetails.getFirstName(), loggedInUser.getId(), loggedInUser.getUsername(), leadQuotationWfDTO.getApproveRequest() ? CommonConstants.WORKFLOW_AUDIT_ACTION.APPROVED : CommonConstants.WORKFLOW_AUDIT_ACTION.REJECTED, LocalDateTime.now(), leadQuotationWfDTO.getApproveRequest() ? CommonConstants.WORKFLOW_AUDIT_ACTION.APPROVED + " with remarks : " + leadQuotationWfDTO.getRemark() + " by :- " + loggedInUser.getUsername() : CommonConstants.WORKFLOW_AUDIT_ACTION.REJECTED + "with remarks : " + leadQuotationWfDTO.getRemark() + " by :- " + loggedInUser.getUsername());
            //workflowAuditService.saveAudit(Integer.parseInt(map.get("eventId")), CommonConstants.EVENT_NAME.LEAD_QUOTATION, leadQuotationDetails.getQuotationDetailId().intValue(), leadQuotationDetails.getFirstName(), assignedUser.getId(), assignedUser.getUsername(), CommonConstants.WORKFLOW_AUDIT_ACTION.ASSIGNED, LocalDateTime.now(), " Assigned to :- " + assignedUser.getUsername());
            sendWorkflowAssignActionMessage(assignedUser.getCountryCode(), assignedUser.getPhone(), assignedUser.getEmail(), assignedUser.getMvnoId(), assignedUser.getFullName(), action);
            sendUpdateDto.setNextTeamMappingId(Integer.valueOf(map.get("nextTatMappingId")));
            sendUpdateDto.setNextApproveStaffId(assignedToStaff.getId());
            sendUpdateDto.setQuotationId(leadQuotationWfDTO.getQuotationId());
            sendUpdateDto.setCurrentLoggedInStaffId(leadQuotationWfDTO.getCurrentLoggedInStaffId());
...
//        messageSender.send(sendLeadQuotationMessage, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL_QUOTATION);
    ...
    public void rejectDirectFromCreatedStaff(String eventName, Integer entityid) {
        ...
        try {
            switch (eventName) {
                case CommonConstants.WORKFLOW_EVENT_NAME.CAF: {
                    Customers customersCaf = customersRepository.findById(entityid).orElse(null);
...
//                   }
    ...
    public Map<String, Object> getTeamForNextApproveHybrid(Integer mvnoId, Long buId, String eventName, String listType, Boolean isApproveRequest, boolean isCreateRequest, Object entity) {
        log.debug("======================================================Common method called for workflow.================================================================ :isApproveRequest:"+isApproveRequest+":eventName:"+eventName+":");
        Map<String, Object> map = new HashMap<>();
        Optional<Hierarchy> hierarchy;
        Long eventId = 0L;
        QTeamHierarchyMapping qTeamHierarchyMapping = QTeamHierarchyMapping.teamHierarchyMapping;
                ...
                if (nextTeamMapping != null) {
                    ...
                    if (nextTeamMapping != null && Objects.nonNull(nextTeamMapping.getIsAutoAssign()) && nextTeamMapping.getIsAutoAssign()) {
                        ...
                        if (staffUsers.size() != 0 && nextTeamMapping.getId() != 0) {
                            ...
                            map.put("workFlowId", String.valueOf(hierarchy.get().getId()));
                            map.put("orderNo", String.valueOf(nextTeamMapping.getOrderNumber()));
                            map.put("isAutoAssign",nextTeamMapping != null && nextTeamMapping.getIsAutoAssign() != null
                                    ? nextTeamMapping.getIsAutoAssign().toString()
                                    : null);
                        }
                        map.put("eventId", 0);
                        map.put("eventName", String.valueOf(eventName));
...
//    }
                ...
                for (String eventName : eventNameList) {
                    switch (eventName) {
                        case CommonConstants.WORKFLOW_EVENT_NAME.CAF:
                            ...
                            processAutoApproval(entity, entityId, triggeredActionName, CommonConstants.NEW_ACTIVATION_STATUS, eventName);
                            System.out.println(" ############################### Completion of CAF Auto Approval Process:  ###############################");
                            break;
                        case CommonConstants.WORKFLOW_EVENT_NAME.TERMINATION:
                            processAutoApproval(entity, entityId, triggeredActionName, CommonConstants.ACTIVE_STATUS, eventName);
                            break;
    ...
    private void processAutoApproval(Object entity, Integer entityId, String triggeredActionName, String requiredStatus, String eventName) {
        ...
        if (entity instanceof Customers) {
            ...
            if (configMap != null && clientServiceValue!=null && (shouldAutoApprove(configMap, triggeredActionName) || clientServiceValue.equalsIgnoreCase("false"))) {
...
//            }
    ...
    public void autoApproveCAF(Customers customers, Integer entityId, Map<String, Object> configMap) {
        try {
            Map<String,Object> map  = new HashMap<>();
            ...
            if (customers != null) {
                ...
                CafDto cafDto = customersService.updateCustomerCafAssignment(customerCafAssignmentPojo, null);
...
//                       messageSender.send(caftoCustomerMessage, SharedDataConstants.QUEUE_CAF_TO_CUSTOMER_DATA_SHARE_REVENUE);
    ...
    public boolean isTeamActionPresentForHierarchy(Integer mvnoId, Long buId, String eventName, String teamAction) {

        QHierarchy qHierarchy = QHierarchy.hierarchy;
        QTeamHierarchyMapping qTeamHierarchyMapping = QTeamHierarchyMapping.teamHierarchyMapping;

        BooleanExpression hierarchyExp = qHierarchy.isNotNull()
                .and(qHierarchy.eventName.eq(eventName))
                .and(qHierarchy.isDeleted.eq(false))
                .and(qHierarchy.mvnoId.eq(mvnoId));

        if (buId != null) {
            hierarchyExp = hierarchyExp.and(qHierarchy.buId.eq(buId));
        } else {
            hierarchyExp = hierarchyExp.and(qHierarchy.buId.isNull());
        }

        JPAQueryFactory queryFactory = new JPAQueryFactory(entityManager);
        Long hierarchyId = queryFactory.select(qHierarchy.id).from(qHierarchy).where(hierarchyExp).fetchOne();

        if (hierarchyId == null) {
            return false;
        }

        BooleanExpression teamMappingExp = qTeamHierarchyMapping.isNotNull()
                .and(qTeamHierarchyMapping.hierarchyId.eq(hierarchyId.intValue()))
                .and(qTeamHierarchyMapping.teamAction.eq(teamAction))
                .and(qTeamHierarchyMapping.isDeleted.eq(false));

        return teamHierarchyMappingRepo.exists(teamMappingExp);
    }
}
