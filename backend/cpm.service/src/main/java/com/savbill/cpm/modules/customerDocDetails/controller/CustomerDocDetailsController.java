package com.savbill.cpm.modules.customerDocDetails.controller;
...
import com.savbill.cpm.core.exceptions.FileNotCreatedException;
...
import com.savbill.cpm.modules.customerDocDetails.model.CustomerDocDeleteModel;
...
public class CustomerDocDetailsController extends ExBaseAbstractController<CustomerDocDetailsDTO> {
    ...
    public GenericDataDTO deleteCustDoc(@RequestBody CustomerDocDeleteModel docDeleteModel, HttpServletRequest req) {
        ...
        String name = customersService.get(docDeleteModel.getCustId()).getFullName();
        ...
        try {
            if (null != docDetailsList) {
                ...
                if (null != customerDocDetailsList && 0 < customerDocDetailsList.size()) {
                    ...
                    genericDataDTO.setDataList(customerDocDetailsService.uploadDocument(customerDocDetailsList, file));
                    genericDataDTO.setResponseCode(HttpStatus.OK.value());
    ...
    public GenericDataDTO uploadDocOnlineForCustomer(@RequestBody CustomerDocDetailsDTO customerDocDetailsDTO, @RequestParam Boolean isUpdate, HttpServletRequest req) {
        ...
        MDC.put("spanId", traceContext.spanIdString());
        Integer RESP_CODE = APIConstants.FAIL;
        String name = customersService.get(customerDocDetailsDTO.getCustId()).getFullName();
        try {
...
//    @PreAuthorize("validatePermission(\"" + AclConstants.OPERATION_CUSTUMER_DOC_ALL + "\",\"" + AclConstants.OPERATION_CUSTUMER_DOC_DELETE + "\")")
    ...
    public GenericDataDTO delete(@RequestBody CustomerDocDetailsDTO entityDTO, Authentication authentication, HttpServletRequest req) throws Exception {
        ...
        Integer RESP_CODE = APIConstants.FAIL;
        try {
            String name = customersService.get(entityDTO.getCustId()).getFullName();
            ...
            if (customers == null || !(getMvnoIdFromCurrentStaff() == 1 || getMvnoIdFromCurrentStaff().intValue() == customers.getMvnoId().intValue() && (customers.getMvnoId() == 1 || customersService.getBUIdsFromCurrentStaff().size() == 0 || customersService.getBUIdsFromCurrentStaff().contains(customers.getBuId())))) {
    ...
    public GenericDataDTO update(@RequestParam String docDetailsList
        ...
        try {
            ...
            CustomerDocDetailsDTO dtoData = customerDocDetailsService.getEntityForUpdateAndDelete(entityDTO.getIdentityKey());
