package com.savbill.cpm.modules.qosPolicy.controller;
...
@Override
    public GenericDataDTO save(@Valid @RequestBody QOSPolicyDTO entityDTO, BindingResult result, Authentication authentication, HttpServletRequest req) throws Exception {
    ...
    try {
            boolean flag = qosPolicyService.duplicateVerifyAtSave(entityDTO.getName());
...
@Override
    public GenericDataDTO update(@Valid @RequestBody QOSPolicyDTO entityDTO, BindingResult result, Authentication authentication, HttpServletRequest req) throws Exception {
        GenericDataDTO dataDTO = new GenericDataDTO();
    TraceContext traceContext = tracer.currentSpan().context();
    HashMap<String, Object> response = new HashMap<>();
    MDC.put("type", "Update");
    ...
    QOSPolicyDTO qosPolicy = null;
        try {
            QOSPolicyDTO qp=qosPolicyService.getEntityForUpdateAndDelete(entityDTO.getId());
           String oldname= qosPolicyService.getEntityForUpdateAndDelete(entityDTO.getId()).getName();
            boolean flag = qosPolicyService.duplicateVerifyAtEdit(entityDTO.getName(), entityDTO.getId().intValue());
            String updatedValues = CommonUtils.getUpdatedDiff(qp,entityDTO);
            if (flag) {
                if(getMvnoIdFromCurrentStaff() != null) {
                    entityDTO.setMvnoId(getMvnoIdFromCurrentStaff());
                    entityDTO.setMvnoName(mvnoRepository.findMvnoNameById(getMvnoIdFromCurrentStaff().longValue()));
...
@Override
    ...
    try{
            entityDTO = qosPolicyService.getEntityForUpdateAndDelete(entityDTO.getIdentityKey());
