package com.savbill.cpm.service.customerServices;
...
import com.savbill.cpm.modules.CafFollowUp.repository.CafFollowUpAuditRepository;
...
import com.savbill.cpm.modules.linkacceptance.mapper.LinkAcceptanceMapper;
...
import com.savbill.cpm.pojo.api.*;
import com.savbill.cpm.pojo.customer.NextBillDatinfo;
import com.savbill.cpm.rabbitMq.MessageSender;
import com.savbill.cpm.rabbitMq.message.CustServiceMappingMessage;
import com.savbill.cpm.rabbitMq.message.CustomerMessage;
import com.savbill.cpm.rabbitMq.message.LeadMasterPojoMessage;
...
//    private HierarchyRepository hierarchyRepository;
    ...
    public CustomersPojo save(CustomersPojo pojo, String requestFrom, boolean isCustomerUpdate) throws Exception {
        ...
        if (pojo.getMvnoId() == null || pojo.getMvnoId() == getMvnoIdFromCurrentStaff()) {
        ...
        if (pojo.getMvnoId() != getMvnoIdFromCurrentStaff()) {
    ...
    public void syncCustomerDataWithOtherServices(CustomersPojo customersPojo) {
        ...
        customersService.sendCustApprovalRegistrationMessage(LocalDateTime.now().toString(), planNamesBuilder.toString(), customersPojo.getUsername(), customersPojo.getPassword(), customersPojo.getCountryCode(), customersPojo.getMobile(), customersPojo.getEmail(), customersPojo.getMvnoId(), customersPojo.getStatus(), customersPojo.getAcctno());
...
//            this.messageSender.send(leadMasterPojoMessage, RabbitMqConstants.QUEUE_LEAD_CAF_CONVERTION);
    ...
    public CustomersPojo createNewCustomer(CustomersPojo subscriber, LocalDateTime currentTime, String requestFrom, BusinessUnit businessUnit) throws Exception {
        ...
        ClientService trialClientService = clientServiceSrv.getByNameAndMvnoIdEquals(Constants.TRIAL_PLAN_PERIOD_THRESHOLD, getLoggedInMvnoId());
        ...
        ClientService noOfTimeTrialclientService = clientServiceSrv.getByNameAndMvnoIdEquals(Constants.ALLOW_NUMBER_OF_TIME_TRAIL, getLoggedInMvnoId());
        ...
        if (plangroupId != null && plangroupId != 0) {
            PlanGroup planGroup = planGroupService.findPlanGroupById(plangroupId, getMvnoIdFromCurrentStaff());
            if (planGroup == null) {
            ...
            List<PlanGroupMapping> planGroupMappingList = planGroupMappingService.findPlanGroupMappingByPlanGroupId(plangroupId, getMvnoIdFromCurrentStaff());
                ...
                if (null != subscriber.getPaymentDetails()) {
                    ...
                    if (requestFrom.equalsIgnoreCase("pw")) {
                        if (paymentPojo.getPaytype() == null) {
                            paymentPojo.setPaytype(com.savbill.cpm.modules.subscriber.model.Constants.ADVANCE);
                        }
                        if (paymentPojo.getType() == null) {
                            paymentPojo.setType(CommonUtils.PAYMENT_TYPE);
                        }
                        if (paymentPojo.getMvnoId() == null) {
                            paymentPojo.setMvnoId(getMvnoIdFromCurrentStaff());
                        }
                    }
                    ...
                    if (null != paymentPojo.getPaymode()) {
                        ...
                        if (requestFrom.equalsIgnoreCase("pw")) {
                            ...
                            creditDocument.setMvnoId(getMvnoIdFromCurrentStaff());
                        }
