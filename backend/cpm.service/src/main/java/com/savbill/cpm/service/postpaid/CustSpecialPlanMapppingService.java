...
import com.savbill.cpm.pojo.api.CustSpecialPlanMapppingPojo;
import com.savbill.cpm.pojo.api.CustSpecialPlanRelMapppingPojo;
import com.savbill.cpm.repository.postpaid.CustSpecialPlanMapppingRepository;
import com.savbill.cpm.repository.postpaid.CustSpecialPlanRelMapppingRepository;
...
//	CustSpecialPlanRelMapper custSpecialPlanRelMapper;
	...
	public CustSpecialPlanRelMappping findById(Long id) {
		...
		if(custSpecialPlanRelMappping != null && ((custSpecialPlanRelMappping.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || custSpecialPlanRelMappping.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()) && (custSpecialPlanRelMappping.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(custSpecialPlanRelMappping.getBuId()))))
	...
	public CustSpecialPlanRelMappping findByIdForUpdateOrDelete(Long id) {
		CustSpecialPlanRelMappping custSpecialPlanRelMappping = relMappingRepo.findById(id).get();
		if(custSpecialPlanRelMappping == null || (!(getMvnoIdFromCurrentStaff() == 1 || getMvnoIdFromCurrentStaff().intValue() == custSpecialPlanRelMappping.getMvnoId().intValue()) && (custSpecialPlanRelMappping.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(custSpecialPlanRelMappping.getBuId()))))
			throw new CustomValidationException(APIConstants.FAIL, Constants.MVNO_DELETE_UPDATE_ERROR_MSG, null);
		return custSpecialPlanRelMappping;
	}

	public CustSpecialPlanMappping findById(Integer id) {
		CustSpecialPlanMappping custSpecialPlanMappping = super.get(id);
		if(custSpecialPlanMappping != null && (custSpecialPlanMappping.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || custSpecialPlanMappping.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()))
	...
	public CustSpecialPlanMappping findByIdForUpdateOrDelete(Integer id) {
		CustSpecialPlanMappping custSpecialPlanMappping = super.get(id);
		if(custSpecialPlanMappping == null || !(getMvnoIdFromCurrentStaff() == 1 || getMvnoIdFromCurrentStaff().intValue() == custSpecialPlanMappping.getMvnoId().intValue()))
			throw new CustomValidationException(APIConstants.FAIL, Constants.MVNO_DELETE_UPDATE_ERROR_MSG, null);
		return custSpecialPlanMappping;
	}
...
//				.stream().filter(custSpecialPlanMappping -> custSpecialPlanMappping.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || custSpecialPlanMappping.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()).collect(Collectors.toList());
	...
	public List<CustSpecialPlanMappping> findAllByCustomers(Integer custId) {
		if (custId != null) {
			Customers customer = customersService.get(custId);
			if (customer != null) {
	...
	public CustSpecialPlanRelMapppingPojo saveCustSpecialPlanMappping(CustSpecialPlanRelMapppingPojo pojo) {
		...
		if (pojo != null) {
			...
			if (getMvnoIdFromCurrentStaff() != null) {
				mapping.setMvnoId(getMvnoIdFromCurrentStaff());
				mapping.setMvnoName(mvnoRepository.findMvnoNameById(getMvnoIdFromCurrentStaff().longValue()));
            }
			...
			if (custSpecialPlanMapppingPojo != null) {
				...
				if (custSpecialPlanMapppingRepository.isDuplicateRecordFound(
					...
					if (custSpecialPlanMapppingPojo.getSpecialPlanId() != null) {
						PostpaidPlan postpaidPlan = postpaidPlanService
								.get(custSpecialPlanMapppingPojo.getSpecialPlanId());
						if (postpaidPlan != null)
						...
						PlanGroup planGroup = planGroupService
								.findPlanGroupById(custSpecialPlanMapppingPojo.getSpecialPlanGroupId(), getMvnoIdFromCurrentStaff());
						if (planGroup != null)
					...
					if(custSpecialPlanMappping.getLeadMaster() != null){
						...
						custSpecialPlanMappping.setMvnoId(getMvnoIdFromCurrentStaff());
...
//		}
				...
				if (custSpecialPlanMapppingRepository.isDuplicateRecordFound(
					...
					if (custSpecialPlanMapppingPojo.getCustomerId() != null) {
						...
						|| custSpecialPlanMapppingRepository.isDuplicateRecordFoundForCustomerPlanGroup(
								custSpecialPlanMapppingPojo.getSpecialPlanGroupId(),
								custSpecialPlanMapppingPojo.getCustomerId(), custSpecialPlanRelMappping.getId()) < 1) {
							Customers customer = customersService.get(custSpecialPlanMapppingPojo.getCustomerId());
							if (customer != null)
				...
				if (custSpecialPlanMapppingRepository.isDuplicateRecordFound(
					...
					if (custSpecialPlanMapppingPojo.getNormalPlanId() != null) {
						if (custSpecialPlanMapppingPojo.getNormalPlanId() != null && custSpecialPlanMapppingRepository
							...
							PostpaidPlan postpaidPlan = postpaidPlanService
									.get(custSpecialPlanMapppingPojo.getNormalPlanId());
							if (postpaidPlan != null)
				...
				if (custSpecialPlanMapppingRepository.isDuplicateRecordFoundForPlanGroup(
					...
					if (custSpecialPlanMapppingPojo.getNormalPlanGroupId() != null) {
						if (custSpecialPlanMapppingPojo.getNormalPlanGroupId() != null && custSpecialPlanMapppingRepository
							...
							PlanGroup planGroup = planGroupService
									.findPlanGroupById(custSpecialPlanMapppingPojo.getNormalPlanGroupId(), getMvnoIdFromCurrentStaff());
							if (planGroup != null)
	...
	public Page<CustSpecialPlanRelMappping> findAllByCustPlanRelWithPagination(Integer pageNumber, Integer customPageSize,
		...
		if(getMvnoIdFromCurrentStaff() != 1)
			booleanExpression = booleanExpression.and(qCustSpecialPlanRelMappping.mvnoId.in(getMvnoIdFromCurrentStaff(), 1));
		if(getBUIdsFromCurrentStaff().size() != 0)
			booleanExpression = booleanExpression
					.and(qCustSpecialPlanRelMappping.mvnoId.eq(1)
							.or(qCustSpecialPlanRelMappping.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qCustSpecialPlanRelMappping.buId.in(getBUIdsFromCurrentStaff()))));
...
//		}
	...
	public Page<CustSpecialPlanRelMappping> searchList(Integer pageNumber, Integer customPageSize,Integer sortOrder, String name) {
		...
		if (name != null) {
			...
			}else if(getMvnoIdFromCurrentStaff() != null && getMvnoIdFromCurrentStaff() != 1) {
				booleanExpression = booleanExpression.and(qCustSpecialPlanMappping.mappingName.containsIgnoreCase(name));
			...
			if(getMvnoIdFromCurrentStaff() != 1)
				booleanExpression = booleanExpression.and(qCustSpecialPlanMappping.mvnoId.in(getMvnoIdFromCurrentStaff(), 1));
			if(getBUIdsFromCurrentStaff().size() != 0)
				booleanExpression = booleanExpression
						.and(qCustSpecialPlanMappping.mvnoId.eq(1)
								.or(qCustSpecialPlanMappping.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qCustSpecialPlanMappping.buId.in(getBUIdsFromCurrentStaff()))));
		}
		return (Page<CustSpecialPlanRelMappping>) relMappingRepo.findAll(booleanExpression, pageRequest);
	}

	@Override
	public CustSpecialPlanMappping get(Integer id) {
		CustSpecialPlanMappping custSpecialPlanMappping = super.get(id);
		if(custSpecialPlanMappping != null) {
			if (getMvnoIdFromCurrentStaff() == 1 || (custSpecialPlanMappping.getMvnoId() == getMvnoIdFromCurrentStaff() || custSpecialPlanMappping.getMvnoId() == 1))
				return custSpecialPlanMappping;
		}
		return null;
	}
	...
	public boolean duplicateVerifyAtSave(String name) {
		...
		if (name != null) {
			name = name.trim();
			Integer count;
			if(getMvnoIdFromCurrentStaff() == 1) count = relMappingRepo.duplicateVerifyAtSave(name);
			else {
				if(getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0) count = relMappingRepo.duplicateVerifyAtSave(name, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
				else count = relMappingRepo.duplicateVerifyAtSave(name, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
			}
			if (count == 0) {
				flag = true;
			}
		...
		if (name != null) {
			...
			if(getMvnoIdFromCurrentStaff() == 1) count = relMappingRepo.duplicateVerifyAtSave(name);
				...
				if(getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0)
					count = relMappingRepo.duplicateVerifyAtSave(name, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
				else
					count = relMappingRepo.duplicateVerifyAtSave(name, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());            }
			if (count >= 1) {
				Integer countEdit;
				if(getMvnoIdFromCurrentStaff() == 1) countEdit = relMappingRepo.duplicateVerifyAtEdit(name, id);
				else {
					if(getBUIdsFromCurrentStaff() == null || getBUIdsFromCurrentStaff().size() == 0)
						countEdit = relMappingRepo.duplicateVerifyAtEdit(name, id, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
					else
						countEdit = relMappingRepo.duplicateVerifyAtEdit(name, id, getMvnoIdFromCurrentStaff(), getBUIdsFromCurrentStaff());
				}
				if (countEdit == 1) {
					flag = true;
				}
...
CustSpecialPlanRelMappping obj =relMappingRepo.findById(custSpecialPlanMappping.getCustSpecialPlanRelMappping().getId()).orElse(null);
	...
	public GenericDataDTO updateSpecialPlanAssignment(CustSpecialPlanRelMapppingPojo pojo) {
		...
		if (pojo.getId() != null && pojo.getNextStaff() != null) {
			...
			StaffUser staffUser = staffUserService.get(pojo.getNextStaff());
	...
	public CustSpecialPlanMappping getEntityForUpdateAndDelete(Integer id) {
		CustSpecialPlanMappping custSpecialPlanMappping = get(id);
		if (custSpecialPlanMappping == null || (!(getMvnoIdFromCurrentStaff() == 1 || getMvnoIdFromCurrentStaff().intValue() == custSpecialPlanMappping.getMvnoId().intValue()) && (custSpecialPlanMappping.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(custSpecialPlanMappping.getCustSpecialPlanRelMappping().getBuId()))))
			throw new CustomValidationException(APIConstants.FAIL, Constants.MVNO_DELETE_UPDATE_ERROR_MSG, null);
		return custSpecialPlanMappping;
	}
    ...
    public GenericDataDTO getSpecialPlanMappingApprovals(PaginationRequestDTO paginationRequestDTO) {
		...
		if (getMvnoIdFromCurrentStaff() != 1)
			booleanExpression = booleanExpression.and(qCustSpecialPlanRelMappping.mvnoId.in(1, getMvnoIdFromCurrentStaff()));
		if (getBUIdsFromCurrentStaff() != null && staffUserService.getBUIdsFromCurrentStaff().size() > 0) {
			booleanExpression = booleanExpression.and(qCustSpecialPlanRelMappping.mvnoId.eq(1).or(qCustSpecialPlanRelMappping.mvnoId.eq(getMvnoIdFromCurrentStaff())
