...
import com.savbill.cpm.model.common.QStaffUserServiceAreaMapping;
...
import com.savbill.cpm.modules.InventoryManagement.NonSerializedItem.NonSerializedItemRepository;
import com.savbill.cpm.modules.InventoryManagement.NonSerializedItemHierarchy.NonSerializedItemHierarchyServiceImpl;
...
import com.savbill.cpm.modules.InventoryManagement.RequestInventory.RequestInvenotryProductMapping;
import com.savbill.cpm.modules.InventoryManagement.RequestInventory.RequestInventory;
import com.savbill.cpm.modules.InventoryManagement.RequestInventory.RequestInventoryProductMappingRepo;
import com.savbill.cpm.modules.InventoryManagement.RequestInventory.RequestInventoryRepo;
import com.savbill.cpm.modules.InventoryManagement.inventoryMapping.InventoryMapping;
import com.savbill.cpm.modules.InventoryManagement.inventoryMapping.InventoryMappingMapper;
...
import com.savbill.cpm.modules.InventoryManagement.item.QItem;
import com.savbill.cpm.modules.InventoryManagement.outward.*;
import com.savbill.cpm.modules.InventoryManagement.outward.*;
import com.savbill.cpm.modules.InventoryManagement.productOwner.*;
...
import com.savbill.cpm.modules.InventoryManagement.warehouse.QWareHouse;
import com.savbill.cpm.modules.InventoryManagement.warehouse.QWareHouseServiceAreaMapping;
import com.savbill.cpm.modules.InventoryManagement.warehouse.WareHouse;
import com.savbill.cpm.modules.InventoryManagement.warehouse.WarehouseManagementRepository;
...
import com.savbill.cpm.modules.servicePlan.domain.QServices;
...
//                Inward inward = inwardRepository.findById(entity.id).get();
        ...
        if (wareHouseList.size() > 0) {
            ...
            if (getMvnoIdFromCurrentStaff() != 1) {
                if (inwardNumber != null) {
                    paginationList = inwardRepository.findAllByinwardNumberContainingIgnoreCaseAndDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(inwardNumber, warehouseResult, warehouseDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
                } else {
                    paginationList = inwardRepository.findAllByDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(warehouseResult, warehouseDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
                }
            }
        }
    ...
    public List<Inward> getAllPOPForInward(String inwardNumber) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            aBoolean = aBoolean.and(qPopManagement.mvnoId.in(getMvnoIdFromCurrentStaff(), 1));
        }
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            if (inwardNumber != null) {
                paginationList = inwardRepository.findAllByinwardNumberContainingIgnoreCaseAndDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(inwardNumber, popResult, popDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            } else {
                paginationList = inwardRepository.findAllByDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(popResult, popDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            }
        }
        return paginationList;
    }
    ...
    public List<Inward> getAllStaffForInward(String inwardNumber) {
        ...
        List<Inward> paginationList = null;
        if (getMvnoIdFromCurrentStaff() != 1) {
            List<StaffUser> staffUserList = staffUserRepository.findByIdAndStatusAndIsDeleteIsFalseAndMvnoIdIn(getLoggedInUserId(), status, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            if (inwardNumber != null) {
                paginationList = inwardRepository.findAllByinwardNumberContainingIgnoreCaseAndDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(inwardNumber, resultStaffId, staffDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            } else {
                paginationList = inwardRepository.findAllByDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(resultStaffId, staffDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            }
        }
        return paginationList;
    }
    ...
    public List<Inward> getAllServiceAreaForInward(String inwardNumber) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            aBoolean = aBoolean.and(qStaffUser.mvnoId.in(getMvnoIdFromCurrentStaff(), 1));
        }
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            if (inwardNumber != null) {
                paginationList = inwardRepository.findAllByinwardNumberContainingIgnoreCaseAndDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(inwardNumber, serviceAreaStaffResult, serviceAreaDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            } else {
                paginationList = inwardRepository.findAllByDestinationIdInAndDestinationTypeAndIsDeletedIsFalseAndMvnoIdIn(serviceAreaStaffResult, serviceAreaDestinationType, Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            }
        }
        return paginationList;
    }
...
//                throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(), "No record found with this inwardId", null);
    ...
    public InwardDto saveInwardByOutwardApproval(List<Inward> inwardList, String inwardApprovalStatus, String approval_remark) {
        try {
            ...
            List<Outward> outwardList = Lists.newArrayList(outwardRepository.findAll(aBoolean))
                    .stream().filter(outward -> outward.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || outward.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()).collect(Collectors.toList());
        ...
        try {
            if (inwardList != null) {
                if (inwardList.size() > 0) {
                    ...
                    InwardDto inwardDto = null;
                    inwardDto = getEntityForUpdateAndDelete(inwardList.get(0).getId());
                    if (inwardApprovalStatus.equalsIgnoreCase("Approve")) {
    ...
    public InwardDto getInwardOfOutwardByOutwardId (Long outwardId) throws Exception {
        try {
            QInward qInward = QInward.inward;
            ...
            return getEntityForUpdateAndDelete(inwardId);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
