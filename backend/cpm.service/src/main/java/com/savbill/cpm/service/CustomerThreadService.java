...
import com.savbill.cpm.rabbitMq.message.CustServiceActiveMsg;
import com.savbill.cpm.rabbitMq.message.CustServiceInActiveMsg;
...
//            } else
        ...
        if (updateCustomerServiceMappingList != null) {
            updateCustomerServiceMappingList.forEach(service -> {
                ...
                if(service.getStatus().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.ACTIVE)){
                    ...
                    sendCustServiceActiveMessage(customers.getUsername(), customers.getMobile(), customers.getEmail(), customers.getStatus(), customers.getMvnoId(),  buid);
                ...
                if(service.getStatus().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.INAVCTIVE)){
                    ...
                    sendCustServiceInActiveMessage(customers.getUsername(), customers.getMobile(), customers.getEmail(), customers.getStatus(), customers.getMvnoId() ,customers.getBuId());
    ...
    public void sendCustServiceActiveMessage(String username, String mobileNumber, String emailId, String status, Integer mvnoId,Integer buId) {
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    CustServiceActiveMsg custServiceActiveMsg = new CustServiceActiveMsg(username, mobileNumber, emailId, status, mvnoId, RabbitMqConstants.CUSTOMER_SERVICE_ACTIVE_EVENT, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY,buId);
                    Gson gson = new Gson();
...
//                    messageSender.send(custServiceActiveMsg, RabbitMqConstants.QUEUE_CUSTOMER_SERVICE_ACTIVE_NOTIFICATION);
    ...
    public void sendCustServiceInActiveMessage(String username, String mobileNumber, String emailId, String status, Integer mvnoId,Long buId) {
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    CustServiceInActiveMsg custServiceInActiveMsg = new CustServiceInActiveMsg(username, mobileNumber, emailId, status, mvnoId, RabbitMqConstants.CUSTOMER_SERVICE_INACTIVE_EVENT, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY,buId);
                    Gson gson = new Gson();
