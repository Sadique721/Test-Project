...
public interface CustomerServiceMappingRepository extends JpaRepository<CustomerServiceMapping, Integer>, QuerydslPredicateExecutor<CustomerServiceMapping> {
    ...
    List<CustomerServiceMapping> findAllByCustIdAndServiceId(Integer cutId, Long serviceIds);
    ...
    @Query(value = "SELECT m.id AS custServiceMappingId, " +
            ...
            "LEFT JOIN tblcustomers c ON m.custid = c.custid " +
            "WHERE (m.connection_no IS NULL OR TRIM(m.connection_no) = '')", nativeQuery = true)
    ...
    @Query("SELECT CASE WHEN COUNT(c)>0 THEN true ELSE false END FROM CustomerServiceMapping c WHERE c.customer.id=:custId AND c.isSGImgUpload=1")
    boolean existsByCustIdAndIsSgImgUpload(@Param("custId") Integer custId);
}