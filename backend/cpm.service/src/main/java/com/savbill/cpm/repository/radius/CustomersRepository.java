...
import com.savbill.cpm.modules.subscriber.model.CustomerDTO;
...
import com.savbill.cpm.pojo.api.CustomersDetailsPojo;
...
public interface CustomersRepository extends JpaRepository<Customers, Integer>, QuerydslPredicateExecutor<Customers> {
    ...
    @Query("select new com.savbill.cpm.pojo.api.CustomersDetailsPojo(c.id,c.username,c.firstname,c.lastname,c.title,c.custname,c.status,c.custtype,c.planMappingList,c.mvnoId,c.parentCustomers.id) from Customers c where c.id =:id and c.isDeleted=false")
    CustomersDetailsPojo findCustomersDetailsByIdAndIsDeletedIsFalse(@Param("id") Integer id);

    @Query("select new com.savbill.cpm.pojo.api.CustomersDetailsPojo(c.id,c.username,c.firstname,c.lastname,c.title,c.custname,c.status,c.custtype,c.planMappingList,c.mvnoId,c.parentCustomers.id) from Customers c where c.id =:id and c.isDeleted=false and c.mvnoId in :mvnoIds")
...
*/
    ...
    @Query("SELECT DISTINCT new Customers(c.id, c.mobile, c.email, c.countryCode, c.username, c.mvnoId, c.status, c.custtype, c.walletbalance, c.buId) " +
            "FROM Customers c " +
    ...
    @Query("SELECT DISTINCT new Customers(c.id, c.mobile, c.email, c.countryCode, c.username, c.mvnoId, c.status, " +
            "c.custtype, c.walletbalance, c.buId) " +
            "FROM Customers c " +
            "JOIN CustomerPackage d ON d.customers.id = c.id " +
            "join DebitDocument db on d.debitdocid=db.id "+
    ...
    @Query("select DISTINCT new Customers(c.id,c.mobile,c.email,c.countryCode,c.username,c.mvnoId,c.status, c.custtype,c.walletbalance,c.buId)   from Customers c, DebitDocument d where  c.dunningType =:dunningType" +
    ...
    @Query("select DISTINCT new Customers (c.id,c.mobile,c.email,c.countryCode,c.username,c.mvnoId,c.status, c.custtype,c.walletbalance,c.buId)   from Customers c, DebitDocument d where  c.dunningSector =:dunningSector" +
            ...
            + "  (c.status='Active') and c.isDeleted =false and d.customer.id = c.id and c.branch IN (:branchIds) and c.custtype =:customerPayType and c.isDunningEnable =true and  cast (datediff(curdate(),d.duedate) as integer) = (:dateDiff + COALESCE(d.debitdocGraceDays, 0)) and d.paymentStatus !='Fully Paid'")
    List<Customers> getCustomersForDunningSectorPostpaid(@Param(value = "dunningSector") String dunningSector, @Param(value = "dunningCategory") String dunningCategory, @Param(value = "dateDiff") Integer dateDiff,@Param(value = "customerPayType") String customerPayType, @Param("branchIds") List branchIds);
    ...
    @Query("select DISTINCT new Customers(c.id,c.mobile,c.email,c.countryCode,c.username,c.mvnoId,c.status, c.custtype,c.walletbalance,c.buId)   from Customers c, DebitDocument d where " +
    ...
    @Query("SELECT DISTINCT new Customers(c.id, c.mobile, c.email, c.countryCode, c.username, c.mvnoId, c.status, c.custtype, c.walletbalance, c.buId) " +
            ...
            "AND c.isDunningEnable = true " +
            "AND c.branch IN (:branchIds) " +
            "AND DATEDIFF(d.endDate, CURRENT_DATE) = :dateDiff")
    ...
    @Query("SELECT DISTINCT new Customers(c.id, c.mobile, c.email, c.countryCode, c.username, c.mvnoId, c.status, c.custtype, c.walletbalance, c.buId) " +
            ...
            "    FROM CustomerPackage d2 " +
            "    WHERE d2.customers.id = c.id " +
            ") " +
            "AND DATEDIFF(d.endDate, CURRENT_DATE) = :dateDiff")
    ...
    @Query("select DISTINCT new  Customers(c.id,c.mobile,c.email,c.countryCode,c.username,c.mvnoId,c.status, c.custtype,c.walletbalance,c.buId)   from Customers c, CustomerPackage d where c.dunningCategory = :dunningCategory " +
...
//    List<Integer> findAllIdByIsDeletedIsFalseAndStatusIn(List<String> status);
    ...
    @Query(value = "SELECT t.firstname, t.cstatus FROM tblcustomers t WHERE t.lead_id IN :ids order by lead_id desc", nativeQuery = true)
...
//            Long parentId, String invoiceType, Integer mvnoId, Integer mvnoIdFromCurrentStaff, List<Long> buIds);
    ...
    @Query("SELECT c FROM Customers c WHERE c.isKraSynced = false AND c.isDeleted = false AND lower(c.status) = 'active'")
    List<Customers> findAllUnsyncedActiveCustomers();
    ...
    @Query(value = "select c.mobile AS customerMsisdn , c.username AS userName ,c.password AS passWord ," +
            ...
            "c.lastname AS lastName, c.cstatus, c.custid AS custId, c.MVNOID AS mvnoId, c.BUID AS buId,"+
            "c.customertype AS custtype from tblcustomers c " +
            "where c.accountnumber =:accountNumber",nativeQuery = true)
    List<Object[]> findCustomersByOnlyAccountNumber(@Param("accountNumber") String accountNumber);
    ...
    @Query("SELECT new com.savbill.cpm.pojo.NewCustPojos.CustFieldsPojo( " +
   ...
   Customers  findByIdLight(@Param("customerId") Integer CustomerId);
    ...
    @Query("SELECT c.id, c.username, c.firstname, c.lastname, c.partner.id, c.acctno, c.countryCode " +
    ...
    List<Customers> findByAcctnoAndMobile(String acctno, String mobile);
    ...
    @Query(
            value = "SELECT " +
                    ...
                    " c.cityid AS cityid, " +
                    " c.name AS cityname " +
                    "FROM savbillcpm.tblmarea a " +
                    "LEFT JOIN savbillcpm.tblmcity c ON c.cityid = a.cityid " +
                    "WHERE a.areaid = :areaId",
            nativeQuery = true
    )
    PincodeAreaCityProjection getAreaCityDetails(@Param("areaId") Integer areaId);

    @Query("SELECT COUNT(c) FROM Customers c WHERE c.status = :status and c.currentAssigneeId = :currentAssigneeId")
    int countCafByStatus(@Param("status") String status, @Param("currentAssigneeId") Integer currentAssigneeId);

    @Query("SELECT new com.savbill.cpm.modules.subscriber.model.CustomerDTO(c.id,c.username,c.firstname,c.lastname,c.email,c.acctno,c.mobile,c.countryCode,c.mvnoId,sa.id,sa.name) FROM Customers c LEFT JOIN c.servicearea sa WHERE c.acctno = :accountnumber AND c.mobile = :mobile")
    List<CustomerDTO> getByAccountAndMobile(@Param("accountnumber") String accountnumber, @Param("mobile") String mobile);
}
