...
import com.savbill.cpm.MicroSeviceDataShare.SharedMessages.AutoRenewalPreferenceMessage;
...
import com.savbill.cpm.mapper.postpaid.CustQuotaDtlsMapper;
...
import com.savbill.cpm.modules.Matrix.domain.TatMatrixWorkFlowDetails;
...
import com.savbill.cpm.modules.NetworkDevices.service.SlotService.OLTSlotService;
import com.savbill.cpm.modules.NetworkDevices.service.SlotService.OltPortService;
...
import com.savbill.cpm.modules.PartnerLedger.repository.PartnerLedgerDetailsRepository;
...
import com.savbill.cpm.modules.Pincode.repository.PincodeRepository;
...
import com.savbill.cpm.modules.RvenueClient.RevenueClient;
...
import com.savbill.cpm.modules.SubscriberUpdates.Utils.SubscriberUpdateUtils;
import com.savbill.cpm.modules.SubscriberUpdates.Utils.UpdateConstant;
...
import com.savbill.cpm.modules.integrations.NexgeVoice.service.NexgeVoiceProvisionService;
...
import com.savbill.cpm.modules.ippool.service.IPAllocationService;
import com.savbill.cpm.modules.ippool.service.IPPoolDtlsService;
import com.savbill.cpm.modules.ippool.service.IPPoolService;
import com.savbill.cpm.modules.ippool.utils.IpConfigConstant;
import com.savbill.cpm.modules.payments.repository.ServiceAuditRepository;
...
import com.savbill.cpm.modules.planUpdate.service.QuotaDtlsService;
...
import com.savbill.cpm.modules.subscriber.Domain.QServiceAudit;
import com.savbill.cpm.modules.subscriber.Domain.ServiceAudit;
import com.savbill.cpm.modules.subscriber.controller.CustPlanMappingDropdownPojo;
import com.savbill.cpm.modules.subscriber.mapper.ServiceAuditMapper;
import com.savbill.cpm.modules.subscriber.model.*;
import com.savbill.cpm.modules.subscriber.model.*;
import com.savbill.cpm.modules.subscriber.model.Constants;
import com.savbill.cpm.modules.subscriber.queryScript.PlanQueryScript;
import com.savbill.cpm.nepaliCalendarUtils.model.EnglishDateDTO;
import com.savbill.cpm.nepaliCalendarUtils.model.NepaliDateDTO;
import com.savbill.cpm.nepaliCalendarUtils.service.DateConverterService;
...
import com.savbill.cpm.pojo.api.*;
import com.savbill.cpm.pojo.customer.plans.ExtendPlanValidityInBulk;
import com.savbill.cpm.pojo.customer.plans.PromiseToPayPojo;
import com.savbill.cpm.pojo.customer.plans.PromiseToPayPojoInBulk;
...
public class SubscriberService extends AbstractService<Customers, CustomersPojo, Integer> {
    ...
    public QosPolicyDetailsModel getQosPolicyDetails(Integer custId) throws Exception {
        ...
        try {
            ...
            detailsModel.setQosPolicyList(qosPolicyService.getAllEntities());
        } catch (Exception ex) {
            ApplicationLogger.logger.error(SUBMODULE + ex.getMessage(), ex);
            throw ex;
        }
    ...
    public CustomersBasicDetailsPojo changeQos(ChangeQosRequestDTO requestDTO, Customers customers) throws Exception {
        ...
        try {
            if (requestDTO.getPlanId() != null) {
                ...
                PostpaidPlan postpaidPlan = planService.getPostpaidPlanForEdit(custPlanMappping.getPlanId());
                ...
                if (custPlanMapppingPojo != null) {
                    QOSPolicyDTO oldQosPolicyDTO = qosPolicyService.getEntityById(custPlanMapppingPojo.getQospolicyId());
                    ...
                    custPlanMapppingPojo.setCustomer(customerMapper.domainToDTO(customersService.get(requestDTO.getCustId()), new CycleAvoidingMappingContext()));
                    ...
                    //Subscriber Update
                    QOSPolicyDTO newQosPolicyDTO = qosPolicyService.getEntityById(requestDTO.getQosPolicyId());
...
//                    }
//                }
                return getBasicDetailsOfSubscriber(customersService.get(requestDTO.getCustId()));
            } else {
    ...
    public CustomersBasicDetailsPojo changeVoiceDetails(CustomerVoiceDetailsDTO pojo) throws Exception {
        ...
        try {
            Customers customers = customersService.get(pojo.getId());
    ...
    public CustomersBasicDetailsPojo updateSubscriberOtherDetails(SubscriberPopupDTO subscriberPopupDTO) throws Exception {
        Customers customers;
        try {
            customers = customersService.get(subscriberPopupDTO.getCustId());
            //Old Value
    ...
    public List<CustomerPlansModel> getActivePlanList(Integer customerId, Boolean isNotChangePlan) {
...
//        List<CustomerPlansModel> customerPlansModelList = getCustomerPlanList(custId, isNotChangePlan);
        ...
        if (customerPlansModelList != null && 0 < customerPlansModelList.size()) {
            List<CustomerPlansModel> list = customerPlansModelList.stream()
                    .filter(dto -> null != dto.getPlanstage() && (
                            dto.getPlanstage().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.HOLD)
                                    ||
                                    (
                                            (
                                                    dto.getPlanstage().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.ACTIVE)
                                                            || dto.getPlanstage().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.SUSPEND)
                                                            || dto.getPlanstage().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.DISABLE)
                                                            || dto.getPlanstage().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.INGRACE)
                                            )
                                                    && dto.getServiceEndDate().isAfter(LocalDateTime.now())
                                    )
                    ))
            ...
            list.stream().forEach(data -> {
                // ======= Bas yahi add karo =============
                Integer serviceMappingId = data.getCustomerServiceMappingId();
                if (serviceMappingId != null) {
                    data.setCustPlanMapppingId(serviceMappingId);
                }
                // =======================================

                java.sql.Date startDateTmp = data.getStartDate();
                java.sql.Date endDateTmp = data.getExpiryDate();

                Long validity = ChronoUnit.DAYS.between(startDateTmp.toLocalDate(), endDateTmp.toLocalDate());
                if (validity == 0L) validity = 1L;

                PostpaidPlan plan = postpaidPlanService.get(data.getPlanId());
                if (plan != null) {
                ...
                if (data.getPromiseToPayStartDate() != null && data.getPromiseToPayEndDate() != null &&
                        data.getPromiseToPayStartDate().before(new Date()) &&
                        data.getPromiseToPayEndDate().after(new Date())) {
                ...
                if (data.getDbExpiryDate() != null) {
                    remainingDays = ChronoUnit.DAYS.between(LocalDateTime.now(), data.getDbEndDate());
                }
                data.setRemainingDays(remainingDays);
                data.setCustomerInventorySerialnumberDtos(
                        customerInventoryMappingService.getActiveSerialnumberByConnectionNo(data.getConnection_no(), data.getCustId())
                );
            });
            return list;
        }
    ...
    public List<CustomerPlansModel> getActivePlanList2(Integer customerId, Boolean isNotChangePlan) {
//        Customers customers = customersRepository.findById(custId).orElse(null);
        Customers customers = customersService.get(customerId);
//        List<CustomerPlansModel> customerPlansModelList = getCustomerPlanList(custId, isNotChangePlan);
        List<CustomerPlansModel> customerPlansModelList = getCustomerPlanList(customers, isNotChangePlan);
        ...
        if (customerPlansModelList != null && 0 < customerPlansModelList.size()) {
            List<CustomerPlansModel> list = customerPlansModelList.stream()
                    .filter(dto -> null != dto.getPlanstage() &&
                            (
                                    dto.getPlanstage().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.EXPIRED)
                                            ||
                                            dto.getPlanstage().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.HOLD)
                                            ||
                                            (
...
//        Customers customers = customersRepository.getOne(custId);
     ...
     */
    private Long getPlanValidity(java.sql.Date startDate, java.sql.Date endDate, Integer planId) {
...
//        Optional<PostpaidPlan> plan = postpaidPlanRepo.findById(planId);
        Optional<PostpaidPlan> plan = Optional.ofNullable(postpaidPlanService.get(planId));
        if (plan.isPresent()) {
            if (plan.get().getUnitsOfValidity().equalsIgnoreCase(CommonConstants.VALIDIDY_UNIT_HOURS)) {
...
//            Customers customer = customersRepository.findById(custId).get();
            ...
            for (CustomerPlansModel customerPlansModel : customerPlansModelList) {
                ...
                if (customerPlansModel.getPlangroupid() != null) {
                    PlanGroup planGroup = planGroupService.findPlanGroupById(customerPlansModel.getPlangroupid(), getMvnoIdFromCurrentStaff());
...
//                        PlanService planService1 = planServiceRepository.findById(customerServiceMapping.getServiceId().intValue()).get();
                        PlanService planService1 = planServiceService.get(customerServiceMapping.getServiceId().intValue());
...
//                            customerPlansModel.setMaxHoldAttempts(customerServiceMapping.getServiceHoldAttempts());
                        ...
                        customerPlansModel.setChildValidity(getPlanValidity(customerPlansModel.getStartDate(), customerPlansModel.getExpiryDate(), customerPlansModel.getPlanId()));
                        customerPlansModel.setValidity(Double.valueOf(getPlanValidity(customerPlansModel.getStartDate(), customerPlansModel.getExpiryDate(), customerPlansModel.getPlanId())));
                    ...
                    Long remainingDays = 0L;
                    if (customerPlansModel.getDbExpiryDate() != null) {
                        remainingDays = ChronoUnit.DAYS.between(LocalDateTime.now(), customerPlansModel.getDbEndDate());
                    }
                    customerPlansModel.setRemainingDays(remainingDays);
                }
            }
...
//                Long systemPromiseToPayCount = Long.valueOf(clientServiceSrv.getValueByNameAndmvnoId(ClientServiceConstant.PROMISETOPAY_COUNT, customers.getMvnoId()));
                ...
                if (customerPlansModel.getCustomerServiceMappingId() != null) {
                    ...
                    if (customerServiceMapping != null) {
                        ...
                        customerPlansModel.setChildValidity(getPlanValidity(customerPlansModel.getStartDate(), customerPlansModel.getEndDate(), customerPlansModel.getPlanId()));
                        customerPlansModel.setValidity(Double.valueOf(getPlanValidity(customerPlansModel.getStartDate(), customerPlansModel.getEndDate(), customerPlansModel.getPlanId())));
                        if (custPlanMappping.getRemarks() != null)
                            customerPlansModel.setRemarks(custPlanMappping.getRemarks());
                        if (custPlanMappping.getBillTo() != null)
                            customerPlansModel.setBillTo(custPlanMappping.getBillTo());
...
//            statusModel.setCustId(customerId);
    ...
    public CustomersBasicDetailsPojo changeQuota(QuotaDtlsModel requestDTO, Customers customers) throws Exception {
        ...
        try {
            ...
            dto.setCustomer(customersService.get(requestDTO.getCustId()));
    ...
    /// /            statusModel.setCurrentStatus(customersService.get(customerId).getStatus());
...
//                }
    ...
    public ReversableChargeResponseDTO reverseCharge(ReverseChargeRequestDTO requestDTO, Customers customers) throws Exception {
        ...
        try {
            ...
            if (!custChargeDetails.getIs_reversed()) {
                ...
                charge = this.chargeService.get(custChargeDetails.getChargeid());
    ...
    public MacUpdateModel getMacDetails(Integer customerId) {
        ...
        try {
            macDetailsModel.setMacTelFlag(customersService.get(customerId).getMactelflag());
    ...
    public CustomersBasicDetailsPojo updateMacDetails(MacUpdateModel requestDTO) throws Exception {
        ...
        try {
            if (requestDTO.getMacTelFlag()) {
                requestDTO.getMacAddresses().stream().forEach(dto -> {
                    CustMacMappping custMacMappping = dto.getId() == null ? new CustMacMappping() : custMacMapppingService.get(dto.getId());
                    oldCustMacMapppingList.add(new CustMacMappping(custMacMappping));
                    custMacMappping.setCustomer(customersService.get(requestDTO.getCustId()));
    ...
    public CustomersBasicDetailsPojo resetMacDetails(Integer customerId, String remarks) throws Exception {
        ...
        try {
            ...
            return getBasicDetailsOfSubscriber(customersService.get(customerId));
        } catch (Exception ex) {
            ApplicationLogger.logger.error(SUBMODULE + ex.getMessage(), ex);
            throw ex;
        }
    ...
    public CustomersBasicDetailsPojo updateContactDetails(ContactDetailsDTO contactDetailsDTO) throws Exception {
        ...
        Customers customers = customersService.get(contactDetailsDTO.getCustId());
        ...
        return getBasicDetailsOfSubscriber(customersService.get(contactDetailsDTO.getCustId()));
    }
    ...
    public CustomersBasicDetailsPojo updateBasicDetails(BasicDetailsDTO basicDetailsDTO) throws Exception {
        ...
        Customers customers = customersService.get(basicDetailsDTO.getCustId());
        ...
        return getBasicDetailsOfSubscriber(customersService.get(basicDetailsDTO.getCustId()));
    }
    ...
    public NetworkDetailsModel getNetworkDetails(Customers customers) throws Exception {
        ...
        try {
            ...
            if (ipPoolService.getAllEntities() != null && ipPoolService.getAllEntities().size() > 0) {
                networkDetailsModel.setDefaultPool(ipPoolService.getAllEntities());
            }
            if (serviceAreaService.getAllEntities() != null && serviceAreaService.getAllEntities().size() > 0) {
                networkDetailsModel.setServiceArea(serviceAreaService.getAllEntities());
            }
            ...
            if (customers.getDefaultpoolid() != null) {
                if (ipPoolService.getEntityById(customers.getDefaultpoolid()) != null) {
                    networkDetailsModel.setSelectedDefaultIpPool(ipPoolService.getEntityById(customers.getDefaultpoolid()));
                }
            }
            ...
            if (customers.getOltslotid() != null) {
                if (oltSlotService.getEntityById(customers.getOltslotid()) != null) {
                    networkDetailsModel.setSelectedOltSlotDetailDTO(oltSlotService.getEntityById(customers.getOltslotid()));
                }
            }
            if (customers.getOltportid() != null) {
                if (oltPortService.getEntityById(customers.getOltportid()) != null) {
                    networkDetailsModel.setSelectedOltPortDetailsDTO(oltPortService.getEntityById(customers.getOltportid()));
                }
            }
    ...
    public CustomersBasicDetailsPojo updateNetworkDetails(NetworkDetailsDTO networkDetailsDTO) throws Exception {
        ...
        Customers customers = customersService.get(networkDetailsDTO.getCustId());
        ...
        return getBasicDetailsOfSubscriber(customersService.get(networkDetailsDTO.getCustId()));
    }
    ...
    public AddressDetailDTO getAddressDetails(Integer customerId) {
        ...
        try {
            CustomerAddress presentAddress = customerAddressService.findByAddressTypeAndCustomer(SubscriberConstants.CUST_ADDRESS_PRESENT, customersService.get(customerId));
            CustomerAddress permanentAddress = customerAddressService.findByAddressTypeAndCustomer(SubscriberConstants.CUST_ADDRESS_PERMANENT, customersService.get(customerId));
            CustomerAddress paymentAddress = customerAddressService.findByAddressTypeAndCustomer(SubscriberConstants.CUST_ADDRESS_PAYMENT, customersService.get(customerId));
            if (presentAddress != null) {
                ...
                addressDetailsModel.setCountry(countryService.get(presentAddress.getCountryId()).getName());
                addressDetailsModel.setState(stateService.get(presentAddress.getStateId()).getName());
                addressDetailsModel.setCity(cityService.get(presentAddress.getCityId()).getName());
            ...
            if (permanentAddress != null) {
                ...
                addressDetailsModel.setCountry(countryService.get(permanentAddress.getCountryId()).getName());
                addressDetailsModel.setState(stateService.get(permanentAddress.getStateId()).getName());
                addressDetailsModel.setCity(cityService.get(permanentAddress.getCityId()).getName());
            ...
            if (paymentAddress != null) {
                ...
                addressDetailsModel.setCountry(countryService.get(paymentAddress.getCountryId()).getName());
                addressDetailsModel.setState(stateService.get(paymentAddress.getStateId()).getName());
                addressDetailsModel.setCity(cityService.get(paymentAddress.getCityId()).getName());
    ...
    public CustomersBasicDetailsPojo updateAddressDetails(AddressUpdateDTO addressUpdateDTO, Customers customers) throws Exception {
        ...
        String oldValue;
        CustomerAddress customerAddress = customerAddressService.findByAddressTypeAndCustomer(addressUpdateDTO.getAddressType(), customersService.get(addressUpdateDTO.getCustId()));
        ...
        try {
            if (null == customerAddress) {
                ...
                customerAddress.setArea(areaService.getMapper().dtoToDomain(areaService.getEntityById(addressUpdateDTO.getAddress().getAreaId().longValue()), new CycleAvoidingMappingContext()));
                customerAddress.setPincode(pincodeService.getMapper().dtoToDomain(pincodeService.getEntityById(addressUpdateDTO.getAddress().getPincodeId().longValue()), new CycleAvoidingMappingContext()));
                customerAddress.setCountry(countryService.get(addressUpdateDTO.getAddress().getCountryId()));
                customerAddress.setState(stateService.get(addressUpdateDTO.getAddress().getStateId()));
                customerAddress.setCity(cityService.get(addressUpdateDTO.getAddress().getCityId()));
        ...
        return getBasicDetailsOfSubscriber(customersService.get(addressUpdateDTO.getCustId()));
    }
    ...
    public RecordpaymentResponseDTO recordPayment(RecordPaymentRequestDTO requestDTO, Customers customers) throws Exception {
        ...
        try {
            ...
            creditDocument.setXmldocument(this.creditDocService.assemblePaymentXML(creditDocument, CommonUtils.ADDR_TYPE_PRESENT));
            if (CommonUtils.PAYMENT_MODE_CHEQUE.equalsIgnoreCase(requestDTO.getPaymentMode())) {
            ...
            if (requestDTO.getPaymentMode().equalsIgnoreCase(SubscriberConstants.PAYMENT_TYPE_TDS)) {
                CreditDocument originaltxn = this.creditDocService.get(requestDTO.getCredit_doc_id());
    ...
    public ReversePaymentResponseDTO reversePayment(ReversePaymentRequestDTO requestDTO, Customers customers) throws Exception {
        ...
        try {
            CreditDocument creditDocument = this.creditDocService.get(requestDTO.getPayment_id());
    ...
    public RecordpaymentResponseDTO updatePayment(RecordPaymentRequestDTO requestDTO, CreditDocument creditDocument, Customers customers) throws Exception {
        ...
        try {
            ...
            creditDocument.setRemarks(CommonUtils.PAYMENT_STATUS_APPROVED);
            creditDocument.setCreatedate(LocalDateTime.now());
    ...
    public ChangePlanDTO getSubscriberCurrentPlan(Customers customers) throws Exception {
        ...
        if (customerPlansModel != null) {
            PostpaidPlan postpaidPlan = this.planService.get(customerPlansModel.getPlanId());
...
//        	List<CustomerPlansModel> futurePlanList = this.subscriberService.getFuturePlanList(customers.getId());
            ...
            PostpaidPlan postpaidPlan = planService.get(requestDTO.getPlanId());
            ...
            if (!requestDTO.getIsAdvRenewal() && requestDTO.getPurchaseType().equalsIgnoreCase(SubscriberConstants.PLAN_PURCHASE_RENEW)) {
                ...
                if (activeCustPlanModel != null) {
                    tempPostpaidPlan = this.planService.get(activeCustPlanModel.getPlanId());
                    if (parentCustomerId != null) {
...
//                    }
                    if (parentCustomerId != null) {
                        ...
                        if (requestDTO.getAddonStartDate() != null) {
                            ...
                            startDate = custPlanMappping.getExpiryDate().plusSeconds(1);
                    ...
                    if (parentCustomerId != null) {
                        ...
                        if (requestDTO.getAddonStartDate() != null) {
                            ...
                            startDate = startDate.plusSeconds(1L);
...
//                                expiryDate = calculateExpiryDate(customersPojo, postpaidPlan, startDate);//LocalDateTime.of(startDate.plusDays(maxValidity.longValue()).toLocalDate(), LocalTime.from(startDate));
            ...
            if (requestDTO.getPurchaseType().equalsIgnoreCase(SubscriberConstants.PLAN_ADDON)) {
                ...
                //This is in not use
                PostpaidPlan tempPostpaidPlan = this.planService.get(requestDTO.getPlanId());
            ...
            if (!CollectionUtils.isEmpty(requestDTO.getPlanMappingList())) {
                ...
                if (planMappingPojo.isPresent()) {
                    ...
                    else planMapppingPojo1.setBillTo(com.savbill.cpm.constants.Constants.CUSTOMER);
...
//                    planMapppingPojo1.setIsInvoiceToOrg(planMappingPojo.get().getIsInvoiceToOrg());
            ...
            for (CustPlanMapppingPojo planMapppingPojo : Collections.singleton(planMapppingPojo1)) {
                PostpaidPlanPojo plan = postpaidPlanMapper.domainToDTO(planService.get(planMapppingPojo.getPlanId()), new CycleAvoidingMappingContext());
                if (null != plan && null != plan.getQuotatype()) {
                    Double totalQuotaForSeconds = 0.0;
                    Double totalQuotaForKB = 0.0;
                    if (null != plan.getQuotaunittime() && plan.getQuotaunittime().equalsIgnoreCase(com.savbill.cpm.constants.Constants.MINUTE)) {
                        totalQuotaForSeconds = plan.getQuotatime() * 60;
                    }
                    if (null != plan.getQuotaunittime() && plan.getQuotaunittime().equalsIgnoreCase(com.savbill.cpm.constants.Constants.HOUR)) {
                        totalQuotaForSeconds = plan.getQuotatime() * 60 * 60;
                    }
                    if (null != plan.getQuotaUnit() && plan.getQuotaUnit().equalsIgnoreCase(com.savbill.cpm.constants.Constants.MB)) {
                        totalQuotaForKB = (double) plan.getQuota() * 1024;
                    }
                    if (null != plan.getQuotaUnit() && plan.getQuotaUnit().equalsIgnoreCase(com.savbill.cpm.constants.Constants.GB)) {
            ...
            if (services.isPresent()) {
                if (services.get().getExpiry() != null) {
                    if (services.get().getExpiry().equalsIgnoreCase(com.savbill.cpm.constants.Constants.AT_MIDNIGHT)) {
...
//                mapping = customersService.generateConnectionNumber(mapping);
            ...
            PostpaidPlanPojo planPojo = postpaidPlanMapper.domainToDTO(postpaidPlan, new CycleAvoidingMappingContext());
            customersService.saveCustomerChargeHistory(postpaidPlan, customersPojo, mapping, requestDTO.getPlanGroupId(), false, null);
            if (planMapppingPojo1.getBillTo().equalsIgnoreCase(com.savbill.cpm.constants.Constants.ORGANIZATION) && planMapppingPojo1.getIsInvoiceToOrg()) {
            ...
            sendRenewRechargeMessage(customers.getFullName(),customers.getUsername(), customers.getCountryCode(), customers.getMobile(), customers.getEmail(), customers.getMvnoId(), postpaidPlan.getName(), requestDTO.getPurchaseType());
...
//            Optional<CustomerServiceMapping> customerServiceMapping = customerServiceMappingRepository.findById(requestDTO.getCustServiceMappingId());
            ...
            PostpaidPlan postpaidPlan = planService.get(requestDTO.getPlanId());
//            Services service = serviceRepository.findById(postpaidPlan.getServiceId().longValue()).get();
            Services service = planServiceService.getServices(postpaidPlan.getServiceId());
...
//            List<CustomerPlansModel> activeCustPlanModelList = this.subscriberService.getCustomerPlanList(customers.getId());
            ...
            if (!requestDTO.getIsAdvRenewal() && requestDTO.getPurchaseType().equalsIgnoreCase(SubscriberConstants.PLAN_PURCHASE_RENEW)) {
                if (!customerServiceMapping.get().getStatus().equalsIgnoreCase("Hold")) {
                    if (activeCustPlanModel != null) {
                        if (parentCustomerId != null) {
                            startDate = Instant.ofEpochMilli(activeCustPlanModel.getEndDate().getTime()).atZone(ZoneId.systemDefault()).toLocalDateTime().plusSeconds(1L);
                            if (activeCustPlanModel.getCustPlanStatus().equalsIgnoreCase(CommonConstants.STOP_STATUS))
                                startDate = LocalDateTime.now();
                            expiryDate = calculateExpiryDate(customersPojo, postpaidPlan, startDate);
                            endDate = expiryDate;
                            PostpaidPlan plan = this.planService.get(requestDTO.getPlanId());
                            ...
                            if (dateOverrideDto != null) {
                                ...
                                if (requestDTO.getAddonStartDate() != null && requestDTO.getPurchaseType().equalsIgnoreCase(SubscriberConstants.PLAN_ADDON))
                                    ...
                                    startDate = Instant.ofEpochMilli(activeCustPlanModel.getEndDate().getTime()).atZone(ZoneId.systemDefault()).toLocalDateTime().plusSeconds(1);
                                    if (activeCustPlanModel.getCustPlanStatus().equalsIgnoreCase(CommonConstants.STOP_STATUS))
                                        startDate = LocalDateTime.now();
                                }
                                expiryDate = calculateExpiryDate(customersPojo, postpaidPlan, startDate);
                            ...
                            PostpaidPlan plan = this.planService.get(requestDTO.getPlanId());
//                            Optional<PlanService> services = planServiceRepository.findById(plan.getServiceId());
                            Optional<PlanService> services = Optional.ofNullable(planServiceService.get(plan.getServiceId()));
                            if (services.isPresent()) {
                                if (services.get().getExpiry() != null) {
                                    if (services.get().getExpiry().equalsIgnoreCase(com.savbill.cpm.constants.Constants.AT_MIDNIGHT)) {
                                        expiryDate = LocalDateTime.of(expiryDate.toLocalDate(), LocalTime.of(23, 59, 59));
                ...
                PostpaidPlan plan1 = this.planService.get(requestDTO.getPlanId());
                ...
                if (getFuturePlanList.size() > 0) {
                    if (parentCustomerId != null) {
                        startDate = Instant.ofEpochMilli(getFuturePlanList.get(0).getEndDate().getTime()).atZone(ZoneId.systemDefault()).toLocalDateTime().plusSeconds(1L);
                        expiryDate = calculateExpiryDate(customersPojo, postpaidPlan, startDate);
                        endDate = expiryDate;
                        PostpaidPlan plan = this.planService.get(requestDTO.getPlanId());
                        Optional<PlanService> services = planServiceRepository.findById(plan.getServiceId());
...
//            Optional<PlanService> services = planServiceRepository.findById(postpaidPlan.getServiceId());
            Optional<PlanService> services = Optional.ofNullable(planServiceService.get(postpaidPlan.getServiceId()));
...
//                    mapping = customersService.generateConnectionNumber(mapping);
            ...
            if (!CollectionUtils.isEmpty(planMapppingList)) {
                ...
                planMapppingPojo1.setInvoiceType(com.savbill.cpm.constants.Constants.INDEPENDENT_INVOICE_TYPE);
            }
            ...
            AutoRenewalPreferenceMessage autoRenewalPreferenceMessage = new AutoRenewalPreferenceMessage(com.savbill.cpm.constants.Constants.AUTO_RENEWAL_PREFERANCE,customers.getUsername(),customers.getId(),customers.getMvnoId(), customers.getEmail(),customers.getBuId(),mapping.getRenewalForBooster(),customers.getPhone());
            Gson gson = new Gson();
            ...
            PostpaidPlanPojo planPojo = postpaidPlanMapper.domainToDTO(postpaidPlan, new CycleAvoidingMappingContext());
            customersService.saveCustomerChargeHistory(postpaidPlan, customersPojo, mapping, requestDTO.getPlanGroupId(), false, requestDTO.getPurchaseType());
            if (planMapppingPojo1.getBillTo().equalsIgnoreCase(com.savbill.cpm.constants.Constants.ORGANIZATION)) {// && planMapppingPojo1.getIsInvoiceToOrg()) {
                planMapppingPojo1.setCustRefId(customersPojo.getId());
...
//            CustomerServiceMapping customerServiceMapping = customerServiceMappingRepository.findById(requestDTO.getCustServiceMappingId()).orElse(null);
    ...
    private LocalDateTime getParentMaxExpiryDateByService(Integer serviceId, Customers customers) {
        String serviceName = planServiceService.get(serviceId).getName();
    ...
    public CustomersBasicDetailsPojo allocateIp(Customers customers, AllocateIpDTO requestDTO, Boolean is_replace) throws Exception {
        ...
        try {
            CustChargeDetailsPojo chargeDetailsPojo = custChargeDetailMapper.domainToDTO(custChargeService.get(requestDTO.getChargeId()), new CycleAvoidingMappingContext());
            //OLD VALUE
            String newValue = null;
            if (chargeDetailsPojo != null) {
                ChargePojo chargePojo = chargeService.convertChargeModelToChargePojo(chargeService.get(chargeDetailsPojo.getChargeid()));
                IPPoolDtlsDTO ipPoolDtlsDTO = ipPoolDtlsService.getEntityById(requestDTO.getIpPoolDtlsId());
                ...
                custChargeDetailsPojoList.forEach(data -> {
                    try {
                        ...
                        if (ipPoolDtlsDTO != null && ipPoolDtlsDTO.getAllocatedId() != null) {
                            IPPoolDTO ipPoolDTO = ipPoolService.getEntityById(ipPoolDtlsDTO.getPoolId());
                            if (ipPoolDTO != null) {
    ...
    public IPPoolDtlsDTO releaseIp(IPPoolDtlsDTO ipPoolDtlsDTO, Customers customers, String reason) throws Exception {
        ...
        try {
            if (ipPoolDtlsDTO != null) {
                IPPoolDtlsDTO tempIpPoolDtlsDTO = ipPoolDtlsService.getEntityById(ipPoolDtlsDTO.getPoolDetailsId());
                if (!tempIpPoolDtlsDTO.getStatus().equalsIgnoreCase(IpConfigConstant.IP_STATUS_FREE)) {
                    ...
                    IPAllocationDTO ipAllocationDTO = ipAllocationService.getEntityById(ipPoolDtlsDTO.getAllocatedId());
    ...
    public CustChargeDetailsPojo changeIPExpiry(ChangeIPExpiryDTO requestDTO, Customers customers) throws Exception {
        ...
        try {
            CustChargeDetailsPojo chargeDetailsPojo = custChargeDetailMapper.domainToDTO(custChargeService.get(requestDTO.getCurrentChargeId()), new CycleAvoidingMappingContext());
...
//                customerLedgerService.save(custLedger);
                ...
                RecordPaymentRequestDTO recordPaymentRequestDTO = new RecordPaymentRequestDTO(CommonUtils.PAYMENT_MODE_ADJUST, LocalDate.now(), requestDTO.getAmount(), false, requestDTO.getRemarks(), customers.getId());
    ...
    public SubscriberPlanDetailsDTO getSubscriberPlanDetails(CustPlanMapppingPojo customerPlansModel) throws Exception {
        ...
        try {
            ...
            PostpaidPlanPojo postpaidPlanPojo = postpaidPlanMapper.domainToDTO(planService.get(customerPlansModel.getPlanId()), new CycleAvoidingMappingContext());
    ...
    public CustomersBasicDetailsPojo activatePlan(ActivatePlanReqModel requestDTO, Customers customers, CustPlanMapppingPojo customerPlansModel) throws Exception {
        ...
        try {
            ...
            //Subscriber Update
            PostpaidPlan tempPostpaidPlan = this.planService.get(custPlanMappping.getPlanId());
    ...
    public void sendRenewRechargeMessage(String name,String username, String countryCode, String mobileNumber, String emailId, Integer mvnoId, String plan, String purchaseType) {
        try {
            if (purchaseType.equals("Renew")) {
                ...
                if (optionalTemplate.isPresent()) {
                    if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                        ...
                        CustomerRenewalMessage custRenewMessage = new CustomerRenewalMessage(name,username, countryCode, mobileNumber, emailId, mvnoId, RabbitMqConstants.CUSTOMER_RENEW, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, plan, purchaseType, buId);
                        Gson gson = new Gson();
...
//                        messageSender.send(custRenewMessage, RabbitMqConstants.QUEUE_BSS_CUSTOMER_RENEWAL_SUCCESS);
                ...
                if (optionalTemplate.isPresent()) {
                    if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                        ...
                        CustomerRechargeMessage customerRechargeMessage = new CustomerRechargeMessage(name,username, countryCode, mobileNumber, emailId, mvnoId, RabbitMqConstants.CUSTOMER_RECHARGE, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, plan, purchaseType, buId);
                        Gson gson = new Gson();
...
//                      nepaliEndDateDTO = dateConverterService.calculateEndDateNepaliByDay(nepaliCurrentDateDTO, (int) plusDays);
            ...
            if (subscriber.getCalendarType() != null && subscriber.getCalendarType().equalsIgnoreCase(CommonConstants.CAL_TYPE_ENGLISH)) {
                if (postpaidPlan.getUnitsOfValidity().equalsIgnoreCase(CommonConstants.VALIDIDY_UNIT_DAYS)) {
                    ...
                    expDate = startDate.plusDays(CommonUtils.getDaysForExpiryDateByMonth(postpaidPlan.getValidity(), startDate.toLocalDate()));
                } else if (postpaidPlan.getUnitsOfValidity().equalsIgnoreCase(CommonConstants.VALIDIDY_UNIT_YEARS)) {
                    expDate = startDate.plusDays(CommonUtils.getDaysForExpiryDateByYear(postpaidPlan.getValidity(), startDate.toLocalDate()));
        ...
        if (custId != null) {
            ...
            if (list != null && list.size() > 0) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    PostpaidPlan plan = planService.get(list.get(i));
...
//            }
            ...
            for (DeactivatePlanReqDTO reqDTO : requestDTOs.getDeactivatePlanReqDTOS()) {
                reqDTO.setSkipQuotaUpdate(requestDTOs.getSkipQuotaUpdate());
...
//        Optional<Customers> customers = customersRepository.findById(requestDTO.getCustId());
        Optional<Customers> customers = Optional.ofNullable(customersService.get(requestDTO.getCustId()));
...
//        String pendingRevenue = clientService.getValueByName("CHECK_PENDING_REVENUE_CHANGEPLAN");
//        if (pendingRevenue.equalsIgnoreCase("1")) {
//            if (isRevenuePending(requestDTO, customers.get())) {
//                throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(), "Change plan not allowed as Revenue is Pending !", null);
//            }
//        }
...
//                      debitDocService.createInvoice(customers.get(), null, 200, null, "", requestDTO.getPaymentOwner(), requestDTO.getPaymentOwnerId(),false);
                    ...
                    requestDTO.setPlanMapppingList(planMapppingList);
                    for (CustPlanMappping planMappping : planMapppingList) {
                        if(dateOverrideDtos == null && requestDTO.getChangePlanBillingCycle()!=null && requestDTO.getChangePlanBillingCycle().equalsIgnoreCase(CommonConstants.CHANGEPLANBILLINGCYCLECONSTANT.Existing_Billing_Cycle)){
                            dateOverrideDtos=new DateOverrideDto(true,LocalDateTime.now(),planMappping.getEndDate());
                        }
...
//                debitDocService.createInvoice(customers.get(), null, 200, debitdocIds, "", requestDTO.getPaymentOwner(), requestDTO.getPaymentOwnerId(),false);
//            }
            List<Integer> custServiceMappingIds = requestDTO.getDeactivatePlanReqModels()
                    .stream()
                    .map(DeactivatePlanReqModel::getCustServiceMappingId)
                    .collect(Collectors.toList());

            List<CustomerServiceMapping> customerServiceMappings = customerServiceMappingRepository.findAllByIdIn(custServiceMappingIds);
            if (!CollectionUtils.isEmpty(customerServiceMappings)) {
                customerServiceMappings.forEach(serviceMapping -> serviceMapping.setServiceHoldAttempts(0));
    ...
    public void orgCustInvoiceForChangePlan(CustomersPojo subscriber, DeactivatePlanReqDTO requestDTO) {
        ...
        try {
            ...
            if (subscriber.getCusttype().equalsIgnoreCase("Prepaid")) {
                value = clientServiceRepository.findValueByNameandMvnoId("ORGANIZATION", getLoggedInMvnoId());
            } else {
                value = clientServiceRepository.findValueByNameandMvnoId("ORGANIZATIONPOST", getLoggedInMvnoId());
            }
            ...
            for (CustPlanMappping custPlanMappping : planMapppingList) {
                ...
                custPlanMapppingPojo.setBillTo(com.savbill.cpm.constants.Constants.ORGANIZATION);
        ...
        try {
            ...
            if (custPlanMappping != null) {
                ...
                if (custPlanMappping.getDebitdocid() != null) {
                    ...
                    expression = expression.and(qDebitDocument.billrunstatus.notIn(com.savbill.cpm.constants.Constants.VOID, com.savbill.cpm.constants.Constants.CANCELLED));
...
//            save(customers);
            if (model.isBillToOrg() && model.getNewAmount() != null) {
                ...
                planMapppingPojo.setBillTo(com.savbill.cpm.constants.Constants.ORGANIZATION);
            }
...
//                        mapping = customersService.generateConnectionNumber(mapping);
                        ...
                        if (customerServiceMapping.isPresent()) {
                            ...
                            else customerServiceMapping.get().setStatus(CommonConstants.ACTIVE_STATUS);
...
//                    }
                    ...
                    customersService.saveCustomerChargeHistory(postpaidPlanObj, subscriber, custPlanMappping, plangroupId, false, changePlanDate, null);
...
//                    planMapppingPojo.setPlanValidityDays(customersService.calculatePlanValidityDays(subscriber, postpaidPlanObj, Long.valueOf(validity), LocalDateTime.now()));
    ...
    public List<CustPlanMappping> fetchTrialPlanList(Integer custId, Integer planGroupId, Integer cprId) {
        ...
        if (planGroupId != null) {
            booleanExpression = booleanExpression.and(custPlanMappping.planGroup.planGroupId.eq(planGroupId));
        } else {
            booleanExpression = booleanExpression.and(custPlanMappping.id.eq(cprId));
        }
...
//        return promiseToPanMultipleCpr(planMapppingList, graceDays, promiseToPayPojoInBulk.getPromise_to_pay_remarks());
    ...
    public Customers addPromiseToPay(Integer custId, Integer graceDays, String promise_to_pay_remarks) {
        Integer gracePeriod = Integer.valueOf(clientServiceRepository.findValueByNameandMvnoId("graceperiod", getLoggedInMvnoId()));
        if (null == graceDays) {
            graceDays = gracePeriod;
        }
        if (gracePeriod < graceDays) {
    ...
    public List<CustPlanMapppingPojo> holdServiceInBulk(DeactivatePlanReqDTOList reqDTOList) {
        /*Adding hold days 999 as requirement : DIRA-318*/
        reqDTOList.setHoldDays(999);
        ...
        if (reqDTOList.isServiceStopBulkFlag()) {
            ...
            List<CustPlanMappping> custPlanMapppingList = custPlanMappingRepository.findAllByCustServiceMappingIdInAndCustPlanStatusAndEndDateIsAfter(custServiceIds,"Active",LocalDateTime.now());
            if (!CollectionUtils.isEmpty(custPlanMapppingList)) {
        ...
        String maxAttemptStr = clientServiceRepository.findValueByNameandMvnoId("SERVICE_HOLD_ATTEMPT", getLoggedInMvnoId());
        Integer maxAllowedHoldAttempt = (maxAttemptStr == null || maxAttemptStr.trim().isEmpty())
                ? null
                : Integer.valueOf(maxAttemptStr);
        customerServiceMappings.stream().forEach(customerServiceMapping->{
            Integer holdAttempts = customerServiceMapping.getServiceHoldAttempts();
            if (maxAllowedHoldAttempt != null && holdAttempts >= maxAllowedHoldAttempt) {
                throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(),
                        "Service Hold attempt exceeded. Allowed only " + maxAllowedHoldAttempt + " time(s).", null);
            }
            reqDTOList.setServiceResumeDate(LocalDate.now().plusDays(reqDTOList.getHoldDays()+1));
            List<CustPlanMappping> custPlanMapppings = custPlanMappingRepo.findAllByCustServiceMappingIdInAndCustPlanStatusAndEndDateIsAfter(Collections.singletonList(customerServiceMapping.getId()),"Active",LocalDateTime.now());
            ...
            for(CustPlanMappping custPlanMappping : custPlanMapppings){
                if (custPlanMappping.getStartDate().isAfter(LocalDateTime.now()) && custPlanMappping.getEndDate().isAfter(LocalDateTime.now())){
                  throw new CustomValidationException(HttpStatus.EXPECTATION_FAILED.value(), "Service can not be hold as future plan available for this service", null);
                }
            }
            /*Comment start -  As per requirement : DIRA-318*/
...
//            if ( postpaidPlan.getMaxHoldAttempts() > 0 && postpaidPlan.getMaxHoldDurationDays() > 0) {
...
//                }
//            }
            /*Comment end -  As per requirement : DIRA-318*/
        });
        if (!CollectionUtils.isEmpty(custServiceIds)) {
            custPlanMappingService.changeStatusOfCustServices(custServiceIds, StatusConstants.CUSTOMER_SERVICE_STATUS.HOLD, reqDTOList.getDeactivatePlanReqModels().get(0).getRemarks(), false);
        } else {
        ...
        List<CustPlanMappping> custPlanMapppings = custPlanMappingRepo.findAllByCustServiceMappingIdInAndCustPlanStatusAndEndDateIsAfter(custServiceIds,"Active",LocalDateTime.now());
        custPlanMapppings.forEach(custPlanMappping -> {
            custPlanMappping.setStatus(StatusConstants.CUSTOMER_SERVICE_STATUS.HOLD);
            custPlanMappping.setCustPlanStatus(StatusConstants.CUSTOMER_SERVICE_STATUS.HOLD);
            ...
            saveserviceAudit(custPlanMappping, pojo, StatusConstants.CUSTOMER_SERVICE_STATUS.HOLD);
        });
...
//        List<CustomerServiceMapping> customerServiceMappingList = customerServiceMappingRepository.findAllByIdIn(custServiceIds);
        customerServiceMappings.stream().forEach(i->{
            ...
            i.setRemarks("Service Hold Done");
            ...
            i.setStatus(CommonConstants.CUSTOMER_STATUS_HOLD);
            i.setPreviousStatus(CommonConstants.CUSTOMER_STATUS_HOLD);
        });
        customerServiceMappingRepository.saveAll(customerServiceMappings);
        return result;
    }
...
//        });
        if (!CollectionUtils.isEmpty(custServiceIds)) {
            List<CustomerServiceMapping> customerServiceMappings = customerServiceMappingRepository.findAllByIdIn(custServiceIds);
            List<Integer> custserviceMappingIds = customerServiceMappings.stream().map(CustomerServiceMapping::getId).collect(Collectors.toList());
            Map<Integer, Long> idToHoursDiffMap = new HashMap<>();
            Map<Integer, LocalDateTime> idToDateMap = new HashMap<>();
            Map<Integer, Long> custPlanIdActualHoursMap = new HashMap<>();
            List<CustPlanMappping> custPlanMapppingList = custPlanMappingRepository.findAllByCustServiceMappingIdInAndCustPlanStatusAndEndDateIsAfter(custserviceMappingIds,"Hold",LocalDateTime.now());
            Map<Integer, CustPlanMappping> mappingIdToPlanMap = custPlanMapppingList.stream()
                    .sorted(Comparator.comparing(CustPlanMappping::getId))
                    .collect(Collectors.toMap(CustPlanMappping::getCustServiceMappingId,Function.identity(),(existing, replacement) -> existing, LinkedHashMap::new));
            if (customerServiceMappings.size() > 0) {
            customerServiceMappings.stream().forEach(i -> {
            i.setStatus(CommonConstants.ACTIVE_STATUS);
            ...
            idToDateMap.put(i.getId(), i.getServiceHoldDate());
            long hoursElapsed = ChronoUnit.HOURS.between(i.getServiceHoldDate(), LocalDateTime.now());
//            int updatedHold = i.getActualHoldDurationDays() - Math.toIntExact(daysElapsed);
            CustPlanMappping plan = mappingIdToPlanMap.get(i.getId());
            if (plan != null && plan.getExpiryDate() != null) {
//                long expiryDiffHours = Math.abs(ChronoUnit.HOURS.between(LocalDateTime.now(), plan.getExpiryDate()));
                long expiryDiffHours = Math.abs(ChronoUnit.HOURS.between(i.getServiceHoldDate(), plan.getExpiryDate()));
                long hoursToExtend = Math.min(expiryDiffHours, hoursElapsed);
                logger.info("DEBUG HOLD={} RESUME={} EXPIRY={} | hoursElapsed={} | expiryDiffHours={} | hoursToExtend={}",i.getServiceHoldDate(),LocalDateTime.now(),plan.getExpiryDate(),hoursElapsed,expiryDiffHours,hoursToExtend);
                i.setActualHoldDurationDays((int)Math.ceil(hoursToExtend / 24.0));
                custPlanIdActualHoursMap.put(plan.getId(), hoursToExtend);
//                idToDateDiffMap.put(i.getId(), Long.valueOf(ChronoUnit.DAYS.between(i.getServiceHoldDate(), i.getServiceResumeDate())));
                idToHoursDiffMap.put(i.getId(), hoursToExtend);
            }
            });
            customerServiceMappingRepository.saveAll(customerServiceMappings);
            ...
            serviceChnageStatus.setStartTime(idToDateMap.get(custplanmapping.getCustServiceMappingId()));
            serviceChnageStatus.setEndTime(LocalDateTime.now());
            serviceChnageStatus.setDebitDocId(custPlanMappingRepository.getCurrentDebitdocIdByPlanMappId(custplanmapping.getId()));
            serviceChnageStatus.setTotalDays((int) (idToHoursDiffMap.get(custplanmapping.getCustServiceMappingId()) / 24.0));
            if ("Postpaid".equalsIgnoreCase(custplanmapping.getCustomer().getCusttype())) {
            ...
            result.add(custPlanMappingService.convertDomainToDto(custplanmapping));
            });
            ...
            customerServiceMappingRepository.updateServiceStatus(custserviceMappingIds, CommonConstants.ACTIVE_STATUS);
                if (!CollectionUtils.isEmpty(custPlanMapppingList)) {
                    custPlanMapppingList.forEach(custPlanMappping -> {
                        Long hoursToExtend = custPlanIdActualHoursMap.get(custPlanMappping.getId());
                        if (hoursToExtend != null) {
                            LocalDateTime baseDate;
                            if (LocalDateTime.now().isAfter(custPlanMappping.getExpiryDate())) {
                                baseDate = LocalDateTime.now(); // resume
                            } else {
                                baseDate = custPlanMappping.getExpiryDate(); // expiry
                            }
                            logger.info("FINAL DEBUG | baseDate={} | hoursToExtend={} | newEndDate={}",baseDate,hoursToExtend,baseDate.plusHours(hoursToExtend));
                            custPlanMappping.setEndDate(baseDate.plusHours(hoursToExtend));
//                          custPlanMappping.setExpiryDate(custPlanMappping.getEndDate().plusDays(custPlanIdActualDaysMap.get(custPlanMappping.getId())).toLocalDate().atStartOfDay());
                            custPlanMappingService.updateCustPlanEndDateInRadius(custPlanMappping, "");
                        }
                    });
                }
            custPlanMappingRepository.saveAll(custPlanMapppingList);
...
//        activeCustPlanModelList.removeIf(customerPlansModel -> (customerPlansModel.getCustPlanStatus().equalsIgnoreCase("STOP") && customerPlansModel.getStopServiceDate() == null));
        activeCustPlanModelList.clear();
        activeCustPlanModelList.addAll(this.subscriberService.getActivePlanList(
                customers.get().getId(), false));
        if (customers.isPresent()) {
            customers.get().setLastBillDate(LocalDate.now());
            Integer debitDocId = null;//new HashSet<>();
            HashSet<Integer> debitdocIds = new HashSet<>();
            ...
            for (DeactivatePlanReqModel model : requestDTO.getDeactivatePlanReqModels()) {
                ...
                if (CollectionUtils.isEmpty(planMapppingList)) {
                    if (flag) {
                        for (DeactivatePlanReqModel deactivatePlanReqModel : requestDTO.getDeactivatePlanReqModels()) {
                            Optional<PostpaidPlan> postpaidPlan = postpaidPlanRepo.findById(deactivatePlanReqModel.getNewPlanId());
                            ...
                            if (customers.get().getPlangroup() != null)
                                saveCustPlanMapping(customerMapper.domainToDTO(customers.get(), new CycleAvoidingMappingContext()), postpaidPlan.get(), customers.get().getPlangroup().getPlanGroupId(), model, customers.get(), deactivatePlanReqModel.getCustServiceMappingId(), renewalId, false, CommonConstants.EVENTCONSTANTS.CHANGE_PLAN, null);
                            else
                                saveCustPlanMapping(customerMapper.domainToDTO(customers.get(), new CycleAvoidingMappingContext()), postpaidPlan.get(), null, model, customers.get(), deactivatePlanReqModel.getCustServiceMappingId(), renewalId, false, CommonConstants.EVENTCONSTANTS.CHANGE_PLAN, null);
                        }
                        flag = false;
                    }
//                  if (!requestDTO.isPlanGroupChange())
//                      debitDocService.createInvoice(customers.get(), null, 200, null, "", requestDTO.getPaymentOwner(), requestDTO.getPaymentOwnerId(),false);
                ...
                if (!CollectionUtils.isEmpty(oldDebitdocIds)) {
                    ...
                    trialDebitDocuments = trialDebitDocuments.stream().peek(debitDoc -> debitDoc.setBillrunstatus(com.savbill.cpm.constants.Constants.CANCELLED)).collect(Collectors.toList());
...
//                custPlanMappping.setCustServiceMappingId(custServiceMappingId);
//                custPlanMappingRepository.saveAndFlush(custPlanMappping);
//                custPlanMappingService.updateCustPlanEndDateInRadius(custPlanMappping, CommonConstants.EVENTCONSTANTS.CHANGE_PLAN);
//
//            }

            if (custPlanMappping != null) {
                custPlanMappping = custPlanMappingService.stopAndSaveOldCpr(custPlanMappping,oldDebitdocIds);
                custServiceMappingId = custPlanMappping.getCustServiceMappingId();
            }
            ...
            if (custPlanMappping != null && planGroupId == null) {
                if (Objects.isNull(custPlanMappping.getTraildebitdocid())) {
                    debitDocService.createInvoice(customers, CommonConstants.INVOICE_TYPE.CHANGE_PLAN, "", null, null, null, false, false, null,null);
                }
            }
            if (custPlanMappping == null) {
                if (planGroupId == null)
    ...
    public List<CustPlanMapppingPojo> extendServiceValidityInBulk(ExtendPlanValidityInBulk extendPlanValidityInBulk) {
        try {
            ...
            extendPlanValidities = extendPlanValidities.stream().filter(CommonUtils.distinctByKey(ExtendPlanValidity::getCustPlanMapppingId)).collect(Collectors.toList());
            for (ExtendPlanValidity extendPlanValidity : extendPlanValidityInBulk.getExtendPlanValidity()) {
                ...
                if (extendPlanValidity.isExtentionforChild()) {
                    if (getBUIdsFromCurrentStaff().size() != 0)
                        booleanExpression = booleanExpression.and(qCustomers.mvnoId.eq(1).or(qCustomers.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qCustomers.buId.in(getBUIdsFromCurrentStaff()))));
    ...
    public GenericDataDTO extendServiceValidity(CustomerServiceMapping customerServiceMapping, ExtendPlanValidity extendPlanValidity) {
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        try {
            if(extendPlanValidity.getDownStartDate() != null && extendPlanValidity.getDownEndDate() != null) {
            ...
            List<CustPlanMappping> custPlanDistinctByService = planMapppingList.stream().sorted(comparing(CustPlanMappping::getStartDate).reversed()).filter(custPlanMappping -> !custPlanMappping.getPurchaseType().equals(CommonConstants.PLAN_GROUP_VOLUME_BOOSTER)).filter(CommonUtils.distinctByKey(CustPlanMappping::getCustServiceMappingId)).collect(Collectors.toList());
            if (!CollectionUtils.isEmpty(volumeBoosterPlans)) {
                custPlanDistinctByService.addAll(volumeBoosterPlans);
            }
            Long validityDays;
            if(extendPlanValidity.getNumberOfDays() != null){
                validityDays = extendPlanValidity.getNumberOfDays().longValue();
            }
            else if(extendPlanValidity.getDownStartDate() != null && extendPlanValidity.getDownEndDate() != null){
                validityDays = ChronoUnit.DAYS.between(extendPlanValidity.getDownStartDate(), extendPlanValidity.getDownEndDate()) + 1;
            }else {
                validityDays = 0L;
            }
            ...
            // Step 1: Extract renew/addon plans
            List<CustPlanMappping> renewAddOnPlans = planMapppingList.stream()
                    .filter(plan -> {
                        String purchaseType = plan.getPurchaseType();
                        return purchaseType != null && (purchaseType.equalsIgnoreCase("renew") || purchaseType.equalsIgnoreCase("addon") || purchaseType.equalsIgnoreCase("Registration and Renewal") || purchaseType.equalsIgnoreCase("Registration"));
                    })
                    .collect(Collectors.toList());

            // Step 2: Find the last/latest renew/addon plan by expiryDate
            Optional<CustPlanMappping> latestRenewAddOnPlanOpt = renewAddOnPlans.stream()
                    .max(Comparator.comparing(plan -> plan.getEndDate() != null ? plan.getEndDate() : LocalDateTime.MIN));

            // Step 3: Perform extension, with condition for last renew/addon plan only
            LocalDateTime now = LocalDateTime.now();

            boolean hasStopStatusPlan = custPlanMappingRepository.findAllByCustomerId(custId).stream()
                    .anyMatch(plan -> plan.getCustPlanStatus() != null
                            && plan.getCustPlanStatus().equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.STOP));

            Integer  onlyExtendThisPlanId = custPlanDistinctByService.get(0).getId();

            custPlanDistinctByService = custPlanDistinctByService.stream().peek(custPlanMappping -> {

                if (!custPlanMappping.getId().equals(onlyExtendThisPlanId)) {
                    return;
                }

                // Base for endDate related calculation
                LocalDateTime base = custPlanMappping.getEndDate() != null && custPlanMappping.getEndDate().isAfter(now)
                        ? custPlanMappping.getEndDate()
                        : now;

                String purchaseType = custPlanMappping.getPurchaseType();
                String status = custPlanMappping.getCustPlanStatus();

                if (hasStopStatusPlan) {
                    // Any STOP plan present → always extend endDate only, to midnight
                    LocalDate newEndDate = base.toLocalDate().plusDays(validityDays);
                    LocalDateTime midnight = newEndDate.atTime(23, 59, 59);
                    custPlanMappping.setEndDate(midnight);
                } else {
                    if (purchaseType != null &&
                            (purchaseType.equalsIgnoreCase("renew")
                                    || purchaseType.equalsIgnoreCase("Registration and Renewal")
                                    || purchaseType.equalsIgnoreCase("Registration"))
                            && status != null
                            && !status.equalsIgnoreCase(StatusConstants.CUSTOMER_SERVICE_STATUS.STOP)) {

                        // Only latest renew/addon/registration gets expiryDate extension (to midnight)
                        if (latestRenewAddOnPlanOpt.isPresent()
                                && latestRenewAddOnPlanOpt.get().getId().equals(onlyExtendThisPlanId)) {

                            LocalDateTime expiryDateBase =
                                    latestRenewAddOnPlanOpt.get().getEndDate() != null
                                            && latestRenewAddOnPlanOpt.get().getEndDate().isAfter(now)
                                            ? latestRenewAddOnPlanOpt.get().getEndDate()
                                            : now;

                            LocalDate newEndDate = expiryDateBase.toLocalDate().plusDays(validityDays);
                            LocalDateTime midnight = newEndDate.atTime(23, 59, 59);
//                            custPlanMappping.setEndDate(midnight);
                            custPlanMappping.setEndDate(midnight);
                        }
                        // Other renew/addon plans: do nothing (same as your original logic)
                    } else {
                        // Non-renew/addon plans → extend endDate to midnight
                        LocalDate newEndDate = base.toLocalDate().plusDays(validityDays);
                        LocalDateTime midnight = newEndDate.atTime(23, 59, 59);
                        custPlanMappping.setEndDate(midnight);
                    }
            ...
            //});
            if (extendPlanValidity.getExtend_validity_remarks() != null) {
                custPlanDistinctByService.forEach(custPlan ->
                        custPlan.setExtendValidityremarks(extendPlanValidity.getExtend_validity_remarks())
                );
            }
...
//            if (customers.stream().anyMatch(i -> i.getInvoiceType().equalsIgnoreCase(CommonConstants.INVOICE_TYPE_GROUP))) {
            ...
            List<CustPlanMappping> custPlanDistinctByService = custPlanMapppingList1.stream().filter(CommonUtils.distinctByKey(CustPlanMappping::getCustServiceMappingId)).collect(Collectors.toList());
            if (!CollectionUtils.isEmpty(volumeBoosterPlans)) {
                custPlanDistinctByService.addAll(volumeBoosterPlans);
            }
...
//linkAcceptanceDTO.setTraiRate(customerServiceMapping.getTraiRate());
    ...
    public LocalDateTime getParentExpiredDate(Customers parentCustomer, Long serviceId) {
        ...
        if (!CollectionUtils.isEmpty(futureList)) {
            ...
            if (!CollectionUtils.isEmpty(futureList)) {
                return CommonUtils.convertToLocalDateTimeViaSqlTimestamp(futureList.get(0).getEndDate());
            }
        }
        ...
        if (!CollectionUtils.isEmpty(activeList)) {
            ...
            if (!CollectionUtils.isEmpty(activeList)) {
                return CommonUtils.convertToLocalDateTimeViaSqlTimestamp(activeList.get(0).getEndDate());
            }
        }
...
//            List<CustPlanMappping> custPlanMapppingList = custPlanMappingRepository.findAllByCustServiceMappingIdIn(custServiceMappIds);
              List<CustPlanMappping> custPlanMapppingList = custPlanMappingRepository.findAllByIdIn(custServiceMappIds);
            if (!CollectionUtils.isEmpty(custPlanMapppingList)) {
                planGroups = custPlanMapppingList.stream().map(CustPlanMappping::getPlanGroup).collect(Collectors.toList());
                if (!CollectionUtils.isEmpty(planGroups)) {
        ...
        /*Here in custServiceMappIds , custPlanMapping is passed from frontend so managed here  */
        List<CustPlanMappping> custPlanMapppingList = custPlanMappingRepository.findAllByIdIn(custServiceMappIds);
        List<Integer> customerServiceMappingIds = custPlanMapppingList.stream().map(CustPlanMappping::getCustServiceMappingId).collect(Collectors.toList());
        List<CustomerServiceMapping> custServiceMappingDTOS = custPlanMappingService.changeStatusOfCustServices(customerServiceMappingIds, StatusConstants.CUSTOMER_SERVICE_STATUS.STOP, reqDTOList.getDeactivatePlanReqModels().get(0).getRemarks(), false);
...
//            messageSender.send(message,RabbitMqConstants.QUEUE_SERVICE_TERMINATION_REVENUE);
    ...
    public List<CustomerPlansModel> getResultOfQuery(Customers customers) {
        ...
        try {
            for (CustPlanMappping mappping : custPlanMappingRepo.findAllByCustomerIdWithQuotaList(customers.getId())) {
...
//                PlanService services = planServiceRepository.findById(plan.getServiceId()).orElse(null);
                PostpaidPlan plan = postpaidPlanService.get(mappping.getPlanId());
                PlanService services = planServiceService.get(plan.getServiceId());
                customerPlansModel.setCustId(customers.getId());
                customerPlansModel.setRenewalId(mappping.getRenewalId());
                customerPlansModel.setPlanId(plan.getId());
                customerPlansModel.setPlanName(plan.getName());
                customerPlansModel.setServiceId(plan.getServiceId());
                customerPlansModel.setService(services.getName());
                customerPlansModel.setPlangroup(plan.getPlanGroup());
                customerPlansModel.setPlanmapid(mappping.getId());
                customerPlansModel.setRenewalForBooster(mappping.getRenewalForBooster());
                if (plan.getOfferprice() != null) {
                    customerPlansModel.setOfferPrice(plan.getOfferprice());
                }
                if(plan.getMaxHoldAttempts() != null ){
                    customerPlansModel.setMaxHoldAttempts(plan.getMaxHoldAttempts());
                }
                if(plan.getMaxHoldDurationDays() != null ){
                    customerPlansModel.setMaxHoldDurationDays(plan.getMaxHoldDurationDays());
                }
                ...
                customerPlansModel.setValidity(plan.getValidity());
                ...
                System.out.println("======================++++++++++++SKY: PlanStage==Active++++++++++=============");
                if ((mappping.getStartDate().isBefore(LocalDateTime.now()) && mappping.getEndDate().isBefore(LocalDateTime.now())) || (mappping.getCustPlanStatus().equalsIgnoreCase("Suspend") || mappping.getCustPlanStatus().equalsIgnoreCase("Terminate") || mappping.getCustPlanStatus().equalsIgnoreCase("DISABLE") || mappping.getCustPlanStatus().equalsIgnoreCase("STOP")))
                    customerPlansModel.setPlanstage("EXPIRED");
                System.out.println("=============++++++++++++++++++SKY: PlanStage==cd Expired=========================");
                ...
                if (services != null) {
                    customerPlansModel.setIs_qosv(services.getIsQoSV());
                    customerPlansModel.setIs_dtv(services.getIs_dtv());
                }
...
//                PostpaidPlan postpaidPlan = postpaidPlanRepo.findById(mappping.getPlanId()).orElse(null);
    ...
    public CustomersPojo saveSubscriberFromForm(CustomersPojo subscriber, String requestFrom, boolean isCustomerUpdate) throws Exception {
        ...
        ClientService trialClientService = clientServiceSrv.getByNameAndMvnoIdEquals(com.savbill.cpm.constants.Constants.TRIAL_PLAN_PERIOD_THRESHOLD, subscriber.getMvnoId());
        ...
        ClientService noOfTimeTrialclientService = clientServiceSrv.getByNameAndMvnoIdEquals(com.savbill.cpm.constants.Constants.ALLOW_NUMBER_OF_TIME_TRAIL, subscriber.getMvnoId());
...
//                }
                ...
                if (profileId != null) {
                    ...
                    if (subscriber.getAcctno() == null || subscriber.getAcctno().trim().isEmpty()) {
                        subscriber.setAcctno(acctNumCreationService.getNewCustomerAccountNo(custAccountProfile, getMvnoIdFromCurrentStaff()));
                    }
                }
                        ...
                        for (PlanGroupMapping planGroupMapping : planGroupMappingList) {
                            ...
                            if (!CollectionUtils.isEmpty(planMappingBySubscriber)) {
                                ...
                                if (custPlanMapppingPojo.isPresent()) {
                                    ...
                                    else planMapping.setBillTo(com.savbill.cpm.constants.Constants.CUSTOMER);
                        ...
                        if (subscriber.getPlanMappingList() != null && subscriber.getPlanMappingList().size() > 0) {
                            for (int i = 0; i < subscriber.getPlanMappingList().size(); i++) {
                                ...
                                if (mapping.getBillTo() == null) {
                                    mapping.setBillTo(com.savbill.cpm.constants.Constants.CUSTOMER);
...
//                                    }
                                ...
                                if (mapping.getPlanId() != null) {
                                    ...
                                    PostpaidPlan plan = planService.get(mapping.getPlanId());
                                    if (plan != null) {
                        ...
                        if (null != subscriber.getPaymentDetails()) {
                            ...
                            if (requestFrom.equalsIgnoreCase("pw")) {
                                if (paymentPojo.getPaytype() == null) {
                                    paymentPojo.setPaytype(Constants.ADVANCE);
                                }
                                if (paymentPojo.getType() == null) {
                                    paymentPojo.setType(CommonUtils.PAYMENT_TYPE);
                                }
                            ...
                            if (paymentPojo.getPaymode() != null && paymentPojo.getAmount() > 0) {
                                ...
                                Partner partner = partnerService.get(subscriber.getPartnerid());
                                if (partner != null)
                            ...
                            if (null != paymentPojo.getPaymode()) {
                                ...
                                creditDocument.setApproverid(getLoggedInUserId());
                                creditDocument.setReferenceno(String.valueOf(CommonUtils.getUniqueNumber()));
                                creditDocument.setStatus(CommonUtils.PAYMENT_STATUS_PENDING);
                                if (requestFrom.equalsIgnoreCase("pw")) {
                                    creditDocument.setStatus(CommonUtils.PAYMENT_STATUS_APPROVED);
                                ...
                                creditDocument.setAdjustedAmount(Double.valueOf(CommonUtils.INITIAL_PAYMENT_ADJUST));
                ...
                if (null != subscriber.getStatus() && subscriber.getStatus().equalsIgnoreCase(CommonConstants.ACTIVE_STATUS) && subscriber.getActivationByName() == null) {
                    StaffUser loggedStaffUser = staffUserService.get(getLoggedInUserId());
                    ...
                    if (subscriber.getStatus().equalsIgnoreCase("NewActivation")) {
                        ...
                        if (clientServiceSrv.getValueByName(ClientServiceConstant.WORKFLOW_AUTOMATIC_ASSIGN).equals("TRUE")) {
                            ...
                            if (map.containsKey("staffId") && map.containsKey("nextTatMappingId")) {
                                ...
                                StaffUser staffUser = staffUserService.get(staffId);
...
//                                               tatUtils.saveOrUpdateDataForTatMatrix( tatMapDetails, staffUser.get(),subscriber.getId(),null);
                ...
                if (null != subscriber.getPlanMappingList() && 0 < subscriber.getPlanMappingList().size()) {
                    ...
                    for (int i = 0; i < subscriber.getPlanMappingList().size(); i++) {
                        ...
                        PostpaidPlan postpaidPlanObj = planService.get(planMapppingPojo.getPlanId());
                        PostpaidPlanPojo plan = postpaidPlanMapper.domainToDTO(postpaidPlanObj, new CycleAvoidingMappingContext());
                        Optional<PlanService> services = planServiceRepository.findById(plan.getServiceId());
                        if (services.isPresent()) {
                            if (services.get().getExpiry() != null) {
...
//                                    planMapppingPojo.setStartDate(planMapppingPojo.getStartDate().toLocalDate().atTime(LocalTime.MIN).plusSeconds(1));
                        ...
                        if (null != plan && null != plan.getQuotatype()) {
                            if (plan.getQospolicyid() != null) {
                                qosPolicy = qosPolicyService.getEntityForUpdateAndDelete(plan.getQospolicyid());
                            }
                            if (!plan.getQuotatype().equalsIgnoreCase(CommonConstants.DID_QUOTA_TYPE) && !plan.getQuotatype().equalsIgnoreCase(CommonConstants.INTERCOM_QUOTA_TYPE) && !plan.getQuotatype().equalsIgnoreCase(CommonConstants.VOICE__BOTH_QUOTA_TYPE)) {
                                Double totalQuotaForSeconds = 0.0;
                                Double totalQuotaForKB = 0.0;
...
//                                mapping.setLeaseCircuitId(LCIds.get(i));
                                ...
                                if (subscriber.getStatus().equalsIgnoreCase("NewActivation")) {
                                    ...
                                    if (mapping != null && mapping.getNextStaff() == null) {
                                        ...
                                        if (clientServiceSrv.getValueByName(ClientServiceConstant.WORKFLOW_AUTOMATIC_ASSIGN).equals("TRUE")) {
                                            ...
                                            if (map.containsKey("staffId") && map.containsKey("nextTatMappingId")) {
                                                staffId = Integer.parseInt(map.get("staffId"));
                                                assignedStaff = staffUserService.get(staffId);
                                                ...
                                                if (mapping != null) {
                                                    ...
                                                    hierarchyService.sendWorkflowAssignActionMessage(assignedStaff.getCountryCode(), assignedStaff.getPhone(), assignedStaff.getEmail(), assignedStaff.getMvnoId(), assignedStaff.getFullName(), action);
                                ...
                                customersService.saveCustomerChargeHistory(postpaidPlanObj, subscriber, custPlanMappping, plangroupId, true, null);
...
//                                mapping = generateConnectionNumber(mapping);
                ...
                Customers customers = get(subscriber.getId());
                ...
                if (null != subscriber && null != subscriber.getPlanMappingList() && 0 < subscriber.getPlanMappingList().size()) {
                    for (int i = 0; i < subscriber.getPlanMappingList().size(); i++) {
                        ...
                        if (mapping.getId() != null) {
                            ...
                            mapping.setPlanValidityDays(customersService.calculatePlanValidityDays(subscriber, planService.get(entityById.getPlanId()), validity, LocalDateTime.now()));
            ...
            customersService.sendCustApprovalRegistrationMessage(localDateTime.toString(), planNamesBuilder.toString(), customersPojo.getUsername(), customersPojo.getPassword(), customersPojo.getCountryCode(), customersPojo.getMobile(), customersPojo.getEmail(), customersPojo.getMvnoId(), customersPojo.getStatus(), subscriber.getAcctno());
            //send message
...
//                this.messageSender.send(customerMessage, RabbitMqConstants.QUEUE_APIGW_SEND_CUSTOMER);
            ...
            if (subscriber.getOverChargeList() != null && !subscriber.getOverChargeList().isEmpty() && subscriber.getOverChargeList().get(0).getBillTo().equalsIgnoreCase(com.savbill.cpm.constants.Constants.ORGANIZATION) && subscriber.getOverChargeList().get(0).getIsInvoiceToOrg()) {
...
//                cust.setDedicatedStaffUserName(custpojo);
                    ...
                    if (custpojo.getParentCustomer().length() > 0) {
                        try {
                            Optional<Customers> customers = customersRepository.findByUsernameAndMvnoId(custpojo.getParentCustomer(), getMvnoIdFromCurrentStaff());
...
//                cust.setParentExperience(custpojo.getParentExperience());
                    try {
                        ...
                        Optional<Pincode> pincode = pincodeRepository.findByPincodeEqualsAndMvnoIdIn(custpojo.getWard(), mvnoIdList);
                        ...
                        if (custpojo.getAreaName() != null) {
                            try {
                                Optional<Area> area = areaRepository.findAllByNameEqualsIgnoreCaseAndIsDeletedIsFalseAndMvnoIdIn(custpojo.getAreaName(), mvnoIdList);
                    ...
                    if (custpojo.getBranchName().length() > 0) {
                        try {
                            ...
                            if (branch.isPresent()) {
                                ...
                                map.put(cust.getUsername(), "No Branch Data found");
                                continue;
                            }
                    ...
                    if (custpojo.getPartner().length() > 0) {
                        try {
                            Optional<Partner> partner = partnerRepository.findAllByNameEqualsAndMvnoIdIn(custpojo.getPartner(), mvnoIdList);
                        ...
                        cust.setPartner(partnerRepository.findById(1).get());
                    }
...
//                    customersMigrationPojo.setPlanName(dataFormatter.formatCellValue(row.getCell(51)));
                    ...
                    if (!pojo.getStatus().equalsIgnoreCase("NewActivation") && pojo.getPlanMappingList().size() > 0 && pojo.getPlanMappingList().get(0).getBillTo().equals(com.savbill.cpm.constants.Constants.ORGANIZATION) && pojo.getPlanMappingList().get(0).getIsInvoiceToOrg()) {
...
//            map.put(plan.getName(), e.getMessage());
    ...
    public void autoRenewOrAddonPlan(ChangePlanRequestDTOList requestDTOs) {
        try {
            logger.info("========Inside autoRenewOrAddonPlan ========= {}",requestDTOs.toString());
            if (requestDTOs != null) {
                ...
                for (ChangePlanRequestDTO requestDTO : requestDTOs.getChangePlanRequestDTOList()) {
                    if (null == requestDTO.getCustId()) {
                        logger.error(LogConstants.REQUEST_FROM + "Bss" + LogConstants.REQUEST_FOR + "fetch change plan for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + LogConstants.LOG_STATUS_CODE + 404);
                        return;
                    }
                ...
                try {
                    if (requestDTOs.getChangePlanRequestDTOList().get(0).getPurchaseType().equalsIgnoreCase("Addon")) {
                        if (customersforInvoice.size() > 1) {
                            ...
                            debitDocService.createInvoice(customersforInvoice, com.savbill.cpm.constants.Constants.ADD_ON, parentId, childIds, requestDTOs.getRecordPayment(), null, false, requestDTOs.getChangePlanRequestDTOList().get(0).getIsAutoPaymentRequired(), requestDTOs.getChangePlanRequestDTOList().get(0).getCreditDocumentPaymentPojoList(),null);
                        } else {
                            debitDocService.createInvoice(
                                    parentCustomers, com.savbill.cpm.constants.Constants.ADD_ON, "", requestDTOs.getRecordPayment(), null, null, false, requestDTOs.getChangePlanRequestDTOList().get(0).getIsAutoPaymentRequired(), requestDTOs.getChangePlanRequestDTOList().get(0).getCreditDocumentPaymentPojoList(),null);
                        }
                    } else {
                        logger.info("========Inside renew else condition ========= purchase type {}",requestDTOs.getChangePlanRequestDTOList().get(0).getPurchaseType());
                        if (customersforInvoice.size() > 1) {
                            Integer parentId = custIdOptional.get();
                            custIds.removeIf(i -> i.equals(parentId));
                            List<Integer> childIds = custIds;
                            debitDocService.createInvoice(customersforInvoice, com.savbill.cpm.constants.Constants.RENEW, parentId, childIds, requestDTOs.getRecordPayment(), null, false, requestDTOs.getChangePlanRequestDTOList().get(0).getIsAutoPaymentRequired(), requestDTOs.getChangePlanRequestDTOList().get(0).getCreditDocumentPaymentPojoList(),null);
                        } else {
                            debitDocService.createInvoice(parentCustomers, com.savbill.cpm.constants.Constants.RENEW, "", requestDTOs.getRecordPayment(), null, null, false, requestDTOs.getChangePlanRequestDTOList().get(0).getIsAutoPaymentRequired(), requestDTOs.getChangePlanRequestDTOList().get(0).getCreditDocumentPaymentPojoList(),null);
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    public void autoRenewOrAddonPlan(AutoRenewOrAddonPlanRequestDto requestDto) {
        logger.info("========Inside autoRenewOrAddonPlan ========= {}",requestDto.toString());
...
//                shorter.setPlanDueDate(((Timestamp) latestPlan[2]).toLocalDateTime());
    ...
    public List<Object[]> getActivePlanListByCustomerIdWithExpireDate(Integer custId) {
        List<Object[]> result = custPlanMapppingRepository.findActivePlanDetailsWithExpirePlan(custId,
                Arrays.asList(SubscriberConstants.PLAN_PURCHASE_RENEW,
                        SubscriberConstants.PLAN_PURCHASE_REGISTRATION_AND_RENEWWAL,
                        SubscriberConstants.PLAN_PURCHASE_NEW));
        return result;
    ...
    public List<Object[]> getLatestActivePlanListByCustomerId2(Integer custId) {
        List<Object[]> result = custPlanMapppingRepository.findActivePlanDetailsLatest2(custId,
                Arrays.asList(SubscriberConstants.PLAN_PURCHASE_RENEW,
                        SubscriberConstants.PLAN_PURCHASE_REGISTRATION_AND_RENEWWAL,
                        SubscriberConstants.PLAN_PURCHASE_NEW));
        return result;
    ...
    public List<Object[]> getLatestFuturePlanListByCustomerId2(Integer custId) {
        List<Object[]> result = custPlanMapppingRepository.findFuturePlanDetails2(custId,
                Arrays.asList(SubscriberConstants.PLAN_PURCHASE_RENEW,
                        SubscriberConstants.PLAN_PURCHASE_REGISTRATION_AND_RENEWWAL,
                        SubscriberConstants.PLAN_PURCHASE_NEW));
        return result;
    ...
    public List<Object[]> getLatestExpiredListByCustomerId2(Integer custId) {
        List<Object[]> result = custPlanMapppingRepository.findExpiredPlanDetails2(custId,
                Arrays.asList(SubscriberConstants.PLAN_PURCHASE_RENEW,
                        SubscriberConstants.PLAN_PURCHASE_REGISTRATION_AND_RENEWWAL,
                        SubscriberConstants.PLAN_PURCHASE_NEW));
        return result;
    ...
    public List<Object[]> getRenewPlanListByCustomerId(Integer custId) {
        return getRenewPlanListByCustomerId(custId, false);
    }

    public List<Object[]> getRenewPlanListByCustomerId(Integer custId, boolean checkForOtherCases) {
        ...
        if (checkForOtherCases) {
            return checkForOtherCustomers(custId);
        }
        else{
    ...
    private List<Object[]> checkForOtherCustomers(Integer custId) {
        List<Object[]> futurePlanList = getLatestFuturePlanListByCustomerId2(custId);
        if(!futurePlanList.isEmpty()){
            return futurePlanList;
        }
        List<Object[]> activePlanList = getLatestActivePlanListByCustomerId2(custId);
        if(!activePlanList.isEmpty()) {
            return activePlanList;
        }
        List<Object[]> expiredPlanList = getLatestExpiredListByCustomerId2(custId);
        if(!expiredPlanList.isEmpty()) {
            return expiredPlanList;
        }
        else{
    ...
    public CustomerPaymentDto generateAirtelRequestByAccountNumber(String accountNo, Double amount, String token) {
        ...
        try {
                List<Customers> customers = customersRepository.findByAcctnoAndMvnoId(accountNo, getMvnoIdFromCurrentStaff());
...
//            }
    ...
    @Transactional
    public GenericDataDTO voidPlan(Integer custpackageid,String planCancelRemarks, HttpServletRequest req) {
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        CustPlanMappping custPlanMappping = custPlanMappingRepo.findById(custpackageid);
        Integer RESP_CODE = APIConstants.FAIL;
        if (custPlanMappping == null) {
            RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            genericDataDTO.setResponseMessage("No plan found with ID: " + custpackageid);
            logger.info(LogConstants.REQUEST_FROM+ req.getHeader("requestFrom")+ LogConstants.REQUEST_FOR +"Void Plan" + LogConstants.REQUEST_BY + getLoggedInUser().getUsername()+ LogConstants.LOG_STATUS + LogConstants.LOG_FAILED +   LogConstants.LOG_INFO  + "No record found for custPlanMappingId: " + custpackageid + LogConstants.LOG_STATUS_CODE + RESP_CODE);
            return genericDataDTO;
        }

        if (custPlanMappping.getOfferPrice()!=0.0 || custPlanMappping.getDebitdocid() !=null) {
            RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
            genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
            genericDataDTO.setResponseMessage("Plan can not be void as some invoice is invoice is available for this plan.");
            logger.info(LogConstants.REQUEST_FROM+ req.getHeader("requestFrom")+ LogConstants.REQUEST_FOR +"Void Plan" + LogConstants.REQUEST_BY + getLoggedInUser().getUsername()+ LogConstants.LOG_STATUS + LogConstants.LOG_FAILED +   LogConstants.LOG_INFO  + "Plan can not be void as some invoice is invoice is available for this plan." + LogConstants.LOG_STATUS_CODE + RESP_CODE);
            return genericDataDTO;
        }

        CustPlanMappping lastPlanMapping = custPlanMappingRepo.lastActivePlan(custPlanMappping.getCustServiceMappingId());

        if(null != lastPlanMapping && lastPlanMapping.getId().equals(custPlanMappping.getId())){
        lastPlanMapping.setExpiryDate(LocalDateTime.now());
        lastPlanMapping.setEndDate(LocalDateTime.now());
        lastPlanMapping.setCustPlanStatus("STOP");
        lastPlanMapping.setRemarks(planCancelRemarks);
        genericDataDTO.setResponseCode(HttpStatus.OK.value());
        genericDataDTO.setResponseMessage("Invoice voided successfully.");
        RESP_CODE = APIConstants.SUCCESS;
        logger.info(LogConstants.REQUEST_FROM + req.getHeader("requestFrom")+ LogConstants.REQUEST_FOR +"Void Plan" + LogConstants.REQUEST_BY + getLoggedInUser().getUsername()+  LogConstants.LOG_STATUS  + LogConstants.LOG_SUCCESS + LogConstants.LOG_STATUS_CODE + RESP_CODE);

        } else {
        RESP_CODE = HttpStatus.EXPECTATION_FAILED.value();
        genericDataDTO.setResponseCode(HttpStatus.EXPECTATION_FAILED.value());
        genericDataDTO.setResponseMessage("Plan can not be void.");
        logger.info(LogConstants.REQUEST_FROM+ req.getHeader("requestFrom")+ LogConstants.REQUEST_FOR +"Void Plan"+ LogConstants.REQUEST_BY + getLoggedInUser().getUsername()+ LogConstants.LOG_STATUS + LogConstants.LOG_FAILED +   LogConstants.LOG_INFO  + "Plan can not be void as future plan available for this plan or plan is already void." + LogConstants.LOG_STATUS_CODE + RESP_CODE);
        return genericDataDTO;
         }

        if (genericDataDTO.getResponseCode() != HttpStatus.EXPECTATION_FAILED.value()) {
            Set<CustPlanMappping> custPlanMapppingList = new HashSet<>();
            custPlanMapppingList.add(lastPlanMapping);
            List<CustPlanMapppingDto> dtos = new ArrayList<>();
            for (CustPlanMappping planMap : custPlanMapppingList) {
                CustPlanMapppingDto custPlanMapppingDto = new CustPlanMapppingDto(planMap);
