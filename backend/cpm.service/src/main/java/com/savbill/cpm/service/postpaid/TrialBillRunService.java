...
import com.savbill.cpm.model.postpaid.TrialBillRun;
import com.savbill.cpm.pojo.SearchTrialBillRun;
import com.savbill.cpm.pojo.TriggerTrialBillRun;
import com.savbill.cpm.pojo.api.SearchTrialBillRunPojo;
import com.savbill.cpm.pojo.api.TrialBillRunPojo;
import com.savbill.cpm.repository.postpaid.TrialBillRunRepository;
import com.savbill.cpm.service.radius.AbstractService;
import com.savbill.cpm.spring.OptionalBooleanBuilder;
import com.savbill.cpm.utils.CommonConstants;
import com.savbill.cpm.utils.CommonUtils;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.itextpdf.text.Document;
import com.savbill.cpm.model.postpaid.QTrialBillRun;
...
// 	}
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.TrialBillRun', '1')")
...
//    		billRunURL="http://167.71.236.158:40080/billing-engine-1.0/billingprocess/generatebill/{billrundate}";
            ...
            billRunURL = CommonConstants.TRIAL_BILL_RUN_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
...
//        	billRunURL=billRunURL.replace("{billrundate}", billRunDate).replace("{custid}", "2");
    ...
    @PreAuthorize("hasPermission('com.savbill.cpm.model.postpaid.TrialBillRun', '2')")
    public boolean generateInvoice(String billRunId) throws Exception {
        String generatePDFURL = CommonConstants.TRIAL_GERERATE_INVOICE_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
                .replace("{port}", CommonUtils.getInvoiceServer().getWebport())
    ...
    public boolean revertbillrun(String billRunId) throws Exception {
        String revertbillrunrul = CommonConstants.TRIAL_REVERT_BILLRUN_URL.replace("{server}", CommonUtils.getInvoiceServer().getServerip())
                .replace("{port}", CommonUtils.getInvoiceServer().getWebport())
                .replace("{billrunid}", billRunId);
