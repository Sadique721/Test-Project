...
import com.savbill.cpm.modules.CustomerDBR.domain.*;
import com.savbill.cpm.modules.CustomerDBR.domain.*;
import com.savbill.cpm.modules.CustomerDBR.model.CustomDailyRevenue;
import com.savbill.cpm.modules.CustomerDBR.model.CustomMonthlyRevenue;
import com.savbill.cpm.modules.CustomerDBR.model.QCustomDailyRevenue;
import com.savbill.cpm.modules.CustomerDBR.repository.*;
import com.savbill.cpm.modules.CustomerDBR.repository.*;
...
import com.savbill.cpm.pojo.AggregateCount;
        ...
        if (customersPojo.getStatus().equalsIgnoreCase(CommonConstants.CUSTOMER_STATUS_ACTIVE)) {
            ...
            for (int i = 0; i < mapppingPojoList.size(); i++) {
                ...
                chargeHistoryList.stream().forEach(data -> {
                    ...
                    for (int k = 0; k < taxService.get(data.getTaxId()).getTieredList().size(); k++)
                        chargePriceIncludingTax = chargePriceIncludingTax + chargePriceIncludingTax * taxService.get(data.getTaxId()).getTieredList().get(k).getRate() / 100.0;
...
//                offerPrice.updateAndGet(v -> v - (v * mapppingPojoList.get(finalI).getDiscount() / 100.0));
                ...
                if (list.size() > 0) {
                    for (int k = 0; k < list.size(); k++) {
                        ...
                        directChargeOfferPrice = directChargeOfferPrice + directChargeOfferPrice * taxService.get(list.get(k).getTaxId()).getTieredList().get(0).getRate() / 100.0;
                        if (taxService.get(list.get(k).getTaxId()).getTieredList().size() > 1)
                            directChargeOfferPrice = directChargeOfferPrice + directChargeOfferPrice * taxService.get(list.get(k).getTaxId()).getTieredList().get(1).getRate() / 100.0;
...
//                offerPrice.updateAndGet(v -> v - (v * mapppingPojoList.get(finalI).getDiscount() / 100.0));
    ...
    private void addEntryIntoCustomerDbrForPostpaid(Integer customerId, double offerPrice, CustPlanMapppingPojo planMapppingPojo) {
        ...
        customerDBR.setPlanname(postpaidPlanService.get(planMapppingPojo.getPlanId()).getName());
        ...
        if (customers.getCusttype().equalsIgnoreCase(CommonConstants.CUST_TYPE_PREPAID)) {
            if (!custChargeDetails.getType().equalsIgnoreCase("One-time")) {
                ...
                for (int i = 0; i < daysDiff; i++) {
                    ...
                    customerDBR.setPlanname(postpaidPlanService.get(custChargeDetails.getPlanid()).getName());
