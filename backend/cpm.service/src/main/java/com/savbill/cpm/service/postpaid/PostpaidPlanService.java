...
import com.savbill.cpm.kafka.GsonConfig;
...
import com.savbill.cpm.modules.Branch.domain.QBranch;
...
import com.savbill.cpm.modules.Customers.LightPostpaidPlanDTO;
...
import com.savbill.cpm.modules.PlanQosMapping.*;
import com.savbill.cpm.modules.PlanQosMapping.*;
...
import com.savbill.cpm.modules.PriceGroup.domain.QPriceBookPlanDetail;
...
import com.savbill.cpm.modules.planUpdate.domain.QCustomerPackage;
import com.savbill.cpm.modules.planUpdate.repository.CustomerPackageRepository;
import com.savbill.cpm.modules.planUpdate.service.CustomerPackageService;
...
import com.savbill.cpm.pojo.api.*;
import com.savbill.cpm.pojo.customer.plans.GetPlansByFilter;
import com.savbill.cpm.rabbitMq.MessageSender;
import com.savbill.cpm.rabbitMq.message.PlanServiceAreaBindingCheckMessage;
import com.savbill.cpm.rabbitMq.message.PostpaidPlanMessage;
import com.savbill.cpm.rabbitMq.message.SavePlanAssignmentMessage;
import com.savbill.cpm.repository.DepartmentPlanMappingRepository;
import com.savbill.cpm.repository.LeadCustPlanMapppingRepository;
...
import static com.savbill.cpm.constants.Constants.*;
...
//                }
            ...
            if (partner != null && partner.getPriceBookId() != null && partner.getPriceBookId().getIsAllPlanSelected() != null && partner.getPriceBookId().getIsAllPlanSelected()) {
                List<PostpaidPlanPojo> plans = getPlanListByServiceArea(getLoggedInUser().getServiceAreaIdList(), type, Constants.PLAN_GROUP_ALL, null, null, null, null, null,0,null);
...
//        Iterable<PostpaidPlan> postpaidPlanList = entityRepository.findAll(booleanExpression);
        ...
        ;
         List<Object[]> result = serviceRepository.findServiceIdAndNameByIds(longList);

         Map<Long, String> serviceIdToName  = result.stream()
                 .collect(Collectors.toMap(
                         row -> (Long) row[0],
                         row -> (String) row[1]
                 ));

         postpaidPlanList.forEach(plan -> {
             if (plan.getServiceId() != null) {
                 String serviceName = serviceIdToName.get(Long.valueOf(plan.getServiceId()));
                 if (serviceName != null) {
                     plan.setServiceName("[" + serviceName + "]");
                 } else {
                     plan.setServiceName("[Unknown]");
                 }
             }
         });
...
//        return entityRepository.findAllByStatusAndIsDeleteIsFalseOrderByIdDesc(CommonConstants.ACTIVE_STATUS).stream().filter(plan -> plan.getMvnoId() == getMvnoIdFromCurrentStaff() || plan.getMvnoId() == null).collect(Collectors.toList());
    ...
    public List<postpaidPlanFetchPojo> getAllActiveEntities1(String type, String planGroup) {
        QPostpaidPlan qPostpaidPlan = QPostpaidPlan.postpaidPlan;
        List<Integer> planIds = new ArrayList<>();
        JPAQuery<?> query = new JPAQuery<>(entityManager);
        QPostPaidPlanServiceAreaMapping qPostPaidPlanServiceAreaMapping = QPostPaidPlanServiceAreaMapping.postPaidPlanServiceAreaMapping;
...
//                .collect(Collectors.toList());
        ...
        /*based on that serviceIds and fetch service area names */;
        //Shailendra commit and added new code to find service name agaist id insetad of all name issue
...
//        return entityRepository.findAllByStatusAndIsDeleteIsFalseOrderByIdDesc(CommonConstants.ACTIVE_STATUS).stream().filter(plan -> plan.getMvnoId() == getMvnoIdFromCurrentStaff() || plan.getMvnoId() == null).collect(Collectors.toList());
    ...
    public List<postpaidPlanFetchPojo> getAllActiveEntitiesOptimized(String type, String planGroup) {
        QPostpaidPlan qPostpaidPlan = QPostpaidPlan.postpaidPlan;
        List<Integer> planIds = new ArrayList<>();
        JPAQuery<?> query = new JPAQuery<>(entityManager);
        QPostPaidPlanServiceAreaMapping qPostPaidPlanServiceAreaMapping = QPostPaidPlanServiceAreaMapping.postPaidPlanServiceAreaMapping;
...
//                }
        ...
        List<postpaidPlanFetchPojo> postpaidPlanList = entityRepository.findAllPlanDetailsByIdsOptimized(finalPlanIds);
...
//        return entityRepository.findAllByStatusAndIsDeleteIsFalseOrderByIdDesc(CommonConstants.ACTIVE_STATUS).stream().filter(plan -> plan.getMvnoId() == getMvnoIdFromCurrentStaff() || plan.getMvnoId() == null).collect(Collectors.toList());
    ...
    public List<PostpaidPlan> getAllEntities() {
        return entityRepository.findAllByIsDeleteIsFalseOrderByIdDesc().stream().filter(plan -> plan.getMvnoId() == getMvnoIdFromCurrentStaff() || plan.getMvnoId() == null).collect(Collectors.toList());
    }
    ...
    public List<PostpaidPlan> getAllPlanList() {
        ...
        if (getMvnoIdFromCurrentStaff() == 1) return postpaidPlanList;
        else
            return postpaidPlanList.stream().filter(postpaidPlan -> (postpaidPlan.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || postpaidPlan.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()) && (postpaidPlan.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(postpaidPlan.getBuId()))).collect(Collectors.toList());
    }

    public List<PostpaidPlan> getAllPrepaidPlans() {
        return entityRepository.findByStatusAndPlantype(CommonConstants.YES_STATUS, CommonConstants.PLAN_TYPE_PREPAID).stream().filter(plan -> plan.getMvnoId() == getMvnoIdFromCurrentStaff() || plan.getMvnoId() == null).collect(Collectors.toList());
    }
    ...
    public List<PostpaidPlan> getAllRenewableDataPrepaidPlans() {
        ...
        if (planService != null && planGrp.trim().length() > 0) {
            return entityRepository.findAllByStatusAndPlantypeAndServiceIdAndPlanGroup(CommonConstants.YES_STATUS, CommonConstants.PLAN_TYPE_PREPAID, planService.getId(), planGrp).stream().filter(plan -> plan.getMvnoId() == getMvnoIdFromCurrentStaff() || plan.getMvnoId() == null).collect(Collectors.toList());
        }
        return null;
    }

    public List<PostpaidPlan> getAllPostpaidPlans() {
        return entityRepository.findByStatusAndPlantype(CommonConstants.YES_STATUS, CommonConstants.PLAN_TYPE_POSTPAID).stream().filter(plan -> plan.getMvnoId() == getMvnoIdFromCurrentStaff() || plan.getMvnoId() == null).collect(Collectors.toList());
    }
    ...
    public void deletePostpaidPlan(Integer id) throws Exception {
        Boolean flag = this.deleteVerification(id);
        if (flag == true) {
            PostpaidPlanPojo postpaidPlanPojo = postpaidPlanMapper.domainToDTO(get(id), new CycleAvoidingMappingContext());
    ...
    public PostpaidPlan getPostpaidPlanForEdit(Integer id) throws Exception {
        return get(id);
    }
    ...
    public PostpaidPlan savePostpaidPlan(PostpaidPlan postpaidPlan) throws Exception {
        ...
        try {
            ...
            if (getMvnoIdFromCurrentStaff() != null) {
                postpaidPlan.setMvnoId(getMvnoIdFromCurrentStaff());
            }
            ...
            if (pojo.getChargeList() != null) {
                ...
                for (PostpaidPlanChargePojo postpaidPlanChargePojo : chargePojos) {
                    if (postpaidPlanChargePojo.getCharge().getId() == null) {
                        ...
                        Charge charge = chargeService.get(postpaidPlanChargePojo.getCharge().getId());
...
//            messageSender.send(message, RabbitMqConstants.QUEUE_APIGW_POSTPAIDPLAN);
    ...
    @Transactional
    public void setChargeByPlanType(PostpaidPlanPojo entity) throws Exception {
        ...
        try {
            ...
            if (null != entity.getOfferprice() && null != entity.getId()) {
                ...
                PostpaidPlanPojo planPojo = postpaidPlanMapper.domainToDTO(get(entity.getId()), new CycleAvoidingMappingContext());
                        ...
                        for (PostpaidPlanChargePojo postpaidPlanChargePojo : entity.getChargeList()) {
                            ...
                            if (null != postpaidPlanChargePojo.getCharge()) {
                                ...
                                if (null == entity.getTaxId()) postpaidPlanChargePojo.getCharge().setTaxid(null);
                                    ...
                                    Tax tax = taxService.get(entity.getTaxId());
...
//            }*/
                ...
                pojo.setKraSynced(postpaidPlan.isKraSynced());
                pojo.setMvnoId(postpaidPlan.getMvnoId());
...
//        }
    ...
    public List<PostpaidPlanPojo> findPlanByService(Integer serviceId, String planType) {
        ...
        try {
            ...
            if (getMvnoIdFromCurrentStaff() == 1) return postPaidPlanList.stream().map(data -> {
                try {
            ...
            if (null != postPaidPlanList && 0 < postPaidPlanList.size()) {
                if (getBUIdsFromCurrentStaff().size() == 0) return postPaidPlanList.stream().map(data -> {
                    ...
                    return null;
                }).collect(Collectors.toList()).stream().filter(postpaidPlanPojo -> postpaidPlanPojo.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || postpaidPlanPojo.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()).collect(Collectors.toList());
                    ...
                    return null;
                }).collect(Collectors.toList()).stream().filter(postpaidPlanPojo -> (postpaidPlanPojo.getMvnoId() == 1 || getMvnoIdFromCurrentStaff() == 1 || postpaidPlanPojo.getMvnoId() == getMvnoIdFromCurrentStaff().intValue()) && (postpaidPlanPojo.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(postpaidPlanPojo.getBuId()))).collect(Collectors.toList());
            }
        } catch (Exception ex) {
            ApplicationLogger.logger.error(SUBMODULE + ex.getMessage(), ex);
        }
    ...
    public List<PostpaidPlanPojo> findPlanByTypeServiceModeStatusAndServiceArea(String type, Integer serviceId, List<Integer> serviceAreaId, String mode, String status, String planGroup, String planCategory, Integer validity, String unitsOfValidity, Integer custId) {
        String SUBMODULE = MODULE + " [findPlanByTypeServiceModeStatusAndServiceArea()] ";
        try {
            return getPlanListByServiceArea(serviceAreaId, mode, planGroup, planCategory, validity, unitsOfValidity, custId,null,0,null).stream().filter(postpaidPlanPojo -> postpaidPlanPojo.getPlantype().equalsIgnoreCase(type) && postpaidPlanPojo.getMode().equalsIgnoreCase(mode) && postpaidPlanPojo.getStatus().equalsIgnoreCase(status) && postpaidPlanPojo.getServiceId().equals(serviceId)).collect(Collectors.toList());
        } catch (Exception ex) {
            ApplicationLogger.logger.error(SUBMODULE + ex.getMessage(), ex);
        }
        return new ArrayList<>();
    ...
    @Override
    public Page<PostpaidPlan> search(List<GenericSearchModel> filterList, Integer page, Integer pageSize, String sortBy, Integer sortOrder) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            booleanExpression = booleanExpression.and(qPostpaidPlan.mvnoId.in(getMvnoIdFromCurrentStaff(), 1));
        }
        if (getBUIdsFromCurrentStaff().size() != 0) {
...
////
                            ...
                            booleanExpression = booleanExpression.and(qPostpaidPlan.endDate.loe(LocalDate.from((LocalDateTime.parse(searchModel.getFilterValue() + "T00:00:00")))).and(qPostpaidPlan.endDate.loe(LocalDate.from((LocalDateTime.parse(searchModel.getFilterValue() + "T23:59:59"))))));
                        ...
                        } else if (searchModel.getFilterColumn().trim().equalsIgnoreCase(SearchConstants.IS_KRA_SYNCED)) {
                                booleanExpression = booleanExpression.and(qPostpaidPlan.isKraSynced.eq(Boolean.parseBoolean(searchModel.getFilterValue())));
                        }
...
//       return postpaidPlanRepo.findAll(booleanExpression, pageRequest);
    ...
    public Page<PostpaidPlan> getPlanByNameOrSacCode(String s1, String s2, String dataType, PageRequest pageRequest) {
        ...
        if (getMvnoIdFromCurrentStaff() == 1) return getRepository().findAll(booleanExpression, pageRequest);
        else booleanExpression = booleanExpression.and((qPostpaidPlan.mvnoId.in(getMvnoIdFromCurrentStaff(), 1)));
        if (getBUIdsFromCurrentStaff().size() != 0)
            booleanExpression = booleanExpression.and(qPostpaidPlan.mvnoId.eq(1).or(qPostpaidPlan.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qPostpaidPlan.buId.in(getBUIdsFromCurrentStaff()))));
    ...
    public List<PostpaidPlanPojo> getPlanByPartnerId(PlanByPartnerReqDTO reqDTO) throws Exception {
        ...
        Partner partner = partnerService.get(reqDTO.getPartnerId());
        if (partner != null) {
            if (partner.getPriceBookId() != null) {
                PriceBookDTO priceBookDTO = priceBookService.getEntityById(partner.getPriceBookId().getId());
    ...
    public Page<PostpaidPlan> getList(Integer pageNumber, Integer customPageSize, String sortBy, Integer sortOrder, List<GenericSearchModel> filterList, String access) {
        ...
        if (null == filterList || 0 == filterList.size()) {
            ...
            if (getBUIdsFromCurrentStaff().size() != 0 && businessUnit.getPlanBindingType().equalsIgnoreCase(CommonConstants.PREDEFINED)) {
                if (getMvnoIdFromCurrentStaff() != 1) {
                    booleanExpression = booleanExpression.and(qPostpaidPlan.mvnoId.eq(1).or(qPostpaidPlan.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qPostpaidPlan.buId.in(getBUIdsFromCurrentStaff())).and(qPostpaidPlan.businessType.isNull().or(qPostpaidPlan.businessType.equalsIgnoreCase(CommonConstants.RETAIL)))));
                ...
                if (getMvnoIdFromCurrentStaff() != 1) {
                    booleanExpression = booleanExpression.and(qPostpaidPlan.mvnoId.eq(1).or(qPostpaidPlan.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qPostpaidPlan.buId.in(getBUIdsFromCurrentStaff())).and(qPostpaidPlan.businessType.equalsIgnoreCase(CommonConstants.ENTERPRISE))));
                } else if (getMvnoIdFromCurrentStaff() == 1) {
                ...
                if (getMvnoIdFromCurrentStaff() != 1) {
                    booleanExpression = booleanExpression.and(qPostpaidPlan.mvnoId.eq(1).or(qPostpaidPlan.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qPostpaidPlan.businessType.isNull().or(qPostpaidPlan.businessType.equalsIgnoreCase(CommonConstants.RETAIL)))));
                } else if (getMvnoIdFromCurrentStaff() == 1) {
    ...
    public List<PostpaidPlanPojo> getPlanListByServiceArea(List<Integer> serviceAreaId, String planmode, String planGroup, String planCategory, Integer validity, String unitsOfValidity, Integer custId, String plantype,Integer currPlanId,String rfHeader) {
...
//        List<Integer> planIds = list.stream().map(PostPaidPlanServiceAreaMapping::getPlanId).collect(Collectors.toList());
        ...
        if (plantype != null && !plantype.equalsIgnoreCase(ALL) && currPlanId != null && currPlanId != 0) {
            String currentQosSpeed = new JPAQuery<String>(entityManager)
                    .select(qPostpaidPlan.qospolicy.qosspeed)
                    .from(qPostpaidPlan)
                    .where(qPostpaidPlan.id.eq(currPlanId))
                    .fetchOne();

            if (currentQosSpeed != null && !currentQosSpeed.isEmpty()) {
                Integer currentSpeed = extractNumericQosSpeed(currentQosSpeed);
                NumberTemplate<Integer> qosspeedNumeric = Expressions.numberTemplate(
                        Integer.class,
                        "CONVERT(SUBSTRING_INDEX({0}, ' ', 1), UNSIGNED)",
                        qPostpaidPlan.qospolicy.qosspeed
                );
                BooleanExpression qosExpression = null;

                if (Constants.UPGRADE.equalsIgnoreCase(plantype)) {
                    qosExpression = qosspeedNumeric.isNotNull()
                            .and(qosspeedNumeric.goe(currentSpeed));
                } else if (Constants.DOWNGRADE.equalsIgnoreCase(plantype)) {
                    qosExpression = qosspeedNumeric.isNotNull()
                            .and(qosspeedNumeric.loe(currentSpeed));
                }

                if (qosExpression != null) {
                    booleanExpression = booleanExpression.and(qosExpression);
                }
            }
        }

        Integer departmentId = staffUserRepository.findByDepartment(getLoggedInStaffId());
        if(departmentId != null && departmentId != 0){
            List<Integer> list = departmentPlanMappingRepository.findDistinctPlanIdsByDepartmentId(departmentId);
            booleanExpression = booleanExpression.and(qPostpaidPlan.id.in(list));
        } else {
            booleanExpression = booleanExpression.and(qPostpaidPlan.id.in(planIds));
        }

        if (!planmode.equals(ALL)) booleanExpression = booleanExpression.and(qPostpaidPlan.mode.eq(planmode));
...
//            }
        ...
        BooleanBuilder accessibilityFilter = new BooleanBuilder();
        if (rfHeader != null && rfHeader.equalsIgnoreCase("cw")) {
            accessibilityFilter.or(qPostpaidPlan.Accessibility.eq("CustomerPortal"));
            accessibilityFilter.or(qPostpaidPlan.Accessibility.eq("Both"));
            accessibilityFilter.or(qPostpaidPlan.Accessibility.isNull());
        } else {
            accessibilityFilter.or(qPostpaidPlan.Accessibility.eq("MobileApp"));
            accessibilityFilter.or(qPostpaidPlan.Accessibility.eq("Both"));
            accessibilityFilter.or(qPostpaidPlan.Accessibility.isNull());
        }
        booleanExpression = booleanExpression.and(accessibilityFilter);
...
//        }).collect(Collectors.toList());
    ...
    public List<PostpaidPlanPojo> getPartnerPlanListByServiceArea(Integer partnerId, List<Integer> serviceAreaId, String planmode, String planGroup) {
    ...
    public List<PostpaidPlanPojo> getPlanListByServiceAreaAndCustId(Integer serviceAreaId, String planmode, String planGroup, Integer custId) {
        ...
        try {
            ...
            for (CustomerServiceMapping customerServiceMapping : customerServiceMappingList)
                servicesList.add(servicesService.getEntityById(customerServiceMapping.getServiceId()));
...
//            log.warn("::::::::: Total get postpaid data from cache time ::::::::: "+ (endtime - startTime));
            ...
            if (postpaidPlan != null) {
                if (postpaidPlan.getServiceAreaNameList() == null ||
                    postpaidPlan.getServiceAreaNameList().isEmpty()) {
                     log.info("Stale cache detected for planId: {} — Evicting", id);
                    cacheService.removeCacheAsync(cacheKey);
                     postpaidPlan = null;
                }else{
...
//            postpaidPlan = super.get(id);
            //Added changes for getting serviceAreaNameList along with postpaidPlan
            postpaidPlan=entityRepository.findPlanWithServiceAreas(id);

            if (postpaidPlan != null) {
                PostpaidPlan filteredPlan = filterPostpaidPlan(postpaidPlan);
    ...
    private PostpaidPlan filterPostpaidPlan(PostpaidPlan postpaidPlan) {
...
//        List<Long> buids = getBUIdsFromCurrentStaff();
        ...
        if (getMvnoIdFromCurrentStaff() == 1 ||
                ((postpaidPlan.getMvnoId() == getMvnoIdFromCurrentStaff().intValue() || postpaidPlan.getMvnoId() == 1) &&
    ...
    public PostpaidPlan getEntityForUpdateAndDelete(Integer id) {
        PostpaidPlan postpaidPlan = get(id);
        if (postpaidPlan == null || (!(getMvnoIdFromCurrentStaff() == 1 || getMvnoIdFromCurrentStaff().intValue() == postpaidPlan.getMvnoId().intValue()) && (postpaidPlan.getMvnoId() == 1 || getBUIdsFromCurrentStaff().size() == 0 || getBUIdsFromCurrentStaff().contains(postpaidPlan.getBuId()))))
            throw new CustomValidationException(APIConstants.FAIL, Constants.MVNO_DELETE_UPDATE_ERROR_MSG, null);
        return postpaidPlan;
    }

    public List<PostpaidPlan> getAllPlanForCustomerCreationOrChangePlan(String type) {
        QPostpaidPlan qPostpaidPlan = QPostpaidPlan.postpaidPlan;
        JPAQuery<?> query = new JPAQuery<>(entityManager);
        QPostPaidPlanServiceAreaMapping qPostPaidPlanServiceAreaMapping = QPostPaidPlanServiceAreaMapping.postPaidPlanServiceAreaMapping;
        BooleanExpression booleanExpression = qPostpaidPlan.isNotNull().and(qPostpaidPlan.isDelete.eq(false)).and(qPostpaidPlan.status.eq(CommonConstants.ACTIVE_STATUS));
    ...
    public List<PostpaidPlanPojo> getPlansForChildCustomers(Integer parentCustId) {
        ...
        if (getBUIdsFromCurrentStaff().size() != 0)
            boolExp = boolExp.and(qPostpaidPlan.mvnoId.eq(1).or(qPostpaidPlan.mvnoId.eq(getMvnoIdFromCurrentStaff()).and(qPostpaidPlan.buId.in(getBUIdsFromCurrentStaff()))));
    ...
    public GenericDataDTO updatePlanAssignment(CustomerCafAssignmentPojo pojo) throws Exception {
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        if (pojo.getPlanId() != null && pojo.getStaffId() != null) {
            PostpaidPlan plan = getEntityForUpdateAndDelete(pojo.getPlanId());
            ...
            if (savedPlan.getStatus().equalsIgnoreCase("Active")) {
                ...
                genericDataDTO.setData(postpaidPlanPojo);
    ...
    public String updatePlanStatus(Integer id, String status) {
    ...
    public GenericDataDTO getPlanApprovalsList(List<GenericSearchModel> filters, Integer page, Integer pageSize, String sortBy, Integer sortOrder) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1)
            booleanExpression = booleanExpression.and(qPostpaidPlan.mvnoId.in(1, getMvnoIdFromCurrentStaff()));
    ...
    private BooleanExpression getPlanByPartnerBundle(QPostpaidPlan qPostpaidPlan, BooleanExpression booleanExpression) {
        if (getLoggedInUserPartnerId() != 1) {
    ...
    public List<PostpaidPlan> getPlansByFilters(String requestFrom, GetPlansByFilter requestDto, Integer parentPlanId) {
...
//            ********************************************Common filters*************************************************
            ...
            Integer currPlanId=requestDto.getCurrPlanId();
            String changePlanType=requestDto.getChangePlanType();

            if (plantype != null && !plantype.equalsIgnoreCase(Constants.ALL) && currPlanId != null && currPlanId != 0 && changePlanType.equalsIgnoreCase(Constants.CHANGE_PLAN)) {
                String currentQosSpeed = new JPAQuery<String>(entityManager)
                        .select(qPostpaidPlan.qospolicy.qosspeed)
                        .from(qPostpaidPlan)
                        .where(qPostpaidPlan.id.eq(currPlanId))
            ...
            if("cw".equalsIgnoreCase(requestFrom)) {
                exp = exp.and((qPostpaidPlan.Accessibility.equalsIgnoreCase(CUSTOMER_PORTAL)).or(qPostpaidPlan.Accessibility.equalsIgnoreCase(BOTH)).or(qPostpaidPlan.Accessibility.isNull()));
...
//                    if (clientServiceSrv.getValueByName(ClientServiceConstant.CHECK_PENDING_REVENUE_CHANGEPLAN).equals(1))
                    ...
                    case Constants.RENEW:
                        ...
                        if (clientServiceSrv.getValueByName(ClientServiceConstant.RENEW_BY_BUSINESS_PROMOTION_ONLY).equalsIgnoreCase("1")) {
                            ...
                            BooleanExpression exp4 = qCustPlanMappping.isNotNull().and(qCustPlanMappping.custServiceMappingId.eq(requestDto.getCustomerServiceMappingID())).
                                    and(qCustPlanMappping.customer.id.eq(requestDto.getCustId())).and(qCustPlanMappping.billTo.equalsIgnoreCase(Constants.ORGANIZATION)).
                    ...
                    case Constants.ADD_ON:
                        ...
                        if (services.getIs_dtv()) {
                            ...
                            exp = exp.and((qPostpaidPlan.planGroup.equalsIgnoreCase(Constants.VOLUME_BOOSTER)).or(qPostpaidPlan.planGroup.equalsIgnoreCase(Constants.BANDWIDTH_BOOSTER)));
                ...
                exp = exp.and((qPostpaidPlan.Accessibility.equalsIgnoreCase(ADMIN_PORTAL)).or(qPostpaidPlan.Accessibility.equalsIgnoreCase(BOTH)).or(qPostpaidPlan.Accessibility.isNull()));
                switch (changePlanType) {
                    case Constants.CHANGE_PLAN:
                        exp = exp.and(qPostpaidPlan.serviceId.eq(serviceId.intValue()));
                        exp = exp.and(qPostpaidPlan.mode.notEqualsIgnoreCase(Constants.SPECIAL));
...
//          serviceId filter && ServiceArea filter
    ...
    public List<LightPostpaidPlanDTO> getPlanDetailsByServiceAreaSiteNameAndServiceId(List<Integer> serviceAreaId, List<Integer> serviceId, List<String> type) {
...
//        lightPostpaidPlanDTOS = lightPostpaidPlanDTOS.stream().peek(lightPostpaidPlanDTO -> lightPostpaidPlanDTO.setServiceAreaIds(serviceAreaIds.stream().map(Integer::longValue).collect(Collectors.toList()))).collect(Collectors.toList());
        ...
        return updatePostpaidData(lightPostpaidPlanDTOS);
    }

    public List<LightPostpaidPlanDTO> updatePostpaidData(List<LightPostpaidPlanDTO> lightPostpaidPlanDTOS) {

        lightPostpaidPlanDTOS = lightPostpaidPlanDTOS.stream()
                .peek(lightPostpaidPlanDTO -> {
                    Double price = getProratedPrice(lightPostpaidPlanDTO);
    ...
    private Double getProratedPrice(LightPostpaidPlanDTO lightPostpaidPlanDTO) {
        LocalDate nextBillDate = null;
        Double proratedPrice;
        LocalDate minNextBillDate = LocalDate.now().plusDays(400);
        try {
            PostpaidPlan plan = postpaidPlanService.get(lightPostpaidPlanDTO.getId());
...
//                kafkaMessageSender.send(new KafkaMessageData(message, message.getClass().getSimpleName()));
    ...
    checkCustomerPlanChargeChange(PostpaidPlan plan, PostpaidPlanPojo pojo) {
    ...
    private Long normalizeToMB(Long quota, String unit) {
        if (unit == null) return quota;
        if ("GB".equalsIgnoreCase(unit)) return quota * 1024;
        return quota;
    }
}
