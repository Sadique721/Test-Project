...
import com.savbill.cpm.model.common.*;
import com.savbill.cpm.modules.Template.domain.QTemplateNotification;
...
import com.savbill.cpm.pojo.UpdatePasswordResetDto;
...
import com.savbill.cpm.rabbitMq.message.CustomerOtpRegistrationMessage;
import com.savbill.cpm.rabbitMq.message.OtpMessage;
import com.savbill.cpm.repository.common.OTPRepository;
...
public class OTPService extends AbstractService {
    ...
    @Transactional
    public void generateOTP(GenerateOtpDto generateOtpDto) {
        try {
            if (ValidateCrudTransactionData.validateStringTypeFieldValue(generateOtpDto.getCountryCode()) && !generateOtpDto.getCountryCode().contains("+")) {
                ...
                OTPManagement management = Optional.ofNullable(otpManagementService.findByProfileName(generateOtpDto.getProfile(), false)).orElseThrow(() -> new RuntimeException(UrlConstants.EXPIRED_USER + " OTP profile not found"));
...
//            templateNotification.getTemplateName();
            if (templateNotification!=null) {
                if (templateNotification.isSmsEventConfigured() || templateNotification.isEmailEventConfigured()) {
                    ...
                    OtpMessage otpMessage = new OtpMessage(otp, OTP_GENERATED, RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, templateNotification, emailId,mvnoId,otp.getGeneratedTime().toString(), otp.getValidTillTime().toString(), buId);
        ...
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured()
                    ...
                    CustomerOtpRegistrationMessage customerOtpRegistrationMessage = new CustomerOtpRegistrationMessage(RabbitMqConstants.CUSTOMER_OTP_REGISTRATION_TEMPLATE_HEADER, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, customers);
