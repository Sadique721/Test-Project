package com.savbill.cpm.soap.Controller;
...
import com.savbill.cpm.modules.Customers.Services.CustIpMgmtService;
...
import com.savbill.cpm.repository.common.CustIpMappingRepo;
...
import com.savbill.cpm.schedulers.QuotaResetScheduler;
import com.savbill.cpm.service.common.CustomersService;
import com.savbill.cpm.service.postpaid.*;
import com.savbill.cpm.soap.Dto.*;
import com.savbill.cpm.service.postpaid.*;
import com.savbill.cpm.soap.Dto.*;
import com.savbill.cpm.soap.Services.ChangePlanService;
import com.savbill.cpm.soap.Services.IntegrationDataService;
import com.savbill.cpm.soap.SoapConstant.SoapConstant;
...
//            pojo.setEmail(customers.getEmail());
            ...
            Customers old1 = customersService.get(customers.getId());
            ...
            response.put("customer", pojo);
            RESP_CODE = APIConstants.SUCCESS;
            Customers newcust = customersService.get(customers.getId());
...
//                    custMacMapppingRepository.save(custmac);
            ...
            String updatedValues = CommonUtils.getUpdatedDiff(customers22, newcust);
...
//                Customers parentCustomers = customersService.get(custId);
                ...
                for (ChangePlanRequestDTO requestDTO : requestDTOs.getChangePlanRequestDTOList()) {
                    ...
                    if (null == requestDTO.getCustId()) {
                        ...
                        RESP_CODE = HttpStatus.NOT_ACCEPTABLE.value();
                        log.error(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch change plan for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                        return genericDataDTO;
                    }
                ...
                try {
                    if (requestDTOs.getChangePlanRequestDTOList().get(0).getPurchaseType().equalsIgnoreCase("Addon")) {
                        if (customersforInvoice.size() > 1) {
                            ...
                            debitDocService.createInvoice(customers, Constants.ADD_ON, "", requestDTOs.getRecordPayment(), null, null, false,false,null,null);
                        }
                    } else {
                        if (customersforInvoice.size() > 1) {
                            Integer parentId = custIdOptional.get();
                            ...
                            debitDocService.createInvoice(customers, Constants.RENEW, "", requestDTOs.getRecordPayment(), null, null, false,false,null,null);
                        }
                    }
//                            if (requestDTOs.getChangePlanRequestDTOList().get(0).getPurchaseType().equalsIgnoreCase("Addon")){
...
//                Customers parentCustomers = customersService.get(custId);
                ...
                for (ChangePlanRequestDTO requestDTO : requestDTOs.getChangePlanRequestDTOList()) {
                    if (null == requestDTO.getCustId()) {
                        genericDataDTO.setResponseMessage("Please provide customer id!");
                        RESP_CODE = HttpStatus.NOT_ACCEPTABLE.value();
                        log.debug(LogConstants.REQUEST_FROM + req.getHeader("requestFrom") + LogConstants.REQUEST_FOR + "fetch change plan for customer " + customersService.get(custId).getUsername() + LogConstants.REQUEST_BY + getLoggedInUser().getUsername() + LogConstants.LOG_STATUS + LogConstants.LOG_FAILED + LogConstants.LOG_ERROR + LogConstants.LOG_STATUS_CODE + RESP_CODE);
                        return genericDataDTO;
                    }
                    if (requestDTO.getPaymentOwner() == null) {
                        requestDTO.setPaymentOwner("");
...
//                messageSender.send(sendApproverForLeadMsg, RabbitMqConstants.QUEUE_SEND_APPROVER_DETAIL);
    ...
    public void sendCustStatusInActiveMessage(String username, String mobileNumber, String emailId, String status, Integer mvnoId) {
        try {
            ...
            if (optionalTemplate.isPresent()) {
                if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                    ...
                    CustomerStatusInActiveMessage customerStatusInActiveMessage = new CustomerStatusInActiveMessage(username, mobileNumber, emailId, status, mvnoId, RabbitMqConstants.CUSTOMER_STATUS_INACTIVATE_EVENT, optionalTemplate.get(), RabbitMqConstants.SOURCE_NAME_SAVBILL_BSS_GATEWAY, buId);
                    Gson gson = new Gson();
...
//                    messageSender.send(customerStatusInActiveMessage, RabbitMqConstants.QUEUE_CUSTOMER_STATUS_INACTIVATE_NOTIFICATION);
    ...
    private GenericDataDTO changeSubscriberAccountStatus(Integer id, String status, HttpServletRequest req, String newPassword, String locationeLock) throws Exception {
        ...
        String msg = "";
        GenericDataDTO genericDataDTO = new GenericDataDTO();
        Customers customers = customersService.getEntityForUpdateAndDelete(id);
