...
import com.savbill.cpm.rabbitMq.message.CustomerQuotaNotificationMessage;
import com.savbill.cpm.rabbitMq.message.SendQuotaMsg;
...
public class CustQuotaService extends AbstractService<CustQuotaDetails, CustQuotaDtlsPojo, Integer> {
    ...
    public CustQuotaDetails convertCustQuotaDtlsPojoToCustQuotaDtls(CustQuotaDtlsPojo custQuotaDtlsPojo) {
        ...
        if (custQuotaDtlsPojo != null) {
            ...
            custQuotaDetails.setPostpaidPlan(postpaidPlanService.get(custQuotaDtlsPojo.getPlanId()));
...
//            pojo.setCustomer(customerMapper.domainToDTO(customers, new CycleAvoidingMappingContext()));
    ...
    public List<CustQuotaDetails> convertQuotaPojoListToQuotaDomainList(List<CustQuotaDtlsPojo> custQuotaDetailsPojoList) {
        ...
        if (null != custQuotaDetailsPojoList && custQuotaDetailsPojoList.size() > 0) {
            for (CustQuotaDtlsPojo custQuotaDtlsPojo : custQuotaDetailsPojoList) {
                pojoList.add(convertCustQuotaDtlsPojoToCustQuotaDtls(custQuotaDtlsPojo));
            }
        }
        return pojoList;
    }
    ...
    public void sendNotificationOfQuota(Map<String , Object> quota){
        ...
        if(custPlanMappping.isPresent()) {
            if (custPlanMappping.get().getCustomer() != null) {
                ...
                sendNotificationToCustomer(customers.getUsername(), customers.getMobile(), customers.getEmail(), Double.parseDouble(quota.get("percentage").toString()), postpaidPlan.getName(), customers.getCountryCode(), customers.getMvnoId(), buId);
            }
        }

    }

    public void sendNotificationToCustomer(String name , String mobileno , String email , Double percentage , String planname,String countrycode,Integer mvnoId ,Long buId){
        try {
            ...
            if(percentage >= 100) {
                ...
                if (optionalTemplate.isPresent()) {
                    if (optionalTemplate.get().isSmsEventConfigured() || optionalTemplate.get().isEmailEventConfigured()) {
                        CustomerQuotaNotificationMessage customerQuotaNotificationMessage = new CustomerQuotaNotificationMessage(mobileno,email,optionalTemplate.get().getTemplateName(),optionalTemplate.get(),name,roundedUppercentage,planname,countrycode,mvnoId,buId);
                        Gson gson = new Gson();
