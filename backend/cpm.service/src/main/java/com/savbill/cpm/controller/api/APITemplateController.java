...
public class APITemplateController extends ApiBaseController {
    ...
    public ResponseEntity<?> getTemplateStaffById(@PathVariable Integer id, HttpServletRequest req) throws Exception {
        HashMap<String, Object> temp = new HashMap<>();
        Map<String, Object> responseMap = (Map<String, Object>) apiController.getStaffById(id, req).getBody();
    ...
    public ResponseEntity<?> getTemplatePlanServiceList(HttpServletRequest req) throws Exception {
        HashMap<String, Object> temp = new HashMap<>();
        Map<String, Object> responseMap = (Map<String, Object>) apiController.getPlanServiceList(req).getBody();
        temp.put(CommonConstants.DATA_LIST, responseMap.get("serviceList"));
        return apiResponse(HttpStatus.OK.value(), temp);
    }

    @GetMapping("/template/postpaidplan/all")
    public ResponseEntity<?> getTemplateAllPlanList(@RequestParam(defaultValue = "NORMAL") String type, @RequestParam(defaultValue = Constants.PLAN_GROUP_ALL, required = false) String planGroup, HttpServletRequest req) throws Exception {
        HashMap<String, Object> temp = new HashMap<>();
        Map<String, Object> responseMap = (Map<String, Object>) apiController.getAllPlanList(type, planGroup, req).getBody();
...
//    }
    ...
    public ResponseEntity<?> getTemplateChargeListByType(@PathVariable String chargeType, @RequestParam(name = "serviceId", required = false) Integer serviceId, HttpServletRequest req) {
        HashMap<String, Object> temp = new HashMap<>();
        Map<String, Object> responseMap = (Map<String, Object>) apiController.getChargeListByType(chargeType,serviceId, req).getBody();
        temp.put(CommonConstants.DATA_LIST, responseMap.get("chargelist"));
        return apiResponse(HttpStatus.OK.value(), temp);
    ...
    public ResponseEntity<?> findTemplateAllPlanGroups(@RequestParam String mode,
                                                       @RequestParam(name = "planCategory", required = false) String planCategory, @RequestParam(required = false) Integer custId, @RequestParam(required = false) String accessibility,HttpServletRequest req) {
        HashMap<String, Object> temp = new HashMap<>();
        Map<String, Object> responseMap = (Map<String, Object>) apiController.findAllPlanGroups(mode, planCategory, custId, accessibility,null,req).getBody();
