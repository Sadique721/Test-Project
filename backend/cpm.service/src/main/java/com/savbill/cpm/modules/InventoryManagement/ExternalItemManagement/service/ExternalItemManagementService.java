package com.savbill.cpm.modules.InventoryManagement.ExternalItemManagement.service;
...
import com.savbill.cpm.modules.InventoryManagement.ExternalItemManagement.mapper.ExternalItemManagementMapper;
...
import com.savbill.cpm.modules.InventoryManagement.RequestInventory.CommonResponceDto;
...
//    }
    ...
    public GenericDataDTO getInwardList(String externalItemGroupNumber, PageRequest pageRequest) {
        ...
        if (getMvnoIdFromCurrentStaff() != 1) {
            ...
            ServiceAreaService serviceAreaService = SpringContext.getBean(ServiceAreaService.class);
            List<Long> serviceAreaIds = serviceAreaService.getServiceAreaByStaffIdLong();
            List<ExternalItemManagement> inwardServiceAreaStaffList = externalItemManagementRepository.findAllByexternalItemGroupNumberContainingIgnoreCaseAndServiceAreaIdIdInAndIsDeletedIsFalseAndMvnoIdIn(externalItemGroupNumber,serviceAreaIds,Arrays.asList(getMvnoIdFromCurrentStaff(), 1));
            if (inwardServiceAreaStaffList != null) {
                if (inwardServiceAreaStaffList.size() > 0) {
                    for (int s = 0; s < inwardServiceAreaStaffList.size(); s++) {
                        resultPaginationList.add(inwardServiceAreaStaffList.get(s).getId());
