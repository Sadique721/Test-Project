...
public class RadiusPacket {
	...
	private static final Logger log = LoggerFactory.getLogger(RadiusPacket.class);
	...
	public void addAttribute(String typeName, String value) {
		...
		//	throw new IllegalArgumentException("value is empty");
			log.error("Value is null for type: "+typeName+" value: "+value);
			return;
		}
	...
	public void encodeRequestPacketProxy(OutputStream out, String sharedSecret) 
			throws IOException {
		encodePacketProxy(out, sharedSecret, null);
	}
	/**
	...
	protected void encodePacketProxy(OutputStream out, String sharedSecret, RadiusPacket request) 
		throws IOException 
	{
			// check shared secret
			if (sharedSecret == null || sharedSecret.length() == 0)
			...
			byte[] attributes = getAttributeBytesProxy();
			int packetLength = RADIUS_HEADER_LENGTH + attributes.length;
			if (packetLength > MAX_PACKET_LENGTH)
				throw new RuntimeException("packet too long");
	...
	throws IOException, RadiusException {
		...
		//		System.out.println("Radius Packet Proxy Response:"+rp);
		// request packet?
		if (request == null) {
			// decode attributes
	...
	throws RadiusException {
		//		System.out.println("Key:"+sharedSecret+":Lengh:"+packetLength+":Signature Auth:"+requestAuthenticator);
		...
		//		System.out.println("Current Authenticator:"+authenticator+":Received Packet Authenticator:"+receivedAuth);
		for (int i = 0; i < 16; i++)
			if (authenticator[i] != receivedAuth[i]) 
			{
				////		System.out.println("Autheticator not match moving ahead. still");
	...
	protected byte[] getAttributeBytesProxy() 
			throws IOException {
		ByteArrayOutputStream bos = new ByteArrayOutputStream(MAX_PACKET_LENGTH);
		for (Iterator i = attributes.iterator(); i.hasNext();) {
			RadiusAttribute a = (RadiusAttribute)i.next();
			a.setTag(tag);
			bos.write(a.writeAttributeProxy());
		}
		bos.flush();
		return bos.toByteArray();
	}

	public RadiusPacket copyPacket() {
		RadiusPacket copyPacket = createRadiusPacket(packetType);
		copyPacket.setPacketIdentifier(packetIdentifier);
		copyPacket.setLength(length);
		copyPacket.setAuthenticator(this.authenticator);

		List<RadiusAttribute> attributes = getAttributes();
		List<RadiusAttribute> copiedList = attributes.stream().map(attribute -> new RadiusAttribute(attribute.getAttributeType(), attribute.getAttributeData())).collect(Collectors.toList());
		copyPacket.setAttributes(copiedList);
