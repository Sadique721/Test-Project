...
public class VendorSpecificAttribute extends RadiusAttribute {
	...
	public void readAttribute(byte[] data, int offset, int length)
		...
		int initialLength = ((int) data[offset + 1] & 0x000000ff);
		int vsaLen = initialLength - 6;
		...
		boolean bMultiByteSubType=false;
		if(vendorId==637) {
			bMultiByteSubType=true;
		}
		...
		while (pos < vsaLen) {
			...
			if(bMultiByteSubType) {
				vsaSubLen = data[(offset + 6) + pos + 2] & 0x0ff;
			}else {
		...
		while (pos < vsaLen) {
			...
			int sublength =-1;
			if(bMultiByteSubType) {
				int val1=data[(offset + 6) + pos] & 0x0ff;
				int val2=data[(offset + 6) + pos+1] & 0x0ff;  
				subtype = (16*16)*val1+val2;
				sublength = data[(offset + 6) + pos + 2] & 0x0ff;
			}else {
			...
			if(bMultiByteSubType) {
				a.readAttribute(true,data, (offset + 6) + pos, sublength);
			}else {
		...
		byte[] attrData = new byte[initialLength - 2];
		System.arraycopy(data, offset + 2, attrData, 0, initialLength -2);
