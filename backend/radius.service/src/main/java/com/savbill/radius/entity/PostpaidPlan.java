...
*/
    ...
    @Column(name = "addon_to_base", length = 3)
    ...
    public PostpaidPlan(CustomMessage customMessage) throws JsonProcessingException {
        ...
        if (message.get("usageQuotaType") != null)
            this.usageQuotaType = message.get("usageQuotaType").toString();
        else
            this.usageQuotaType = "TOTAL";
...
//            this.serviceAreaNameList = Double.parseDouble(message.get("serviceAreaNameList").toString());
        ...
        if (message.get("chunk") != null)
            this.chunk = Double.valueOf(message.get("chunk").toString());
        if (message.get("useQuota") != null)
            this.useQuota = Boolean.parseBoolean(message.get("useQuota").toString());
        if (message.get("addonToBase") != null)
            this.addonToBase = Boolean.parseBoolean(message.get("addonToBase").toString());
    }
}
