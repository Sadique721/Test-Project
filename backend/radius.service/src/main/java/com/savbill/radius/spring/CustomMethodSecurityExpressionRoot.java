...
//                            logger.info("Authorized against role:" + role + ",Required:" + operation + ",Granted: " + operation);
//                            isPrivileged = true;
//                            break;
//                        }
//                    }
...
//    }
    ...
    public boolean validatePermission(String operation, String... operationAll) {
        ...
        HashMap<Long,List<String>> data= (HashMap<Long, List<String>>) cacheRetrival.getPermissionServerData();
        //Cache permissionsCache = cacheManager.getCache("permissionCache");
        Object principal = authentication.getPrincipal();
        if (principal.toString().equalsIgnoreCase(CommonConstants.ANONYMOUS_USER)) {
            isPrivileged = true;
        }
            ...
            for (Long roleId: roleIds) {
                    List<String> operationList = data.get(roleId);
                    if (operationList.contains(operation) || Arrays.stream(operationAll).anyMatch(op -> operationList.contains(op))) {
                      //  logger.info("Authorized against role:" + roleId + ",Required:" + operation + ",Granted: " + operation);
                        isPrivileged = true;
                        break;
                    }
            }
