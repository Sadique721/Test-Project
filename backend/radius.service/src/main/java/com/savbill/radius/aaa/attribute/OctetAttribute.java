...
public class OctetAttribute extends RadiusAttribute {
	 ...
	 */
	public OctetAttribute(int type, String value) {
...
//				int codePoint = (int) c;
//				octalString.append("\\").append(Integer.toOctalString(codePoint));
//			}
		byte[] byteArray = new byte[value.length()/2]; // Create a byte array with size 8
		for (int i = 0; i < value.length()/2; i++) {
			String byteString = value.substring(i*2, (i * 2) + 2);
			byteArray[i] = (byte) Integer.parseInt(byteString, 16);  // Parse hex string to integer, then cast to byte (caution with overflow)
			System.out.println(byteString);
		}
		setAttributeData(byteArray);
