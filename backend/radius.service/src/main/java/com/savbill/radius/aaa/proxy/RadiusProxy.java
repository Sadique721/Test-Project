 ...
 */
package com.savbill.radius.aaa.proxy;
...
extends RadiusServer {
	 ...
	 */
	public void start(boolean listenAuth, boolean listenAcct,boolean listendyna,boolean listenProxy) {
		super.start(listenAuth, listenAcct,listendyna);
		if (listenProxy) {
			new Thread() {
				public void run() {
     ...
     */
    protected void proxyPacket(RadiusPacket packet, RadiusProxyConnection proxyConnection,RadiusPacket accessResponse)
    throws IOException, RadiusException {    	
		...
		proxySocket.setSoTimeout(Integer.parseInt("3000"));
		proxySocket.send(datagram);
	    ...
	    accessResponse=RadiusPacket.decodeRequestPacket(in, "secret");
	    //		System.out.println("Response is "+accessResponse);
	    proxySocket.close();
    }
