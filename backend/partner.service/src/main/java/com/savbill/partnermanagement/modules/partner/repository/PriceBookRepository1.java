...
@Repository
public interface PriceBookRepository1 extends JpaRepository<PriceBook1, Long>, QuerydslPredicateExecutor<PriceBook1> {


    @Query("select t from PriceBook t where t.status='Active' AND t.isDeleted = false")
    List<PriceBook1> getAllByStatus();

    @Query("SELECT t from PriceBook t where t.isDeleted = false ")
    Page<PriceBook1> findAll(Pageable pageable);

    @Query("SELECT t from PriceBook t where t.isDeleted = false and (MVNOID = 1 or (MVNOID = :mvnoId and BUID in :buIds))")
    Page<PriceBook1> findAll(Pageable pageable, @Param("mvnoId") Integer mvnoId, @Param("buIds") List buIds);


    @Query("SELECT t from PriceBook t where t.isDeleted = false and MVNOID in :mvnoIds")
    Page<PriceBook1> findAll(Pageable pageable, @Param("mvnoIds") List mvnoIds);

    Page<PriceBook1> findAllByBooknameContainingIgnoreCaseAndIsDeletedIsFalse(String name, Pageable pageable);

    Page<PriceBook1> findAllByBooknameContainingIgnoreCaseAndIsDeletedIsFalseAndMvnoIdIn(@Param("name") String name, Pageable pageable, @Param("mvnoIds") List mvnoIds);
    ...
    Page<PriceBook1> findAllByNameAndIsDeleteIsFalse(@Param("s1") String s1, Pageable pageable, @Param("mvnoId") Integer mvnoId, @Param("buIds") List buIds);
