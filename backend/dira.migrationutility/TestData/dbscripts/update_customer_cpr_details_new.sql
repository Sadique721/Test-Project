...
BEGIN
	...
	DECLARE varUserName varchar(256);
	...
	DECLARE cursor_i CURSOR FOR SELECT t.username,t.custid,t1.registered,t1.renewed,t1.expires,t1.AccountNo,t1.Updates FROM tblcustomers t join tempcustdetailsheet t1 on t1.username=t.username;
	...
	OPEN cursor_i;
	read_loop: LOOP
		FETCH cursor_i INTO varUserName,varCustId,varStartDate,varRenewedDate,varEndDate,varAccountNo,varUpdates;
		IF done THEN
			LEAVE read_loop;
		END IF;
		...
		SELECT custpackageid INTO cprid FROM Savbillcpm.tblcustpackagerel WHERE custid IN (varCustId);
		...
		SELECT concat('username=', varUserName,',','cprid=', cprid,',','varCustId=', varCustId,',','startdate=', varStartDate,',','enddate=', varEndDate,',','varAccountNo=', varAccountNo,',','varUpdates=', varUpdates,',','varCreatedBy=', varCreatedBy,',','varCreatedByName=', varCreatedByName)  into msg;
