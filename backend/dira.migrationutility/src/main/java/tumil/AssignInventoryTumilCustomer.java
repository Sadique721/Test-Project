...
public class AssignInventoryTumilCustomer extends RestExecution {

	
			private static String logFileName = "AssignInventoryTumil.log";
			...
			//	System.out.println("api"+ queryParam);
			...
			// here there problem we get replace get to post -->
		...
		//		String queryParam = "?productId=" + productId + "&ownerId=" + ownerId + "&ownerType=Staff";
			...
			//	String apiBody=getJsonGetItemPagination(1,5000,productId,ownerId,"Staff");
				String apiBody=getJsonGetItemPaginationSearch(1,50,productId,ownerId,"Staff",assignedSerialNumber);
 //System.out.println(apiBody);
				JSONObject JSONResponseBody = httpPost(apiURL,apiBody);  // here get to post
				int status = JSONResponseBody.getInt("responseCode");
			...
			// new development tumil 
			// new developemnt
			public String getJsonGetItemPagination(int page, int pageSize, int productId, String ownerId, String ownerType) {
				try {
			        // Create the outer JSONObject for the main fields
			        JSONObject result = new JSONObject();
			        
			        // Add productId, ownerId, and ownerType to the outer JSONObject
			        result.put("productId", productId);
			        result.put("ownerId", ownerId);
			        result.put("ownerType", ownerType.toLowerCase());  // Ensure the ownerType is in lowercase

			        // Create a nested JSONObject for the paginationRequestDTO
			        JSONObject paginationRequestDTO = new JSONObject();
			        paginationRequestDTO.put("page", page);
			        paginationRequestDTO.put("pageSize", pageSize);

			        // Add the nested paginationRequestDTO to the outer JSONObject
			        result.put("paginationRequestDTO", paginationRequestDTO);
		...
		// paginationnserch by post	
			public String getJsonGetItemPaginationSearch(int page, int pageSize, int productId, String ownerId, String ownerType, String assignedSerialNumber) {
			    try {
			        // Create the outer JSONObject for the main fields
			        JSONObject result = new JSONObject();
			        ...
			        result.put("productId", productId);
			        result.put("ownerId", ownerId);
			        result.put("ownerType", ownerType);  // Keep original case
			        ...
			        // Create one filter object as an example
			        JSONObject filter = new JSONObject();
			        filter.put("filterColumn", "serialNumber");
			        filter.put("filterValue",assignedSerialNumber );
			        filter.put("filterCondition", "AND");

			        // Add the filter object to the filters array
			        filters.put(filter);

			        // Add the filters array to the paginationRequestDTO
			        paginationRequestDTO.put("filters", filters);

			        // Add paginationRequestDTO to the outer result
			        result.put("paginationRequestDTO", paginationRequestDTO);

			        // Return the final JSON as a string
			        return result.toString(2); // Indented for readability
			    } catch (Exception e) {
			        // Log the error or handle it as needed
			        System.err.println("Error creating pagination JSON: " + e.getMessage());
			        return "{}"; // Return empty JSON if something goes wrong
