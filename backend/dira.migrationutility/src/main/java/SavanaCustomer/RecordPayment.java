...
public class RecordPayment extends RestExecution {

    private static String logFileName = "Savannacustomer.log";
...
//		}
    ...
    public void recordCustomerPaymentDetails(List<Map<String, String>> customerMapList) {
        ...
        // Thread pool — tune thread count based on your server CPU
        ...
        for (Map<String, String> map : customerMapList) {
            ...
            executor.submit(() -> {
                try {
                    ...
                    if (commonGetAPI.checkcustomerUsernameIsAlreadyExists(userName)) {
                        Utility.printLog(logFileName, logModuleName, "Sheet Raw Data", map.toString());
                        recordCustomerPaymentDetails(map); // your existing logic
                    } else {
                        System.out.println("Customer UserName is not Exists! - " + userName);
                    }
    ...
    public List<Map<String, String>> readUniqueCustomerPaymentDetailsList() {
        ...
        for (int i = 0; i < sheetMap.size(); i++) {
            ...
            if ((!"".equals(userName)) && (!"Success".equalsIgnoreCase(mStatus))) {
                ...
                valuemap.put("ChequeDate", safeTrim(cellValue.get("ChequeTransactionDate")));
...
//                        JSONObject docJson = getDocNumberByCustId(customerId);
...
//    private JSONObject getDocNumberByCustId(
//            int customerId
//    ) {
//
//        // --- Define endpoint (no query params — POST) ---
//        String apiURL = "Revenue/trial/invoice/search?billrunid=&docnumber=&customerid=" + customerId + "&billfromdate=&billtodate=&custmobile=&isInvoiceVoid=true";
//        apiURL = getAPIURL(apiURL);
//
//        // --- Build payload ---
//        JSONObject payload = new JSONObject();
//        {
//            payload.put("billrunid", "");
//            payload.put("docnumber", "");
//            payload.put("customerid", customerId);
//            payload.put("billfromdate", "");
//            payload.put("billtodate", "");
//            payload.put("custmobile", "");
//            payload.put("isInvoiceVoid", true);
//        }
//
...
//
//        int status = JSONResponseBody.optInt("status", 0);
//        JSONObject itemJson = null;
//
//        if (status == 200) {
//            JSONArray jsonArray = JSONResponseBody.optJSONArray("invoicesearchlist");
//
//            if (jsonArray != null && jsonArray.length() > 0) {
//                for (int i = 0; i < jsonArray.length(); i++) {
//                    JSONObject item = jsonArray.getJSONObject(i);
//
//                    String docNumber = item.optString("docnumber", "");
//
