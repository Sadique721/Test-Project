 ...
 */
public class CAF_PastPayment extends RestExecution {

    private static final Logger log = LoggerFactory.getLogger(CAF_PastPayment.class);
     ...
     */
    private ArrayList<Object> createCAFPaymentWithRetry(Map<String, String> paymentDetails) {
            ...
            if (apiBody != null && !apiBody.isEmpty()) {
                ...
                while (attempts < retryLimit && !success) {
                    try {
                        ...
                        JSONResponseBody = httpPostFormData4(apiURL, apiBody, fileName);
                        success = true;
    ...
    private void handleAPIResponse(JSONObject response, String rowIndex, long elapsedTime, Map<String, String> paymentDetails) {
        try {
            ...
            if (response.has("customerid")) {
                customerId = response.optString("customerid", null);
            }
            if (customerId != null) {
...
//            }
            ...
            if (customerId != null) {
                columnValue = "customerId:" + customerId + "#Status:Success";
            } else {
     ...
     */
    public void createCAFPaymentRecord(List<Map<String, String>> paymentList) {
        ...
        for (Map<String, String> paymentDetails : paymentList) {
            futures.add(executorService.submit(() -> {
                ...
                try {
                    ...
                    result = createCAFPaymentWithRetry(paymentDetails);
...
//                Thread.sleep(2000);
    ...
    public List<Map<String, String>> readCAFPaymentList() {
    ...
    private String getPaymentJson(Map<String, String> customerDetails) {
        ...
        try {
            ...
            if (userName != null && !"".equals(userName)) {
                int customerId = commonGetAPI.getCAFCustomerId(userName);
...
//            paymentJson.put("reciptNo", customerDetails.get("ReceiptNumber"));
            paymentJson.put("reciptNo", JSONObject.NULL);
