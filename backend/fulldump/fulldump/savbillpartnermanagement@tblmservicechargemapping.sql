...
--
-- Host: localhost    Database: savbillpartnermanagement    Table: tblmservicechargemapping
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tblmservicechargemapping` (
  ...
  CONSTRAINT `tblmservicechargemapping_fk2` FOREIGN KEY (`chargeid`) REFERENCES `tblmcharges` (`chargeid`)
