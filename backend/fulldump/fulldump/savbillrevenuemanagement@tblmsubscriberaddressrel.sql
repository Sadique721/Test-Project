...
--
-- Host: localhost    Database: savbillrevenuemanagement    Table: tblmsubscriberaddressrel
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tblmsubscriberaddressrel` (
  ...
  KEY `TBLMSUBSCRIBERADDRESSREL_fk_chid` (`CITYID`),
  KEY `TBLMSUBSCRIBERADDRESSREL_fk_stid` (`STATEID`),
  KEY `TBLMSUBSCRIBERADDRESSREL_fk_ctryid` (`COUNTRYID`),
  KEY `TBLMSUBSCRIBERADDRESSREL_fk_pinid` (`PINCODEID`),
  KEY `TBLMSUBSCRIBERADDRESSREL_address_custid_version` (`ADDRESSTYPE`,`SUBSCRIBERID`,`version`),
  KEY `idx_area_city` (`AREAID`,`CITYID`),
  KEY `idx_subscriberid` (`SUBSCRIBERID`),
  CONSTRAINT `TBLMSUBSCRIBERADDRESSREL_fk_areaid` FOREIGN KEY (`AREAID`) REFERENCES `tblmarea` (`areaid`),
  CONSTRAINT `TBLMSUBSCRIBERADDRESSREL_fk_chid` FOREIGN KEY (`CITYID`) REFERENCES `tblmcity` (`cityid`),
  CONSTRAINT `TBLMSUBSCRIBERADDRESSREL_fk_ctryid` FOREIGN KEY (`COUNTRYID`) REFERENCES `tblmcountry` (`countryid`),
  CONSTRAINT `TBLMSUBSCRIBERADDRESSREL_fk_pinid` FOREIGN KEY (`PINCODEID`) REFERENCES `tblmpincode` (`pincodeid`),
  CONSTRAINT `TBLMSUBSCRIBERADDRESSREL_fk_stid` FOREIGN KEY (`STATEID`) REFERENCES `tblmstate` (`stateid`)
