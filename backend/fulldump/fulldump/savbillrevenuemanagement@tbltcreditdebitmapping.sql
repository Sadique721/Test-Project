...
--
-- Host: localhost    Database: savbillrevenuemanagement    Table: tbltcreditdebitmapping
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tbltcreditdebitmapping` (
  ...
  `trialdebitdocumentid` bigint DEFAULT NULL,
  PRIMARY KEY (`creddebtmappingid`),
  UNIQUE KEY `tblmcreddebtdocmapping_creddebtmappingid_unq` (`creddebtmappingid`),
  KEY `idx_creditdebit_debitid` (`debitdocumentid`),
  KEY `idx_next_approve_staff_id` (`CREDITDOCID`),
  KEY `idx_creditdocid` (`CREDITDOCID`)
