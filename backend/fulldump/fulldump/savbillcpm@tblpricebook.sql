...
--
-- Host: localhost    Database: savbillcpm    Table: tblpricebook
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tblpricebook` (
  ...
  `plan_group` varchar(250) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  PRIMARY KEY (`bookid`),
  UNIQUE KEY `pricebook_bookid_unq` (`bookid`),
  KEY `tblpricebook_ibfk_1` (`MVNOID`),
  KEY `businessunit_pricebook_fk` (`BUID`),
  CONSTRAINT `businessunit_pricebook_fk` FOREIGN KEY (`BUID`) REFERENCES `tblmbusinessunit` (`businessunitid`),
  CONSTRAINT `tblpricebook_ibfk_1` FOREIGN KEY (`MVNOID`) REFERENCES `tblmmvno` (`MVNOID`)
