...
--
-- Host: localhost    Database: savbillcpm    Table: tblmhierarchy
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tblmhierarchy` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `hierarchy_id_unq` (`id`),
  KEY `idx_hierarchy_event_mvno_bu` (`event_name`,`mvno_id`,`BUID`,`is_deleted`)
