...
--
-- Host: localhost    Database: commongateway    Table: tbltbranchservicearearel
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tbltbranchservicearearel` (
  ...
  CONSTRAINT `tblmbranchservicearearel_change` FOREIGN KEY (`servicearea_id`) REFERENCES `tblmservicearea` (`service_area_id`),
  CONSTRAINT `tblmbranchservicearearel_unique` FOREIGN KEY (`branchid`) REFERENCES `tblmbranch` (`branchid`)
) ENGINE=InnoDB AUTO_INCREMENT=820701 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
