...
--
-- Host: localhost    Database: savbillinventorymanagement    Table: tblmplangroup
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tblmplangroup` (
  ...
  KEY `plangroup_mvno_id_fk` (`MVNOID`),
  CONSTRAINT `plangroup_mvno_id_fk` FOREIGN KEY (`MVNOID`) REFERENCES `tblmmvno` (`MVNOID`)
