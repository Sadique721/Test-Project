...
--
-- Host: localhost    Database: savbillinventorymanagement    Table: tblmbusinessunit
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tblmbusinessunit` (
  ...
  KEY `businessunit_mvno_id_fk` (`MVNOID`),
  CONSTRAINT `businessunit_mvno_id_fk` FOREIGN KEY (`MVNOID`) REFERENCES `tblmmvno` (`MVNOID`)
