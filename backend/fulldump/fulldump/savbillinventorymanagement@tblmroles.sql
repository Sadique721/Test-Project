...
--
-- Host: localhost    Database: savbillinventorymanagement    Table: tblmroles
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tblmroles` (
  ...
  KEY `roles_mvno_id_fk` (`MVNOID`),
  CONSTRAINT `roles_mvno_id_fk` FOREIGN KEY (`MVNOID`) REFERENCES `tblmmvno` (`MVNOID`)
