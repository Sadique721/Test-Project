...
--
-- Host: localhost    Database: savbillrevenuemanagement    Table: tblparentchildmappingrel
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tblparentchildmappingrel` (
  ...
  KEY `fk_create_by_staffids` (`create_by_staff_id`),
  KEY `fk_parent_customer` (`parent_cust_id`),
  CONSTRAINT `fk_create_by_staffids` FOREIGN KEY (`create_by_staff_id`) REFERENCES `tblstaffuser` (`staffid`),
  CONSTRAINT `fk_parent_customer` FOREIGN KEY (`parent_cust_id`) REFERENCES `tblcustomers` (`custid`)
