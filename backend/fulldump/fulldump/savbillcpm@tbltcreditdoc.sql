...
--
-- Host: localhost    Database: savbillcpm    Table: tbltcreditdoc
-- ------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `tbltcreditdoc` (
  ...
  `batchassigned` bit(1) DEFAULT b'0',
  PRIMARY KEY (`CREDITDOCID`),
  UNIQUE KEY `tcreditdoc_creditdocid_unq` (`CREDITDOCID`),
  KEY `tbltcreditdoc_ibfk_1` (`CUSTID`),
  KEY `TBLTCREDITDOC_fk_02` (`invoiceid`),
  KEY `TBLTCREDITDOC_ibfk2_1` (`MVNOID`),
  KEY `businessunit_creditdocument_fk` (`BUID`),
  KEY `idx_creditdoc_status_approver` (`STATUS`,`APPROVEDBYSTAFFID`,`is_delete`),
  KEY `idx_creditdoc_mvno_bu` (`MVNOID`,`BUID`,`STATUS`,`is_delete`),
  KEY `idx_creditdoc_cust` (`CUSTID`),
  KEY `idx_creditdoc_createdate` (`CREATEDATE` DESC),
  CONSTRAINT `businessunit_creditdocument_fk` FOREIGN KEY (`BUID`) REFERENCES `tblmbusinessunit` (`businessunitid`),
  CONSTRAINT `TBLTCREDITDOC_ibfk2_1` FOREIGN KEY (`MVNOID`) REFERENCES `tblmmvno` (`MVNOID`),
  CONSTRAINT `tbltcreditdoc_ibfk_1` FOREIGN KEY (`CUSTID`) REFERENCES `tblcustomers` (`custid`)
