...
class CustomerCAFBasicDetailController extends GetxController {
  ...
  TextEditingController feasibility = TextEditingController();
      ...
      onSuccess: (ResponseModel responseModel) {
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              Map<String, dynamic> map = responseModel.result;
              CustomerFeasibilityRes responseData = CustomerFeasibilityRes.fromJson(map);
              if (responseData.responseCode == 200) {
                if (responseData.dataList != null &&
                    responseData.dataList!.isNotEmpty) {
          ...
          if (responseModel.result != null) {
            try {
              ...
              if ((responseData.status != null && responseData.status == 200) ||
                ...
                for (CustomerFeasibility element in feasibilityList!) {
                  if (element.text == customerDetail?.feasibilityRequired) {
                    selectedFeasibilityData = element;
                  }
                }

                feasibility.text =
                customerDetail?.feasibilityRequired != null
                    ? customerDetail?.feasibilityRequired!
                    : "";
