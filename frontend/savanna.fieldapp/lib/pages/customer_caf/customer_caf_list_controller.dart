...
import '../task_management/model/response/get_all_team_list_res.dart';
...
class CustomerCafListController extends GetxController
  ...
  CustomerCafDropDownStaffList? defaultSelectedStaffList;
  ...
  void onInit() {
    ...
    searchCategory?.add(
        CustomerSearchData(text: 'CAF Created By', value: 'createbyname'));
    ...
    custTypeList!.clear();
    custTypeList!
        .add(DropdownDetail(id: "Parent", text: "Parent", type: "custparent"));
    custTypeList!
        .add(DropdownDetail(id: "Child", text: "Child", type: "custchild"));
    custTypeList!.add(DropdownDetail(
        id: "Individual", text: "Individual", type: "custindividual"));

    selectedSearchCategory = searchCategory![19];
    initPlatformState();
  ...
  getCustomerListData() {
    ...
    if (isFilterApply) {
      ...
      filters.add(Filters(
          ...
          filterValue: selectedSearchCategory!.value!
                  .equalsIgnoreCase("currentAssignedTeam")
              ? selectedAllTeamList != null
                  ? selectedAllTeamList!.name
                  : searchController.text
              : selectedSearchCategory!.value!.equalsIgnoreCase("createbyname")
                  ? selectedCustomerAllStaffList != null
                      ? selectedCustomerAllStaffList!.fullName
                      : searchController.text
                  : (selectedSearchCategory!.value!
                              .equalsIgnoreCase("currentAssigneeName") &&
                          selectedCustomerAllStaffList != null)
                      ? selectedCustomerAllStaffList!.username
                      : searchController.text));
  ...
  approveRejectCafCustomer(
      ...
      required List<FileDetail>? allFiles}) async {
...
// ✅ Populate fields safely
    request.fields['custcafId'] = safeString(customerDetail?.id);
    request.fields['flag'] = safeString(status);
    request.fields['remark'] = safeString(remark);
    request.fields['staffId'] = safeString(userDetail?.userId);
...
// ✅ Print URL & Payload before sending
      ...
      if (response.statusCode == 200 && data != null) {
        ...
        if (responseData.result!.dataList != null &&
          ...
          showAssignStaffDialog(responseData.result!.dataList!, status, context,
              customerDetail?.id, searchStaffController);
        } else {
          if (status!.equalsIgnoreCase(Strings.rejected.toLowerCase())) {
  ...
  showAssignStaffDialog(
      List<ApproveRejectCafDataList> item,
      String? staffStatus,
            ...
            try {
              ...
              if ((responseData.responseCode != null &&
                ...
                if (responseData.dataList != null &&
                  ...
                  showReAssignWorkFlowGetStaffDialog(
                      responseData.dataList!, searchAssignStaffController);
                } else {
                  Utils.showSnackbar(
                      Strings.INFO,
                      responseData.responseMessage ?? "",
  ...
  showReAssignWorkFlowGetStaffDialog(List<ReassignWorkflowList> item,
      TextEditingController? assignSearchController) {
    showDialog(
        context: Get.context!,
        barrierDismissible: true,
        builder: (BuildContext context) {
          return CustomerReAssignWorkFlowDialog(
            customerReassignWorkflowAction: this,
            itemsOrgLst: item,
            textEditingController: assignSearchController,
          );
        });
  ...
  reassignWorkflowAssignCall(
    ...
    String apiUrl =
        "${UrlConstants.reassignWorkflow}?entityId=$entityId&eventName=$eventName&assignToStaffId=$assignToStaffId&remark=$remark";
    isLoading = true;
    update();
  ...
  void cafCustomerApproveRejectStatus({
    ...
    BuildContext? context,
    CustomerDetail? customerDetail,
    List<FileDetail>? allFiles, // ✅ Added here
  }) {
    Get.back();
    ...
    if (identifier != null && identifier.equalsIgnoreCase(Strings.approve)) {
      approveRejectCafCustomer(
        ...
        customerDetail: customerDetail,
        context: context!,
        allFiles: allFiles, // ✅ Pass list here
      );
      ...
      onSuccess: (ResponseModel responseModel) {
        ...
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              Map<String, dynamic> map = responseModel.result;
              CustomerCafDropDownRes responseData =
                  CustomerCafDropDownRes.fromJson(map);
              log("map==>${jsonEncode(responseData)}");
              if (responseData.responseCode == 200) {
                customerAllStaffList = responseData.dataList;
                for (CustomerCafDropDownStaffList element
                    in customerAllStaffList!) {
                  if (userDetail!.userId == element.id) {
                    selectedCustomerAllStaffList = element;
                    defaultSelectedStaffList = element;
                    applyFilter();
                  }
                }
              } else {
  ...
  getAllTeamList() {
    ...
    allTeamList!.clear();
    update();
    CustomerProvider().getAllTeamListRequest(
      onSuccess: (ResponseModel responseModel) {
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              Map<String, dynamic> map = responseModel.result;
              GetAllTeamListRes responseData = GetAllTeamListRes.fromJson(map);
              if (responseData.responseCode != null &&
                  responseData.responseCode == 200) {
                if (responseData.dataList != null &&
                    responseData.dataList!.isNotEmpty) {
                  allTeamList!.addAll(responseData.dataList!);
                  for (AllTeamDataList element in allTeamList!) {
                    if (userDetail!.userId == element.id) {
                      selectedAllTeamList = element;
                      applyFilter();
                    }
                  }
                } else {
