...
class TicketDocumentUploads extends StatefulWidget {
  @override
  _TicketDocumentUploadsState createState() => _TicketDocumentUploadsState();
}

class _TicketDocumentUploadsState extends State<TicketDocumentUploads>
    with WidgetsBindingObserver, TickerProviderStateMixin
    implements LocationBtnAction,
        PermissionDenyBtnAction {
  final ticketChangeStatusController = Get.put(TicketChangeStatusController());
  ...
  _backScreen() {
    Get.back(result: ticketChangeStatusController.isChangeData);
  }
  ...
  void uploadAllDocuments() {
    ...
    if (allSectionsData.isNotEmpty) {
      ...
      // uploadDocuments(allSectionsData, allFiles, ticketChangeStatusController.customerInventoryDataList!.id);
    } else {
  ...
  Future<void> uploadDocuments(List<Map<String, dynamic>> sectionsData,
    ...
    try {
      ticketChangeStatusController.isLoading = true;
      final streamedResponse = await request.send();
      final response = await http.Response.fromStream(streamedResponse);
      final responseData = jsonDecode(response.body);
      log("response body => ${response.body}"); // ✅ correct
      if (response.statusCode == 200) {
        ticketChangeStatusController.isLoading = false;
    ...
    return WillPopScope(
      ...
      child: GetBuilder<TicketChangeStatusController>(builder: (controller) {
        return Stack(children: [
          ...
          ProgressBar(isLoader: ticketChangeStatusController.isLoading),
        ]);
      }), /**/
    );
  }
                                        ...
                                        children: [
                                          ...
                                          DropdownButtonHideUnderline(
                                            child: DropdownButtonFormField(
                                              ...
                                              isExpanded: true,
                                              isDense: true,
                                              value: ticketChangeStatusController
                                                  .selectPowerOpticalRange,
                                              items: ticketChangeStatusController
                                                  .opticalRangeData
                                              ...
                                              validator: (value) {
                                                // if (value == null ||
                                                //     ticketChangeStatusController.selectPowerOpticalRange == null) {
  ...
  Future<void> checkCameraPermission(int tabIndex) async {
    ...
    var storagePermission = isAndroid13OrAbove ? Permission.photos : Permission.storage;
  ...
  getCurrentPosition(bool fromTryAgain,int tabIndex) async {
    ...
    if (!serviceEnabled) {
      ticketChangeStatusController.setBtnClickEvent(true);
    ...
    ticketChangeStatusController.isLoading = true;
    ticketChangeStatusController.update();
    ...
    geolocatorPlatform
      ...
      locationSettings: settings,
        ...
        .then((position) {
      if (position != null) {
        ticketChangeStatusController.setBtnClickEvent(false);
        ticketChangeStatusController.isLoading = false;
  ...
  openLocationListScreen() async {
    ...
    if (result != null) {
      ...
      if (data != null) {
        ticketChangeStatusController.selectedLocation = data;
        ticketChangeStatusController.update();
        ticketChangeStatusController.getLocationToLatLong();
      }
    }
  }
