...
import 'package:savbill/pages/task_management/task_mgmt/upload_doc/task_upload_doc_controller.dart';
...
class TaskUploadDocumentScreen extends StatefulWidget {
  @override
  _TaskUploadDocumentState createState() => _TaskUploadDocumentState();
}

class _TaskUploadDocumentState extends State<TaskUploadDocumentScreen>
    with WidgetsBindingObserver
    implements ImageOptionBtnAction, PermissionDenyBtnAction {
  final uploadDocumentController = Get.put(TaskUploadDocumentController());
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: GetBuilder<TaskUploadDocumentController>(builder: (controller) {
  ...
  validateForm() {
    if (uploadDocumentController.fileDetail != null &&
      ...
      uploadDocumentController.taskDocumentUpload();
    } else {
  ...
  openCameraGallery(ImageSource source) async {
    try {
      XFile? image;
      image = await imagePicker.pickImage(source: source,imageQuality: 20);
