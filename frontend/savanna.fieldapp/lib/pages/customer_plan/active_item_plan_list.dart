        ...
        Card(
          ...
          child: Column(
            ...
            children: [
              ...
              cardDataRow(
                  Strings.serial_no,
                  item.customerInventorySerialnumberDtos?.isNotEmpty == true
              ...
              line(),
              cardDataRow(Strings.validity, controller!.customerDetail!.custtype!.equalsIgnoreCase("Postpaid") ? "N/A" :item.validity!.toInt().toString()),
              line(),
              Padding(
              ...
              cardDataRow(Strings.service_expiry_date,
                  controller!.customerDetail!.custtype!.equalsIgnoreCase("Postpaid") ? "N/A" : getDateTimeFormat(item.dbEndDate.toString())),
              line(),
              cardDataRow(Strings.billing_end_date,
                  controller!.customerDetail!.custtype!.equalsIgnoreCase("Postpaid") ? "N/A" : getDateTimeFormat(item.dbExpiryDate.toString())),
              line(),
              cardDataRow(Strings.remaining_days,
                  controller!.customerDetail!.custtype!.equalsIgnoreCase("Postpaid") ? "N/A"  :remainingDuration(item.dbEndDate.toString())),
              line(),
              Padding(
                padding: const EdgeInsets.all(Constant.MEDIUM_PADDING),
                child: Row(
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    controller!.customerDetail!.status != "Terminate"
                        ? buttonView(
                            openTicketInvoice,
                    ...
                    item.debitdocid == null
                        && item.offerPrice == 0.0 && PermissionService().hasAclPermission([AclPreCustConstants.PRE_CUST_PLAN_VOID]) == true
                        ? buttonView(
                        voidPlan,
                        AppTheme.colorPrimaryTheme,
                        AppTheme.colorWhite,
                        onTapVoidPlan!) : Container(),
                    ...
                    // (controller!.customerDetail!.status!.equalsIgnoreCase("Prepaid") &&
                    (controller!.customerDetail!.status!.equalsIgnoreCase("Active") &&
            isLatestFuturePlan(
                                item.custPlanMapppingId,
                                item.expiryDate,
                                futurePlanList,
                                activePlanList) &&
            ...
            !controller!.customerDetail!.status!.equalsIgnoreCase("Terminate")  &&
...
isLatestFuturePlan(
  ...
  if (futurePlanList != null && futurePlanList.isNotEmpty) {
    ...
    return (futurePlanList
        .where((element) => element.custPlanMapppingId == custPlanMappingId).isEmpty &&
        activePlanList.where((element) =>
  ...
  return (activePlanList!.where((element) =>
  element.custPlanMapppingId == custPlanMappingId &&
      DateFormat(Constant.DATE_NEW_TIME_FORMAT)
          .parse(element.expiryDate.toString())
          .millisecondsSinceEpoch
...
checkPlanGroup(CustPlanDataList plan, List<CustPlanDataList>? futurePlanList) {
  if (plan.plangroupid != null) {
    ...
    return (futurePlanList!.where((element) => element.plangroupid == plan.plangroupid).toList() == plan);
  }
  return true;
}
