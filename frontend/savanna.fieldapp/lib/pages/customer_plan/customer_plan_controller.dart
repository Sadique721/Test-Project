...
class CustomerPlanController extends GetxController {
  ...
  getTrialPlanListData() {
    isLoading = true;
    update();
    CustomerPlanProvider().getTrialPlanList(
            ...
            try {
              ...
              if (responseData.responseCode == 200 ||
                ...
                if (responseData.dataList != null &&
                  ...
                  trialPlanList?.clear();
                  log("trialPlanList===>>>${jsonEncode(responseData.dataList!)}");
                  trialPlanList?.addAll(responseData.dataList!);
                }
  ...
  voidPlan(CustPlanDataList? item , String? voidPlanRemarks) {
    isLoading = true;
    update();

    CancelTrailPlanReq request = CancelTrailPlanReq(
      custpackageid: item?.planmapid,
      planCancelRemarks : voidPlanRemarks,
    );

    CustomerPlanProvider().voidPlan(
      cancelTrailPlanReq: request,
      onSuccess: (ResponseModel responseModel) {
        isLoading = false;
        update();
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              Map<String, dynamic> map = responseModel.result;
              CustPlanDetailResponse responseData = CustPlanDetailResponse.fromJson(map);
              if (responseData.responseCode == 200) {
                Get.back(result: true);
                Utils.showSnackbar(Strings.INFO, responseData.responseMessage,
                    AppTheme.colorWhite, AppTheme.colorBlueRView);
