...
class PAChangeDiscountItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (item.startDate != null && item.startDate!.isNotEmpty) {
      DateTime date =
          DateFormat(Constant.DATE_TIME_FORMAT_API).parse(item.startDate!);
      startDt = DateFormat(Constant.DATE_FORMAT).format(date);
    }

    if (item.endDate != null && item.endDate!.isNotEmpty) {
      DateTime date =
          DateFormat(Constant.DATE_TIME_FORMAT_API).parse(item.endDate!);
      endDt = DateFormat(Constant.DATE_FORMAT).format(date);
    }
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Padding(
            ...
            child: basicDetailItem(
                Strings.plan,
                (item.planId != null) ? item.planId!.toString() : "-",
          ...
          Padding(
            ...
            child: basicDetailItem(
                Strings.start_date,
                (startDt.isNotEmpty) ? startDt : "-",
                Strings.end_date,
                (endDt.isNotEmpty) ? endDt : "-"),
          ),
          const SizedBox(
            height: Constant.SMALL_PADDING,
          ),
