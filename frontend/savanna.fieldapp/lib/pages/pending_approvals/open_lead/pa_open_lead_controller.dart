...
class PAOpenLeadController extends GetxController {
  bool isLoading = false;
  ScrollController? controller;
  int page = 1;
  bool isShowLoadMore = false;
  GetStorage getStorage = GetStorage();
  // List<PAInvoice>? invoiceList = [];
  List<PAOpenLeadContent>? openLeadList = [];
  PAOpenLeadRes? paOpenLeadRes;
  ...
  void onInit() {
    ...
    controller?.addListener(() {
      ...
      if (extentAfter! < 300 && !isShowLoadMore) {
        if (paOpenLeadRes != null && paOpenLeadRes!.leadMasterList!.totalPages != page) {
          isShowLoadMore = true;
          page = page + 1;
          update();
          getPAOpenLeadList();
  ...
  getPAOpenLeadList() {
    ...
    PendingApprovalsProvider().getOpenLeadApproval(
      request: request,
        ...
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              ...
              PAOpenLeadRes responseData = PAOpenLeadRes.fromJson(map);
              if ((responseData.status != null && responseData.status == 200) ||
                  (responseData.responseCode != null &&
                      responseData.responseCode == 200)) {
                paOpenLeadRes = responseData;
                if (page == 1) {
                  openLeadList?.clear();
                }
                if (responseData.leadMasterList!.content != null &&
                    responseData.leadMasterList!.content!.isNotEmpty) {
                  openLeadList?.addAll(responseData.leadMasterList!.content!);
                }
              } else {
                if (page == 1) {
                  openLeadList?.clear();
