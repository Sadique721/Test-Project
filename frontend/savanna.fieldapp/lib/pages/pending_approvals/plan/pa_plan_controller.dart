...
class PAPlanController extends GetxController {
  bool isLoading = false;
  ScrollController? controller;
  int page = 1;
  bool isShowLoadMore = false;
  GetStorage getStorage = GetStorage();
  List<ApprovalPendingPlan>? planList = [];
  ApprovalPendingPlanRes? approvalPendingPlanRes;
  ...
  void onInit() {
    ...
    controller?.addListener(() {
      ...
      if (extentAfter! < 300 && !isShowLoadMore) {
        if (approvalPendingPlanRes != null &&
            approvalPendingPlanRes!.totalPages != page) {
          isShowLoadMore = true;
          page = page + 1;
          update();
          getPAPlanList();
  ...
  getPAPlanList() {
    ...
    PendingApprovalsProvider().getPlanPendingApprovals(
      request: request,
        ...
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              ...
              ApprovalPendingPlanRes responseData =
                  ApprovalPendingPlanRes.fromJson(map);
              if ((responseData.status != null && responseData.status == 200) ||
                ...
                if (responseData.dataList != null &&
                  ...
                  planList?.addAll(responseData.dataList!);
                }
              } else {
                if (page == 1) {
                  planList?.clear();
  ...
  approveRejectPlan(PlanApproveRejectReq request) {
    isLoading = true;
    update();
    PendingApprovalsProvider().approveRejectPlan(
      request: request,
      onSuccess: (ResponseModel responseModel) {
        isLoading = false;
        update();
