...
class PaymentTabController extends GetxController
  ...
  void onInit() {
    ...
    paymentStatusList?.add(PaymentStatus(status: Strings.rejected,label: Strings.rejected));
  }
  ...
  getPaymentListData() {
    ...
    if (isFirstTime) {
      ...
      apiUrl = "${apiUrl}&staff=&approveId=&paymode=&branchname=&buID=&referenceno=&receiptNo=&chequedate=&paydetails1=&destinationBank=&partnerName=&serviceAreaId=&page=$page&pageSize=${Constant.PAGE_LOAD_DATA_LIMIT_FULL}";
      isLoading = true;
    }/* else {
      if (selectedCustomer != null) {
        cusId = selectedCustomer?.id;
  ...
  downloadFileApiCall(String url, String savePath) {
   ...
   fileDownloading(String? pageTitle, String? networkPathUrl,String? customerName) async {
    isLoading = true;
    ...
    if (response.statusCode == 200) {
      try {
          ...
          final String customerDirPath = p.join(baseDir.path, customerName);
          final Directory customerDir = Directory(customerDirPath);
          if (!await customerDir.exists()) {
            await customerDir.create(recursive: true);
            log('Created directory: $customerDirPath');
          }
          final String filePath = p.join(customerDir.path, '$filename.pdf');
          ...
          if (Platform.isIOS) {
            ...
            Utils.showSnackbar(Strings.SUCCESS, "File Downloaded Successfully Please Open Document Folder!!",
                AppTheme.colorWhite, AppTheme.colorGreen);
          }
        } catch (e) {
