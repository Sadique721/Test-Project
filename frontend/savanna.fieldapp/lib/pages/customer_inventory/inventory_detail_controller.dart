...
class InventoryDetailController extends GetxController
  ...
  bool nmsEnable = false;
  bool fiberHomeEnable = false;
  ...
  Future<void> initPlatformState() async {
    ...
    if (!isCustCaf) {
      getNmsEnable("NMS_ENABLE");
      // getFiberHomeEnable("FiberHome");
    }
  }

  getNmsEnable(String type) {
    CustomerProvider().getSystemConfiguration(
      type: type,
          ...
          try {
            ...
            if (responseData.data != null) {
              if (responseData.data!.name!.isNotEmpty &&
                ...
                nmsEnable = responseData.data?.value?.toLowerCase() == 'true';
              }
            }
  ...
  getFiberHomeEnable(String type) {
    CustomerProvider().getSystemConfiguration(
      type: type,
      onSuccess: (ResponseModel responseModel) {
        if (responseModel.statusCode == 200) {
          try {
            ...
            if (responseData.data != null) {
              if (responseData.data!.name!.isNotEmpty &&
                  type.equalsIgnoreCase(type)) {
                fiberHomeEnable =
                    responseData.data?.value?.toLowerCase() == 'true';
              }
            }
          } on Exception catch (e) {
            print(e.toString());
  ...
  getArgumentData() {
    ...
    if (arguments != null) {
      ...
      if (arguments[Constant.ISCUSTCAF] != null) {
        isCustCaf = arguments[Constant.ISCUSTCAF];
      }
    }
    update();
  }
  ...
  approveReplaceInventoryApi(
    ...
    final mappingItemAssemblyName = item.custInventoryListId;
    // Filter matching inventory list
    var inventoryDetailsMappings = inventoryDataList
        !.where((inventory) => inventory.custInventoryListId == mappingItemAssemblyName)
        .toList();
    // final inventoryDetailsMapping = inventoryDetailsMappings[0].inOutWardMACMapping;
    for (var inventoryDetail in inventoryDetailsMappings) {
      final mapping = inventoryDetail.inOutWardMACMapping;

      final oldId1 = mapping?[0].id;
      final newId1 = mapping!.length > 1 ? mapping[1].id : "";

      ReplaceInventoryReq replaceInventoryReq = ReplaceInventoryReq(
        oldMacMappingId: oldId1,
        newMacMappingId: newId1,
      );
  ...
  void downloadDocument(CustomerInventoryDataList? inventoryFileData) {
    ...
    print(
        "File task_catg_detail_screen processed: ${inventoryFileData.fileDetails}");
  }
