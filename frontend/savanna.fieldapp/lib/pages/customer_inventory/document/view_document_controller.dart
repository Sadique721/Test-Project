...
import '../response/get_all_customer_inventory_list_res.dart';
...
class ViewDocumentController extends GetxController {
  ...
  showInventoryDocData(
    ...
    try {
      final response = await http.get(Uri.parse("$url$inventoryId/$fileName/$uniqueName/"), headers: headers,);
  ...
  inventoryDocumentDownload(
    ...
    final response = await http
        .get(Uri.parse("$url$inventoryId/$uniqueName/$sectionName/"), headers: headers,);
    ...
    if (Platform.isIOS) {
      ...
      Utils.showSnackbar(
          Strings.SUCCESS, "File Downloaded Successfully Please Open Download Folder!!",
          AppTheme.colorWhite, AppTheme.colorGreen);
    }
  ...
  inventoryDocumentDelete(String? fileName, String? uniqueName,String? sectionName) {
    isLoading = true;
    update();
    InventoryProvider().inventoryDownloadDeleteDocument(
