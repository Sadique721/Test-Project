...
class AddEditLeadController extends GetxController {
      ...
      onSuccess: (ResponseModel responseModel) {
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              Map<String, dynamic> map = responseModel.result;
              NewServicesAreaRes responseData = NewServicesAreaRes.fromJson(map);
              if (responseData.responseCode == 200) {
                if (responseData.dataList != null &&
                    responseData.dataList!.isNotEmpty) {
                  ...
                  getStorage.write(Constant.LEAD_SERVICE_AREA_DATA, jsonEncode(newServicesAreaList));
                }
              } else {
                if (responseData.responseMessage!.isNotEmpty) {
                  Utils.showSnackbar(
  ...
  List<NewServiceDataList> getNewServicesAreaList() {
    String? jsonString = getStorage.read(Constant.LEAD_SERVICE_AREA_DATA); // Read stored JSON
    ...
    try {
      ...
      return jsonData.map((e) => NewServiceDataList.fromJson(e)).toList(); // Convert to List<ServicesAreaDetail>
    } catch (e) {
              ...
              if (responseData.responseCode != null &&
                ...
                if (responseData.dataList != null &&
                  ...
                  if(branchesByServiceAreaList!.isNotEmpty) {
                    selectBranchesByServiceAreaData = branchesByServiceAreaList![0];
                  }
            ...
            try {
              ...
              if ((responseData.responseCode != null &&
                ...
                if (responseData.leadCustomerGender!.isNotEmpty) {
                  ...
                  getStorage.write(Constant.LEAD_GENDER_TYPE_DATA, jsonEncode(leadCustomerGenderList));
                }
              } else {
                if (responseData.responseMessage != null &&
                    responseData.responseMessage!.isNotEmpty) {
  ...
  List<String> getGenderTypeList() {
    String? jsonString = getStorage.read(Constant.LEAD_GENDER_TYPE_DATA); // Read stored JSON
    if (jsonString == null || jsonString.isEmpty) {
      return []; // Return an empty list if data is null or empty
    }
    try {
      List<dynamic> jsonData = json.decode(jsonString); // Decode JSON string
      return jsonData.map((e) => e.toString()).toList();// Convert to List<String>
    } catch (e) {
      print("Error decoding services data: $e");
      return []; // Return empty list if an error occurs
    }
  ...
  createLead() {
    ...
    LeadSaveReq saveReq = LeadSaveReq(
      ...
      feasibilityRemark: remarksController.text.isNotEmpty ? remarksController.text : null,
