...
class TaskViewDocumentController extends GetxController {
  ...
  getArgumentData() {
    ...
    if (arguments != null) {
      ...
      if (arguments[Constant.INVENTORY_ID] != null) {
        taskId = arguments[Constant.INVENTORY_ID];
      }
      if (arguments[Constant.CUSTOMER_NAME] != null) {
        customerName = arguments[Constant.CUSTOMER_NAME];
      }
      if (taskId != null) {
        getTaskDocumentList(taskId);
      }
    }
    update();
  }

  getTaskDocumentList(int? ID) {
    documentList?.clear();
    isLoading = true;
    update();
    TaskSystemProvider().getTaskDocumentViewCall(
      taskId: ID!,
  ...
  showInventoryDocData(
    ...
    try {
      final response = await http.get(Uri.parse("$url$taskId/$fileName/$uniqueName/"), headers: headers,);
  ...
  ticketDocumentDownload(
    ...
    final response = await http
        .get(Uri.parse("$url/$taskId/$uniqueName/$sectionName/"), headers: headers,);
    ...
    var file = File('$directory/${customerName}_${fileName[0] ?? "-"}.png');
  ...
  taskDocumentDelete(String? fileName, String? uniqueName,String? sectionName) {
    isLoading = true;
    update();
    TaskSystemProvider().taskDownloadDeleteDocument(
      taskId: taskId,
      fileName: fileName,
