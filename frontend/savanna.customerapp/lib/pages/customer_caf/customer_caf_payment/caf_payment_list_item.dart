...
class CAFPaymentViewListItem extends StatelessWidget {
  PaymentDetail item;
  int index, userId;
  CustomerCAFPaymentListController? controller;
  bool showStatusBtn = false, showDownloadBtn = false;

  CAFPaymentViewListItem(
      {Key? key,
      required this.index,
      required this.item,
  ...
  Widget build(BuildContext context) {
    ...
    //   } else if (item.status!.equalsIgnoreCase(Strings.approved.toLowerCase()) ||
    ...
    //   } else if (item.status!.equalsIgnoreCase(Strings.rejected.toLowerCase())) {
    ...
    if (item.status!.equalsIgnoreCase("Fully Adjusted")) {
      status = "Verified";
      statusTxtColor = AppTheme.statusApprove;
    } else if (item.status!.equalsIgnoreCase("advance")) {
      status = "Verified";
      statusTxtColor = AppTheme.statusApprove;
      ...
      status = "Submitted";
      statusTxtColor = AppTheme.statusPending;
    } else if (item.status!.equalsIgnoreCase("Partialy Adjusted")) {
      status = "Verified";
      statusTxtColor = AppTheme.statusApprove;
    } else if (item.status!.equalsIgnoreCase("rejected")) {
      status = item.status.toString().capitalizeFirst!;
      statusTxtColor = AppTheme.statusReject;
    } else if (item.status!.equalsIgnoreCase("Portal_Payment_Pending")) {
      status = "Portal Payment Pending";
      statusTxtColor = AppTheme.statusReject;
    } else if (item.status!.equalsIgnoreCase("Portal_Payment_Approved")) {
      status = "Success";
      statusTxtColor = AppTheme.statusApprove;
    }else if(item.status!.equalsIgnoreCase("Online Payment Pending")){
      statusTxtColor = AppTheme.statusReject;
      status = item.status!;
    }else if(item.status!.equalsIgnoreCase("Online Payment Approved")){
      statusTxtColor = AppTheme.statusApprove;
      status = "Success";
    }
    ...
    if (item.filename == null) {
      fileName = "-";
      clickLink = AppTheme.title_dark;
    } else if (item.filename != null) {
      fileName = "Download Invoice";
      clickLink = AppTheme.clickLink;
    }
