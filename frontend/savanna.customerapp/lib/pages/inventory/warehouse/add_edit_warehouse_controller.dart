      ...
      onSuccess: (ResponseModel responseModel) {
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              Map<String, dynamic> map = responseModel.result;
              GetAllTeamBasedInventoryRes responseData = GetAllTeamBasedInventoryRes.fromJson(map);
              if (responseData.responseCode == 200) {
                if (responseData.dataList != null &&
                    responseData.dataList!.isNotEmpty) {
                  allTeamBasedInventoryList?.addAll(responseData.dataList!);
                  if (wareHouseData != null &&
                      wareHouseData!.teamsIdsList != null &&
                      wareHouseData!.teamsIdsList!.isNotEmpty) {
                    String teamName = "";
                    for (AllTeamDataList element
                    in allTeamBasedInventoryList!) {
                      for (int value
                      in wareHouseData!.teamsIdsList!) {
                        if (value == element.id) {
                          selectedAllTeamInventoryList!.add(element.id!);
                          teamName =
                          "$teamName${element.displayName!}, ";
                          element.selected = true;
                        }
                      }
                    }
