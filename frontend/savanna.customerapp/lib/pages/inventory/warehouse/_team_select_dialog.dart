...
class TeamSelectionDialog extends StatefulWidget {
  final TeamSelectionAction teamSelectionAction;
  final List<AllTeamDataList>? itemTeam;
  final String fromFor;

  const TeamSelectionDialog({
    Key? key,
    required this.teamSelectionAction,
    this.itemTeam,
...
class _TeamSelectionState extends State<TeamSelectionDialog> {
  List<AllTeamDataList>? itemsLst = [];

  @override
  void initState() {
    super.initState();
    setState(() {
      if(widget.itemTeam!.isNotEmpty) {
        itemsLst!.addAll(widget.itemTeam!);
      }
  ...
  Widget build(BuildContext context) {
    String title = "";
    if (widget.fromFor.equalsIgnoreCase(Strings.team)) {
      title = "${Strings.select} ${Strings.team}";
    }
                    ...
                    Flexible(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            AllTeamDataList item = itemsLst![index];
                            return Column(
                              children: [
                                InkWell(
                                  onTap: () {
  ...
  validateSelection() {
    List<AllTeamDataList> selectedItem = [];
    ...
    if (selectedItem.isNotEmpty) {
      widget.teamSelectionAction.teamSelectionBtnAction(
...
abstract class TeamSelectionAction {
