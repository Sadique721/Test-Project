...
class ViewManufacturerController extends GetxController {
  bool isLoading = false;
  ScrollController? controller;
  int page = 1;
  bool isShowLoadMore = false;
  GetStorage getStorage = GetStorage();
  List<ManufacturerDetail>? manufacturerList = [];
  ManufacturerListRes? manufacturerListRes;
  ...
  void onInit() {
    super.onInit();
    getManufactureVendorData();
    controller = ScrollController();
    controller?.addListener(() {
      double? extentAfter = controller?.position.extentAfter;
      if (extentAfter! < 300 && !isShowLoadMore) {
        if (manufacturerListRes != null && manufacturerListRes!.totalPages != page) {
          isShowLoadMore = true;
          page = page + 1;
          update();
          getManufactureVendorData();
  ...
  getManufactureVendorData() {
    ...
    InventoryManagementProvider().getManufactureVendor(
        ...
        if (responseModel.statusCode == 200) {
          if (responseModel.result != null) {
            try {
              ...
              ManufacturerListRes responseData = ManufacturerListRes.fromJson(map);

              if ((responseData.status != null && responseData.status == 200) ||
                  (responseData.responseCode != null &&
                      responseData.responseCode == 200)) {
                manufacturerListRes = responseData;
                if (page == 1) {
                  manufacturerList?.clear();
                }
                if (responseData.dataList != null &&
                    responseData.dataList!.isNotEmpty) {
                  manufacturerList?.addAll(responseData.dataList!);
                }
              } else {
                if (page == 1) {
                  manufacturerList?.clear();
  ...
  deleteManufacturerItemData(int? vendorId, int index) {
    isLoading = true;
    update();

    InventoryManagementProvider().deleteManufacturerItem(
      vendorId: vendorId,
      onSuccess: (ResponseModel responseModel) {
        isLoading = false;
        update();
            ...
            try {
              ...
              if ((responseData.status != null && responseData.status == 200) ||
                ...
                Utils.showSnackbar(
                    ...
                    AppTheme.colorWhite,
                    AppTheme.colorGreen);
                manufacturerList!.removeAt(index);
              } else {
                if (responseData.responseMessage!.isNotEmpty) {
                  Utils.showSnackbar(
                      Strings.ERROR,
