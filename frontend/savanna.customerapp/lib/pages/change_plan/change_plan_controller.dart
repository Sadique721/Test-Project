...
//getPlanChangeGroup
  ...
  selectPlanGroupType(PlanGroupDetail? planTypeDetail, int childIndex) {
    ...
    if (childIndex != -1) {
      planGroupData[childCustList![childIndex].id.toString()] = planMappingListData!;
    } else {
  ...
  filterPlanGroup(String? service, int childIdx) {
    ...
    if (childIdx != -1) {
      List<PlanMappingGroupDetail>? planGroup = planGroupData[childCustList![childIdx].id.toString()];
      if (planGroup != null) {
      ...
      List<PlanMappingGroupDetail>? planGroup = planGroupData[customerDetail!.id.toString()];
  ...
  openChargeDetails(int? custId, BuildContext context){
    var filteredCharge = chargeData.firstWhere((item) => item['custid'] == custId, orElse: () => null);
  ...
  prepareChangePlanPayload(dynamic recordPaymentPojo) {
    if (selectedPlanType!.text!.equalsIgnoreCase("Renew")) {
      ...
      List<Map<String, dynamic>> deactivatePlanReqModelsChild = [];
            ...
            try {
              ...
              if ((responseData.status != null && responseData.status == 200) ||
                ...
                if (responseData.postPaidPlan != null) {
                  ...
                  selPlanData['quotaunittime'] = responseData.postPaidPlan!.quotaunittime;
                  selPlanData['validity'] = responseData.postPaidPlan!.validity!.toInt();
                  selPlanData['offerprice'] = responseData.postPaidPlan!.offerprice;
                  selPlanData['taxamount'] = responseData.postPaidPlan!.taxamount;
                  // selPlanData['activationDate'] = responseData.postPaidPlan!.ac.toString();
                  selPlanData['expiryDate'] = responseData.postPaidPlan!.expiryDate;
                  selPlanData['id'] = responseData.postPaidPlan!.id;
                  selPlanData['name'] = responseData.postPaidPlan!.name;
                  selPlanData['unitsOfValidity'] = responseData.postPaidPlan!.unitsOfValidity;
  ...
  showDirectChargeDialog(dynamic addedChargeList,
      bool displayShowCharge, BuildContext context) {
    showDialog(
        context: context,
        barrierDismissible: true,
        builder: (BuildContext context) {
