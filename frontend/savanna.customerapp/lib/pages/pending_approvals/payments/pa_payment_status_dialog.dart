...
class PAPaymentStatusDialog extends StatefulWidget {
  final PaymentStatusBtnAction paymentStatusBtnAction;
  final PaymentApproveRejectReq paymentApproveRejectReq;
  final String from;

  const PAPaymentStatusDialog(
      {Key? key,
      required this.paymentStatusBtnAction,
      required this.paymentApproveRejectReq,
      required this.from})
      : super(key: key);

  @override
  _PAPaymentDialogtate createState() => _PAPaymentDialogtate();
}

class _PAPaymentDialogtate extends State<PAPaymentStatusDialog> {
  ...
  contentBox(BuildContext context) {
    ...
    if (widget.from.equalsIgnoreCase(Strings.approve)) {
      title = "${Strings.approve} ${Strings.payments}";
    } else if (widget.from.equalsIgnoreCase(Strings.reject)) {
      title = "${Strings.reject} ${Strings.payments}";
    }
                        ...
                        child: InkWell(
                          onTap: () {
                            if (remarkController.text.isNullOrEmpty()) {
                              ...
                              widget.paymentApproveRejectReq.remarks =
                                  remarkController.text;

                              widget.paymentStatusBtnAction
                                  .paymentStatusBtnAction(
                                identifier: widget.from,
                                request: widget.paymentApproveRejectReq,
                              );
                            }
                          },
                          child: Container(
...
abstract class PaymentStatusBtnAction {
