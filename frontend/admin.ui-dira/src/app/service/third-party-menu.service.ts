...
export class ThirdPartyMenuService {
  ...
  baseUrl = RadiusConstants.SAVBILL_INTEGRATION_SYSTEM_BASE_URL;
  ...
  getParamsByEventAndClientName(clientName: string, eventName: string) {
    const url = `${this.baseUrl}/thirdPartyMenu/getParamsByEventAndClientName?clientName=${encodeURIComponent(clientName)}&eventName=${encodeURIComponent(eventName)}`;
