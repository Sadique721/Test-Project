...
export class LeadManagementService {
  ...
  postleadMethod(url, data) {
    return this.http.post(RadiusConstants.SAVBILL_LEAD_BASE_URL + url, data);
  }
  getConnection(url) {
  ...
  assignMethod(url, formData) {
    return this.http.put(RadiusConstants.SAVBILL_PRODUCT_MANAGEMENT_BASE_URL + url, formData);
  }

  deleteMethod(url) {
    return this.http.delete(RadiusConstants.SAVBILL_LEAD_BASE_URL + url);
  }

  updateMethod(url, data, mvnoid, staffid) {
    return this.http.put(RadiusConstants.SAVBILL_LEAD_BASE_URL + url, data, {
      headers: new HttpHeaders({
        "Content-Type": "application/json",
        mvnoid: mvnoid,
        staffid: staffid
  ...
  downloadLeadPDF(type: any): any {
    const url = RadiusConstants.SAVBILL_LEAD_BASE_URL + `${type}`;
  ...
  findCPRForLeadToCAFConvertionForEnterpriseCustomer(leadId: any): any {
    const url =
      RadiusConstants.SAVBILL_LEAD_BASE_URL +
