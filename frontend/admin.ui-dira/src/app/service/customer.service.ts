...
export class CustomerService {
  ...
  baseUrl = RadiusConstants.SAVBILL_RADIUS_BASE_URL;
  baseUrlForNetConf = RadiusConstants.SAVBILL_API_GATEWAY_NETCONF_CUSTOMER;
  mvnoId = localStorage.getItem("mvnoId");
  loggedInUser = localStorage.getItem("loggedInUser");
  userId = localStorage.getItem("userId");

  getCommonFeasibility(){
        const url ="/commonList/feasibility"
        return this.http.get(RadiusConstants.SAVBILL_API_GATEWAY_COMMON_MANAGEMENT + url);
  }

  getChangePlanTypeList(){
    const url ="/commonList/generic/changePlanType";
    return this.http.get(RadiusConstants.SAVBILL_API_GATEWAY_COMMON_MANAGEMENT + url)
 }

  getBillingCycleList(){
    const url ="/commonList/generic/changePlanBillingCycle";
    return this.http.get(RadiusConstants.SAVBILL_API_GATEWAY_COMMON_MANAGEMENT + url)
 }
