...
export class TicketManagementService {
    ...
    assignMethod(url, formData) {
        return this.http.post(RadiusConstants.SAVBILL_TICKET_MANAGEMENT + url, formData);
    }

    deleteMethod(url) {
        return this.http.delete(RadiusConstants.SAVBILL_TICKET_MANAGEMENT + url);
    }

    updateMethod(url, data) {
        return this.http.put(RadiusConstants.SAVBILL_TICKET_MANAGEMENT + url, data);
    }
    ...
    downloadResolveFile(ticketId, uniquename, section): Observable<any> {
        const get_url = RadiusConstants.SAVBILL_TICKET_MANAGEMENT + "/case/document/download/" + ticketId + "/" + uniquename + "/" + section + "/";
