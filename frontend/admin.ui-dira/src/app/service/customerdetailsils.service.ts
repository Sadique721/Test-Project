...
export class CustomerdetailsilsService {
    ...
    commonUrl = RadiusConstants.SAVBILL_API_GATEWAY_COMMON_MANAGEMENT;
    revenueUrl = RadiusConstants.SAVBILL_REVENUE_MANAGEMENT_BASE_URL;
    ...
    baseUrlForPay = RadiusConstants.SAVBILL_PAY;
    ...
    intigrationGetMethod(id, token) {
        let headers = new HttpHeaders({
            Authorization: `${token}`,
        });
        return this.http.get(`${this.intigrationUrl}` + id, { headers });
    }
    ...
    getCustomerAddressDetails(data) {
        return this.http.get(
            RadiusConstants.SAVBILL_PRODUCT_MANAGEMENT_BASE_URL + "/getCustomerAddressDetails/" + data
        );
    }
    ...
    buyPlanUisngMpesaExpressPayLink(data) {
        const token = localStorage.getItem("payLinkToken");
        let headers = new HttpHeaders({
            Authorization: `${token}`,
            requestFrom: "gui"
        ...
        return this.http.post(`${this.intigrationUrl}/c2b/mpesa/express/initiatePayment`, data, { headers });
    }
