...
import { NavService } from "../sidebar/services/nav.service";
...
export class LoginService {
    ...
    baseUrl = RadiusConstants.SAVBILL_API_GATEWAY_COMMON_MANAGEMENT;
    ...
    logout() {
        ...
        localStorage.clear();
        this.cache.clear();
        this.navService.menuSelection.set({ main: "", sub: "" });
    ...
    getRadiusServiceStatus() {
        return this.http.get(`${RadiusConstants.SAVBILL_RADIUS_BASE_URL}/serviceStatus`);
    }

    //   getTacacsServiceStatus() {
    //     return this.http.get(
    //       `${RadiusConstants.SAVBILL_TACACS_MANAGEMENT_SYSTEM_BASE_URL}/tacacs-service/health`
    //     );
    //   }

    getNotificationServiceStatus() {
        return this.http.get(`${RadiusConstants.SAVBILL_NOTIFICATION_BASE_URL}/serviceStatus`);
    }

    getSaleCrmServiceStatus() {
        return this.http.get(`${RadiusConstants.SAVBILL_LEAD_BASE_URL}/serviceStatus`);
    }

    getTaskMgmtServiceStatus() {
        return this.http.get(`${RadiusConstants.SAVBILL_TASK_MGMT_BASE_URL}/serviceStatus`);
    }
    ...
    public getAclEntry(): Observable<any> {
        ...
        return this.getMethod(url).pipe(
            ...
            catchError(err => {
                this.toastr.error(`${err.error.errorMessage}`, 'Something was wrong. Try again!');
    ...
    hasOperationPermission(classId, operationId, accessIdForAllOpreation) {
        ...
        if (this.permissionList.length > 0) {
            ...
            error: (error) => {
                this.toastr.error(`${error.error.ERROR}`, 'Sorry you have not privilege to any operation!!');
    ...
    hideSidebarMenu(menuId, subMenuId, constantVar) {
        ...
        if (this.menuPermissionList) {
            if (this.menuPermissionList.length > 0) {
                ...
                error: (error) => {
                    this.toastr.error(`${error.error.ERROR}`, '"Sorry you have not privilege to access any menu!');
    ...
    getOTP(data) {
        return this.http.post(
            `${RadiusConstants.SAVBILL_PRODUCT_MANAGEMENT_BASE_URL}/otp/generate`,
            data
        );
    }
    ...
    validateOTP(data) {
        return this.http.post(
            `${RadiusConstants.SAVBILL_PRODUCT_MANAGEMENT_BASE_URL}/otp/validate`,
            data
        );
    }
    ...
    changePassword(data) {
        return this.http.put(
            `${RadiusConstants.SAVBILL_API_GATEWAY_COMMON_MANAGEMENT}/staffuser/resetpassword`,
            data
        );
    }
    ...
    SearchResellerName(name) {
        return this.http.get(
            `${RadiusConstants.SAVBILL_PRODUCT_MANAGEMENT_BASE_URL}/getStaffContactByUserName?username=` +
