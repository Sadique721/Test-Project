...
export class ChildManagementService {
  ...
  getAllChild(url, data) {
    return this.http.get(RadiusConstants.SAVBILL_COMMON_BASE_URL + url, data);
  }
  putMethod(url, data) {
    return this.http.put(RadiusConstants.SAVBILL_COMMON_BASE_URL + url, data);
  }
  deleteMethod(url) {
    return this.http.delete(RadiusConstants.SAVBILL_COMMON_BASE_URL + url);
  }
