...
import { type } from "os";
import { SAVBILL_INVENTORY_MANAGEMENT_BASE_URL } from "../RadiusUtils/RadiusConstants";
...
export class InwardService {
    baseUrl = SAVBILL_INVENTORY_MANAGEMENT_BASE_URL;

    mvnoId = localStorage.getItem("mvnoId");
    loggedInUser = localStorage.getItem("loggedInUser");
    constructor(private http: HttpClient) { }

    // private baseUrll = 'http://localhost:30083/api/v1/SavbillInventoryManagement/specificationParameters/getSpecificParametersByid?product_id=1000';
    ...
    downloadInvoice(type: any): any {
        const url = RadiusConstants.SAVBILL_INVENTORY_MANAGEMENT_BASE_URL + `${type}`;
    ...
    getInwardsByGroupId(inwardId) {
        return this.http.get(`${this.baseUrl}/inwards/findByGroupId/${inwardId}`);
    }

    inwardGroupApproval(data) {
        return this.http.put(`${this.baseUrl}/inwards/inwardGroupApproval`, data);
    }
