...
export class RadiusAccessResponseManagementComponent implements OnInit, IDeactivateGuard {
    ...
    accessResponseFormGroup: UntypedFormGroup;
    ...
    @ViewChild('accessResponseDialog') accessResponseDialog!: TemplateRef<any>; dialogRef!: MatDialogRef<any>;
    // this.dialogRef = this.dialog.open(this.accessResponseDialog, {
    //     width: '1000px'
    // });
    constructor(
    ...
    getDisplayedPlanDetailsColumns(): Array<string> {
        if (this.loginService.hasPermission(RADIUS_CONSTANTS.RADIUS_ACCESS_RESPONSE_EDIT) || this.loginService.hasPermission(RADIUS_CONSTANTS.RADIUS_ACCESS_RESPONSE_DELETE)) {
            return ['name', 'action'];
        } else {
            return ['name'];
        }
    ...
    addEditAccessResponseDialog(data?: any) {
        this.isAccessResponseEdit = false;
        this.dialog.open(this.accessResponseDialog, {
            width: '600px',
            data: data || null
        });
    }
            ...
            if (accessResponseId) {
                ...
                this.radiusService.post(url, this.accessResponseFormGroup.value).subscribe(
                    (response: any) => {
                        ...
                        // });
                        this.toastr.success(`Access Response has been Updated successfully.`, 'Success!');
                        this.submitted = false;
                        this.dialog.closeAll();
                        if (this.searchkey) {
                            //   this.searchCountry();
    ...
    getAccessResponseListData(list) {
        ...
        this.radiusService.post(url, plandata).subscribe(
            (response: any) => {
                ...
                this.dataSource = new MatTableDataSource<any>(this.accessResponseListData);
    ...
    //     }
    // }
    deleteConfirmonAccessResponse(item: any) {
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.deleteAccessResponse(item.id)
            } else {
            }
        });
    }
    ...
    pageChangedCountryList(event: any) {
        this.currentPageAccessResponseSlab = event.pageIndex + 1;
        this.accessResponseitemsPerPage = event.pageSize;
        if (this.searchkey) {
            //   this.searchCountry();
        } else {
            this.getAccessResponseListData("");
