...
export class CallDetailsComponent implements OnInit {
    @ViewChild('viewDetailsDialog') viewDetailsDialog: TemplateRef<any>;
    ...
    callDetailsitemsPerPage: any = RadiusConstants.PAGE_SIZE_OPTIONS;
    ...
    openViewModal(row: any): void {
        ...
        if (!uniqueId) {
            this.toastr.info(`Unique ID is missing.`, 'Info!')
        ...
        this.customerManagementService.getMethod(url).subscribe(
            (response: any) => {
                ...
                if (response?.custCallLogDetails?.length) {
                    ...
                    this.currentDialogRef = this.dialog.open(this.viewDetailsDialog, {
                        width: '700px',
                        maxHeight: '90vh',
                        disableClose: false,
                        autoFocus: false
                    ...
                    this.toastr.info(`No call log details found for this entry.`, 'Info!')
                    // this.messageService.add({
                    //     severity: "info",
                    //     summary: "No Data",
            ...
            (error: any) => {
                this.spinner.hide();
                this.toastr.error(`${error.error?.ERROR || "Failed to fetch call log details."}`, 'Failed!')
    ...
    searchCallDetails(phoneNumber: string): void {
        if (!phoneNumber) {
            this.toastr.info(`Please provide a valid phone number before searching.`, 'Info!')
            // this.messageService.add({
            //     severity: "warn",
        ...
        this.customerManagementService.postMethod(url, body).subscribe(
            ...
            (error: any) => {
                ...
                this.spinner.hide();
                this.toastr.error(`${error.error?.ERROR}` || "Unable to fetch call log details!");
