...
export class CustomerTemplateComponent implements OnInit, AfterViewInit {
    ...
    planCreationType() {
        ...
        if (planBindingType === "On-Demand") {
            ...
            error: (error) => {
                this.toastr.error(`${error.error.ERROR}`, 'Staff with multiple BU cannot create Customer!');
            }
    ...
    saveCustomer() {
        ...
        if (this.customerGroupForm.valid) {
            this.custService.postMethod("/customers", this.customerGroupForm.value).subscribe(
                (res: any) => {
                    if (res.status == 200) {
                        this.toastr.success(`${res.responseMessage}`, 'Customer Created Successfully!');


                        this.submitted = false;
                        this.customerGroupForm.reset();
                    } else if (res.status == 406) {
                        this.toastr.error(`${res.responseMessage}`, 'Customer Creation Failed!');
                ...
                (error: any) => {
                    this.toastr.error(`${error.error.ERROR}`, 'Customer Creation Failed!');
            ...
            (error: any) => {
                this.toastr.error(`${error.error.ERROR}`, 'Please fill all the required fields!');
                    ...
                    if (res.responseCode == 200 && res.dataList?.length != 0) {
                            ...
                            this.filteredArray.forEach((el1: any) => {
                                el1.fields.forEach((el2: any) => {
                                    if (el2.fieldType != "object") {
                                        ...
                                        el2.child.forEach((el3: any) => {
                                            if (el2.isMandatory == true) {
                                                childForm[el3.fieldname] = new UntypedFormControl("", [Validators.required]);
                                            } else childForm[el3.fieldname] = new UntypedFormControl("");
                                        });
                        ...
                        this.presentAddressArr[0].fields[0].child.forEach((val: any) => {
                            if (val.isMandatory == true) {
                                presentForm1[val.fieldname] = new UntypedFormControl("", [Validators.required]);
                            } else presentForm1[val.fieldname] = new UntypedFormControl("");
                        });
                        this.permanentAddressArr[0].fields[0].child.forEach((val: any) => {
                            if (val.isMandatory == true) {
                                permanentForm[val.fieldname] = new UntypedFormControl("", [Validators.required]);
                            } else permanentForm[val.fieldname] = new UntypedFormControl("");
                        });
                        this.paymentAddressArr[0].fields[0].child.forEach((val: any) => {
                            if (val.isMandatory == true) {
                                paymentForm[val.fieldname] = new UntypedFormControl("", [Validators.required]);
                            } else paymentForm[val.fieldname] = new UntypedFormControl("");
                        });
