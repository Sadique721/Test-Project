...
export class EnterpriseLeadComponent implements OnInit {
        ...
        this.leadManagementService.getConnection(url).subscribe(
            (response: any) => {
                ...
                if (response.dataList == null) {
                    this.toastr.error(`${response.message}`, 'No staff available to assign..!');
    ...
    createLead() {
        if (!this.createAccess) {
            (error: any) => {
                this.toastr.error(`${error.error.ERROR}`, 'Sorry you have not privilege to create opration!');
            }
        ...
        this.savbillCommonBaseService.get(url).subscribe((response: any) => {
            if (index === "present") {
                ...
                if (response.data.areaList?.length !== 0) {
                    ...
                    this.toastr.info(`Area detals are not available, please select correct pincode`, 'Note!');
        ...
        this.customerManagementService.getMethod(planserviceurl).subscribe((response: any) => {
            ...
            if (planserviceData?.length > 0) {
                ...
                if (this.plantypaSelectData?.length === 0) {
                    this.toastr.info(`Plan not available for this customer type and service `, 'Note!');
    ...
    addEditLead(id: any, custId: any) {
        ...
        if (this.customerGroupForm.valid) {
                    ...
                    .subscribe(
                        async (response: any) => {
                            if (response.status === 200) {
                                ...
                                this.toastr.error(`${response.error.errorMessage}`, 'Failed!');
            ...
            this.scrollToError();
            this.toastr.error(`Fields are Mandatory or Invalid. Please fill or update those field`, 'Required!');
    ...
    getCurrentStaffBUList() {
        ...
        this.leadManagementService.getMethodAPIGatewaytemp(url).subscribe(
            (response: any) => {
    ...
    getEnterpriseLeadList() {
        ...
        const ids = [];
        this.currentStaffBuList.forEach(element => {
            ids.push(element.id);
        });
        const data = {
            buidIdList: ids
        ...
        this.leadManagementService.deleteMethod(url).subscribe(
            async (response: any) => {
                if ((await response.status) === 406) {
                    ...
                    this.toastr.success(`${await response.message}`, 'Successfully!');
                ...
                async (response: any) => {
                    ...
                    if (response.status === 200) {
                        ...
                        if (
                            ...
                            data.id = this.leadId;
                            let planUrl = "/SavbillSalesCrmsBss/leadMaster/verifyPlansWithQuotationApproval";
                            this.customerManagementService.updateMethod(planUrl, data).subscribe(
                                ...
                                (error: any) => {
                                    if (error.status === 400) {
                                        ...
                                        this.toastr.info(`${error.responseMessage}`, 'No Convert Lead To CAF!');
                                        // this.messageService.add({
                                        //     severity: "info",
    ...
    searchLead() {
        ...
        if (!this.searchDeatil) {
            ...
            if (
                ...
                !this.searchkey2 ||
                this.searchkey2 !== this.optionValue.trim()
            ) {
                this.currentPageLeadListdata = 1;
            }
            this.searchkey = this.searchDeatil;
            ...
            this.searchData.filters[0].filterColumn = this.optionValue.trim();
    ...
    approveOrRejectLeadPopup(lead, flag) {
        if (lead.finalApproved) {
            if (flag === "Reject") {
                ...
                this.toastr.success(`Assigned to the next staff`, 'Successfully!');
                ...
                async (response: any) => {
                    ...
                    if ((await response.dataList) && (await response.dataList.length) > 0) {
                        ...
                        if (this.leadApproveRejectDto.approveRequest) {
                            ...
                            this.toastr.success(`Rejected Successfully`, 'Successfully!');
    ...
    getStaffUsersFromLeadMaster() {
        ...
        this.leadManagementService.getMethod(url).subscribe(
            ...
            (error: any) => {
                console.log("Backend response from /leadMaster/findAll/StaffUser api => ", error);
            }
        );
    }
    ...
    getPreviousVendors() {
        ...
        this.leadManagementService.getMethod(url).subscribe(
            ...
            (error: any) => {
                console.log("An error occurred while fetching the list of previous vendors => ", error);
                this.toastr.error(`${error.error.ERROR}`, 'Failed');
    ...
    viewLeadStatusPopupOpen(id) {
        ...
        this.leadManagementService.getMethod(urlLead).subscribe(
            async (response: any) => {
                if ((await response.status) === 200) {
                    ...
                    if (!this.leadMasterObj?.buId) {
                        this.toastr.info(`buid is not present for this request`, 'buid is required!');
                    ...
                    if (this.leadMasterObj?.nextTeamMappingId) {
                        this.leadManagementService.getMethodForSavbillApi(url).subscribe(
                            ...
                            (error: any) => {
                                console.log("An error occurred while fetching the list of lead status => ", error);
                                this.toastr.error(`${error.error.errorMessage}`, 'Failed!');

                            }
                        );
                    } else {
                        this.toastr.info(`Workflow will be started after first approval. So, please approve first and then check status.`, 'Approval configure!');
            ...
            (error: any) => {
                console.log("An error occurred while fetching the lead  => ", error);
                this.toastr.error(`${error.error.errorMessage}`, 'Failed!');
            ...
            this.confirmationService.confirm({
                ...
                accept: () => {
                    ...
                    this.leadManagementService.getMethod(url).subscribe(
                        ...
                        (error: any) => {
                            this.toastr.error(`${error.error.ERROR}`, "Something went wrong while reopening the lead!! + error");
    ...
    leadToCAFConversion() {
        ...
        if (this.customerGroupForm.valid && this.presentGroupForm.valid) {
            if (
                ...
                this.scrollToError();
                this.toastr.error(`Minimum one Plan Details need to add`, 'Required!');

            }
        } else {
            this.scrollToError();
            this.toastr.error(`Fields are Mandatory or Invalid. Please fill or update those field`, 'Failed!');
    ...
    leadToCaf(url, data) {
        this.customerManagementService.postMethod(url, data).subscribe(
            ...
            (error: any) => {
                console.log(error, " error while leadToCAFConversion() function calling!!");
                this.addressListData = [];
                this.toastr.error(`${error.error.ERROR}`, 'Failed!');
            ...
            this.customerManagementService.getMethod(url).subscribe(
                async (response: any) => {
                    ...
                    if (response.responseCode == 417) {
                        ...
                        this.toastr.error(`Assigned to the next staff successfully`, 'Successfully');
    ...
    generateLeadNo() {
        ...
        this.leadManagementService.getMethod(url).subscribe(
            ...
            async (error: any) => {
                console.log("Generated lead no error => ", await error);
                this.toastr.error(`${error.error.ERROR}`, "Something went wrong with 'Lead No.' Generation!");
    ...
    makeACall() {
        this.toastr.info(`Sorry! Please configure call client first..`, 'Call configure!');
        ...
        if (leadId) {
            ...
            this.leadManagementService.getMethod(url).subscribe(
                ...
                (error: any) => {
                    ...
                    this.toastr.error(`Page Not Found`, 'Something went wrong!!');
                }
            );
        }
    }
    ...
    async modalOpenextingCustomer() {
        $("#selectextingCustomer").modal("show");
        this.newFirst = 1;
    ...
    modalCloseextingCustomer() {
        ...
        $("#selectextingCustomer").modal("hide");
