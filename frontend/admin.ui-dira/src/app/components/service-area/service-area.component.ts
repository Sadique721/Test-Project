...
export class ServiceAreaComponent implements OnInit {
    ...
    staffAllData: any = [];
    planAllData: any = [];
    bulkAssignForm: UntypedFormGroup;
    bulkSAStaffList: any = [];
    polygonList: any[] = [];
    showPolygonNameDialog: boolean = false;
    polygonNameForm: UntypedFormGroup;
    currentPolygonCoordinates: any[] = [];

    // after change code
    serviceAreatotalRecords: any;
    currentPageServiceAreaSlab = 1;
    ...
    customValidator(formGroup: UntypedFormGroup) {
    ...
    TotalItemPerPage(event) {
        ...
        if (!this.searchkey) {
            this.getserviceAreaListData(this.showItemPerPage);
        } else {
            this.searchserviceArea();
        }
    }

    getserviceAreaListData(list) {
        ...
        let size;
        this.searchkey = "";
        let pageList = this.currentPageserviceAreaListdata;
        if (list) {
            size = list;
            this.serviceAreaitemPerpage = list;
        } else {
            size = this.serviceAreaitemPerpage;
        }
        let servicearedata = {
            page: pageList,
            pageSize: size
        ...
        this.serviceAreaService.postMethod(url, servicearedata).subscribe(
            (response: any) => {
                ...
                this.dataSource = new MatTableDataSource<any>(this.serviceAreaListData);
                this.serviceAreatotalRecords = response.totalRecords;
                this.searchkey = "";
    ...
    addEditserviceArea(serviceAreaId): void {
        ...
        if (this.serviceAreaGroupForm.valid) {
            if (serviceAreaId) {
                ...
                //     selected: selectedPincodes.includes(pincode.id)
                // }));
                // this.serviceAreaGroupForm.patchValue({ siteName: this.serviceAreaGroupForm.value.name });
                this.createserviceAreaData = this.serviceAreaGroupForm.value;
    ...
    editserviceArea(serviceAreaId: any): void {
        ...
        //         this.isserviceAreaEdit = !this.isViewServiceArea;
        ...
        this.serviceAreaService.getMethod(url).subscribe(
            (response: any) => {
                ...
                this.serviceAreaGroupForm.patchValue({
                    ...
                    mvnoIds: Array.isArray(data.mvnoIds) ? data.mvnoIds : [],
                    locationIds: Array.isArray(data.locationIds) ? data.locationIds : []
                });

                // Patch Multi-select fields
                this.serviceAreaGroupForm.patchValue({
                    pincodes: Array.isArray(data.pincodes) ? data.pincodes : [],
                    mvnoIds: Array.isArray(data.mvnoIds) ? data.mvnoIds : [],
                    locationIds: Array.isArray(data.locationIds) ? data.locationIds : []
                ...
                // Patch polygon list if available
                if (data.polyGoneList && data.polyGoneList.length > 0) {
                    const filteredPolyGoneList = data.polyGoneList.map(coord => ({
                        lat: Number(coord.lat),
                        lng: Number(coord.lng),
                        polyOrder: Number(coord.polyOrder),
                        polygoneName: coord.polygoneName
                    ...
                    // this.drawingManager.setDr(null);
                    if (data.siteName) {
                        this.getPolygonListBySiteName(data.siteName);
                    }
                }

                // Patch ISP / location options for multiselect dropdowns
                this.pincodeOptions = (this.pincodeDetail || []).map(pin => ({
                    pincode: pin.pincode,
                    id: pin.id,
                    selected: (data.pincodes || []).includes(pin.id)
                }));

                this.ispListData = (this.ispListData || []).map(isp => ({
                    id: isp.id,
                    name: isp.name,
                    selected: (data.mvnoIds || []).includes(isp.id)
                }));

                this.locationDetail = (this.locationDetail || []).map(loc => ({
                    id: loc.id,
                    name: loc.name,
                    selected: (data.locationIds || []).includes(loc.id)
                }));
                this.viewDataServiceArea = this.serviceAreaGroupForm.value;
                this.openServiceEDitDialog(this.serviceAreaGroupForm, serviceAreaId);
    ...
    openServiceEDitDialog(data: any, serviceAreaId: any) {
        const dialogRef = this.dialog.open(AddEditServiceAreaManagmentComponent, {
            width: '1000px',
            data: {
                title: 'Update ' + this.serviceAreaTitle,
                yesLabel: 'Update',
                noLabel: 'Cancel',
                createAcS: this.createAccess,
                editAcs: this.editAccess,
                createView: this.createAccess,
                isServiceAreaEdit: true,
                isViewService: this.isViewServiceArea,
                viewAcs: this.viewAccess,
                polygonAccs: this.polygonCreateAccess,
                fomrGrp: data,
                cityListDatas: this.cityListData,
                pincodeListDatas: this.pincodeListData,
                locationDataByPlan: this.locationDataByPlan,
                siteNameListData: this.siteNameListData,
                polygonViewAcs: true,
                ifsearchLocationModal: true,
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.serviceAreaGroupForm = result;
                this.addEditserviceArea(serviceAreaId);
            } else {
            }
        });
    }
    ...
    deleteConfirmonserviceAreaDialog(serviceArea: any) {
        this.customerrMyInventoryView = false;
        this.assignInventoryWithSerial = false;
        // if (serviceArea) {
        //     this.confirmationService.confirm({
        ...
        const dialogRef = this.dialog.open(DeleteConfirmationDialogBoxComponent, {
            ...
            data: {
                ...
                description: `Are you sure you want to delete "${serviceArea.name}"?`,
                yesLabel: 'Delete',
                noLabel: 'Cancel'
            }
        });
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.deleteserviceArea(serviceArea.id);
            } else {
                reject: () => {
                    ...
                    //     severity: "info",
                    //     summary: "Rejected",
                    //     detail: "You have Cancel to Delete."
                    // });
                }
            }
        });
    ...
    pageChangedserviceAreaList(pageNumber) {
        ...
        this.serviceAreaitemPerpage = pageNumber.pageSize;
        if (!this.searchkey) {
            this.getserviceAreaListData("");
        } else {
            this.searchserviceArea();
    ...
    searchserviceArea() {
        if (!this.searchkey || this.searchkey !== this.searchName) {
            this.currentPageServiceAreaSlab = 1;
        }
        this.searchkey = this.searchName;
        if (this.showItemPerPage) {
            this.serviceAreaitemPerpage = this.showItemPerPage;
        }
        this.searchObject.filters[0].filterValue = this.searchName.trim();

        this.searchObject.page = this.currentPageServiceAreaSlab;
        this.searchObject.pageSize = this.serviceAreaitemPerpage;
        ...
        this.serviceAreaService.postMethod(url, this.searchObject).subscribe(
            (response: any) => {
                ...
                this.serviceAreaListData = response.dataList || [];
                this.serviceAreatotalRecords = response.totalRecords || 0;
            ...
            (error: any) => {
                this.serviceAreatotalRecords = 0;
                this.toastr.error(`${error.error.ERROR}`, 'Failed!')
                // this.messageService.add({
                //     severity: "error",
                //     summary: "Error",
    ...
    openSearchModel() {
        ...
        if (this.ifsearchLocationModal) {
            this.dialogRef = this.dialog.open(this.searchLocationDialog, {
                width: '800px',
                height: 'auto',
                autoFocus: false,
                disableClose: true
    ...
    private resetForm() {
        ...
        Object.keys(this.searchLocationForm.controls).forEach(key => {
            const control = this.searchLocationForm.get(key);
            if (control) {
    ...
    getCustomerAssignedList(id): void {
        ...
        this.inwardService.getByOwnerId(data).subscribe(
            (res: any) => {
                ...
                this.inventoryDataSource = new MatTableDataSource<any>(this.assignedInventoryList);
                // this.serviceAreatotalRecords = res.totalRecords;
                this.searchkey = "";
            },
            (error: any) => {
                this.toastr.error(`${error.error.msg}`, 'Failed!')
        ...
        this.serviceAreaService.getMethod(url).subscribe(
            (response: any) => {
                ...
                if (this.pincodeDetail.length > 0) {
                    ...
                    this.pincodeOptions = [];
                    this.toastr.info(`"No " + ${this.pincodeTitle} + " found." `, 'Info!')
                    // this.messageService.add({
                    //     severity: "info",
                    //     summary: "Info",
    ...
    getLocationDetailByServiceArea(selServiceAreaId: number) {
        ...
        this.serviceAreaService.getMethod(url).subscribe(
            ...
            (error: any) => {
                this.toastr.error(`${error.error.message || "An error occurred while fetching location details"}`, 'Failed!')
                // this.messageService.add({
                //     severity: "error",
                //     summary: "Error",
    ...
    getAllLocation() {
        this.locationService.getAllActiveLocation().subscribe((response: any) => {
            this.locationDataByPlan = response.locationMasterList
                .map(location => ({
                    name: location.name,
                    locationMasterId: location.locationMasterId
                }));
    ...
    //     }
    //     this.dialogRef = this.dialog.open(this.mapDialog, {
    //         width: '1000px',
    //         maxWidth: '100vw',
    //         height: 'auto',
    ...
    viewEditServiceArea(serviceAreaId, isServiceAreaView) {
        ...
        this.serviceAreaService.getMethod(url).subscribe(
            (response: any) => {
                ...
                this.viewDataServiceArea = this.serviceAreaGroupForm.value;
                this.openServiceViewDialog(this.serviceAreaGroupForm, serviceAreaId);
        ...
        if (event.target.files.length > 0) {
            ...
            if (this.selectedFile) {
                ...
                if (this.isValidCSVFile(file)) {
                    ...
                    this.selectedFileUploadPreview = [];
                    this.toastr.info(`Please upload a valid .csv or .kml file`, 'Info!')
                    // this.messageService.add({
                    //     severity: "info",
                    //     summary: "Info",
    ...
    @ViewChild('bulkAssignDialog') bulkAssignDialog!: TemplateRef<any>;
    @ViewChild('PolygonNameDialog') PolygonNameDialog!: TemplateRef<any>;
    ...
    bulkAssignViewData(data) {
        ...
        this.dialogRef = this.dialog.open(this.bulkAssignDialog, {
            width: '50%',
            height: 'auto',
            autoFocus: false,
            disableClose: true
    ...
    openPolygonNameDialog() {
        this.dialogRef = this.dialog.open(this.PolygonNameDialog, {
            width: '50%',
            height: 'auto',
            disableClose: true
        });
    ...
    bulkAssignToStaff() {
        ...
        this.serviceAreaService.postMethod(url, obj).subscribe(
            (response: any) => {
                this.dialogRef.close();
                this.toastr.success(`Asssigned Successfully`, 'Success!')
                // this.messageService.add({
                //     severity: "success",
                //     summary: "Success",
                //     detail: response.message,
    ...
    displayedColumns = ['id', 'Name', 'Lat/Long', 'Status', 'Action'];
    displayedInventoryListColumns = ['ProductName', 'Mac', 'SerialNumber', 'AssignedQty.',
        'Status', 'NextApprover', 'AssignedDate', 'ExpiryDate', 'Action'];
    ...
    onSubmit() {
        ...
        if (this.serviceAreaGroupForm.invalid) {
            return;
        }

        if (this.isserviceAreaEdit) {
            this.addEditserviceArea(this.viewserviceAreaListData.id);
        } else {
            this.addEditserviceArea('');
        }
    }
    @ViewChild(CommonInventoryManagementComponent) commonComp!: CommonInventoryManagementComponent;
    opnAssignInventory() {
        this.commonComp.openDialog({ type: 'assignInventory', data: this.inventoryServiceAreaData });
    }

    addEditServiceAreaDialog() {
        const dialogRef = this.dialog.open(AddEditServiceAreaManagmentComponent, {
            width: '1000px',
            data: {
                createView: this.createView,
                isServiceAreaEdit: false,
                isViewService: false,
                title: 'Create ' + this.serviceAreaTitle,
                createAcS: true,
                editAcs: false,
                viewAcs: false,
                loggedinUsermvnoid: this.loggedInUserMvnoId,
                polygonViewAcs: this.polygonViewAccess,
                serviceAreaTyp: this.serviceAreaType,
                yesLabel: 'Create',
                noLabel: 'Cancel',
                cityListDatas: this.cityListData,
                pincodeListDatas: this.pincodeListData,
                ...
                assignInventoryWithSerial: false,
                ifsearchLocationModal: true,
                currentPagesearchLocationList: 1,
                polygonCreateAccess: this.polygonCreateAccess,
                mArea: this.mArea,
                isClearShow: this.isClearShow,
                showPolygonNameDialog: this.showPolygonNameDialog,
                polygonDeleteAccess: this.polygonDeleteAccess
            }
        });
    ...
    @ViewChild('viewserviceAreaDialog') viewserviceAreaDialog!: TemplateRef<any>;
    dialogRefview!: MatDialogRef<any>;

    openServiceViewDialog(data: any, serviceAreaId: any) {

        this.viewDataService = data.value;

        const city = this.cityListData.find(c => c.id === data.value.cityid);
        this.cityName = city.name;


        this.dialogRefview = this.dialog.open(this.viewserviceAreaDialog, {
            width: '1000px',
            maxWidth: '90vw',
            height: 'auto',
            autoFocus: false,
        ...
        this.dialogRefview.afterClosed().subscribe(() => {
