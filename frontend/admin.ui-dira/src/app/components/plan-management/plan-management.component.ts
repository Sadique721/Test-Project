...
export class PlanManagementComponent implements OnInit {
    ...
    qospolicyformArray: UntypedFormArray;
    qosPolicyGatewayMappingListFormArray: UntypedFormArray;
    ...
    chargefromgroup: UntypedFormGroup;
    qospolicyformgroup: UntypedFormGroup;
    ...
    casmasterData: any;
    casPackageData: any;
    planCasMappingFromGroup: UntypedFormGroup;
    planCasMappingFromArray: UntypedFormArray;
    planCasMappingSubmitted = false;

    serviceParamFromGroup: UntypedFormGroup;
    serviceParamFromArray: UntypedFormArray;
    ...
    cities: any = [];
    exampleGroupForm!: UntypedFormGroup;
    ...
    private changeActualPriceSubject = new Subject<{
        price: any;
        id: any;
        index: any;
        actualprice: any;
    ...
    changeActualPriceSubjectSubscription: Subscription;
    ...
    getDisplayedPlanDetailsColumns(): Array<string> {
        if (this.loginService.hasPermission(PRODUCTS.PLAN_EDIT)) {
            return ["id",
                "displayName",
                "plantype",
                "validity",
    ...
    ngOnInit(): void {
        ...
        this.changeActualPriceSubjectSubscription = this.changeActualPriceSubject
            .pipe(debounceTime(1000))
            .subscribe(({ price, id, index, actualprice, event }) => {
                this._changeActualPrice(price, id, index, actualprice, event);
            });

        if (this.billingCycleHide) {
            this.displayedColumns = this.displayedColumns.filter(c => c !== 'billingCycle');
        } else {
            if (!this.displayedColumns.includes('billingCycle')) {
                this.displayedColumns.push('billingCycle');
            }
        }
        this.updateDisplayedColumnsCharge();
    }

    get step1Group(): FormGroup {
        return this.planGroupForm.pick(['name', 'displayName', 'code', 'plantype', 'category', 'mode', 'planGroup', 'serviceId', 'serviceAreaIds', 'accessibility', 'startDate', 'endDate', 'validity', 'unitsOfValidity', 'allowdiscount', 'invoiceToOrg', 'requiredApproval', 'allowOverUsage', 'maxconcurrentsession', 'templateId', 'useQuota', 'chunk', 'desc', 'addonToBase']);
    }

    get step2Group(): FormGroup {
        return this.planGroupForm.pick(['quotatype', 'qospolicyid', 'quotaUnit', 'quota', 'quotaunittime', 'quotatime', 'quotaResetInterval', 'usageQuotaType', 'saccode', 'timebasepolicyId', 'param1', 'param2', 'param3']);
    }
    ...
    getTemplateData() {
        let url = "/billTempleteByType/Billing";
    ...
    getPostoaidPlan(list) {
        ...
        this.planManagementService.postMethod(url, plandata).subscribe(
            (response: any) => {
                ...
                this.dataSource = new MatTableDataSource<any>(this.postPaidPlanList);
    ...
    onAddCasMappingField(selectedCasId: any): void {
        if (!this.planCasMappingFromArray.value.some(value => value.casId == selectedCasId)) {
            ...
            if (this.planCasMappingFromGroup.valid) {
                ...
                this.casMappimgTable.renderRows();
            }
        } else {
            (error: any) => {

                this.toastr.error(`${error.error.ERROR}`, 'CAS already added!');
            }
        }
    }
    ...
    onAddChargeField() {
        ...
        if (this.planGroupForm.value.serviceId) {
            ...
            if (this.chargefromgroup.valid) {
                ...
                this.chargeTable.renderRows();
                this.chargeSubmitted = false;
            } else {
            }
        } else {
            (error: any) => {
                this.toastr.error(`${error.error.ERROR}`, 'Service Field Required!');
            }
        }
    }

    createQosPolicyFormGroup(): UntypedFormGroup {
    ...
    onAddQosPolicy() {
        if (this.planGroupForm.value.qospolicyid) {
            ...
            if (this.qospolicyformgroup.valid) {
                ...
                this.qospolicyTable.renderRows();
            }
        } else {
            (error: any) => {
                this.toastr.error(`${error.error.ERROR}`, 'Qos Policy Field Required!');
    ...
    //     const hasColumn = this.displayedColumnsCharge.includes('billingCycle');

    //     if (this.billingCycleHide && hasColumn) {
    //         // Remove column if hidden
    //         this.displayedColumnsCharge = this.displayedColumnsCharge.filter(
    //             c => c !== 'billingCycle'
    //         );
    //     } else if (!this.billingCycleHide && !hasColumn) {
    //         // Add column if visible (choose position if needed)
    //         const index = this.displayedColumnsCharge.indexOf('amount') + 1; // insert after 'amount', example
    //         this.displayedColumnsCharge.splice(index, 0, 'billingCycle');
    //         this.displayedColumnsCharge = [...this.displayedColumnsCharge]; // trigger change detection
    //     }
    // }
    plantypeEvntData(_event) {
        ...
        if ("Prepaid" == _event.value) {
            ...
            // ✅ Hide Billing Cycle column for Prepaid
            this.billingCycleHide = true;
            this.updateDisplayedColumnsCharge();
        }
        ...
        if ("Postpaid" == _event.value) {
            ...
            // ✅ Show Billing Cycle column for Postpaid
            this.billingCycleHide = false;
            this.updateDisplayedColumnsCharge();
        }
    ...
    updateDisplayedColumnsCharge() {
        const hasBilling = this.displayedColumnsCharge.includes('billingCycle');

        if (this.billingCycleHide && hasBilling) {
            // Remove column if hiding
            this.displayedColumnsCharge = this.displayedColumnsCharge.filter(
                col => col !== 'billingCycle'
            );
        } else if (!this.billingCycleHide && !hasBilling) {
            // Add column back if showing
            const index = this.displayedColumnsCharge.indexOf('chargeName') + 1;
            this.displayedColumnsCharge.splice(index, 0, 'billingCycle');
            this.displayedColumnsCharge = [...this.displayedColumnsCharge];
        }
    }

    addEditPostPaidPlan(postPaidPlanId) {
        Object.keys(this.planGroupForm.controls).forEach(field => {
            const control = this.planGroupForm.get(field);
        ...
        if (this.planGroupForm.valid) {
            if (postPaidPlanId) {
                ...
                if (this.planGroupForm.value.chargeList.length > 0) {
                    ...
                    (error: any) => {
                        this.toastr.info(`${error.message}`, 'Minimum one Charge Details need to add!');
                    }
                ...
                if (this.planGroupForm.value.chargeList.length > 0) {
                    ...
                    (error: any) => {
                        this.toastr.error(`${error.error.ERROR}`, 'Minimum one Charge Details need to add!');
            ...
            (error: any) => {
                this.toastr.error(`${error.error.ERROR}`, 'Please enter all required data to proceed!');
    ...
    canExit() {
            ...
            return true;
        {
            return Observable.create((observer: Observer<boolean>) => {
                ...
                //     }
                // });
                const dialogRef = this.dialog.open(this.confirmChargeDialog, {
                    width: '400px',
                    data: {
                        title: 'Alert',
    ...
    @ViewChild('confirmChargeDialog') confirmChargeDialog!: TemplateRef<any>;
        ...
        if (qosPolicyFieldIndex || qosPolicyFieldIndex === 0) {
            const dialogRef = this.dialog.open(this.confirmChargeDialog, {
                ...
                data: {
                    ...
                    description: `Do you want to delete this QoS Policy?`,
                    yesLabel: 'Delete',
                    noLabel: 'Cancel'
                }
            });
    ...
    deleteConfirmonCasMappingField(index: number) {
        const dialogRef = this.dialog.open(this.confirmChargeDialog, {
            ...
            data: {
                ...
                description: `Do you want to delete this CAS?`,
                yesLabel: 'Delete',
                noLabel: 'Cancel'
            }
        });
    ...
    deleteConfirmonPostPaidPlan(planId) {
        if (planId) {
            const dialogRef = this.dialog.open(this.confirmChargeDialog, {
                ...
                data: {
                    title: 'Delete Confirmation',
                    description: `Do you want to delete this Post Paid Plan?`,
                    yesLabel: 'Delete',
                    noLabel: 'Cancel'
                }
            });
    ...
    getplanDetailById(planId) {
        ...
        this.planManagementService.getMethod(url).subscribe(
            (response: any) => {
                ...
                this.productDeatiltotalRecords = this.planDetailData.productplanmappingList.length
                this.qosPolicyDeatiltotalRecords = this.planDetailData.planQosMappingEntityList.length

                this.chargeDeatiltotalRecords = this.planDetailData.chargeList.length
    ...
    pageChangedPlanList(pageNumber) {
        this.currentPagePostPaidPlan = pageNumber.pageIndex + 1;
        this.postPaidPlanitemsPerPage = pageNumber.pageSize;
    ...
    @ViewChild('qosPolicyDialog') qosPolicyDialog!: TemplateRef<any>;
    getQosPolicyById(qosPolicyId) {

        this.dialog.open(this.qosPolicyDialog, {
            width: '70%',
            disableClose: true // same as data-backdrop="static" data-keyboard="false"
        });
    ...
    getTimeBasePolicyById(getTimeBasePolicyById) {
        const url = "/timebasepolicy/" + getTimeBasePolicyById;
        this.planManagementService.getMethod(url).subscribe(
            (response: any) => {
    ...
    changeActualPrice(price, id, index, actualprice, event: KeyboardEvent) {
        this.changeActualPriceSubject.next({ price, id, index, actualprice, event });
    }

    _changeActualPrice(price, id, index, actualprice, event: KeyboardEvent) {
    ...
    chargeFilterData(id) {
        this.advanceListData = [];
        let ListData: any = [];
        const url1 = "/charge2/" + id + "?service=" + id;
    ...
    approvePlanOpen(planId, nextApproverId) {
        this.dialog.open(this.approverDialog, {
            width: '50%',
            disableClose: true // same as data-backdrop="static" data-keyboard="false"
        });
        this.approved = false;
    ...
    assignToStaff(flag, dialogRef) {
        ...
        if (flag == true) {
            if (this.selectStaff) {
                url = `/teamHierarchy/assignFromStaffList?entityId=${this.assignPlanID}&eventName=${"PLAN"}&nextAssignStaff=${this.selectStaff?.id}&isApproveRequest=${flag}`;
            } else {
        ...
        if (this.assignPlanForm.valid) {
            ...
            this.planManagementService.updateMethod(url, assignCAFData).subscribe(
                (response: any) => {
                    ...
                    this.toastr.success(`Plan Approved`, 'success!');
        ...
        if (this.rejectPlanForm.valid) {
            ...
            this.planManagementService.updateMethod(url, assignCAFData).subscribe(
                (response: any) => {
                    ...
                    dialogRef.close();
                    this.toastr.success(`Plan is Rejected`, 'Success!');
                    this.searchPlan("");
    ...
    ChangeStatus(dialogRef: any) {
        let Data = {};

        let url =
            "/Plan/changeStatus?id=" + this.statusChangePlanID + "&status=" + this.changeStatusValue;
        this.planManagementService.updateMethod(url, Data).subscribe(
            (response: any) => {
                this.changeStatusFlag = false;
                this.changeStatusValue = "";
    ...
    createPlanProductFormGroup(): UntypedFormGroup {
        return this.fb.group({
    ...
    onAddProductField() {
        ...
        if (this.planProductfromgroup.valid) {
            ...
            this.productPlanMappingTable.renderRows();
        }
    }
    ...
    pageChangedAuditList(pageNumber) {
        this.currentPageAuditSlab1 = pageNumber.pageIndex + 1;
        this.AudititemsPerPage1 = pageNumber.pageSize;
                        ...
                        if (this.fieldsArr.length >= 1) {
                            this.fieldsArr.forEach((el2: any) => {
                                if (el2.mandatoryFlag) {
                                    this.form[el2.fieldname] = new UntypedFormControl("", [Validators.required]);
                                } else this.form[el2.fieldname] = new UntypedFormControl("");
                            });
                        }
    ...
    @ViewChild('reasiignDialog') reasiignDialog!: TemplateRef<any>;
    StaffReasignList(data) {
        this.dialog.open(this.reasiignDialog, {
            width: '50%',
            disableClose: true // same as data-backdrop="static" data-keyboard="false"
        });
    ...
    ngOnDestroy() {
        this.changeActualPriceSubjectSubscription.unsubscribe();
    }


    @ViewChild('PlantdetailsDialog') PlantdetailsDialog!: TemplateRef<any>;
    ...
    openPlantdetailsDialog(plan: any): void {
        this.planDetail(plan.id)
        this.dialog.open(this.PlantdetailsDialog, {
            width: '80%',
            disableClose: true // same as data-backdrop="static" data-keyboard="false"
        });
    }
    ...
    @ViewChild('serviceDetailsTable') serviceDetailsTable!: MatTable<any>;
    displayedserviceDetails: string[] = ['label', 'name'];

    openservicePlanDialog(): void {
    ...
    @ViewChild('chargeTable') chargeTable!: MatTable<any>;
    billingCycleHide = true;
    ...
    @ViewChild('approvableStaffTable') approvableStaffTable!: MatTable<any>;
    displayApprovableStaff: string[] = ['select', 'fullName', 'username']


    @ViewChild('casMappimgTable') casMappimgTable!: MatTable<any>;
    displayCasMapping: string[] = ['cas', 'package', 'delete']

    @ViewChild('qospolicyTable') qospolicyTable!: MatTable<any>;
    displayqospolicy: string[] = ['frompercentage', 'topercentage', 'qosid', 'delete']

    @ViewChild('productPlanMappingTable') productPlanMappingTable!: MatTable<any>;
    displayProductMapping: string[] = [
        'name',
        'productCategory',
        'product',
        'productType',
    ...
    displayedplanQosMappingView: string[] = ['from', 'to', 'qospolicy']
    displayedplanChargeView: string[] = [
        'name',
        'chargetype',
        'price',
        'actualprice',
    ...
    displayedProductPlanMappingView: string[] = [
        'name',
        'productCategoryName',
        'productName',
        'product_type',
    ...
    workflowDisplayedColumns: string[] = [
        'planName',
        'action',
        'actionByName',
        'remark',
    ...
    displayedstaffRejectColumns = ['select', 'fullName', 'username'];


    displayedQOSpolicyView: string[] = ['name', 'policyName', 'basePolicyName', ' description']
}
