...
export class ManageBalanceComponent implements OnInit {
    ...
    partnerName: any;
    ifRedirectManageBalance: boolean = false;
    AclClassConstants;
    AclConstants;
    ...
    assignPartnerBalanceForm: UntypedFormGroup;
    rejectPartnerBalanceForm: UntypedFormGroup;
    ...
    constructor(
        ...
        public paymentGatewayConfigService: PaymentGatewayConfigurationService, private dialog: MatDialog,
    ...
    get step1Group(): FormGroup {
        return this.manageBalanceGroupForm.pick(['partnerType', 'partnerId', 'operation', 'mode', 'transfer', 'onlinesource', 'chequeno', 'chequedate', 'bankManagement', 'destinationBank', 'branch', 'referenceno',]);
    }
    ...
    getOpetationType(event) {
        ...
        if (operationType == "AddBalance") {
            ...
            this.ifwithdrawalCommision = false;
            this.balanceTitle = 'Transfer Balance'
            this.clearAddData();
            this.clearWithdrawalCommission();
            ...
            this.balanceTitle = 'Commission Amount'
            this.clearTransferData();
            this.clearAddData();
    ...
    onKeyAmoutTransfer(e: any) {
        if (this.manageBalanceGroupForm.value.transfer == "BalanceToCommission") {
            if (
                ...
                this.toastr.info(`You have rejected the transfer because the value of the amount greater than current balance`, 'Info!');
                // this.messageService.add({
                //     severity: "info",
                //     summary: "Rejected",
                //     detail:
            ...
            if (
                ...
                this.toastr.info(`You have rejected the transfer because the value of the amount greater than commission`, 'Info!');
                // this.messageService.add({
                //     severity: "info",
                //     summary: "Rejected",
                //     detail:
    ...
    cashModeADDBalance(data) {
        if (data.balance != 0 || data.credit != 0) {
            ...
            this.toastr.info(`You have rejected the transfer because the value of the Appliced Balance is Zero`, 'Info!');
            // this.messageService.add({
            //     severity: "info",
            //     summary: "Rejected",
        ...
        if (data.balance != 0 || data.credit != 0) {
            if (data.paymentMode == "ONLINE" && data.onlinesource == "PHONEPE") {
                ...
                if (config) {
                    ...
                    this.toastr.info(`Payment GateWay Not Configured`, 'Info!');
                    // this.messageService.add({
                    //     severity: "info",
                    //     summary: "Info",
    ...
    trasfetBalance(data) {
        if (data.amount != 0) {
            ...
            this.toastr.info(`You have rejected the transfer because the value of the Transfer Amount is Zero`, 'Info!');
            // this.messageService.add({
            //     severity: "info",
            //     summary: "Rejected",
    ...
    getInvoicePendingApprovals(list): void {
        ...
        const url = `/getAllPartnerBalance`;
        const custerlist = {
            page,
            pageSize: size,
        };
    ...
    pageChangedForInvoiceApprovals(pageNumber): void {
        ...
        this.invoiceListdataitemsPerPage = pageNumber.pageSize;
        this.getInvoicePendingApprovals("");
    }
    ...
    rejectPartnerBalanceOpen(partnerBalanceId, nextApproverId) {
        ...
        this.dialog.open(this.rejectPartnerBalanceDialog, {
            width: '450px',
    ...
    approvePartnerBalanceOpen(partnerBalanceId, nextApproverId) {
        ...
        this.assignPartnerBalanceID = partnerBalanceId;
        this.nextApproverId = nextApproverId;
        this.dialog.open(this.approvePartnerBalanceDialog, {
            width: '450px',
            disableClose: true // same as data-backdrop="static" data-keyboard="false"
        });
    ...
    assignPartnerBalance(dialogRef) {
    ...
    rejectPartnerBalance(dialogRef) {
    ...
    assignToStaff(flag, dialogRef: any) {
        let url: any;
        if (!this.selectStaff && !this.selectStaffReject) {
            url = `/teamHierarchy/assignEveryStaff?entityId=${this.assignPartnerBalanceID
                }&eventName=${"PARTNER_BALANCE"}&isApproveRequest=${flag}`;
        } else {
            if (flag == true) {
                url = `/teamHierarchy/assignFromStaffList?entityId=${this.assignPartnerBalanceID
                    }&eventName=${"PARTNER_BALANCE"}&nextAssignStaff=${this.selectStaff
                    }&isApproveRequest=${flag}`;
            } else {
                url = `/teamHierarchy/assignFromStaffList?entityId=${this.assignPartnerBalanceID
                    }&eventName=${"PARTNER_BALANCE"}&nextAssignStaff=${this.selectStaffReject
                    }&isApproveRequest=${flag}`;
            }
        }
    ...
    @ViewChild('approvePartnerBalanceDialog') approvePartnerBalanceDialog!: TemplateRef<any>;

    @ViewChild('rejectPartnerBalanceDialog') rejectPartnerBalanceDialog!: TemplateRef<any>;

    @ViewChild('approveCustomerDialog') approveCustomerDialog!: TemplateRef<any>;

    @ViewChild('selectStaffTable') selectStaffTable!: MatTable<any>;

    @ViewChild('rejectTable') rejectTable!: MatTable<any>;
    @ViewChild('approveTable') approveTable!: MatTable<any>;
    @ViewChild('invoiceTable') invoiceTable!: MatTable<any>;

    approveTableDisplay = ['select', 'fullName', 'username']
    rejectTableDisplyName = ['select', 'fullName', 'username']
    displayedColumns = ['id', 'partnerName', 'transcategory', 'paymentmode', 'amount', 'paymentdate', 'status', 'action']
}
