...
export class ChangePlanComponent implements OnInit {
    ...
    changePlanForm = new UntypedFormGroup({
        planId: new UntypedFormControl('', [Validators.required]),
        advRenewal: new UntypedFormControl(null),
        isIncludeTax: new UntypedFormControl(false),
        isPriceOverride: new UntypedFormControl(false),
        sellPrice: new UntypedFormControl(''),
        planPurchaseType: new UntypedFormControl('', [Validators.required]),
        skip: new UntypedFormControl(),
        remarks: new UntypedFormControl('', [Validators.required]),
        isPaymentReceived: new UntypedFormControl('no', [Validators.required]),
        addonStartDate: new UntypedFormControl(new Date()),
        recordPaymentDTO: new UntypedFormGroup({
            amount: new UntypedFormControl(''),
            payment_date: new UntypedFormControl(''),
            payment_mode: new UntypedFormControl(''),
            referenceNo: new UntypedFormControl(''),
            ...
            tdsDeducted: new UntypedFormControl(false),
            tdsAmount: new UntypedFormControl(''),
            remarks: new UntypedFormControl(''),
            credit_doc_id: new UntypedFormControl(''),
        })
    })
    ...
    getSubscriberBasicDetails() {
        ...
        this.changePlanService.getSubscriberBasicDetails(subscriber_id).subscribe((result) => {
            if (result.responseCode !== 200 && result.responseMessage) {
                ...
                this.toastr.error(`${result.responseMessage}`, 'Failed!');
            ...
            if (result && result.data) {
                ...
                if (this.ui_id !== "purchase-addon-plan") {
                    ...
                    if (this.planList && this.planList.length > 0) {
                        ...
                        if (this.subscriberCurrentPlanData.currentPlanDTO) {
                            this.changePlanForm.get("planId").patchValue(this.subscriberCurrentPlanData.currentPlanDTO.custCurrentPlanId);
    ...
    onClickUpdatePaymentForm() {
        if ((this.changePlanForm.get('recordPaymentDTO').valid === false && this.ui_id !== 'purchase-addon-plan') || (this.changePlanForm.get('recordPaymentDTO').valid === false && this.ui_id === 'purchase-addon-plan')) {
            error: (error) => {
                this.toastr.error(`${error.error.ERROR}`, 'Please Fill All payment Details!');
            }
        ...
        if (this.isOverhead === true) {
            ...
            if (this.changePlanForm.get('sellPrice').valid === false) {
                error: (error) => {
                    this.toastr.error(`${error.error.ERROR}`, 'Please Enter Override Price!');
                }
            ...
            (result) => {
                ...
                if (result.responseCode !== 200 && result.responseMessage) {
                    ...
                    if (this.ui_id == 'purchase-addon-plan') {
                        ...
                        this.toastr.success(`${result.responseMessage}`, 'Change Plan Save Successfully!');
                        this.changePlanForm.reset();
                    }
                }
            }, err => {
            ...
            if ((this.selectedPlan.minPrice !== null && this.selectedPlan.maxPrice !== null)
                ...
                if (this.selectedPlan.minPrice !== null && this.selectedPlan.maxPrice !== null) {
                    if (event < this.selectedPlan.minPrice && event > this.selectedPlan.maxPrice) {
                        error: (err) => {
                            this.toastr.error(`${err.error.ERROR}`, 'Override price Sholud be between!');
                        }
