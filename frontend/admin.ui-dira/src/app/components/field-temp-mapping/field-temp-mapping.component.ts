...
export class FieldTempMappingComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.templateForm = new UntypedFormGroup({
            ...
            businessType: new UntypedFormControl("", [Validators.required]),
            moduleName: new UntypedFormControl("", [Validators.required]),

            aclEntryPojoList: new UntypedFormControl(this.saveSelectedPermission.aclEntryPojoList),
        });
        this.setAccessData();
        this.setData(new AclSave());
    ...
    submit(form: any, type) {
        ...
        if (this.isFormValid) {
            ...
            error: (error) => {
                this.toastr.error(`${error.responseMessage}`, 'Module Name is Mandatory!');
            }
