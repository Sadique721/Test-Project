...
export class TrialBillComponent implements OnInit {
    searchtrailBillFormGroup: UntypedFormGroup
    ...
    searchtrailBill(size) {
        ...
        if (this.searchtrailBillFormGroup.value) {
            if (this.searchtrailBillFormGroup.value.billrunid) {
                searchList.push(
                    `billrunid=${this.searchtrailBillFormGroup.value.billrunid}`,
                )
            }
            if (this.searchtrailBillFormGroup.value.billrunstatus) {
                searchList.push(
                    `billrunstatus=${this.searchtrailBillFormGroup.value.billrunstatus}`,
                )
            }
            if (this.searchtrailBillFormGroup.value.billfromdate) {
                searchList.push(
                    `billfromdate=${this.searchtrailBillFormGroup.value.billfromdate}`,
                )
            }
            if (this.searchtrailBillFormGroup.value.billtodate) {
                searchList.push(
                    `billtodate=${this.searchtrailBillFormGroup.value.billtodate}`,
                )
            }
