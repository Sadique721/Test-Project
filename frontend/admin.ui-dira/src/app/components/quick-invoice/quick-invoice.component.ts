...
export class QuickInvoiceComponent implements OnInit {
    ...
    milestoneGroupForm: UntypedFormGroup;

    addressListFromArray: UntypedFormArray;
    milestoneFormArray: UntypedFormArray;
    chargeGroupFormArray: UntypedFormArray;
    ...
    currentPagesearchLocationList = 0;
    ...
    ) { this.createAccess = loginService.hasPermission(INVOICE_SYSTEMS.CREATE_QUICK_INVOICE); }
    ...
    onAddChargeArray() {
        ...
        if (this.chargeGroupForm.valid) {
            if (this.chargeGroupForm.value.actualprice) {
                ...
                this.chargeGroupTable.renderRows();
            }
            setTimeout(() => {
    ...
    milestoneListFormGroup(): UntypedFormGroup {
        return this.fb.group({
    ...
    deleteConfirmmilestone(attributeIndex: number, price) {
        ...
        this.milestoneTable.renderRows();
    }
    onAddmilestoneList() {
        let orderN = 0;
        ...
        if (this.milestoneGroupForm.valid) {
            ...
            if (Number(remPrice) >= 0) {
                ...
                if (this.milestoneFormArray.value.length == 0) {
                    this.toastr.info(`Please first add charge deatils`, 'Info!')
                    ...
                    this.toastr.info(`Total Charge Amount greater than to Milestone Amounts`, 'Info!')
                    // this.messageService.add({
                    //     severity: "info",
                    //     summary: "Required ",
    ...
    addPlanInCustomer() {
        ...
        if (
            ...
            if (this.customerGroupForm.valid && this.presentGroupForm.valid) {
                ...
                this.toastr.error(`Minimum one chatge Details need to add`, 'Failed!')
                // this.messageService.add({
                //     severity: "error",
                //     summary: "Required ",
                ...
                this.toastr.error(`Minimum one Milestone Details need to add`, 'Failed!')
                // this.messageService.add({
                //     severity: "error",
                //     summary: "Required ",
    ...
    openStaffDetailModal(staffId) {
        ...
        this.savbillCommonBaseService.get(url).subscribe(
            (response: any) => {
                ...
                //     console.log("this.staffBUID", this.staffBUID);
                // }
                if (staff && staff.businessunitids?.length > 0) {
                    this.staffBUID = staff.businessunitids[0];
                }
            },
            (error: any) => {
    ...
    addcustomer() {
        ...
        if (checkRemainingamount !== 0) {
            ...
            this.toastr.error(`Remaining Payment is not zero`, 'Failed!')
            // this.messageService.add({
            //     severity: "error",
            //     summary: "Required ",
    ...
    addService(leadData) {
        ...
        let url = "/SavbillSalesCrmsBss/leadMaster/addNewService";
    ...
    openSearchModel() {
        ...
        this.dialog.open(this.locationDailog, {
            width: '80%',
    ...
    getServiceByServiceAreaID(ids) {
        ...
        this.savbillCommonBaseService.postMethod(url, data).subscribe((response: any) => {
    ...
    onAddProductField() {
        ...
        if (this.planProductfromgroup.valid) {
            ...
            this.planProductMappingTable.renderRows();
        }
    }
    ...
    @ViewChild('locationDailog') locationDailog!: TemplateRef<any>;


    @ViewChild('planProductMappingTable') planProductMappingTable!: MatTable<any>;
    @ViewChild('chargeGroupTable') chargeGroupTable!: MatTable<any>;
    @ViewChild('milestoneTable') milestoneTable!: MatTable<any>;
    @ViewChild('searchLocationTable') searchLocationTable!: MatTable<any>;
    ...
    milestoneDisplay = ['milestoneName', 'amount', 'dueDate', 'action']
    displayedChargeColumns = ['name', 'chargecategory', 'chargetype', 'ledgerId', 'billingCycle', 'taxid', 'actualprice', 'taxamount', 'action']
