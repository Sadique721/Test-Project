...
export class NetworkDeviceComponent implements OnInit, AfterViewInit {
    ...
    zoomInstance: any;
    networkDeviceGroupForm: UntypedFormGroup;
    networkDeviceCategoryList: any;
    submitted: boolean = false;

    displayedColumns = ['displayname', 'devicetype', 'status', 'action']
    ...
    constructor(
        ...
        private customerManagementService: CustomermanagementService, private dialog: MatDialog,
    ...
    addEditnetworkDevice(networkDeviceId, dialogRef) {
        this.submitted = true;

        if (this.networkDeviceGroupForm.valid) {
            if (networkDeviceId) {
                const url = "/NetworkDevice/update";
                ...
                this.networkdeviceService.postMethod(url, this.createnetworkDeviceData).subscribe(
                    (response: any) => {
                        if (response.responseCode == 406) {
                            this.toastr.info(`${response.responseMessage}`, "Error");
                        } else {
    ...
    editnetworkDevice(networkDeviceId: any) {
        if (networkDeviceId) {
            const url = "/NetworkDevice/" + networkDeviceId;
        ...
        if (networkDevice) {
            const dialogRef = this.dialog.open(this.confirmDialog, {
                ...
                data: {
                    ...
                    description: `Do you want to delete this Network Device?`,
                    yesLabel: 'Delete',
                    noLabel: 'Cancel'
                }
            });
    ...
    pageChangednetworkDeviceList(pageNumber) {
        this.currentPagenetworkDeviceListdata = pageNumber.pageIndex + 1;
        this.networkDeviceListdataitemsPerPage = pageNumber.pageSize;
        if (!this.searchkey) {
            this.getnetworkDeviceList("");
        } else {
            this.searchnetworkDevice();
    ...
    mylocation() {
        ...
        if (navigator.geolocation) {
            ...
            this.toastr.info(`Geolocation is not supported by this browser.`, "Error");
    ...
    networkHierarchyDevice(data) {
        this.dialog.open(this.isHierarchyDiagramVisibleDialog, {
            width: '80%',
            disableClose: true
        });
        this.data1 = [];
        ...
        this.networkdeviceService.getMethod(url).subscribe(
            (response: any) => {
                ...
                if (childrenlength.length == 0) {
                    ...
                    this.toastr.info(`Children data is not available.`, "Info!");
    ...
    parentDeviceMapping(data: any) {
        ...
        this.networkdeviceService.getMethod(url).subscribe(
            (response: any) => {
                ...
                this.dialog.open(this.deviceMappingDialog, {
                    width: '80%',
                    disableClose: true
                });
    ...
    getInwardList(productID) {
        ...
        this.networkdeviceService.getAllInwardByProduct(productID).subscribe(
            ...
            (error: any) => {
                ...
                this.toastr.info(`${error.error.msg}`, "Error");
            }
        );
    }
    ...
    // }
    getAvailableParent() {
        const url = `/NetworkDevice/availableParents?id=${this.selectedDeviceId}`;
        this.networkdeviceService.getMethod(url).subscribe(
            (response: any) => {
    ...
    updateNameAndPort(event: any, isParent) {
        ...
        this.dialog.open(this.parentChildPortModalDialog, {
            width: '80%',
            disableClose: true
        });
    ...
    modalOpenParentDevice(deviceType) {
        ...
        this.dialog.open(this.isparentChildDeviceModelOpenDialog, {
            width: '80%',
            disableClose: true
        });
    ...
    displayedColumnspersnol: string[] = ['label', 'value'];
    serviceAreaColumns: string[] = ['index', 'name'];
    searchLocationDisplayColumns: string[] = ['name', 'address'];
    childDisplayShow = ['childPort', 'parentPort', 'actions']
    parentDisplayShow = ['parentPort', 'childPort', 'actions']

    parentDisplayedColumns: string[] = ['childPort', 'parentPort', 'actions'];
    childDisplayedColumns: string[] = ['parentPort', 'childPort', 'actions'];

    @ViewChild('searchLocationDialog') searchLocationDialog!: TemplateRef<any>;
    @ViewChild('parentChildPortModalDialog') parentChildPortModalDialog!: TemplateRef<any>;
    @ViewChild('isHierarchyDiagramVisibleDialog') isHierarchyDiagramVisibleDialog!: TemplateRef<any>;
    @ViewChild('deviceMappingDialog') deviceMappingDialog!: TemplateRef<any>;
    @ViewChild('addEditDialog') addEditDialog!: TemplateRef<any>;
    @ViewChild('detailsDialog') detailsDialog!: TemplateRef<any>;
    @ViewChild('isparentChildDeviceModelOpenDialog') isparentChildDeviceModelOpenDialog!: TemplateRef<any>;
