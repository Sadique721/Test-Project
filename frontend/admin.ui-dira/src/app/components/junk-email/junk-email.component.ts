...
export class JunkEmailComponent implements OnInit {
    displayedColumns = ['sender', 'summary', 'desc', 'action']

    @ViewChild('addEditDialog') addEditDialog!: TemplateRef<any>;
    @ViewChild('descDailog') descDailog!: TemplateRef<any>;
    public loginService: LoginService;
    ticketCreateFormGroup: UntypedFormGroup;
    serviceData: any;
    planData: any;
    ...
    emailListData: any;
    pageLimitOptions = RadiusConstants.ITEMS_PER_PAGE;
    currentPageSize;
    showItemPerPage = 0;
    ...
    constructor(
        ...
        public savbillCommonBaseService: SavbillCommonBaseService, private dialog: MatDialog,
    ) {
    ...
    getDisplayedPlanDetailsColumns(): Array<string> {
        if (this.loginService.hasPermission(TICKETING_SYSTEMS.OPEN_OPPORTUNITY_CREATE) || this.loginService.hasPermission(TICKETING_SYSTEMS.OPEN_OPPORTUNITY_DELETE)) {
            return ['sender', 'summary', 'desc', 'action'];
        } else {
            return ['sender', 'summary', 'desc'];
        }
    }
    ...
    pageChangedTrcList(pageNumber) {
        this.currentPageEmailListdata = pageNumber.pageIndex + 1;
        this.emailListdataitemsPerPage = pageNumber.pageSize
        if (this.searchSenderKey || this.searchOption) {
            this.searchSender("");
        } else {
            this.getJunkEmailList("");
    ...
    createTicket(dialogRef) {
        ...
        if (this.selectedEmail.desc?.length > 15000) {
            this.toastr.info(`The size of the email body exceeds the permissible limit.`, 'Info!');
            // this.messageService.add({
            //     severity: "info",
            //     summary: "Info",
        ...
        if (this.isCustomerAvailable) {
            ...
            this.ticketManagementService.postMethod(url, formData).subscribe(
                ...
                (error: any) => {
                    ...
                    this.toastr.error(`Name not available, Please try different one.`, 'Failed!');
                    // this.messageService.add({
                    //     severity: "error",
                    //     summary: "Error",
            ...
            const urlCustomerCheck =
                "/customer/customerUsernameIsAlreadyExists/" + this.ticketCreateFormGroup.value.name;
            this.customerManagementService.getMethod(urlCustomerCheck).subscribe((response: any) => {
                if (response.isAlreadyExists == true) {
                    this.toastr.error(`Name not available. Please Change name`, 'Failed!');
                    // this.messageService.add({
                    //     severity: "error",
                    //     summary: "Error ",
    ...
    onServiceChange(event: any, dd: any) {
        ...
        this.customerManagementService.getMethod(planserviceurl).subscribe((response: any) => {
            ...
            if (planserviceData.length > 0) {
                ...
                if (this.planData.length === 0) {
                    this.toastr.info(`Plan not available for this service`, 'Info!');
                    // this.messageService.add({
                    //     severity: "info",
                    //     summary: "Note ",
    ...
    onProblemDomainChange(event: any, dd) {
    ...
    openDescriptionModal(desc: string) {
        ...
        this.dialog.open(this.descDailog, {
            width: '80%',
            disableClose: true
        });
