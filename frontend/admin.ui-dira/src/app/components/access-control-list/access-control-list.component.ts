...
export class AccessControlListComponent implements OnInit {
    ...
    createTacacsDeviceGroup(): UntypedFormGroup {
    ...
    createTacacsCommand(): UntypedFormGroup {
    ...
    editDeviceGroup(index: number) {
        const deviceGroupsArray = this.deviceFormGroup.get("tacacsDeviceGroups") as UntypedFormArray;
    ...
    approveCommand(index: number) {
        const tacacsDeviceGroupsArray = this.deviceFormGroup.get("tacacsDeviceGroups") as UntypedFormArray;
        ...
        if (isDuplicate) {
            error: (error) => {
                this.toastr.error(`${error.error.ERROR}`, 'Duplicate tacacsDeviceGroupName found. Please enter a unique value!');
            }
        ...
        const emptyFields = tacacsDeviceGroupsArray.controls.some(group => {
            const tacacsCommandSetDtoGroup = group.get("tacacsCommandSetDto") as UntypedFormGroup;
        ...
        if (emptyFields) {
            error: (error) => {
                this.toastr.error(`${error.error.msg}`, 'Some fields are empty. Please fill in all required values.!');
            }
        }
    ...
    addEditCommandSet(id: any) {
        ...
        if (!this.isApproved) {
            error: (error) => {
                this.toastr.info(error.ERROR || error.responseMessage, 'Please click the "Approve" button to proceed.!')
            }
            return;
        }
        if (id) {
