...
export class RadiusClientCreateComponent implements OnInit {
    ...
    clientGroupAttribute: UntypedFormArray;
    ipPoolAttribute: UntypedFormArray;
    ...
    get step1Group(): FormGroup {
        return this.clientForm.pick(['clientname', 'clientIpAddress', 'vendor', 'ipType', 'sharedKey', 'timeOut', 'clientGroupId', 'snmpEnable', 'sessionPurgeInterval', 'acctOnAttribute']);
    }
    ...
    async addClient() {
        ...
        if (this.isClientIdNull === true && this.clientGroupAttribute.length <= 0) {
            this.toastr.info(`Client Group Details is Mandatory!!`, 'Info!');
        }
        ...
        if (snmpEnable === true && requiredFields.some(field => !field)) {
            this.toastr.info(`SNMP Profile Details is Mandatory!!`, 'Info!');
        }
    ...
    onAddClientGroupAttribute() {
        ...
        this.ClientGroupTable.renderRows()
    }

    createClientGroupAttributeFormGroup(): UntypedFormGroup {
    ...
    createIPPoolNameFormGroup(): UntypedFormGroup {
        return this.fb.group({
            ipPoolMappingId: [""],
            clientId: [""],
            ipPoolId: [""]
    ...
    onAddIPPoolName() {
        ...
        if (selectedPool) {
            ...
            this.IpPoolAttributeTable.renderRows()
        }
    }
            ...
            this.radiusClientService.getClientDataById(clientId).subscribe(
                (response: any) => {
                    ...
                    if (data.clientGroupMappings.length > 0) {
                        ...
                        if (this.clientGroupAttribute.controls.length > 0) {
                            this.ClientGroupTable.renderRows();
                        }
                    }
                    ...
                    if (data.ipPoolMappingList.length > 0) {
                        ...
                        if (this.ipPoolAttribute.controls.length > 0) {
                            this.IpPoolAttributeTable.renderRows();
                        }
                        this.updateFilteredPoolList();
                    }
    ...
    dataSourceData = [{}]
    displyColIppool = ['poolName', 'actions']

    displayedClientGroupColumns: string[] = [
        'checkItem',
        'priority',
        'clientGroupId',
        'actions'
    ...
    @ViewChild('IpPoolAttributeTable') IpPoolAttributeTable!: MatTable<any>;
    @ViewChild('ClientGroupTable') ClientGroupTable!: MatTable<any>;
}
