...
export class InvestmentCodeComponent implements OnInit {
    @ViewChild('createEditInvestmentDialog') createEditInvestmentDialog: TemplateRef<any>;
    dataSource = new MatTableDataSource<any>([]);
    displayedColumns = ["id", "name", "iccode", "status", "action"];
    ...
    searchInvestment() {
        ...
        this.stateManagementService.postMethod(url, this.searchData).subscribe(
            (response: any) => {
                ...
                this.dataSource.data = this.IcListData;
    ...
    openCreateInvestmentDialog(): void {
        this.isIcEdit = false;
        this.submitted = false;
        this.investmentFormGroup.reset();

        this.dialogRef = this.dialog.open(this.createEditInvestmentDialog, {
            width: '800px',
            data: { editMode: this.isIcEdit },
        });
    ...
    openEditInvestmentDialog(id: any): void {
        if (!id) return;
        this.isIcEdit = true;

        this.stateManagementService.getMethod(`/investmentCode/${id}`).subscribe(
            (response: any) => {
                this.viewIcListData = response.data;
                this.investmentFormGroup.patchValue(this.viewIcListData);
    ...
    pageChangedRegionList(event: PageEvent): void {
        this.currentPageIcListdata = event.pageIndex + 1;
        this.IcListdataitemsPerPage = event.pageSize;
        if (this.searchkey) {
            this.searchInvestment();
        } else {
            this.getInvestmentList(this.IcListdataitemsPerPage);
        }
    }
    ...
    deleteConfirmon(icdata: any) {
        ...
        const dialogRef = this.dialog.open(DeleteConfirmationDialogBoxComponent, {
            width: '400px',
            data: {
                title: 'Delete Investment Code',
                description: `Are you sure you want to delete "${icdata.icname}"?`,
                yesLabel: 'Delete',
                noLabel: 'Cancel'
            }
        });
    ...
    private updatePaginator() {
        if (this.paginator) {
            setTimeout(() => {
                this.paginator.length = this.IcListdatatotalRecords;
                this.paginator.pageIndex = this.currentPageIcListdata - 1;
            });
        }
    }
