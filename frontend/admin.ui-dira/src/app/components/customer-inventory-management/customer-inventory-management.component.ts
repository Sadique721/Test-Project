...
import { NETWORKS, POST_CUST_CONSTANTS, PRE_CUST_CONSTANTS } from "src/app/constants/aclConstants";
...
export class CustomerInventoryManagementComponent implements OnInit {
    ...
    approveAssignInventoryColumns: string[] = ['select', 'fullName', 'username'];
    ...
    rejectStaffDisplayedColumns: string[] = ['select', 'name', 'username'];
    ...
    @ViewChild('assignPlanInventoryDialog') assignPlanInventoryDialog!: TemplateRef<any>;
    assignPlanInventoryDialogRef!: MatDialogRef<any>;

    @ViewChild('assignExternalInventoryDialog') assignExternalInventoryDialog!: TemplateRef<any>;
    assignExternalInventoryDialogRef!: MatDialogRef<any>;

    @ViewChild('attachDeviceDialogTemplate') attachDeviceDialogTemplate!: TemplateRef<any>;
    attachDeviceDialogTemplateRef!: MatDialogRef<any>;

    @ViewChild('rejectRemoveInventoryDialog') rejectRemoveInventoryDialog!: TemplateRef<any>;

    @ViewChild('hierarchyDialog') hierarchyDialog!: TemplateRef<any>;
    ...
    @ViewChild('inventorySpecDialog') inventorySpecDialog!: TemplateRef<any>;
    inventorySpecDialogRef!: MatDialogRef<any>;
    inventorySpecColumns: string[] = ['paramName', 'isMandatory', 'paramValue', 'action'];
    @Input() custData: any;
    @Input() isCustCAF: boolean = false;
    @Output() backButton = new EventEmitter();
    replaceDisplayedColumns: string[] = [
        'select',
        'itemId',
        'condition',
        'macAddress',
    ...
    // Replace your current getAllInventoryofCust array with a data source
    ...
    macAddressListCarditemsPerPage = RadiusConstants.PER_PAGE_ITEMS;
    ...
    inventoryDetailsDialogOpen = false
    inventoryColumns = ['productName', 'currentPlan', 'itemType', 'qty', 'status', 'assigneeName', 'action'];
    ...
    dialogRefApproveRemoveInventory: MatDialogRef<any, any>;
    ...
    ngOnInit(): void {
        ...
        this.uploadDocumentAccess = this.loginService.hasPermission(
            this.custData.custtype == "Prepaid"
                ? PRE_CUST_CONSTANTS.PRE_CUST_CAF_INVENTORY_UPLOAD_DOCUMENT
                : POST_CUST_CONSTANTS.POST_CUST_CAF_INVEN_SWAP
        );
        this.downloadDocumentsAccess = this.loginService.hasPermission(
            this.custData.custtype == "Prepaid"
                ? PRE_CUST_CONSTANTS.PRE_CUST_CAF_INVENTORY_DOWNLOAD_DOCUMENTS
                : POST_CUST_CONSTANTS.POST_CUST_CAF_INVEN_SWAP
        );
        this.downloadDocumentAccess = this.loginService.hasPermission(
            this.custData.custtype == "Prepaid"
                ? PRE_CUST_CONSTANTS.PRE_CUST_CAF_INVENTORY_DOWNLOAD_DOCUMENT
                : POST_CUST_CONSTANTS.POST_CUST_CAF_INVEN_SWAP
        );
        this.viewDocumentAccess = this.loginService.hasPermission(
            this.custData.custtype == "Prepaid"
                ? PRE_CUST_CONSTANTS.PRE_CUST_CAF_INVENTORY_VIEW_DOCUMENT
                : POST_CUST_CONSTANTS.POST_CUST_CAF_INVEN_SWAP
        );
        this.deleteDocumentAccess = this.loginService.hasPermission(
            this.custData.custtype == "Prepaid"
                ? PRE_CUST_CONSTANTS.PRE_CUST_CAF_INVENTORY_DELETE_DOCUMENT
                : POST_CUST_CONSTANTS.POST_CUST_CAF_INVEN_SWAP
        );
        ...
        this.refundAmountForm = this.fb.group({
            actualRefundPrice: [{ value: '', disabled: true }],
    ...
    openPortDetail() {
        const dialogRef = this.dialog.open(this.portDetailDialog, {
            width: '65%',
            disableClose: false,
    ...
    openInventoryDetailModal(modalId, data) {
        ...
        this.inventoryDetailsDialogOpen = true;
    ...
    getMacAddressList(event) {
        ...
        let currentPage;
        currentPage = this.currentMacAddressListdata;
        const requestData = {
        ...
        this.customerInventoryManagementService.postMethod(url, requestData).subscribe(
            (res: any) => {
                ...
                if (this.macAddressList.length == 0 || this.macAddressList == null) {
                    this.toastr.info(`${res.responseMessage}`, 'Assignee does not have a product!');
        ...
        this.customerInventoryManagementService.getMethod(url).subscribe(
            (res: any) => {
                ...
                if (this.macAddressList.length == 0 || this.macAddressList == null) {
                    ...
                    this.oldOfferPricePlanFlag = false;
                    error: (error) => {
                        this.toastr.info(`${error.responseMessage}`, 'Assignee does not have a product!');
                    }
    ...
    getPlanLevelCardMacAddressList(event) {
        ...
        const requestData = {
            ...
            paginationRequestDTO: {
                ...
                pageSize: this.macAddressListCarditemsPerPage
            }
        };
        const url = "/outwards/getItemHistoryByProductWith";
        this.customerInventoryManagementService.postMethod(url, requestData).subscribe(
            (res: any) => {
    ...
    getReplaceLevelMacAddressList(event) {
        ...
        this.customerInventoryManagementService.postMethod(url, requestBody).subscribe(
            (res: any) => {
                ...
                if (this.macAddressList.length == 0 || this.macAddressList == null) {
                    // error: (error) => {
                    this.toastr.info('Assignee does not have a product!');
                    // }
    ...
    getMacAddressList1(event) {
        ...
        let currentPage1;
        currentPage1 = this.currentMacAddress1Listdata;
        const requestData = {
    ...
    replaceInventorySubmit(): void {
        ...
        if (this.replaceInventoryForm.valid) {
            if (this.selectedReplaceMACAddress != "") {
                ...
                this.customerInventoryManagementService.postMethod(url, payload).subscribe(
                    (res: any) => {
                        if (res.responseCode == 200) {
                            ...
                            this.getCustomerAssignedList();
                            this.toastr.success(`${res.responseMessage}`, 'Assigned inventory successfully!');
                            this.replaceInventoryDialogRef?.close();
                ...
                // error: (error) => {
                this.toastr.info('"Please select a product to replace!');
                // }
    ...
    replaceAssemblyInventorySubmit(): void {
        ...
        if (this.replaceInventoryForm.valid) {
            if (this.selectedReplaceMACAddress != "") {
                ...
                error: (error) => {
                    this.toastr.info(`${error.responseMessage}`, 'Please select a product to replace!');
                }
            ...
            (response: any) => {
                if (response.dataList != null) {
                    ...
                    if (response.dataList[0].dataList) {
                        ...
                        this.toastr.success(`${response.responseMessage}`, 'Approve replace inventory!');
    ...
    assigneOtherInventory(): void {
        ...
        if (this.selectedMACAddress?.macAddress == "") {
            error: (error) => {
                this.toastr.info(`${error.responseMessage}`, 'Please Enter at mac address in selected item!');
            }
                ;
        } else if (this.selectedMACAddress?.serialNumber == "") {
            error: (error) => {
                this.toastr.info(`${error.responseMessage}`, 'Please Enter at serial number in selected item!');
            }
            ...
            if (this.inventoryAssignForm.valid) {
                if (data.inOutWardMACMapping.length > 0) {
                    ...
                    // data.custServiceMapId = this.selectedService;
                    ...
                    error: (error) => {
                        this.toastr.info(`${error.responseMessage}`, 'Please Select at least one item!');
                    }
        ...
        this.customerInventoryManagementService.getMethod(url).subscribe(
            (response: any) => {
                if (response.responseCode == 200) {
                    this.replaceInventoryModalClose();
                    this.getCustomerAssignedList();
                    this.toastr.success(`${response.message}`, 'Successfully Swapped!');
    ...
    assignExternalInventoryModalOpen() {
        ...
        this.assignExternalInventoryDialogRef = this.dialog.open(this.assignExternalInventoryDialog, {
            width: '75%',
            disableClose: true,
        });
    }
    ...
    assignExternalInventoryModalClose() {
        ...
        this.assignExternalInventoryDialogRef.close();
    }
    ...
    assigneExternalInventory(): void {
        ...
        if (this.externalInventoryAssignForm.valid) {
            if (data.inOutWardMACMapping.length > 0) {
                ...
                (error: any) => {
                    this.toastr.info(`${error.error.msg}`, 'Please select a product to assign!');
                }
    ...
    assignPlanInventoryModalOpen() {
        ...
        this.assignPlanInventoryDialogRef = this.dialog.open(this.assignPlanInventoryDialog, {
            width: '65%',
            disableClose: false,
        });
    }
    ...
    assignPlanInventoryModalClose() {
        ...
        this.assignPlanInventoryDialogRef.close();
    }
    ...
    assigneOtherInventoryForNonSerializedItem(): void {
        ...
        if (this.inventoryAssignForm.invalid) {
            ...
            Object.values(this.inventoryAssignForm.controls).forEach(control => {
        ...
        this.customerInventoryManagementService.getMethod(url).subscribe(
            (res: any) => {
                ...
                if (
                    ...
                    error: (error) => {
                        this.toastr.info(`${error.responseMessage}`, 'Assignee does not have sufficient product quantity!');
                    }
    ...
    // generateRemoveInventoryRequest(id, custinventoryid, ItemId) {
    //     this.editInventory = false;
    //     this.editSTBCradInventory = false;
    ...
    getAllInventoryHistoryModalOpen() {
        ...
        this.openInventoryDetailsDialog();
        ...
        this.customerInventoryManagementService.getMethod(url).subscribe(
            (res: any) => {
                if (res && res.dataList) {
                    ...
                    this.totalRecords = res.totalRecords || res.dataList.length;

                    // Set paginator if it exists
    ...
    onPageChange(event: PageEvent) {
        ...
        // If your API supports server-side pagination, call it here
        // this.getAllInventoryHistoryWithPagination();

        // For client-side pagination (if data is already loaded)
        // The mat-paginator will handle it automatically
    }
    ...
    approveRemoveInventoryOpen(inventory, nextApproverId) {
        ...
        this.dialogRefApproveRemoveInventory = this.dialog.open(this.approveRemoveInventoryDialog, {
            width: '60%',
            disableClose: true
        });
        ...
        this.customerInventoryManagementService.getMethod(url).subscribe(
            (response: any) => {
                if (response.responseCode == 200 || response.responseCode == 0) {
                    ...
                    if (this.dialogRefApproveRemoveInventory) {
                        this.dialogRefApproveRemoveInventory.close();
                    }
    ...
    rejectRemoveInventoryOpen(inventory, nextApproverId) {
        ...
        // this.rejectAssignInventoryForm.reset();
        this.rejectRemoveInventorySubmitted = false;
        this.dialogRefApproveRemoveInventory = this.dialog.open(this.rejectRemoveInventoryDialog, {
            width: '60%',
            disableClose: true
        });
    ...
    // }
    openHierarchyDialog() {
        const dialogRef = this.dialog.open(this.hierarchyDialog, {
            width: '80%',
            disableClose: false,
    ...
    viewSpecificationParameters(id, data) {
        ...
        this.inventorySpecDialogRef = this.dialog.open(this.inventorySpecDialog, {
            width: '800px',
            disableClose: true,
        });
    }
    ...
    //     this.dialog.open(this.inventorySpecDialog, {
    //         width: '75%',
    //         disableClose: true,
    //     });
    //     this.specDetailsShow = true;
    ...
    //     this.inwardService.getInventoryParamsByMappingID(this.productData.id).subscribe((response: any) => {
    //         if (response.responseCode == 200) {
    //             //   this.specDetailsShow = true;
    //             this.inventorySpecificationDetails = response.dataList;
            ...
            (response: any) => {
                if (response.responseCode == 200) {
                    if (response.data) {
                        ...
                        this.toastr.error(`${response.responseMessage}`, 'Something went wrong!!!');
    ...
    uploadDocument(inventory) {
        ...
        this.uploadDocForm.forEach((formGroup, tabIndex) => {
            ...
            formGroup.get('sectionName')?.disable();
        });
        ...
        this.customerInventoryManagementService
            ...
            .subscribe(
                blob => {
                    if (blob.status == 200) {
                        error: (error) => {
                            this.toastr.success(`${error.responseMessage}`, 'Download Successfully!');
                        }
                        ...
                        error: (error) => {
                            this.toastr.error(`${error.responseMessage}`, 'File Not Found!');
                        }


                    } else {
                        error: (error) => {
                            this.toastr.error(`${error.responseMessage}`, 'Something went wrong!');
                        }
    ...
    wifiModalOpen(inventory) {
        ...
        this.navService.postMethod(urldoc, obj).subscribe(
            (response: any) => {
                if (response.responseCode == 200) {
                    this.wifiModel = true;
                    if (response.data.ssidUsername !== null && response.data.ssidPassword !== null) {
    ...
    saveWifi() {
        ...
        if (this.wifiForm.valid) {
            ...
            this.navService.postMethod(urldoc, data).subscribe(
                (response: any) => {
                    if (response.responseCode == 200) {
                        this.toastr.success(`${response.responseMessage}`, 'sucecess!');
        ...
        if (inputElement.files && inputElement.files.length > 0) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (
                    ...
                    this.toastr.info(`${file?.name}`, 'Invalid file type: ${file?.name}. Must be png, jpg, jpeg, or pdf.!');
    ...
    uploadAllDocuments(): void {
        ...
        if (invalidMandatoryTabs.length > 0) {
            error: (error) => {
                this.toastr.info(`${error.responseMessage}`, 'Fields are mandatory in these tabs: ${invalidMandatoryTabs.join(", ")}');
            }
    ...
    collectSectionData(
        ...
        const section = {
            name: formGroup.getRawValue().sectionName,
    ...
    attachDeviceInventoryModalOpen() {
        ...
        this.attachDeviceDialogTemplateRef = this.dialog.open(this.attachDeviceDialogTemplate, {
            width: '60%',
            disableClose: true
        });
    }
    ...
    attachDeviceInventoryModalClose() {
        ...
        this.masterDbList = [];
        this.getCustomerNetworkDeviceBindData();
        this.attachDeviceDialogTemplateRef.close();
    }
    ...
    getPOPList() {
        ...
        this.customerInventoryManagementService.getMethod(url).subscribe(
            ...
            err => {
                this.toastr.error(`${err?.error?.ERROR || "Something went wrong"}`, 'Failed!');
    ...
    getCustomerNetworkDeviceBindData() {
        const bindUrl = `/customer/getCustomerNetworkBindByCustId?custId=${this.custData.id}`;
        ...
        apiCall.subscribe(
            (res: any) => {
                if (res.responseCode === 200) {
                    ...
                    this.toastr.error(`${res.responseMessage || "Something went wrong"}`, 'Failed!');
        ...
        this.inwardService.postMethod(url, this.searchMacData).subscribe(
            (response: any) => {
                if (response.dataList.length > 0) {
                    ...
                    this.macAddressList = [];
                    this.macAddressListtotalRecords = 0;
                    this.toastr.info(`${response.message}`, 'No Search Record Found!');
    ...
    @ViewChild("inventoryDetailsDialog") inventoryDetailsDialog!: TemplateRef<any>;

    openInventoryDetailsDialog() {
        this.dialogRef = this.dialog.open(this.inventoryDetailsDialog, {
            width: "1500px",
            disableClose: true,
