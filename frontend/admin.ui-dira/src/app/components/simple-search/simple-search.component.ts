...
export class SimpleSearchComponent implements OnInit {
    ...
    selectedColumns = this.exportColumns ? this.exportColumns : []; // now it spreads an empty array safely
    ...
    // This tells Angular Material how to compare values in mat-select
    compareColumns = (o1: any, o2: any): boolean => {
    ...
    updateDisplayedColumns(): void {
        ...
        const headerData = this.selectedCols.map(col => col.field); // or col.header depending on what you use in HTML

        if (headerData.length > 0) {
            if (this.showCheckbox) {
                this.displayedColumns = ['select', ...headerData];
            } else {
    ...
    //     return this.selectedCols.some(c => c.field === 'Actions');
    // }
    ...
    deleteSelectedUsers() {
        if (!this.selectedInstances || this.selectedInstances.length === 0) {
            this.toastr.info(`No users selected for deletion.`, 'Info!')
            // this.messageService.add({
            //     severity: "info",
            //     summary: "Info",
    ...
    exportToCSV() {
        ...
        const headers = Object.keys(this.instanceData[0]);
        csvRows.push(headers.join(','));

        for (const row of this.instanceData) {
