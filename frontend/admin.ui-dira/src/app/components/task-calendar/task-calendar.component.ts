...
@Component({
    ...
    providers: [DatePipe, provideNativeDateAdapter(), CalendarDateFormatter],
    standalone: false
})
export class TaskCalendarComponent implements OnInit {
    ...
    ticketId: any;
    cdr: any;
    evntFormGroup: UntypedFormGroup;
    ...
    @ViewChild('createNewEventDialog') createNewEventDialog!: TemplateRef<any>;
    @ViewChild('selectCustomerDialog') selectCustomerDialog!: TemplateRef<any>;
    @ViewChild('customerDetailDialog') customerDetailDialog!: TemplateRef<any>;
    newEventdialogRef!: MatDialogRef<any>;
    selectustomerDialogRef!: MatDialogRef<any>;
    customerDetailDialogRef!: MatDialogRef<any>;
    ...
    openEventDialog(): void {
        ...
        this.evntFormGroup.get('caseType').disable();
        this.evntFormGroup.get('customersId').disable();

        this.newEventdialogRef = this.dialog.open(this.createNewEventDialog, {
            width: '1200px',
            maxWidth: '90vw',
            autoFocus: false,
            disableClose: true
        ...
        this.newEventdialogRef.afterClosed().subscribe((result) => {
            if (result) {
            }
        });
    ...
    <div class="tooltip-content">
      ...
      <span class="tooltip-date">Duration: ${isAllDay ? "All Day" : `${startTime} to ${endTime}`
            }</span>
    </div>
  `;
    ...
    checkAvailablity() {
        ...
        if (this.evntFormGroup.valid) {
            ...
            if (
                ...
                const startTime = this.evntFormGroup.get("startTime")?.value;
                const endTime = this.evntFormGroup.get("endTime")?.value;

                const startDateFormat = this.formatDateTime(startDate);
                const startTimeFormat = this.formatDateTime(startTime);
                const onlyStartDate = startDateFormat.split(" ")[0];
                const onlyStartTime = startTimeFormat.split(" ")[1];

                const endDateFormat = this.formatDateTime(endDate);
                const endTimeFormat = this.formatDateTime(endTime);
                const onlyEndDate = endDateFormat.split(" ")[0];
                const onlyEndTime = endTimeFormat.split(" ")[1];

                const startDateTimeFinal = `${onlyStartDate}T${onlyStartTime}`;
                const endDateTimeFinal = `${onlyEndDate}T${onlyEndTime}`;

                startDateTime = startDateTimeFinal;
                endDateTime = endDateTimeFinal;
            }
            ...
            this.taskManagementService.getMethod(url).subscribe(
                (response: any) => {
                    ...
                    if (response.dataList.length > 0) {
                        const dialogRef = this.dialog.open(DeleteConfirmationDialogBoxComponent, {
                            ...
                            data: {
                                title: 'Alert',
                                description: `This staff member is already assigned an event at the specified time. Do you want to allocate a new event?`,
                                yesLabel: 'Delete',
                                noLabel: 'Cancel'
                            }
                        });
                        ...
                        // this.confirmationService.confirm({fv
                        //     header: "Alert",
                        //     message:
                ...
                (error: any) => {
                    console.log("error", error)
                    this.toastr.error(`${error.error.responseMessage}`, 'You have rejected!');
    ...
    checkSelectCust(event: any) {
        this.selectedParentCust = event.value;
    }

    async modalOpenParentCustomer() {
        await this.getParentCustomerData();
        // this.parentCustList = Array.isArray(this.parentCustList) ? this.parentCustList : [];
        this.selectustomerDialogRef = this.dialog.open(this.selectCustomerDialog, {
            width: '1000px',
            maxWidth: '90vw',
            autoFocus: false,
            disableClose: true
        ...
        this.selectustomerDialogRef.afterClosed().subscribe((result) => {
            if (result) {
            }
        });
        // await this.getParentCustomerData();
    ...
    async saveSelCustomer(isOpenModal) {
        this.evntFormGroup.get("customersId").setValue(Number(this.selectedParentCust.id));
        ...
        // console.log("heeeeeeeeeeeeeeee11111111111111", this.evntFormGroup.value);
        // console.log("h222222222", he);
    }

    showSavedCustomerDetails() {
        if (this.savedCustomerDetails) {
            const url = "/customers/" + this.evntFormGroup.controls.customersId.value;
            this.customerService.getMethod(url).subscribe(
                (response: any) => {
                    if (response.status == 200) {
                        ...
                        this.customerDetailDialogRef = this.dialog.open(this.customerDetailDialog, {
                            width: '800px',
                            maxWidth: '90vw',
                            autoFocus: false,
                            disableClose: true
                        ...
                        this.customerDetailDialogRef.afterClosed().subscribe((result) => {
                            if (result) {
                            }
                        });
                    }
    ...
    modalCloseParentCustomer() {
        ...
        this.selectustomerDialogRef.close();
    ...
    getCaseStatus() {
        ...
        this.commondropdownService.getMethodWithCache(url).subscribe(
            (response: any) => {
                ...
                const data = this.createStatusList.forEach(e => {
                    if (e.value === 'Open') {
                        this.searchFormGroup.controls.caseStatus.setValue(e.value);
                    }
                });
    ...
    closeCustomerDetailsModel() {
        ...
        this.customerDetailDialogRef.close();
    }
    closeCustomer() {
        this.displaySavedCustomerDialog = false;
    }
    ...
    events2 = signal<CalendarEvent[] | any>([
        {
            start: subDays(startOfDay(new Date()), 1),
            end: addDays(new Date(), 1),
            title: 'A 3 day event',
