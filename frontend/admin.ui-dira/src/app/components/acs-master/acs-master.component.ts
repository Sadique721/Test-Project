...
export class AcsMasterComponent implements OnInit {
    title = "ACS";
    acsFormGroup: UntypedFormGroup;
    acsPackageMappings: UntypedFormArray;
    acsParameterMappingsfromgroup: UntypedFormGroup;
    acsParameterMappingsFromArray: UntypedFormArray;
    acsUrlParameterMappingsfromgroup: UntypedFormGroup;
    acsUrlParameterMappingsFromArray: UntypedFormArray;
        ...
        if (this.acsFormGroup.valid) {
            if (id) {
                ...
                if (
                    ...
                    error: (error) => {
                        this.toastr.error(`${error.error.ERROR}`, 'Minimum one Parameter mapping Details and API mapping need to add!');
                    }
    ...
    createUrlParameterMappingFormGroup(): UntypedFormGroup {
        return this.fb.group({
    ...
    getVendorInfo(event) {
        ...
        this.acsManagementService.getMethod(url).subscribe((response: any) => {
            ...
            if (response.data != null) {
                error: (error) => {
                    this.toastr.error(`${error.error.ERROR}`, 'Vendor already configured!');
                }
