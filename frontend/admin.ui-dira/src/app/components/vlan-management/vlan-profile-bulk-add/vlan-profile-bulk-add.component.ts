...
export class VlanProfileBulkAddComponent implements OnInit {
    ...
    customerMigration: UntypedFormGroup;
    ...
    downloadSampleFile() {
        this.vlanProfileService.downloadVlanFile(this.fileName).subscribe(
            ...
            (error: any) => {
                this.toastr.error(`${error.error.errorMessage}`, 'Something went wrong!!!!');
    ...
    download() {
        this.dialog.open(this.isFIleDialog, {
            width: '550px',
    ...
    @ViewChild('isFIleDialog') isFIleDialog!: TemplateRef<any>;
}
