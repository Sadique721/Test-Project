...
export class AddServiceComponent implements OnInit {
    ...
    movePlanDetail() {
        ...
        if (this.generalInfoForm.valid) {
            ...
            error: (error) => {
                this.toastr.error(`${error.error.ERROR}`, 'Link Acceptance or Contact details are required!');
            }
    ...
    onAddPlanServiceField() {
        if (this.planDto) {
            ...
            error: (error) => {
                this.toastr.error(`${error.error.ERROR}`, 'Please fill the Details!');
            }
    ...
    addPlanService() {
        if (this.isDTOReady && this.planDto) {
            ...
            if (this.addServicePlanData.planMappingList.length <= 0) {
                error: (error) => {
                    this.toastr.error(`${error.error.ERROR}`, 'lease add atleast one service and plan!');
                }
    ...
    planServiceAdd(data) {
        ...
        if (this.custData.businessType == "Enterprise") {
            ...
            if (this.isLeadMaster) {
                if (this.isServiceEdit) {
                    url =
                        "/SavbillSalesCrmsBss/leadMaster/updateLeadService?leadMasterServiceId=" +
    ...
    getLeadServiceList() {
        const url =
            "/SavbillSalesCrmsBss/leadMaster/findCircuitDetailsByLeadId?leadId=" + this.custData.id;
        this.customerManagementService.getMethod(url).subscribe(
            (response: any) => {
                if (response.status == 200) {
    ...
    playService() {
        ...
        setTimeout(() => {
            this.getActivePlanDetails();
            error: (error) => {
                this.toastr.success(`${error.message}`, 'Started Successfully!');
            }
    ...
    getCircuitDetail(id) {
        let url = "/SavbillSalesCrmsBss/leadMaster/findLeadServiceMappingById?leadServiceMappingId=" + id;
        this.customerManagementService.getMethod(url).subscribe(
            (response: any) => {
                if (response.status == 200) {
