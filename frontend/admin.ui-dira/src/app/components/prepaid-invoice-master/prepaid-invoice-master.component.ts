...
export class PrepaidInvoiceMasterComponent implements OnInit {
    ...
    @ViewChild(MatPaginator) paginators: MatPaginator = Object.create(null);
    ...
    currentPageinvoicePaymentList = 0;
    ...
    searchinvoiceMaster(id, size) {
        ...
        url =
            ...
            "&billfromdate=" +
            this.formatDate(this.searchInvoiceMasterFormGroup.value.billfromdate) +
            "&billtodate=" +
            this.formatDate(this.searchInvoiceMasterFormGroup.value.billtodate) +
            "&custmobile=" +
            this.searchInvoiceMasterFormGroup.value.custMobile +
            "&serviceAreaId=" +
    ...
    invoicePaymentList(invoice) {
        ...
        if (invoice.adjustedAmount >= invoice.totalamount) {
            ...
            this.dialog.open(this.PaymentDetails, {
                width: '80%',
    ...
    invoicePaymentAdjsment(dialogRef) {
        const data = {
    ...
    // voidInvoice(dialogRef): void {
    //     const dialogRef1 = this.dialog.open(this.confirmDialog, {
    //         width: '400px',
    //         data: {
    //             title: 'VOID Invoice Confirmation',
    //             description: `Do you wish to VOID this invoice?`,
    //             yesLabel: 'Yes',
    //             noLabel: 'No'
    //         }
    //     });
    ...
    voidInvoice(): void {
        ...
        this.revenueManagementService.getMethod(url).subscribe(
            (response: any) => {
                ...
                if (response?.responseCode === 417) {
                    this.toastr.info(response.responseMessage || 'Info', 'Info!');
                    this.dialog.closeAll()
                } else {
                    this.toastr.success(response?.message || response?.responseMessage || 'Operation successful', 'Success!');
                    this.dialog.closeAll()
                }
            },
            (error: any) => {
    ...
    displayNote(invoiceId) {
        ...
        this.dialog.open(this.noteDailog, {
            width: '80%',
            disableClose: true // same as data-backdrop="static" data-keyboard="false"
        });
    ...
    cancelRegenrateInvoice(dialogRef?: any) {
        const data = {};
        ...
        this.revenueManagementService.postMethodPasssHeader1(url, data).subscribe(
            ...
            (error: any) => {
                this.toastr.error(error?.error?.ERROR || 'Something went wrong', 'Error!');
                this.dialog.closeAll()
            }
        );
    }
    ...
    getParentCustomerData() {
        ...
        // const url = "/customers/list";
        const url = `/customers/list/` + RadiusConstants.CUSTOMER_TYPE.PREPAID + "?orgcusttype=false";
    ...
    async saveSelCustomer(event, dialogRef) {
        this.parentCustList = [
            {
    ...
    openModal(invoiceData, custId) {
        ...
        this.dialog.open(this.modalTitleDialog, {
            width: '80%',
            disableClose: true // same as data-backdrop="static" data-keyboard="false"
        });
    ...
    onChangeOFTDS(event) {
        ...
        if (diff < 0 && tdsAmount != 0) {
            this.paymentFormGroup.controls.tdsAmount.setValue(0);
            this.toastr.error(`TDS/ABBS total can not be greater than amount.`, 'Failed!');
            // this.messageService.add({
            //     severity: "error",
            //     summary: "Error",
    ...
    getRecordPayment() {
        ...
        this.dialog.open(this.recordPaymentDialog, {
            width: '80%',
            disableClose: true // same as data-backdrop="static" data-keyboard="false"
        });
        let amount = 0;
    ...
    getStaff() {
        ...
        this.savbillCommonBaseService.post(url, data).subscribe(
            (response: any) => {
                if (response.staffUserlist.length > 0) {
                    this.staffData = response.staffUserlist;
                    this.dataSource = new MatTableDataSource<any>(this.staffData);
    ...
    displayedColumnsNotes: string[] = ['event', 'date', 'remark'];
    displayMiliston = ['milestoneNumber', 'amount', 'dueDate', 'action']
    displayStaff = ['select', 'name', 'username', 'partner']
    displayedColumnsCutomer: string[] = ['select', 'name', 'username'];


    @ViewChild('milstoneTable') milstoneTable!: MatTable<any>;
    ...
    @ViewChild('modalTitleDialog') modalTitleDialog!: TemplateRef<any>;
    @ViewChild('noteDailog') noteDailog!: TemplateRef<any>;
