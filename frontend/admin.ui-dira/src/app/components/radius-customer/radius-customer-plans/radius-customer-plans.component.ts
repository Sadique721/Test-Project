...
export class RadiusCustomerPlansComponent implements OnInit {
    ...
    custCurrentPlanListDataSource = new MatTableDataSource<any>([]);
    custFuturePlanListDataSource = new MatTableDataSource<any>([]);
    custExpiredPlanListDataSource = new MatTableDataSource<any>([]);
    expiredPlanDisplayedColumns: string[] = [
        'service',
        'planName',
        'validity',
        'status',
    ...
    @ViewChild('quotaDetailsDialog') quotaDetailsDialog!: TemplateRef<any>;
    ...
    async ngOnInit() {
        this.pageSizeOptions = this.pageLimitOptions.map(o => o.value);
    ...
    ngAfterViewInit(): void {
        this.custCurrentPlanListDataSource.paginator = this.paginator;
        this.custCurrentPlanListDataSource.sort = this.sort;
        this.custFuturePlanListDataSource.paginator = this.paginator;
        this.custFuturePlanListDataSource.sort = this.sort;
        this.custExpiredPlanListDataSource.paginator = this.paginator;
        this.custExpiredPlanListDataSource.sort = this.sort;
    }
        ...
        this.rediusprofileService.getMethodForRadius(url).subscribe(
            (response: any) => {
                if (response.dataList) {
                    ...
                    //  this.custCurrentPlanListDataSource = new MatTableDataSource<any>(this.custCurrentPlanList);
    ...
    getcustFuturePlan(custId, size) {
        ...
        this.rediusprofileService.getMethodForRadius(url).subscribe(
            (response: any) => {
                ...
                this.customerFuturePlanListdatatotalRecords = response.dataList;
                // this.custFuturePlanListDataSource = new MatTableDataSource<any>(this.custFuturePlanList);
    ...
    getCustQuotaList(custId, plan) {
        this.customerManagementService.getCustQuotaListFromRadius(custId).subscribe(
            (response: any) => {
                ...
                this.dialog.open(this.quotaDetailsDialog, {
                    width: '65%'
                });
            },
            (error: any) => {
    ...
    resetQuota(custId, plan) {
        this.customerManagementService.resetQuota(custId, plan.planmapid).subscribe(
            (response: any) => {
                ...
                //   icon: "far fa-times-circle"
                // });
                this.toastr.success('Quota Reset successfully', 'Success!');
    ...
    getcustExpiredPlan(custId, size) {
        ...
        this.rediusprofileService.getMethodForRadius(url).subscribe(
            (response: any) => {
                ...
                this.customerExpiryPlanListdatatotalRecords = response.dataList;
                // this.custExpiredPlanListDataSource = new MatTableDataSource<any>(this.custExpiredPlanList);
    ...
    quotaDisplayedColumns: string[] = [
        ...
        'totalQuota',
        'usedQuota',
        'currentSessionUsageVolume',
        'quotaUnit',
        'timeTotalQuota',
        'timeQuotaUsed',
        'timeQuotaUnit',
        'currentSessionUsageTime',
        'parentQuotaType',
        'usageQuotaType'
    ];
    closeModel() {
        // If using MatDialogRef to close Angular Material dialog

        this.dialog.closeAll();


        // Reset PlanQuota with default values
        this.PlanQuota.next({
            custid: "",
            PlanData: ""
        });
