...
export class CustChangePlanComponent implements OnInit {
  ...
  getCustomersDetail(custId: number) {
    ...
    this.customerManagementService.getMethod(url).subscribe(
      (response: any) => {
        ...
        console.log("billableCustList:", this.billableCustList);
        ...
        //   this.currency = this.custDetails.currency;
  ...
  onChangePlanType(event) {
    if (event === "Upgrade") {
      ...
      this.isBilling = true;
      this.changeBillCycle = this.selectedBillingCycle;
      this.getUpgradePlan();
    }
    ...
    this.childCustList.forEach(element => {
      if (element.serviceMappingData != null && element.serviceMappingData.length == 1) {
    ...
    if (this.custServiceData != null && this.custServiceData.length == 1) {
  ...
  getUpgradePlan() {
    this.connectionList = this.custServiceData.map(item => {
      return {
        text: item.connection_no,
        value: item.connection_no
      };
    });
  }
  ...
  getStaffDisplayName(): string {
    ...
    if (
      paymentOwnerId &&
      this.staffListDatasource.data &&
      this.staffListDatasource.data.length > 0
    ) {
  ...
  getCustomerChangePlanDueAmount(isCheckAmount?) {
    ...
    let pojo = {
      ...
      purchaseType: this.changePlanTypeSelection
    };
  ...
  changePlanGroupBulk() {
    ...
    console.log("changePlanNewForm values:", this.changePlanNewForm.value);
    console.log("isPaymentReceived value:", this.changePlanNewForm.value.isPaymentReceived);
    console.log("isPaymentReceived type:", typeof this.changePlanNewForm.value.isPaymentReceived);
    ...
    console.log("Payment Received:", isPaymentReceived, "Type:", typeof isPaymentReceived);
    ...
    if (!isPaymentReceived) {
      ...
      console.log("❌ Payment Received = NO - Opening payment modal");
      this.openRecordPayment();
    }
  }

  getCustomerChangePlanDueAmountList() {
    const url = "/customers/getCustomerChangePlanDueAmount";
    this.custServiceData.forEach(element => {
      this.custPackRelId = element.planmapid;
      this.oldPlanId = element.planId;
    ...
    let pojo = {
      ...
      newPlanId: this.selPlanData.id,
      changePlanBillingCycle: this.changePlanNewForm.value.billCycle,
      purchaseType: this.changePlanTypeSelection == "Upgrade" ? "Change Plan" : "Renew"
    };

    this.custServiceData.forEach(element => {
      if (element.custServMappingStatus == "NewActivation") {
        this.prepareChangePlanPayload(event);
      } else {
        this.customerManagementService.getCustomerChangePlanDueAmount(url, pojo).subscribe(
          (response: any) => {
  ...
  getCustomerInsufficientBalance() {
    const url = "/checksufficentwallteblanceforplan";
    const data = {
      custId: this.custDetails.id,
      planId: this.selPlanData.id
    };
    this.customerManagementService.getCustomerInsufficientBalance(url, data).subscribe(
      (response: any) => {
        ...
        if (this.customerInsufficientBalance.responseCode === 200) {
          this.prepareChangePlanPayload(null);
        } else if (
          this.customerInsufficientBalance.responseCode === 400 &&
          this.customerInsufficientBalance.responseMessage === "Insufficient wallet balance"
        ) {
          this.amountCheckDialog();
        }
      },
  ...
  openRecordPayment() {
    ...
    if (this.recordPaymentDialog) {
      ...
      console.error("recordPaymentDialog template not found!");
    }
  }
  ...
  modalCloseInvoiceList() {
    ...
    console.log(this.paymentFormGroup.value.invoiceId);
  ...
  getPlanPurchaseType() {
    ...
    this.savbillCommonBaseService.get(url).subscribe(
      (response: any) => {
        ...
        this.changePlanType = response.dataList;
        let data: any =
          this.custCurrentPlanList.length > 0 ? this.custCurrentPlanList[0].planstage : "";
        if (data && data.toString().toLowerCase() === "hold") {
          this.changePlanType = this.changePlanType.filter(
            type => type.text !== "Renew" && type.text !== "Change Plan"
          );
        }
